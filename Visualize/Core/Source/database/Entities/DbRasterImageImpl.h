...
class ISM_EXPORT OdTvDbRasterImageImpl : public OdTvDbEntityImpl
{
  static OdTvDbRasterImageImpl* getImpl(const OdTvDbRasterImage *pObj)
  { return (OdTvDbRasterImageImpl*)OdTvDbSystemInternals::getImpl(pObj);}
...
public:
  ...
  OdTvDbHardOwnershipId m_ReactorId;
...
public:
  ...
  OdResult getVertices(const OdTvDbRasterImage *pImage, OdGePoint3dArray& pnts, bool bLoadIfNeed = true);
protected:
  ...
  friend class OdTvDbRasterImage;
};


inline void odtvdbDrawImageFrame(const OdTvDbEntity *pEntity, OdGiGeometry &geom, OdGiSubEntityTraits &traits, OdGiRegenType regenType,
  const OdGePoint3d *pFramePoints, OdUInt32 nFramePoints, bool bDrawFrame, OdGsMarker baseSubEntMarker = -1)
{
  if (!bDrawFrame)
  {
    if ((regenType == kOdGiForExplode) ||
      ...
      return;
    traits.setTransparency(OdTvCmTransparency(OdUInt8(0u))); // Don't know why it is need, but acad does
