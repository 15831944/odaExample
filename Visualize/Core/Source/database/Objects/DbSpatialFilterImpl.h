...
class OdTvDbSpatialFilterImpl : public OdTvDbFilterImpl, public OdGiClipBoundary
{
  static OdTvDbSpatialFilterImpl* getImpl(const OdTvDbSpatialFilter *pObj)
  { return (OdTvDbSpatialFilterImpl*)OdTvDbSystemInternals::getImpl(pObj);}
...
public:
  OdTvDbSpatialFilterImpl();

  static void getMatrices(const OdTvDbSpatialFilter& filter,
                          OdGeMatrix3d& inverseBlockMatrix,
                          OdGeMatrix3d& clipBoundMatrix)
  {
    filter.assertReadEnabled();
    OdTvDbSpatialFilterImpl * pImpl = OdTvDbSpatialFilterImpl::getImpl(&filter);
    inverseBlockMatrix = pImpl->m_xInverseBlockRefXForm;
    clipBoundMatrix = pImpl->m_xToClipSpace;
  }

  static void setMatrices(OdTvDbSpatialFilter& filter,
                          OdGeMatrix3d& inverseBlockMatrix,
                          OdGeMatrix3d& clipBoundMatrix)
  {
...
private:
  ...
  friend class OdTvDbSpatialFilter;
