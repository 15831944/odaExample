...
public:
  ...
  bool TryMeAsInfoAndAdd2Table(const OdString &str, OdTvDbLinetypeTablePtr pTbl, OdTvDbTextStyleTablePtr pTStyleTbl, OdDb::DuplicateLinetypeLoading dlt)
  {
    OdString tmp;
    ...
    OdTvDbLinetypeTableRecordPtr pLTRec;
    pLTRec = OdTvDbLinetypeTableRecord::createObject();
    if (pLTRec.isNull())
    ...
    do
    {
      ...
      if (tmp[0] == '[')
      {
        ...
        else // shape
        {
          ...
          OdTvDbTextStyleTableRecordPtr pR;
          OdTvDbSymbolTableIteratorPtr pIt = pTStyleTbl->newIterator();
          while (!pIt->done())
          { 
      ...
      else
      {
        ...
        {
          OdTvDbLinetypeTableRecordImpl::resizeDashes(pLTRec, indx + 1);
        }
      ...
      if (m_hasAt)
      {
        ...
        if (!m_IdAt)
        {
          if (!m_IsShapeFileAt)
            return false;
          OdTvDbTextStyleTableRecordPtr pR;
          pR = OdTvDbTextStyleTableRecord::createObject();
        ...
        if (m_IsShapeFileAt)
        {
          OdUInt16 ShapeIndx = OdTvDbTextStyleTableRecordImpl::shapeIndexByName(m_IdAt, m_TextAt);
          if (!ShapeIndx)
            return false;
    ...
    OdTvDbLinetypeTableRecordImpl::resizeDashes(pLTRec, indx + 1); // real count
    ...
    catch (const OdError_TvDuplicateRecordName& err)
    {
      switch (dlt)
      {
      case OdDb::kDltReplace:
        {
          OdTvDbSymbolTableRecordPtr pOldRec = err.existingRecordId().safeOpenObject(OdDb::kForWrite);
