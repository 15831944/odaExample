...
class OdTvViInfo : public OdUCSInfo
{
public:
  OdTvViInfo();
  ...
  void auditViInfo (OdTvDbAuditInfo* pAuditInfo, OdTvDbObject* pObj);
...
protected:
  ...
  // new AC2007 fields
  OdTvDbSoftPointerId   m_background;
  OdTvDbHardPointerId   m_visualStyle;
  OdTvDbSoftPointerId   m_shadePlotId;
  bool                m_bUseDefaultLights; // use default lighting instead OdTvDbLights in this viewport
  OdUInt8             m_defaultLightingType; // OdGiViewportTraits::DefaultLightingType
  double              m_dBrightness, m_dContrast;
  OdTvCmColor           m_ambientLightColor;
  OdTvDbHardOwnershipId m_sun;
  OdInt16             m_gridMajor; // groupcode 61, the frequency of major grid lines compared to minor grid lines

  void composeForLoadRenderMode(OdTvDbObject *pObj, OdDb::SaveType format, OdDb::DwgVersion version);
