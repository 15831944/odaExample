...
class OdTvProxyClassImpl;

class TOOLKIT_EXPORT OdTvProxyClass : public OdRxClass
{
protected:
  OdTvProxyClass(OdTvProxyClassImpl*);
public:
  OdTvProxyClass();
  ODRX_DECLARE_MEMBERS(OdTvProxyClass);
...
typedef OdSmartPtr<OdTvProxyClass> OdProxyClassPtr;
...
{
  OdProxyClassPtr pProxyClass = OdTvProxyClass::cast(pClass);
  if(pProxyClass.get())
    return pProxyClass->getItemClassId();
  return OdUInt16(pClass->isDerivedFrom(OdTvDbEntity::desc()) ? 498 : 499);
}
