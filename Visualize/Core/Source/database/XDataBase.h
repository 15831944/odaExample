...
#define StaticXDataR15Iterator OdStaticRxObject<OdObjectWithImpl<OdTvXDataIterator, OdTvXDataIteratorImpl> >
#define StaticXDataR21Iterator OdStaticRxObject<OdObjectWithImpl<OdTvXDataIterator, OdTvXDataR21IteratorImpl> >
...
public:
  ...
  public:
    ...
    void initReadIterator(OdTvXDataIteratorImpl& i, bool treatLayerNameAsHandle = false) const
    {
    ...
    OdXDataIteratorPtr getReadIterator(bool treatLayerNameAsHandle = false) const
    {
      ...
      if (m_pXData->isInR15Format())
        p = OdRxObjectImpl<OdObjectWithImpl<OdTvXDataIterator, OdTvXDataIteratorImpl> >::createObject();
      else
        p = OdRxObjectImpl<OdObjectWithImpl<OdTvXDataIterator, OdTvXDataR21IteratorImpl> >::createObject();
      initReadIterator(*OdTvXDataIteratorImpl::getImpl(p), treatLayerNameAsHandle);
      return p;
    }
    OdXDataIteratorPtr getWriteIterator(OdBinaryData* bin, bool bXDataIsInR15Format, bool treatLayerNameAsHandle) const
    {
      ...
      if (bXDataIsInR15Format)
        ...
        p = OdRxObjectImpl<OdObjectWithImpl<OdTvXDataIterator, OdTvXDataR21IteratorImpl> >::createObject();
      OdTvXDataIteratorImpl::getImpl(p)->init(bin);
      OdTvXDataIteratorImpl::getImpl(p)->setTreatLayerNameAsHandle(treatLayerNameAsHandle);
      return p;
    }
  ...
  void convertToFormat(bool bToR15Format, OdDbBaseDatabase* pDb)
  {
    ...
    OdBinaryData bin;
    OdTvXDataIteratorImpl::getImpl(pDst)->init(&bin);
    ...
    {
      itemSrc.initReadIterator(*OdTvXDataIteratorImpl::getImpl(pSrc));
      pDst->start();
      bin.clear();
      itemDst.setAppId(itemSrc.getAppId(pDb));
      while (!pSrc->atEndOfApp())
      {
        OdTvResBufPtr pNext = pSrc->readItem(pDb);
        if (OdTvDxfCode::_getType(pNext->restype()) == OdTvDxfCode::Unknown)
          break;
  ...
  bool nextItem(Position& pos, Item& item) const
  {
    ...
    {
        OdTvDbObjectId id;
        memcpy(&id, pBuff, sizeof(OdTvDbObjectId));
        ODA_ASSERT(!id.isNull());
        item.setAppId(id);
        pBuff += sizeof(OdTvDbObjectId);
    }
    else
    {
  ...
  void setNextItem(Position& pos, Item& item, OdDbBaseDatabase* pDb)
  {
    ...
    if (item.m_nDataSize > 0)
    {
      ...
      if(m_bAppAsId)
      {
        nItemSize = sizeof(OdTvDbObjectId) + item.m_nDataSize + sizeof(OdUInt16);
        pBuff = itemResize(pos, nItemSize);
        OdTvDbObjectId id = item.getAppId(pDb);
        if (id.isNull())
        {
        ...
        //nItemSize = sizeof(OdTvDbObjectId) + item.m_nDataSize + sizeof(OdUInt16);
        //pBuff = itemResize(pos, nItemSize);
        memcpy(pBuff, &id, sizeof(OdTvDbObjectId));
        pBuff += sizeof(OdTvDbObjectId);
      }
      else
      {
  ...
  bool find(const OdTvDbObjectId &appId, Item& item, OdTvDbDatabase* pDb) const
  {
    if (m_bAppAsId)
    {
      Position pos = firstItemPos();
    ...
    {
      return find(OdTvDbSymUtil::getSymbolName(appId), item);
    }
    return false;
  }
