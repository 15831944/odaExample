...
// OdTvDbDxfLoader.h - DXF parser interface
...
public:
  ...
  OdTvDbDxfFiler * m_pFiler;
};

typedef OdSmartPtr<DxfLoadResolver> DxfLoadResolverPtr;


class OdTvDbDxfLoader : public OdTvDbFilerController
{
  class DXFClassItem
  {
    OdString      m_key;
...
private:
  OdTvDbObjectId                  m_objectBeingLoading;
  int                           m_nProgress;
  OdUInt64                      m_nMax;
  DxfName2RxClassMap            m_RxClassMap;
  OdSmartPtr<OdTvDbCommonDxfFilerImpl> m_pFiler;
...
public:
  ...
  OdTvDbObjectId objectBeingLoading() { return m_objectBeingLoading; }
  virtual void addResolver(DxfLoadResolver* pRes);
  void processResolvers();  
  virtual void setFiler(OdTvDbDxfFiler* pFiler);
  static void addResolver(DxfLoadResolver* pRes, OdTvDbDxfFiler* pFiler)
  {
    OdTvDbFilerController* pLoader = pFiler->controller();
    if (pLoader)
    {
...
private:
  ...
  void adjustXrefDependentSTRs(OdTvDbSymbolTable * pBlockTable, OdTvDbObjectId idTbl);

  void setHeaderVar(const OdString& varName);
  OdDbHandle getObjectHandle();
  OdTvDbObjectPtr createObject(const OdString& dxfName);
  void loadDrawing();
  ...
  // AC27 support
  void loadDsData();
  OdTvDs::ObjectPtr loadDsObject(OdUInt64 offset);
  OdTvDs::ObjectPtr createDsObject(const OdString& dxfName);
protected:
  OdTvDbDxfLoader();
public:
  ODRX_DECLARE_MEMBERS(OdTvDbDxfLoader);
  ...
  void pmMeterProgress()
  {
    ...
    {
      OdTvDbFilerController::pmMeterProgress();
    }
  }
  ~OdTvDbDxfLoader();
  ...
  void endDbLoading();
  TOOLKIT_EXPORT OdTvDbObjectPtr loadObject(OdUInt64 offset);

  virtual void getPreviewImg(OdThumbnailImage* pPreview);

//  virtual void setAuditInfo(OdTvDbAuditInfo* pAuditInfo) {m_pAuditInfo = pAuditInfo;}
//  virtual OdTvDbAuditInfo* getAuditInfo(void) { return m_pAuditInfo; }

  virtual OdTvDbDxfLoader* castToDxfLoader() { return this; }
};
