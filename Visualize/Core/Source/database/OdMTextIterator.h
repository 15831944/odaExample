...
public:
    ...
    , m_DecShift(0.0)
  {
	...
	void calculateSize(TextProps& tp, OdTvDbDatabase* pDb);
...
public:
  ...
  OdMTextFragmentData() :
    ...
    , m_Alignment(0)
    {}
  void calculateExtents(OdTvDbDatabase* pDb);
  ...
  OdString getWSpace(double dSpace, OdTvDbDatabase* pDb, bool bR15);
};
...
public:
  ...
  double endingSpace();
	...
	void   setAsLineBreak();
	void   calculateSize(OdTvDbDatabase* pDb, bool bRaw = true);
	void   drawAsStacked(OdGiWorldDraw* pWd, OdGiTextStyle& ts, const OdGeMatrix3d& cordSys);
  double drawAsNonBreaking(OdGiWorldDraw* pWd, OdTvDbDatabase* pDb,  OdGiTextStyle& ts, const OdGeMatrix3d& cordSys, bool bRaw );
  ...
  double getNonBreakingFragments(OdTvDbDatabase* pDb, FragmentsArray* fragments, bool bRaw ) const;
  void   calculateScorePos();
  OdString getSpaces(double dSpace, OdTvDbDatabase* pDb);
  bool   isHasText();
  OdString getString()
  {
...
public:
	OdMTextIterator(OdTvDbDatabase* pDb, const OdString& str,
...
public:
  ...
  void addWordProp(TextProps* word, OdTvDbDatabase* pDb);
  ...
  // void calculateSize(OdTvDbDatabase* pDb);
...
public:
  ...
  OdString getContentString(OdMTextFragmentData& style, OdTvDbDatabase *pDb, bool bR15 = false);
...
public:
  ...
  void breakToLines(OdComplexWordsArray& words, const OdMTextRendererData* data, TextPropsList* wordList, OdTvDbDatabase* pDb);
  void breakToLinesV(OdComplexWordsArray& words, const OdMTextRendererData* data, TextPropsList* wordList, OdTvDbDatabase* pDb);
...
#ifdef TD_UNICODE
DBENT_EXPORT OdAnsiString convertMTextToDwgCodePage(const OdString& str, OdTvDbDatabase* pDb, const OdTvDbObjectId& textStyleId);
DBENT_EXPORT OdString correctMTextCoding(const OdString& str, OdTvDbDatabase* pDb, const OdTvDbObjectId& textStyleId);
#else
OdString convertMTextToCodePage(const OdString& str, OdTvDbDatabase* pDb, const OdTvDbObjectId& textStyleId, OdCodePageId  from, OdCodePageId  to);
#endif
