...
namespace OdTvDs
{
  ...
  public:
    ...
    virtual OdStreamBufPtr GetData(OdTvDbDwgFiler* pFiler = NULL) const = 0;
  };
  ...
  public:
    ...
    virtual OdStreamBufPtr GetData(OdTvDbDwgFiler* pFiler = NULL) const
    {
...
typedef std::multimap<OdDbHandle, OdTvDs::DataLocatorPtr> OdDsRecords;
...
namespace OdTvDs
{
  ...
  public:
    ...
    void addDsRecord(RecordType type, const OdDbHandle& h, OdStreamBufPtr& pBinStream)
    {
      ...
      dsRecords(type).insert( std::pair<const OdDbHandle, OdSharedPtr<OdTvDs::DataLocator> >(h, pDataLocator));
    }
    ...
    OdStreamBufPtr extractDsAcisData(RecordType type, OdDbHandle h)
    {
      ...
      else if (findresult.first != dsRecs.end())
      {
    ...
    OdArray<OdTvDs::SchemaPtr> m_schemas;
    typedef std::map<OdUInt32, OdTvDs::SchemaPtr> SchemasMap;
    SchemasMap m_schemasMap;
    OdArray<OdTvDs::SchemaAttributeData> m_attributeData;
    OdArray<OdTvDs::SchemaSearchData> m_schemaSearch;
    OdStaticRxObject<OdTvFlatFiler> filer;
  private:
