...
/** \details
    This class represents Lightweight Polyline entities in an OdTvDbDatabase instance.

    \sa
    TD_Db

    OdTvDbPolyline entities differ from OdTvDb2dPolyline entities as follows:
    * OdTvDbPolyline entities are stored as single objects, thereby improving
       performance and reducing overhead compared to OdTvDb2DPolyline objects.
    * Curve fitting and Spline fitting of OdTvDbPolyline entities are not supported.  
    
    \remarks
    The number of vertices in an OdTvDbPolyline must be at least two. 
...
*/
class DBENT_EXPORT OdTvDbPolyline : public OdTvDbCurve
{
public:
  DWGMAP_DECLARE_MEMBERS(OdTvDbPolyline);

  OdTvDbPolyline();

  /** \details
    Fills this OdTvDbPolyline entity with data from the specified OdTvDb2dPolyline entity.  
    
    \param pSource [in]  Pointer to the source OdTvDb2dPolyline entity.
    \param transferId [in]  True to do a handOverTo() between the source OdTvDb2dPolyline 
                   entity and this OdTvDbPolyline entity.  

    \remarks
    If and only if transferId is true, 
      * This OdTvDbPolyline entity will be made database resident.
      * It will assume the objectId, handle, extended entity data, extension
         dictionary, and reactors of the source OdTvDb2dPolyline entity
      * The source OdTvDb2dPolyline entity will be deleted.
            
    \remarks 
    The source OdTvDb2dPolyline entity
    ...
    eIllegalEntityType     *pSource is not an OdTvDb2dPolyline
    eNotApplicable         *pSource is not k2dSimplePoly or there is extended
                           entity data attached to a vertex.
    </table>
  */
  OdResult convertFrom(
    OdTvDbEntity* pSource, 
    bool transferId = true);

  /** \details
    Fills the specified OdTvDb2dPolyline with data from this OdTvDbPolyline entity.

    \param pDest [in]  Pointer to the destination OdTvDb2dPolyline entity.
    \param transferId [in]  True to do a handOverTo() between this OdTvDbPolyline entity 
    and the destination OdTvDb2dPolyline entity.  

    \remarks
    If and only if transferId is true,
      * The destination OdTvDb2dPolyline entity will be made database resident.
      * It will assume the objectId, handle, extended entity data, extension
         dictionary, and reactors of this OdTvDbPolyline entity. 
      * This OdTvDbPolyline entity will made non- database resident, and may be deleted with the C++ delete operator.
  ...
  */
  ...
  virtual OdResult subGetTransformedCopy(
    const OdGeMatrix3d& mat, OdTvDbEntityPtr& pCopy) const ODRX_OVERRIDE;
  ...
  virtual OdResult dwgInFields(
    OdTvDbDwgFiler* pFiler) ODRX_OVERRIDE;

  virtual void dwgOutFields(
    OdTvDbDwgFiler* pFiler) const ODRX_OVERRIDE;

  virtual OdResult dxfInFields(
    OdTvDbDxfFiler* pFiler) ODRX_OVERRIDE;

  virtual OdResult dxfInFields_R12(
    OdTvDbDxfFiler* pFiler) ODRX_OVERRIDE;

  virtual void dxfOutFields(
    OdTvDbDxfFiler* pFiler) const ODRX_OVERRIDE;
  ...
  */
  ...
  TD_USING(OdTvDbCurve::getFirstDeriv);
  TD_USING(OdTvDbCurve::getSecondDeriv);
...
/** \details
    This template class is a specialization of the OdSmartPtr class for OdTvDbPolyline object pointers.
*/
typedef OdSmartPtr<OdTvDbPolyline> OdTvDbPolylinePtr;
