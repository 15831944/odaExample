...
public:  
  ...
  */
  virtual OdResult createModeler(OdArray<OdModelerGeometryPtr> &models, 
    ...
    OdTvDbAuditInfo* pAuditInfo = NULL,
  ...
  /** \details
    ...
    \param models [in]  Array of SmartPointers to the OdTvDbModelerGeometry objects to be written.
  ...
  */
  ...
  /** \details
    ...
    \param entities [in]  Array of SmartPointers to the OdTvDbEntity objects to be written.
    \param pStreamBuf [in]  Pointer to the StreamBuf object to which the data are to be written.   
    \param typeVer [in] Type and version of ACIS data to write.
    \param standardSaveFlag [in] Controls the saving and restoring of use count data in the save file.
	...
	\remarks
    Color attributes are also added to the SAT/SAB file from the specified OdTvDbEntity objects.
    Only the following OdTvDbEntity types are valid: OdTvDb3dSolid, OdTvDbBody, and OdTvDbRegion/
  */
  virtual OdResult createSat(const OdTvDbEntityPtrArray &entities, 
    OdStreamBuf* pStreamBuf, 
    AfTypeVer typeVer, 
    bool standardSaveFlag = true) = 0;
