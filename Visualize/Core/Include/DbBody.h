...
/** \details
  This class represents 3D body entities in an OdTvDbDatabase instance.

  \sa
  TD_Db
  
  \remarks
  An OdTvDbBody entity is a wrapper for an ACIS model and cannot be
  represented as an OdTvDb3dSolid entity or an OdTvDbRegion entity.
  
  <group OdTvDb_Classes> 
*/
class DBENT_EXPORT OdTvDbBody : public OdTvDbEntity
{
public:
  DWGMAP_DECLARE_MEMBERS(OdTvDbBody);
  ...
  */
  static OdResult acisOut(
    ...
    const OdTvDbEntityPtrArray& solids, 
  ...
  */
  ...
  /** \details
    ...
    Returns eOk if successful, or an appropriate error code if not.  
  */
  virtual OdResult subGetGsMarkersAtSubentPath(const OdTvDbFullSubentPath& subPath, OdGsMarkerArray& gsMarkers) const;

  /** \details
    Returns the array of OdTvDbFullSubentPath object(s) by the specified GS marker.
    Each OdTvDbFullSubentPath object represents a subentity.
    ...
    \param subentPaths [out] Array of OdTvDbFullSubentPath object(s) generated from the GS marker. 
  ...
  */
  static OdResult setSubentPath(OdBrEntity &ent, OdTvDbFullSubentPath& subpath);
  ...
  /** \details
    ...
    Returns eOk if successful, or an appropriate error code if not.  
  */
  static OdResult generateMesh(const OdTvDbEntity *pEnt, const OdBrMesh2dFilter & filter, OdBrMesh2d &mesh2d);
};

/** \details
    This template class is a specialization of the OdSmartPtr class for OdTvDbBody object pointers.
*/
typedef OdSmartPtr<OdTvDbBody> OdTvDbBodyPtr;
