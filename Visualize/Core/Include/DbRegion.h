...
/** \details
    This class represents Region entities in an OdTvDbDatabase instance.

    An OdTvDbRegion entity is a wrapper for an ACIS model that represents
    the geometry of the OdTvDbRegion entity.
...
*/
class DBENT_EXPORT OdTvDbRegion : public OdTvDbEntity
{
public:
  DWGMAP_DECLARE_MEMBERS(OdTvDbRegion);
  ...
  */
  ...
  /** \details
  Creates OdTvDbRegion entities from the closed loops defined by the specified curve segments.
  
  \param curveSegments  [in]  Array of curve segments. Each curve segment must be one of the following: OdTvDb3dPolyline, OdTvDbArc, OdTvDbCircle, OdTvDbEllipse, OdTvDbLine, OdTvDbPolyline, or OdTvDbSpline.
  ...
  to an OdTvDbDatabase or to delete them.
  */
  ...
  */
  ...
  TD_USING(OdTvDbEntity::getPlane);
...
* kBoolSubtract - subtracts the region from the first one.
  ...
  for other modelers. 
  */
  virtual OdResult booleanOper(OdDb::BoolOperType operation, OdTvDbRegion* otherRegion);
...
/** \details
This template class is a specialization of the OdSmartPtr class for OdTvDbRegion object pointers.
*/
typedef OdSmartPtr<OdTvDbRegion> OdTvDbRegionPtr;
