...
class OdTvTfBinarySchemaIteratorImpl;

/** \details
This class is designed for working with XML schemes for revision control.
*/
class TFDLL_EXPORT OdTvTfSchemaManager : public OdTfSchemaManagerInterface
{
public:
  ODRX_DECLARE_MEMBERS(OdTvTfSchemaManager);
  OdTvTfSchemaManager() : m_appServices(NULL) {};
  virtual ~OdTvTfSchemaManager() {};
  ...
  */
  ...
  /** \details
  Set HostAppServices that will be used bu OdTvTfSchemaManager.
  \param pHostAppServices [in] pointer to OdTvDbHostAppServices.
  \note setAppServices used in ::odTfInitialize(pHostApp);
  */
  void setAppServices(OdTvDbHostAppServices* pHostAppServices);
  /** \details
...
protected:
  ...
  OdTvDbHostAppServices* m_appServices;
};

typedef OdSmartPtr<OdTvTfSchemaManager> OdTvTfSchemaManagerPtr;

/** \details
Used to register basic(core) scheme.
\param pAppServices [in] OdTvDbHostAppServices.
\note pAppServices can't be NULL
*/
TFDLL_EXPORT bool odTfInitialize(OdTvDbHostAppServices* pAppServices);
