...
DBIO_EXPORT OdSmartPtr<OdGiWorldDraw> odtvdbBeginProxyGraphics( OdTvDbDatabase* pDbCtx );
DBIO_EXPORT void odtvdbEndProxyGraphics( OdGiWorldDraw* pWdSaver, OdUInt8Array& graphics );
...
inline void
OdRemoteGeomProxy::setDatabase(OdTvDbDatabase* pDb) {
  m_pDb = pDb;
}

inline void OdRemoteGeomProxy::begin() {
  m_pWd = ::odtvdbBeginProxyGraphics( m_pDb );
}
    ...
    if(hBufferWritten != NULL && hBufferProcessed != NULL)
    {
      ...
      if(hIPCFile != NULL)
      {
        ...
        if(pBuffer != NULL)
        {
          ...
          ::odtvdbEndProxyGraphics( m_pWd, graphics );
