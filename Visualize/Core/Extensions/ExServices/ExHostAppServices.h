...
*/
class ExHostAppServices : public OdTvDbHostAppServices2, public OdDbHostAppProgressMeter
{
  ...
  /*
  class DwgCollection : public OdStaticRxObject<OdTvDbDatabaseReactor>
  {
    std::map<OdString, OdTvDbDatabase*> m_loadedDwgs;
    void goodbye(const OdTvDbDatabase* pDb);
  public:
    OdTvDbDatabase* lookUp(const OdString& keyFileName);
    void add(OdTvDbDatabase* pDb);
  }         m_dwgCollection;
  */
...
public:
  ...
  TD_USING(OdTvDbHostAppServices2::warning); 
#endif
  ...
  */
  ...
  TD_USING(OdTvDbHostAppServices2::readFile);
};
