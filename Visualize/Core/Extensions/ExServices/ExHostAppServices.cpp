...
/*
void ExHostAppServices::DwgCollection::goodbye(const OdTvDbDatabase* pDb)
{
  std::map<OdString, OdTvDbDatabase*>::iterator iter = m_loadedDwgs.begin(), end = m_loadedDwgs.end();
  while(iter!=end)
  {
    if(iter->second==pDb)
    {
...
OdTvDbDatabase* ExHostAppServices::DwgCollection::lookUp(const OdString& keyFileName)
{
  std::map<OdString, OdTvDbDatabase*>::iterator iter = m_loadedDwgs.find(keyFileName);
...
void ExHostAppServices::DwgCollection::add(OdTvDbDatabase* pDb)
{
  ODA_ASSERT(lookUp(pDb->getFilename())==0);
  m_loadedDwgs.insert(std::map<OdString, OdTvDbDatabase*>::value_type(pDb->getFilename(), pDb));
  pDb->addReactor(this);
}
*/
OdTvDbDatabasePtr ExHostAppServices::readFile(const OdString& fileName,
    bool bAllowCPConversion, bool bPartial, Oda::FileShareMode shmode, const OdPassword& password)
{
  OdTvDbDatabasePtr pRes;// = m_dwgCollection.lookUp(fileName);
  //if(pRes.isNull())
  {
    pRes = OdTvDbHostAppServices2::readFile(fileName, bAllowCPConversion, bPartial, shmode, password);
