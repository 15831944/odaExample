...
public:
  ...
  {
    OdTvDbDatabase* pDb = OdTvDbDatabase::cast(m_pRxDb).get();
...
protected:
  ...
  virtual void subViewportDraw(OdGiViewportDraw* pVd) const ODRX_OVERRIDE
  {
    if (m_bApplicable)
    {
      pVd->subEntityTraits().setTransparency(OdTvCmTransparency(0.5));
...
OdTvDbUnitsFormatter& ExDbCommandContext::formatter()
{
  return database()->formatter();
}

OdTvDbCommandContextPtr ExDbCommandContext::createObject(OdEdBaseIO* pIOStream, OdTvDbDatabase* pDb) {
  return ExDbCommandContext::createObject(pIOStream, static_cast<OdRxObject*>(pDb));
}

OdTvDbCommandContextPtr ExDbCommandContext::createObject(OdEdBaseIO* pIOStream, OdRxObject* pRxDb)
{
  OdTvDbCommandContextPtr pRes = OdRxObjectImpl<ExDbCommandContext, OdTvDbCommandContext>::createObject();
...
OdSelectionSetPtr ExDbCommandContext::box(const OdGePoint3d& pt1, OdDbVisualSelection::SubentSelectionMode ssm)
{
  ...
  return OdTvDbSelectionSet::select(database()->activeViewportId(), 2, pts, OdDbVisualSelection::kBox, ssm);
}
                                   ...
                                   bool bDoIt) const //  = true
{
  ...
  OdTvDbDatabase* pDb = NULL;
  OdUInt32 cntr(0);
  for (; !pIter->done(); pIter->next())
  {
    highlight(pIter, bDoIt);
    if ((cntr % DBCC_PAGE_EACH_OBJECT) && (pDb || (pDb = OdTvDbObjectId(pIter->id()).database())))
    {
      pDb->pageObjects();
      cntr = 0;
    }
...
void ExDbCommandContext::highlight(OdSelectionSetIterator* pIter, 
                                   bool bDoIt) const //  = true
{
  OdTvDbObjectPtr pObj = OdTvDbObjectId(pIter->id()).safeOpenObject();
  if (!pObj->isKindOf(OdTvDbEntity::desc()))
    return;
  OdTvDbEntityPtr pEnt = pObj;
  if (pIter->subentCount())
  {
    for (OdUInt32 se = 0; se < pIter->subentCount(); se++)
    {
      OdTvDbFullSubentPath subEntPath;
...
inline bool isOverallVp(const OdTvDbEntity* pObj)
{
  OdTvDbViewportPtr pVp = OdTvDbViewport::cast(pObj);
...
static OdUInt32 getGripObjLimit(const OdRxObject* cpRxDb)
{
  ...
  if (pBaseAppServices)
  {
    OdTvDbHostAppServices* pAppServices = OdTvDbHostAppServices::cast(pBaseAppServices).get();
    if (pAppServices)
...
bool checkCurrentSpaceForNestedBlocks(OdTvDbEntityPtr pEnt, OdTvDbObjectId BTRId)
{
  OdTvDbBlockTableRecordPtr pBlockTRec = OdTvDbBlockTableRecord::cast(pEnt->ownerId().openObject());
  bool notInCurrentSpace = pEnt->ownerId() != BTRId;

  if (!pBlockTRec.isNull())// TODO AlexeyTyurin
  {
    OdTvDbObjectIdArray idsblk;
    pBlockTRec->getBlockReferenceIds(idsblk);

    for (unsigned int i = 0; i < idsblk.length(); i++)
    {
      OdTvDbEntityPtr blkRefer = OdTvDbEntity::cast(idsblk[i].openObject());
      if (!blkRefer.isNull())
      {
                               ...
                               OdEdUserIO* pIO) const //  = NULL
{
  ...
  OdUInt32 numResEntities = pRes->numEntities(),
           ...
           limitGripObj = getGripObjLimit(pRxDb);
  ...
  OdTvDbObjectId BTRId;
  OdTvDbDatabase* pDb = OdTvDbDatabase::cast(pRxDb).get();
  if (pDb && !GETBIT(options, OdEd::kSelAllowInactSpaces))
  {
    OdTvDbObjectId idMS = pDb->getModelSpaceId();
    BTRId = pDb->getActiveLayoutBTRId();
    if (BTRId != idMS)
    { // Paper Space Layout
      OdTvDbLayoutPtr pLayout = pDb->currentLayoutId().safeOpenObject();
  ...
  for (pIter = pSSet->newIterator(); !pIter->done(); pIter->next())
  {
    ...
    if (pDb)
    {
      OdTvDbEntityPtr pEnt = OdTvDbEntity::cast(OdTvDbObjectId(id).openObject());

      if (pEnt.get())
      {
        bool notInCurrentSpace = checkCurrentSpaceForNestedBlocks(pEnt, BTRId);
        OdTvDbLayerTableRecordPtr l = pEnt->layerId().openObject();
...
class ExSelectionMethods
{
  protected:
    ...
    static bool isEntityVisible(OdTvDbEntityPtr pEntity)
    {
      if (!pEntity.isNull() && pEntity->visibility() == OdDb::kVisible)
      {
        OdTvDbLayerTableRecordPtr pLayer = OdTvDbLayerTableRecord::cast(pEntity->layerId().openObject(OdDb::kForRead));
    ...
    static OdTvDbObjectId selectionBlockId(const OdTvDbObjectId& vpId)
    {
      OdTvDbDatabase* pDb = vpId.database();
      OdTvDbViewportPtr pVp = OdTvDbViewport::cast(vpId.safeOpenObject());
      if (!pVp.isNull())
  ...
  public:
    static void selectLast(OdTvDbDatabase *pDb, OdTvDbObjectIdArray &ids)
    {
      OdTvDbObjectId blockId = selectionBlockId(pDb->activeViewportId());
      OdTvDbBlockTableRecordPtr pBlockRec = OdTvDbBlockTableRecord::cast(blockId.openObject(OdDb::kForRead));
      OdTvDbObjectIteratorPtr pIt = pBlockRec->newIterator(false);
      OdUInt32 cntr(0);
      while (!pIt->done())
      {
        OdTvDbEntityPtr pEntity = pIt->entity(OdDb::kForRead);
    ...
    static void selectAll(OdTvDbDatabase *pDb, OdTvDbObjectIdArray &ids)
    {
      OdTvDbObjectId blockId = selectionBlockId(pDb->activeViewportId());
      OdTvDbBlockTableRecordPtr pBlockRec = OdTvDbBlockTableRecord::cast(blockId.openObject(OdDb::kForRead));
      OdTvDbObjectIteratorPtr pIt = pBlockRec->newIterator();
      OdUInt32 cntr(0);
      while (!pIt->done())
      {
        OdTvDbEntityPtr pEntity = pIt->entity(OdDb::kForRead);
    ...
    static void selectRect(OdSmartPtr<OdEdBaseIO> pIoStream, OdTvDbUserIO* pIO, OdDbVisualSelection::SubentSelectionMode &ssm, OdGePoint3dArray &pts)
    {
      pts.resize(2);
    ...
    static void selectPolyline(OdSmartPtr<OdEdBaseIO> pIoStream, OdTvDbUserIO* pIO, OdDbVisualSelection::SubentSelectionMode &ssm, OdGePoint3dArray &pts)
    {
      pts.clear();
    ...
    static void selectPolygon(OdSmartPtr<OdEdBaseIO> pIoStream, OdTvDbUserIO* pIO, OdDbVisualSelection::SubentSelectionMode &ssm, OdGePoint3dArray &pts)
    {
      pts.clear();
...
{
  OdTvEdSSetTracker* pTracker = OdTvEdSSetTracker::cast(pBaseTracker).get();
  OdTvDbUserIO* pIO = dbUserIO();
  OdTvDbDatabase* pDb = database();
  OdDbSelectionSetPtr pRes = OdTvDbSelectionSet::createObject(pDb);
...
selection_loop_start:
    ...
    try
    {
      try
      {
        ...
        pSSet = OdTvDbSelectionSet::select(database()->activeViewportId(), 1, &pts[0], OdDbVisualSelection::kPoint, ssm);
      ...
      catch(const OdEdOtherInput& inp)
      {
        ...
        {
          pSSet = OdTvDbSelectionSet::createObject(pDb);
          for (OdTvResBufPtr pRb(result()); pRb.get(); pRb = pRb->next())
          {
    ...
    case  0: // Window
      ...
      pSSet = OdTvDbSelectionSet::select(database()->activeViewportId(), 2, pts.getPtr(), OdDbVisualSelection::kWindow, ssm);
      break;
    case  1: // Last
      {
        OdTvDbObjectIdArray ids;
        pSSet = OdTvDbSelectionSet::createObject(database());
    ...
    case  2: // Crossing
      ExSelectionMethods::selectRect(m_pIoStream, pIO, ssm, pts);
      pSSet = OdTvDbSelectionSet::select(database()->activeViewportId(), 2, pts.getPtr(), OdDbVisualSelection::kCrossing, ssm);
      break;
    case  3: // BOX
    ...
    case  5: // Fence
      ...
      if (pts.size() > 1)
        pSSet = OdTvDbSelectionSet::select(database()->activeViewportId(), (int)pts.size(), pts.getPtr(), OdDbVisualSelection::kFence, ssm);
      break;
    case  6: // WPolygon
      ExSelectionMethods::selectPolygon(m_pIoStream, pIO, ssm, pts);
      if (pts.size() > 2)
        pSSet = OdTvDbSelectionSet::select(database()->activeViewportId(), (int)pts.size(), pts.getPtr(), OdDbVisualSelection::kWPoly, ssm);
      break;
    case  7: // CPolygon
      ExSelectionMethods::selectPolygon(m_pIoStream, pIO, ssm, pts);
      if (pts.size() > 2)
        pSSet = OdTvDbSelectionSet::select(database()->activeViewportId(), (int)pts.size(), pts.getPtr(), OdDbVisualSelection::kCPoly, ssm);
      break;
    case  8: // Group
      bNYI = true;
      break;
                                   ...
                                   OdEdRealTracker* pTracker)
{
  ...
  try
  {
    ...
    if (pTracker && vt == OdTvResBuf::kDxfXdDist && GETBIT(options, OdEd::kGdsSignedDist))
                                   ...
                                   OdEdRealTracker* pTracker)
{
  ...
  return getReal(prompt, options, defVal, OdTvResBuf::kDxfXdDist, sKeywords, pTracker);
}
                                    ...
                                    OdEdRealTracker* pTracker)
{
  ...
  {
    if (OdTvDbDatabase::cast(baseDatabase()).get())
                               ...
                               OdEdIntegerTracker* pTracker)
{
  ...
  for (;;)
  {
    ...
    catch (const OdError& e)
    {
      ...
      putError(sKeywords.find(L' ') < 0 ? L"Requires integer value."
                                        : L"Requires integer value or optional keyword.");
    }
  }
}
                                   ...
                                   OdEdIntegerTracker* pTracker)
{
  ...
  for(;;)
  {
    ...
    try
    {
      // ODE: overriden getString is a key method to support auto-urls by keywords in prompt
      sInput = getString(prompt, options,
                         bDef ? kwIndex.at(defVal).keyword() : OdString::kEmpty, sKeywords);
    }
...
{
  OdString sInput;
  OdTvDbDatabasePtr db = OdTvDbDatabase::cast(baseDatabase());
  OdTvDbHostAppServices* hostAppSvcs = db.isNull() ? 0 : db->appServices();
  if(hostAppSvcs) {
    if(!hostAppSvcs->getFILEDIA()) {
      sInput = OdTvDbUserIO::getFilePath(prompt, options, dialogCaption, defExt, fileName, filter, keywords, pTracker);
      if(sInput != OD_T("~"))
      {
        sInput.trimLeft(' ');
        sInput.trimRight(' ');
  ...
  return OdTvDbUserIO::getFilePath(prompt, options, dialogCaption, defExt, fileName, filter, keywords, pTracker);
}
...
OdTvCmColor ExDbCommandContext::getColor(const OdString& sPrompt,
                                       int options,
                                       const OdTvCmColor* pDefVal,
                                       const OdString& sKeywords,
                                       OdEdColorTracker* pTracker)
{
  ...
  OdTvCmColor* pColor = dynamic_cast<OdTvCmColor*>(color.get());
                                                          ...
                                                          OdEdColorTracker* pTracker)
{
  ...
  for(;;)
  {
    OdString sInput = getString(prompt, options,
    ...
    if (lspMod.isNull())
    {
      ...
      if(s.left(7).makeLower() == OD_T("handent"))
      {
        ...
        while (odSScanf(s.trimLeft(L" \r\n\t\","), L"%" PRIx64W, &handle))
        {
          ...
          if (s[0] != L')' && s[0] != L',')
            break;
          OdTvResBufPtr pRbNext = OdTvResBuf::newRb(OdTvResBuf::kRtHandle);
