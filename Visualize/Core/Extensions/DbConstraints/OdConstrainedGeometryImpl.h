...
public:
  ...
  */
  virtual OdResult init(const OdTvDbObjectId depId);
  ...
  */
  OdTvDbObjectId geomDependencyId() const;
  ...
  */
  virtual OdResult getFullSubentPaths(OdTvDbFullSubentPathArray& subentPaths) const;
...
protected:
  ...
  */
  OdTvDbAssocGeomDependencyPtr geomDependency(const OdTvDbObjectId geomDepId,
                                              OdDb::OpenMode mode = OdDb::kForRead);

  OdTvDbAssocGeomDependencyPtr geomDependency(const OdTvDbObjectId geomDepId) const;

  /** \details
    Returns the associated geometry object.
  */
  OdTvDbObjectPtr geomObject(const OdTvDbObjectId geomDepId, OdDb::OpenMode mode = OdDb::kForRead);

  /** \details
    Converts the transformation matrix to WCS.
  */
  void matrToWrdCoord(const OdTvDbObjectId geomDepId, OdGeMatrix3d &matr);

  /** \details
    Returns an array of full subent paths.
  */
  OdResult getFullSubentPath(OdTvDbFullSubentPath& path) const;

  /*!DOM*/
  OdTvDbObjectId                m_idDependency;
  /*!DOM*/
