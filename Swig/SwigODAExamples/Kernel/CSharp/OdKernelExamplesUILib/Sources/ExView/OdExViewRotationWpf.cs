...
{
  public class OdExViewRotationWpf : OdExViewRotation, OdExViewFunctionalityInterface
  {
    private UserControl m_targetControl;

    public OdExViewRotationWpf(UserControl TargetControl) : base()
    {
    ...
    private void M_targetControl_MouseDown(object sender, System.Windows.Input.MouseButtonEventArgs e)
    {
      ...
      this.OnStartRotationHandler((int)point.X, (int)point.Y);
    }

    private void M_targetControl_MouseMove(object sender, System.Windows.Input.MouseEventArgs e)
    {
      Point point = e.GetPosition(m_targetControl);
      OnMoveRotationHandler((int)point.X, (int)point.Y);
    }
