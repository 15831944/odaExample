...
{
  public class OdExFileStream : OdStreamBuf
  {
    private System.IO.FileStream _fs;
    public OdExFileStream(System.IO.FileStream fs) { _fs = fs; }

    public override uint getBytesByNum(OdUInt8Array data, ulong startPos, uint numBytes)
    {
      byte[] rawData = new byte[numBytes];
      _fs.Read(rawData, 0, (int)numBytes);
      data = new OdUInt8Array(rawData);
      return numBytes;
    }
  }
}