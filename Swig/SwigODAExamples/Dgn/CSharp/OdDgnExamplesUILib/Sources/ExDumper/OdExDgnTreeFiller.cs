...
namespace OdDgnExamplesUIMgdLib.ExDumper
{
  ...
  public class OdExDgnTreeFiller
  {
    private TableMap m_nonDBROItems = new TableMap();
    private TreeView m_pObjTree;
    private OdDgDatabase m_pDb;
    private TreeNode m_dbTreeItem;

    public OdExDgnTreeFiller(OdDgDatabase pDb, TreeView pObjTree)
    {
      m_pObjTree = pObjTree;
      m_pDb = pDb;
    ...
    public TreeNode addElement(OdDgElement pElm, TreeNode hParent, TreeNode hInsertAfter)
    {
      if (pElm != null)
      {
        ...
        TreeNode hItem = addTreeItem(sName, hParent, hInsertAfter);
    ...
    private void addSubElements(OdDgElement pElm, TreeNode hParent)
    {
