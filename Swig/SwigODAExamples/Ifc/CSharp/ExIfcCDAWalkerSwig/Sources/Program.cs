...
namespace ExIfcCDAWalkerSwigMgd
{
  class Program
  {
    ...
    private static String sUsage =
      "\n\nusage: ExIfcCDAWalkerSwigMgd <filename>" +
      " \n<filename> - input .ifc file\n";

    static void Main(string[] args)
    {
      Console.WriteLine("\nExIfcCDAWalkerSwigMgd sample program. Copyright (c) 2020, Open Design Alliance\n");
      ...
      IfcCore_Globals.odIfcInitialize(true /* No CDA */, false /* No geometry calculation needed */);

      try
      {
        String ifcFileName = args[0];

        OdIfcFile pIfcFile = services.m_HostAppServices.createDatabase(IfcSchema.kScmUndefined);
        OdResult res = pIfcFile.readFile(ifcFileName);
        OdDAI_Model pModel = pIfcFile.getModel(SdaiAccessMode.sdaiRW);
        ...
        OdExCDAWalker walker = new OdExCDAWalker(new OdExCDATreePrinter());
        walker.run(pIfcFile);
      }
      catch (OdError e)
      {
