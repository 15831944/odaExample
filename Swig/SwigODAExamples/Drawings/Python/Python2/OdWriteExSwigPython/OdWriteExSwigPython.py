...
def main(argv):
    ...
    if(len(sys.argv) < 2):
        ...
        print("Usage: OdWriteExSwigPython.py <dwg_path>")
        sys.exit(2)
    ...
    print("OdWriteExSwigPython developed using {0} ver {1}\n".format(hostApp.product(), hostApp.versionString()))
    print("OdWriteExSwigPython developed using minor major ver {0}\n".format(hostApp.releaseMajorMinorString()))
    ...
    database = hostApp.createDatabase(True, ODA_Kernel.kEnglish)
    pSumInfo = oddbGetSummaryInfo(database)
    pSumInfo.setComments("File was created by OdWriteExSwigPython")
    ...
    print("OdWriteExSwigPython finished")
