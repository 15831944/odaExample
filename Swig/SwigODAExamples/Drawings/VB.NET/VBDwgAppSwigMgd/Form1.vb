...
Public Class Form1
  ...
  Public Sub New()
    ...
    root_transaction = MemoryManager.GetMemoryManager().StartTransaction()
  ...
  Private Sub Form1_Load(sender As Object, e As EventArgs) Handles MyBase.Load
    ...
    sysServ = New RxSystemServicesImpl()
    hostApp = New ExHostAppServices()
    TD_Db.odInitialize(sysServ)
  End Sub

  Private Sub Form1_FormClosing(sender As Object, e As FormClosingEventArgs) Handles MyBase.FormClosing
    MemoryManager.GetMemoryManager().StopTransaction(root_transaction)
  ...
  Private Sub FillLayouts(ByRef aLayouts As OdStringArray, ByVal bAllLayouts As Boolean)
    If True = bAllLayouts Then
      Dim pLayoutDict As OdDbDictionary = CType(pDb.getLayoutDictionaryId().safeOpenObject(), OdDbDictionary)
      Dim pIter As OdDbDictionaryIterator = pLayoutDict.newIterator()
      While False = pIter.done()
        Dim id As OdDbObjectId = pIter.objectId()
        Dim pLayout As OdDbLayout = CType(id.safeOpenObject(), OdDbLayout)
  ...
  Private Sub ExportToPdf_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles ExportToPdf.Click
    ...
    'Dim compressLevel As PRCCompressionLevel = PRCCompressionLevel.kA3DLooseCompression
    'Dim bCompress As Boolean = True
    '      Select Case dlgPdf.ui_Compression
    '          Case 0
    'bCompress = False
    '          Case 2
    'compressLevel = PRCCompressionLevel.kA3DMeddiumCompression
    '          Case 3
    'compressLevel = PRCCompressionLevel.kA3DHighCompression
    '      End Select
    ...
    If bMonochrome = True Then
      ...
      While f < lSize
        Dim pLayout As OdDbLayout = CType(pLayoutDict.getAt(Params.layouts()(f)).safeOpenObject(OpenMode.kForWrite), OdDbLayout)
