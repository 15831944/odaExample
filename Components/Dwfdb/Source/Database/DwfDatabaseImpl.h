...
#endif
  bool m_isDwfx; // _xps
  bool m_bDwf3dOrthoOnly; friend class DWF_TK_Camera; // use true to avoid global GS zooming problem with perspective mode
  bool m_bDwf3dLoadObjectProps;
  OdDwfHostAppServices::Dwf3dModelTransformType m_typeDwf3dModelTransform;
#if defined(__BCPLUSPLUS__) || defined(__BORLANDC__)
protected:
#endif 
  ...
  OdResult proceedSectionByRole(DWFToolkit::DWFSection* pSection, DWFToolkit::DWFPackageReader* pReader,
                                OdArray<DWFCore::DWFInputStream*>& w2dStreams, OdStringArray& mimes, OdArray<OdGeMatrix3d>& globals, // out
                                const OdString& sTitle, DWFToolkit::DWFPaper* pPaper,
                                const wchar_t* pRole = NULL); // for DWFXML::kzRole_Graphics2d
