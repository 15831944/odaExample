...
#ifndef _PRCDEFINES_INCLUDED_
#define _PRCDEFINES_INCLUDED_ 
...
#if defined(WIN32)
#if _MSC_VER >= 1600 || defined(__BCPLUSPLUS__)|| defined(__BORLANDC__)
#include <stdint.h>
#else
...
//const uint32_t OdPrcVersion=7094;   // For Adobe reader 8 or later
const uint32_t OdPrcVersion=8137; // For Adobe reader 9 or later
...
#define PRC_TYPE_SURF_Pipe            ( PRC_TYPE_SURF + 10 )  // Pipe. This surface type is currently not supported and reserved for future use.
...
#define PRC_TYPE_ASM_ProductOccurrence            ( PRC_TYPE_ASM + 10 )  // Basic contruct for assemblies. 
...
#define PRC_TEXTURE_APPLYING_MODE_NONE          0x0000 // let the application choose
#define PRC_TEXTURE_APPLYING_MODE_LIGHTING      0x0001 // use lighting mode
#define PRC_TEXTURE_APPLYING_MODE_ALPHATEST     0x0002 // use alpha test
#define PRC_TEXTURE_APPLYING_MODE_VERTEXCOLOR   0x0004 // combine a texture with one-color-per-vertex mode
...
enum EPRCPictureDataFormat {
  ...
  /** JPEG format.*/
  KEPRCPicture_JPG,               
  /** BITMAP format. Pixel data represented as RGB elements.*/
  KEPRCPicture_BITMAP_RGB_BYTE,   
  /** BITMAP format. Pixel data represented as RGBA elements.*/
  KEPRCPicture_BITMAP_RGBA_BYTE,  
  /** BITMAP format. Pixel data represented as a luminance value.*/
  KEPRCPicture_BITMAP_GREY_BYTE,  
  /** BITMAP format. Pixel data represented with a luminance value and alpha pair.*/
  KEPRCPicture_BITMAP_GREYA_BYTE  
};

/** \details
Enumeration represents the product occurrence's load status. 
Load status determines whether an external resource was available at the moment of creating the .prc file.
*/
...
/** \details
Enumeration represents methods of curve and surface extensions. 
There are two types of extensions: C and G continuous.
*/
enum EPRCExtendType { 
...
\remarks
This value is currently unused and should be set to KEPRCBSplineCurveFormUnspecified.
*/
enum EPRCBSplineCurveForm {           
  /** Unspecified curve form.*/
...
enum EPRCTextureMappingType { 
  ...
  KEPRCTextureMappingType_Unknown /*= 1*/,      
  /** Use the mapping coordinates that are stored on a 3D tessellation object.*/
  KEPRCTextureMappingType_Stored /*= 2*/,       
  /** Retrieve the UV coordinates on the surface as mapping coordinates.*/
  KEPRCTextureMappingType_Parametric /*= 3*/,   
  /** Use the defined Texture mapping operator to calculate mapping coordinates.*/
  KEPRCTextureMappingType_Operator /*= 4*/      
};
...
enum EPRCTextureFunction {            
  /** Let the application choose.*/
  KEPRCTextureFunction_Unknown /*= 1*/,       
  /** Combine lighting with texturing. This is the default value.*/
  KEPRCTextureFunction_Modulate /*= 2*/,      
  /** Replace the object color with texture color data.*/
  KEPRCTextureFunction_Replace /*= 3*/,       
  /** Reserved for future use.*/
  KEPRCTextureFunction_Blend /*= 4*/,         
  /** Reserved for future use.*/
  KEPRCTextureFunction_Decal /*= 5*/          
};

/** \details
Enumeration represents operators used for computing the mapping coordinates.
*/
enum EPRCTextureMappingOperator {           
  /** Default value.*/
...
/** \details
Enumeration represents methods of texture blending. 
Reserved for future use.
*/
...
/** \details
Enumeration represents method of using a texture alpha test.
Reserved for future use. 
*/
enum EPRCTextureAlphaTest {
  /** Default value.*/
  KEPRCTextureAlphaTest_Unknown,    
  /** Reserved for future use.*/
  KEPRCTextureAlphaTest_Never,      
  /** Reserved for future use.*/
  KEPRCTextureAlphaTest_Less,       
  /** Reserved for future use.*/
  KEPRCTextureAlphaTest_Equal,      
  /** Reserved for future use.*/
  KEPRCTextureAlphaTest_Lequal,     
  /** Reserved for future use.*/
  KEPRCTextureAlphaTest_Greater,    
  /** Reserved for future use.*/
  KEPRCTextureAlphaTest_Notequal,   
  /** Reserved for future use.*/
  KEPRCTextureAlphaTest_Gequal,     
  /** Reserved for future use.*/
  KEPRCTextureAlphaTest_Always      
};

// Bit field for graphics behaviour
...
#define PRC_MARKUP_IntegerMask    0x000FFFFF // Integer mask to retrieve sizes.
#define PRC_MARKUP_ExtraDataType  0x03E00000 // Mask to retrieve the integer type of the entity.
...
#define PRC_HCG_NewLoop               0
#define PRC_HCG_EndLoop               1
#define PRC_HCG_IsoPlane              2
#define PRC_HCG_IsoCylinder           3
#define PRC_HCG_IsoTorus              4
#define PRC_HCG_IsoSphere             5
#define PRC_HCG_IsoCone               6
#define PRC_HCG_IsoNurbs              7
#define PRC_HCG_AnaPlane              8
#define PRC_HCG_AnaCylinder           9
#define PRC_HCG_AnaTorus              10
#define PRC_HCG_AnaSphere             11
#define PRC_HCG_AnaCone               12
#define PRC_HCG_AnaNurbs              13
#define PRC_HCG_AnaGenericFace        14
#define PRC_HCG_Line                  0
#define PRC_HCG_Circle                1
#define PRC_HCG_BsplineHermiteCurve   2
#define PRC_HCG_Ellipse               12
#define PRC_HCG_CompositeCurve        13

#define PRC_Teigha_Application_ID_1   0
#define PRC_Teigha_Application_ID_2   0
#define PRC_Teigha_Application_ID_3   0
#define PRC_Teigha_Application_ID_4   0

//Parametric position on the first surface of an intersection curve is filled. 
#define   PRC_INTERSECTION_CROSS_POINT_SURFACE1                 0x0001
//Parametric position on the second surface of an intersection curve is filled. 
#define   PRC_INTERSECTION_CROSS_POINT_SURFACE2                 0x0002
//The crossing point is inside the curve interval.
#define   PRC_INTERSECTION_CROSS_POINT_INSIDE_CURVE_INTERVAL    0x0004

/** \details 
Provides information about an intersection curve limit. 
*/
enum EPRCIntersectionLimitType
{
...
enum EPRCSchemaToken
{
  ...
  /** Parent type of the current object.*/
  ...
  /** Condition block.*/
  EPRCSchema_Else,
  /** Define an instruction block.*/
  EPRCSchema_Block_Start,
  /** Define a versioned instruction block.*/
  ...
  /** Declare and set a global value.*/
  ...
  /** Specific value (see Schema processing).*/
...
#define PRC_MAJOR_VERSION        TD_MAJOR_VERSION
#define PRC_MINOR_VERSION        TD_MINOR_VERSION
#define PRC_COMPANY_NAME_W       TD_COMPANY_NAME_W
#define PRC_COPYRIGHT_W          TD_COPYRIGHT_W
#define PRC_BUILD_COMMENTS_W     PRC_PRODUCT_NAME_W L" " PRC_PRODUCT_VER_STR_W

#endif // _PRCDEFINES_INCLUDED_
