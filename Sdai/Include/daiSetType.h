...
#ifndef _DAI_SET_TYPE_H
#define _DAI_SET_TYPE_H
...
namespace OdDAI {
  ...
  */
  class DAI_EXPORT SetType : public VariableSizeAggregationType
  {
  public:

    //DOM-IGNORE-BEGIN
    ODRX_DECLARE_MEMBERS(SetType);
    //DOM-IGNORE-END

    /** \details
    Creates a new aggregation type object with specified parameters.
    \param elementType [in] A pointer to a data type contained in the set type.
    ...
    */
    static OdSmartPtr<SetType> createObject(
      const BaseType *elementType,
      const BoundPtr &lowerBound,
      const BoundPtr &upperBound = BoundPtr());

    /** \details
    Creates a new set type definition with default parameters.
    */
    SetType() {};
  ...
  /** \details
  A data type that represents a smart pointer to an <link OdDAI::SetType, set type> object.
  */
  typedef OdSmartPtr<SetType> SetTypePtr;
}

#endif // _DAI_SET_TYPE_H
