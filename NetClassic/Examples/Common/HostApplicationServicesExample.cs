...
namespace ODASampleCommon
{

  class HostApplicationServicesExample : HostApplicationServices
  {
    public HostApplicationServicesExample()
    {
    }
    ...
    public override String FindFile(String file, Database db, FindFileHint hint)
    {
      ...
      if ( ext == String.Format(".STB") || ext == String.Format(".CTB"))
        return System.IO.Path.Combine(Environment.GetEnvironmentVariable("DDPLOTSTYLEPATHS"), file);  // return FindConfigFile( String.Format("PrinterStyleSheetDir"), file);
