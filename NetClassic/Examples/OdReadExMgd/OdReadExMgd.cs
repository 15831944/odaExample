...
namespace OdReadExMgd
{
  ...
  class Program
  {
    static void Main(string[] args)
    {
      ...
      using (Teigha.Runtime.Services srv = new Teigha.Runtime.Services())
      {
        try
        {
          HostApplicationServices.Current = new ODASampleCommon.HostApplicationServicesExample();

          /**********************************************************************/
          /* Display the Product and Version that created the executable        */
          /**********************************************************************/
          Console.WriteLine("\nOdReadExMgd developed using {0} ver {1}", HostApplicationServices.Current.Product, HostApplicationServices.Current.VersionString);

          if (args.Length < 1)
          {
            Console.WriteLine("\n\n\tusage: OdReadExMgd <filename> [-NoWait]");
          ...
          /* first parameter means - do not initialize database- it will be read from file
            * second parameter is not used by ODA.NET Classic - it is left for ARX compatibility.
        ...
        {
          bSuccess = false;
          Console.WriteLine("OdReadExMgd Error: " + e.Message);
        }
      }

      if (bSuccess)
      {
        Console.WriteLine("\n\nOdReadExMgd Finished Successfully");
  ...
  class DbDumper
  {
    ...
    class SubEntityTraitsDumper : SubEntityTraits
    {
      ...
      public override SelectionFlags SelectionFlags
      {
...
*/
    ...
    {
      using (BlockTableRecord btr = (BlockTableRecord)pDb.CurrentSpaceId.Open(OpenMode.ForRead))
      {
        using (Layout pLayout = (Layout)btr.LayoutId.Open(OpenMode.ForRead))
        {
          string layoutName = "";
