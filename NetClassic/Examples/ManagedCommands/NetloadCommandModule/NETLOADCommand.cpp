...
void NETLOAD_cmd::execute(OdEdCommandContext* pCmdCtx)
{
  ...
  try
  {
    ...
    {
      // PROD-124 // support for OdaDrawingsExplorer
      Teigha::Runtime::RXObject^ pObj = Teigha::Runtime::RXObject::Create(IntPtr(((OdDbDatabase*)pCmdCtx->baseDatabase())->appServices()), false);
      Teigha::DatabaseServices::HostApplicationServices^ appSrv = (Teigha::DatabaseServices::HostApplicationServices^)pObj;
      Teigha::DatabaseServices::Database^ db = (Teigha::DatabaseServices::Database^)Teigha::Runtime::RXObject::Create(IntPtr(pCmdCtx->baseDatabase()), false);
      s = appSrv->FindFile(s, db, Teigha::DatabaseServices::FindFileHint::ArxApplication);
    }

    // if path till now contains nothing (for example user cancelled choosing assembly in dialog), notify that path was not set and return
    if (s == "")
    {
      String^ sStr = "Assembly path was not set.";
...
{
  if (OdDbDatabase::cast(pCommandContext->baseDatabase()).isNull())
    throw OdError(eNoDatabase); // PROD-124 // support for OdaDrawingsExplorer
