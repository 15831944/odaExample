...
namespace OdWriteExMgd
{
  public partial class DbFiller
  {
    ...
    public void fillDatabase(Database pDb)
    {
      ...
      Transaction tr = pDb.TransactionManager.StartTransaction();
      ...
      using (BlockTable blockTable = (BlockTable)pDb.BlockTableId.GetObject(OpenMode.ForRead))
      {
        ...
        /**********************************************************************/
        addExtendedData(modelSpaceId, 6, 8, odaLayer1Id, ttfStyleId);
    ...
    {
      using (RegAppTable regappTable = (RegAppTable)pDb.RegAppTableId.GetObject(OpenMode.ForWrite))
      {
        using (RegAppTableRecord regAppRecord = new RegAppTableRecord())
        {
          regAppRecord.Name = name;
          return regappTable.Add(regAppRecord).IsNull == false;
        }
      }
    }
    ...
    ObjectId addMaterial(Database pDb, string name, string desc)
    {
      ...
      using (DBDictionary pMatDic = (DBDictionary)pDb.MaterialDictionaryId.GetObject(OpenMode.ForWrite))
      {
        ...
        using (Material pMaterial = new Material())
        {
          ...
          MaterialMap materialMap = new MaterialMap(Source.File, new MaterialTexture(), 0, new Mapper());
    ...
    /************************************************************************/
    /* Append an XData Pair to a test object                                */
    /************************************************************************/
    void addExtendedData(ObjectId modelSpaceId, int boxRow, int boxCol, ObjectId layerId, ObjectId styleId)
    {
      using (BlockTableRecord bBTR = (BlockTableRecord)modelSpaceId.GetObject(OpenMode.ForWrite))
      {
        ...
        double h = m_EntityBoxes.getHeight();
        double w = m_EntityBoxes.getWidth(boxRow, boxCol);
        Point3d centralPoint = m_EntityBoxes.getBoxCenter(boxRow, boxCol);

        /**********************************************************************/
        /* Add the label                                                      */
        /**********************************************************************/
        addTextEnt(bBTR, point + m_textOffset, point + m_textOffset, "XData on circle",
          m_textSize, TextHorizontalMode.TextLeft, TextVerticalMode.TextTop, layerId, styleId);
        ...
        /**********************************************************************/
        ObjectId circleId;
        using (Circle circleWithXData = new Circle(centralPoint, new Vector3d(0, 0, 1), Math.Min(h, w) * 0.4))
        {
          circleWithXData.SetDatabaseDefaults(modelSpaceId.Database);
          circleId = bBTR.AppendEntity(circleWithXData);
        }
    ...
    {
      //TODO: createNurbSurface() removed. You should use BrepBuilder for creating nurb surfaces (see OdWriteEx project)
      return;
