...
void importGradient( OdDbHatchPtr& pHatch,
                       const OdDgGradientFill& gradientFill
                   )
{
  pHatch->setHatchObjectType( OdDbHatch::kGradientObject );
  pHatch->setGradientAngle( gradientFill.getAngle() );
  pHatch->setGradientOneColorMode( gradientFill.getKeyCount() < 2 );

  switch( gradientFill.getGradientType() )
  {
    case OdDgGradientFill::kLinear:
    {
      if( gradientFill.getInvertFlag() )
      {
  ...
  if( gradientFill.getKeyCount() == 0 )
  {
    OdCmColor curColor = pHatch->color();
    arrColors.push_back( curColor );
    curColor.setRGB( (OdUInt8)(255 * gradientFill.getWhiteIntensity()),
      (OdUInt8)(255 * gradientFill.getWhiteIntensity()),
      (OdUInt8)(255 * gradientFill.getWhiteIntensity())
      );
    arrColors.push_back( curColor );
    arrValues.push_back(0);
    arrValues.push_back(1);
  }
  else if( gradientFill.getKeyCount() == 1 )
  {
    OdDgGradientKey gradKey = gradientFill.getKey(0);
    ...
    arrColors.push_back( curColor );
    arrValues.push_back(gradKey.dKeyPosition);
    arrValues.push_back(1);
  }
  else
  {
    OdDgGradientKey firstKey = gradientFill.getKey(0);
    OdDgGradientKey lastKey = gradientFill.getKey(gradientFill.getKeyCount() - 1);

    arrValues.push_back(firstKey.dKeyPosition);
    arrValues.push_back(lastKey.dKeyPosition);
...
void applyDgnSymbolPatternSymbologyToHatch( OdDbHatchPtr& pHatch,
                                              OdDgDatabase* pDb, 
                                                const OdDgSymbolHatchPattern* pPaternLinkage
                                     )
{
...
OdGePoint2d getHatchSeedPoint( OdDgElement* pElm, 
                                 OdDgHatchPatternPtr pHatchPattern, 
                                   OdDbHatchPtr& pHatch,
                              ...
                              )
{
  ...
  if( !pElm || pHatchPattern.isNull() || pHatch.isNull() || pHatch->isErased() )
  {
    return ptSeedBase.convert2d();
  }

  if( pHatchPattern->getUseOffsetFlag() )
    pHatchPattern->getOffset( ptSeedBase );

  OdGePoint3d ptElementOrigin(0,0,0);

  if( pHatchPattern->getType() == OdDgHatchPattern::kDWGPattern )
  {
...
void importLinearPattern( OdDbHatchPtr& pHatch,
                            OdDgElement* pElm, 
                              const OdDgLinearHatchPatternPtr& pPaternLinkage,
                                double dUORsToModelScale
                        )
{
  ...
  patternLine.m_patternOffset.set(0.0,pPaternLinkage->getSpace());
  patternLine.m_dashes.push_back(pPaternLinkage->getSpace());

  linearPattern.push_back( patternLine );

  OdGePoint2d ptBase = getHatchSeedPoint( pElm, pPaternLinkage, pHatch, dUORsToModelScale );

  pHatch->setPattern(OdDbHatch::kUserDefined, L"HATCH", pPaternLinkage->getAngle(), 1.0, linearPattern, ptBase );
...
void importCrossPattern( OdDbHatchPtr& pHatch,
                           OdDgElement* pElm, 
                             const OdDgCrossHatchPatternPtr& pPaternLinkage,
                               double dUORsToModelScale
                         )
{
  OdDgDatabase* pDb = pElm->database();
  ...
  patternLine.m_patternOffset.set(0.0,pPaternLinkage->getSpace1());
  patternLine.m_dashes.push_back(pPaternLinkage->getSpace1());

  crossPattern.push_back( patternLine );

  patternLine.m_basePoint.set(0.0,0.0);
  patternLine.m_dLineAngle = pPaternLinkage->getAngle2();
  patternLine.m_patternOffset.set(0.0,pPaternLinkage->getSpace2());
  patternLine.m_dashes.clear();
  patternLine.m_dashes.push_back(pPaternLinkage->getSpace2());
  crossPattern.push_back( patternLine );

  OdGePoint2d ptBase = getHatchSeedPoint( pElm, pPaternLinkage, pHatch, dUORsToModelScale );

  pHatch->setPattern(OdDbHatch::kUserDefined, L"CROSSHATCH", 0.0, 1.0, crossPattern, ptBase );
...
void importSymbolPattern( OdDbHatchPtr& pHatch, 
                            OdDgElement* pElm, 
                              const OdDgSymbolHatchPatternPtr& pPaternLinkage, 
                                double dUORsToModelScale,
                        ...
                        )
{
  ...
  OdDgSymbolHatchPatternPtr pSymbolHatch = pPaternLinkage;

  if( pSymbolHatch->getSymbolId() != 0 )
  {
    OdDgElementId elementId = pElm->database()->getElementId( OdDbHandle( pSymbolHatch->getSymbolId() ) );
  ...
  if( !pSymbol.isNull() )
  {
    OdString strHatchName = pSymbol->getName();
    OdString repairedName;

    if( OdDbSymUtil::repairSymbolName(repairedName, strHatchName, pHatch->database() ) == eOk )
    {
      if( !repairedName.isEmpty() )
        strHatchName = repairedName;
    }
                                                ...
                                                pPaternLinkage->getSpace1() / pPaternLinkage->getScale(), hatchDesc) )
    {
      ...
      double dAngle = pPaternLinkage->getAngle1();

      if (pPaternLinkage->getUseRotationFlag() && pElm->isKindOf(OdDgGraphicsElement::desc()))
      {
        OdDgGraphicsElementPtr pGrElm = pElm;

        if (!pGrElm->get3dFormatFlag())
        {
          OdGeMatrix2d matRotation;
          pPaternLinkage->getRotation2d(matRotation);
          OdGeVector2d vrX = OdGeVector2d::kXAxis;
          vrX.rotateBy(dAngle);
          vrX.transformBy(matRotation);
          dAngle = OdGeVector2d::kXAxis.angleToCCW(vrX);
        }
      }

      pHatch->setPattern(OdDbHatch::kUserDefined, strHatchName, dAngle, pPaternLinkage->getScale() * hatchDesc.m_dHatchScale, hatchDesc.m_hatchPattern, ptBase );
    ...
    else
    {
                                              ...
                                              dwgHatch, dSymbolScale, uHatchColorIndex ) )
      {
        ...
        pHatch->setPattern(OdDbHatch::kUserDefined, strHatchName, dAngle, pPaternLinkage->getScale() * dSymbolScale, dwgHatch, ptBase );
        bCorrectHatch = true;
      }
    }
...
void importDwgBasedPattern( OdDbHatchPtr& pHatch,
                              OdDgElement* pElm,
                                const OdDgDWGHatchPatternPtr& pPaternLinkage,
                                  double dUORsToModelScale
                         )
{
  pHatch->setHatchStyle( OdDbHatch::kNormal );
  ...
  OdDgDWGHatchPattern::DWGHatch dwgHatch = pPaternLinkage->getHatch();
  ...
  for( OdUInt32 i = 0; i < dwgHatch.size(); i++ )
  {
    ...
    patternLine.m_basePoint = dwgHatch[i].m_throughPoint;
    patternLine.m_dLineAngle = dwgHatch[i].m_angle;
    patternLine.m_patternOffset = dwgHatch[i].m_offset.asVector();

    for( OdUInt32 j = 0; j < dwgHatch[i].m_dashes.size(); j++ )
    {
      patternLine.m_dashes.push_back(dwgHatch[i].m_dashes[j]);
    }
  ...
  OdString strHatchName = pPaternLinkage->getPatternName();
  OdString repairedName;

  if( strHatchName.isEmpty() )
    strHatchName = L"_USER";
  else if( OdDbSymUtil::repairSymbolName(repairedName, strHatchName, pHatch->database() ) == eOk )
  {
    if( !repairedName.isEmpty() )
      strHatchName = repairedName;
  }

  pHatch->setPattern(OdDbHatch::kUserDefined, strHatchName, 0.0, 1.0, dwgPattern, ptBase );
}

//-----------------------------------------------------------------------------------------

OdDbEntityPtr createHatchEntity( const OdGePoint3dArray& arrPts, bool b3dPoints )
{
  if( arrPts.size() < 3 )
    return OdDbEntityPtr();

  bool bNormalIsCalculated = true;
  ...
  else
  {
    if (geCalculateNormal(arrPts, &vNormal) == OdGe::kOk)
    {
      ...
      bNormalIsCalculated = false;
  }

  if( bNormalIsCalculated )
  {
...
OdDbEntityPtr createHatchEntity( const OdDbObjectIdArray& arrElements, bool bSplitContours, bool b3dPoints)
{
  ...
  else if( getLoopNormal( arrElements, vrNormal) )
  {
    pHatch->setNormal( vrNormal );
  }
...
{
  if( !OdDgnImportContext::getActualViewFlags().getShowPatternsFlag() )
    return;

  OdRxObjectPtrArray arrFillLinkages;
  OdArray<OdDgHatchPatternPtr> arrPatterns;

  bool bImportGradient = false;
  OdDgGradientFill gradientFill;
  OdDgHatchPatternPtr        pPaternLinkage;
  ...
  if( !idOwner.isNull() )
  {
    ...
    {
      OdDgGraphicsElementPtr pGrOwner = pOwner;

      OdDgHatchPatternPtr pHatchPattern = pGrOwner->getHatchPattern();

      if( !pHatchPattern.isNull() )
        arrPatterns.push_back(pHatchPattern);
    }

    if( arrPatterns.size() > 0 )
    {
      pBaseElement = pOwner;
    }
  }

  if( pShape->getFillType() == OdDg::kGradientFill )
  {
    bImportGradient = true;
    gradientFill    = pShape->getGradientFill();
  }

  if( arrPatterns.size() == 0 )
  {
    if( pShape->isKindOf(OdDgMultiline::desc()) )
    {
      OdDgMultilinePtr pMLine = pShape;
    ...
    {
      OdDgHatchPatternPtr pHatchPattern = pShape->getHatchPattern();
  ...
  for( OdUInt32 j = 0; j < arrPatterns.size(); j++ )
  {
    if( arrPatterns[j]->getType() != OdDgHatchPattern::kProxyPattern )
    {
      pPaternLinkage = arrPatterns[j];
      break;
    }
  }

  if( bImportGradient || !pPaternLinkage.isNull() )
  {
    ...
    if( bUsePoints )
    {
      ...
      if( !pHatch.isNull() && !arrElementIds.isEmpty() )
      {
        pHatch->setAssociative(true);
    ...
    {
      pHatch = createHatchEntity( arrElementIds, false, pShape->get3dFormatFlag());
    }
...
//       }
    ...
    if( !pPaternLinkage.isNull() && pPaternLinkage->isKindOf( OdDgLinearHatchPattern::desc()) )
    {
      importLinearPattern( pHatch, pShape, pPaternLinkage, dUORsToModelScale );
    }
    else if( !pPaternLinkage.isNull() && pPaternLinkage->isKindOf( OdDgCrossHatchPattern::desc()) )
    {
      importCrossPattern( pHatch, pShape, pPaternLinkage, dUORsToModelScale );
    }
    else if( !pPaternLinkage.isNull() && pPaternLinkage->isKindOf( OdDgDWGHatchPattern::desc()) )
    {
      importDwgBasedPattern( pHatch, pShape, pPaternLinkage, dUORsToModelScale );
    }
    else if( !pPaternLinkage.isNull() && pPaternLinkage->isKindOf( OdDgSymbolHatchPattern::desc()) )
    {
...
OdResult getAnyPoint(OdDgElement* e, OdGePoint3d& ptHatch)
{
  OdResult retVal = eInvalidInput;

  if (e->isKindOf(OdDgCurveElement3d::desc()))
  {
    OdDgCurveElement3dPtr pCurve3d = e;
    retVal = pCurve3d->getStartPoint(ptHatch);
  }
...
void setHatchElevation(OdDbHatchPtr& pHatch, OdDgElement* e)
{
  OdGePlane hatchPlane;
  OdDb::Planarity hatchPlanarity;

  if( (pHatch->getPlane(hatchPlane, hatchPlanarity) == eOk) && (hatchPlanarity != OdDb::kNonPlanar) )
  {
    OdGePoint3d ptDgHatch;
    if( getAnyPoint(e, ptDgHatch) == eOk )
    {
      OdGePoint3d ptDbHatch = ptDgHatch.project(hatchPlane, hatchPlane.normal());
      OdGeVector3d vrElevation = ptDgHatch - ptDbHatch;

      if( !vrElevation.isZeroLength() )
      {
        if( vrElevation.isCodirectionalTo(hatchPlane.normal()))
          pHatch->setElevation(vrElevation.length());
        else
          pHatch->setElevation(-vrElevation.length());
      }
    }
...
void importClosedElementFill( OdDgElement* e, OdDbEntity* dbEll, OdDbBlockTableRecord* owner )
{
  ...
  {
    OdDgGraphicsElementPtr pGrElm = e;

    if( pGrElm->getFillType() == OdDg::kFillColor )
    {
      bUseFillColor   = true;
      uFillColorIndex = getDgnElementColorIndex(pGrElm->getFillColorIndex(), pGrElm->getLevelId());
    }
  }
