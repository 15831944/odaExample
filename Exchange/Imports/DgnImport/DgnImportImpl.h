...
class DgnProperties : public OdRxDispatchImpl<>
{
  ...
  bool    _bImportDgTextsAsDbMTexts;
  bool    _bCropXRefExtraClipByExtents;
  OdInt8  _iImportView;
  bool    _bDontImportInvisibleElements;
  bool    _bForceDgnFileMTLoading;
  bool    _bShowNewXRefLevels;
...
public:
  DgnProperties()
  {
    ...
    _bCropXRefExtraClipByExtents = true;
    _bForceDgnFileMTLoading = true;
    _bShowNewXRefLevels = false;
  ...
  const bool &get_ImportDgTextsAsDbMTexts() const { return _bImportDgTextsAsDbMTexts; }
  void put_ImportDgTextsAsDbMTexts( const bool& bSet ){ _bImportDgTextsAsDbMTexts = bSet; }
  ...
  const bool &get_CropXRefExtraClipByExtents() const { return _bCropXRefExtraClipByExtents; }
  void put_CropXRefExtraClipByExtents( const bool& bSet ){ _bCropXRefExtraClipByExtents = bSet; }
  OdInt8 get_ImportViewIndex() const { return _iImportView; }
  void put_ImportViewIndex( const OdInt8& iImportView ){ _iImportView = iImportView; }
  ...
  const OdString &get_dgnFastFont() const { return _str_dgnFastFont; }
  void put_dgnFastFont(const OdString& strFontName) { _str_dgnFastFont = strFontName; }
  ...
  const bool &get_EnableDgnFileMTLoading() const { return _bForceDgnFileMTLoading; }
  void put_EnableDgnFileMTLoading(const bool& bSet) { _bForceDgnFileMTLoading = bSet; }
  const bool &get_ShowNewXRefLevels() const { return _bShowNewXRefLevels; }
  void put_ShowNewXRefLevels(const bool& bSet) { _bShowNewXRefLevels = bSet; }
...
public:
  ...
  virtual OdDbObjectId importLineStyle( OdDgDatabase* pDgnDb, OdRxObject* pDwgDbObj, const OdString& strLineStyleName );
};
...
class DgnImporter : public OdDgnImport
{
  ...
  OdDbObjectId m_idDgnContinuous;
  ...
  OdDbObjectId copyLayout(OdDgModel* m, OdDbDatabase* pDb);
  ...
  void setLayoutViewport(OdDgDatabase* pDg, const OdDbObjectId& idLayout , const OdDgModelPtr& pBaseModel);
  ...
  void unregisterElementLoaders( OdDbHostAppServices* pHostAppServices );
	void setLevelVisibilityByLevelMaskMode( OdDgDatabase* pDg );
	static void updateLayerVisibilityByLevelMask( const OdDgDatabase* pDg );
...
public:
  void copyLayers(const OdDgLevelTable* src, OdDbDatabase* dst, bool bRenameLayers = false, const OdString& strFilename = OdString::kEmpty );
  ...
  void copyBlocks(OdDgDatabase* pDg, OdDbDatabase* pDb, bool bDimTermBlocksOnly = false, bool bSkipDimTermBlocks = false );
  void setDimensionAssociation( OdDgDatabase* pDg, OdDbDatabase* pDb );
  void createShxFontFromRscFont( const OdString& strFontName );
  void copyDatabaseDefaults(OdDgDatabase* pDg, OdDbDatabase* pDb);
  ...
  OdDbObjectId getDgnContinuousLineStyle() const;
  void         setDgnContinuousLineStyle(const OdDbObjectId& idDgnContinuous);
public:
