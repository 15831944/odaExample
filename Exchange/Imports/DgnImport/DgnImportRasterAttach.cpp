  ...
  if( !OdDgnImportContext::isImportInvisible() )
  {
    ...
    if( !idRasterFrame.isNull() )
    {
      ...
      if (!OdDgnImportContext::getActiveView().isNull())
        bRet = pRasterFrame->getDisplayAtViewFlag(OdDgnImportContext::getActiveView()->getIndex());
    }
  }
...
void OdDgnRasterAttachmentImportPE::subImportElement(OdDgElement* e, OdDbBlockTableRecord* owner)
{
  ...
  if( !idRasterFrame.isNull() )
  {
    ...
    if( !pRasterFrame.isNull() )
    {
      ...
      if( !OdDgnImportContext::getActiveView().isNull() )
        pImage->setVisibility( pRasterFrame->getDisplayAtViewFlag(OdDgnImportContext::getActiveView()->getIndex()) ? OdDb::kVisible : OdDb::kInvisible);
    }
  }
