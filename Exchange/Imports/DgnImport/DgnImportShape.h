...
template <class T> struct OdDgShapeImportPE : OdDgnImportPE
{
  void subImportElement(OdDgElement* e, OdDbBlockTableRecord* owner) ODRX_OVERRIDE
  {
    ...
    bool     bUseFillColor   = pShape->getFillType() == OdDg::kFillColor;
    OdUInt32 uFillColorIndex = 0;

    OdDgViewPtr pActiveView = OdDgnImportContext::getActiveView();

    if (bUseFillColor)
      uFillColorIndex = getDgnElementColorIndex(pShape->getFillColorIndex(),pShape->getLevelId());
    ...
    if( bUseFillColor || bImportAsPolyline ) // Create polyline and solid hatch
    {
      ...
      if( pPolyline.isNull() && pHatch.isNull() )
      {
    ...
    else // Create Face or Region
    {
      ...
      {
        bool bCreateFace = true;

        if( arrVertices[1].isEqualTo(arrVertices[2]) )
        {
          if( (arrVertices.size() == 3) || arrVertices[0].isEqualTo(arrVertices[3]) )
          {
            OdDbLinePtr pLine = OdDbLine::createObject();
            pLine->setStartPoint(arrVertices[0]);
            pLine->setEndPoint(arrVertices[1]);

            owner->appendOdDbEntity(pLine);

            copyEntityProperties(pShape, pLine);

            idCreatedObject = pLine->objectId();
