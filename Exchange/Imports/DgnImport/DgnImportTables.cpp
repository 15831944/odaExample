...
void DgnImporter::copyLayerProperties(OdDgLevelTableRecord* l, OdDbLayerTableRecord* l_d)
{
  l_d->setLineWeight(odDgnImportLineweightMapping(l->getElementLineWeight()));
  {
    ...
    {
      OdDgElementId idLineStyle = l->database()->getLineStyleTable()->getAt(l->getElementLineStyleEntryId());

      OdDgnImportPathToDwgObject dwgPath = _idMap[idLineStyle];

      if( dwgPath.m_idPath.objectIds().isEmpty() && l_d->database())
      {
        OdDgLineStyleTableRecordPtr pDgnLineStyle = idLineStyle.openObject(OdDg::kForRead);

        if (!pDgnLineStyle.isNull())
        {
          OdDbLinetypeTablePtr pDwgLSTable = l_d->database()->getLinetypeTableId().openObject(OdDb::kForRead);

          if (!pDwgLSTable.isNull())
          {
            OdDbObjectId ltpId = pDwgLSTable->getAt(pDgnLineStyle->getName());

            if (!ltpId.isNull())
              dwgPath.m_idPath.objectIds().push_back(ltpId);
          }
        }
      }
...
void DgnImporter::copyLayers(const OdDgLevelTable* src, OdDbDatabase* dst, bool bRenameLayers, const OdString& strFilename)
{
  OdDgLevelTablePtr lt = src;
  for (OdDgElementIteratorPtr i = lt->createIterator(); !i->done(); i->step())
  {
    ...
    if( bRenameLayers )
    {
      if( !l_d_id.isNull() )
      {
        OdString strXRefName = strFilename;
        strXRefName.replace(L'\\', L'/');

        if (strXRefName.reverseFind(L'/') != -1)
          strXRefName = strXRefName.right(strXRefName.getLength() - strXRefName.reverseFind(L'/') - 1);

        if (strXRefName.find(L".") != -1)
          strXRefName = strXRefName.left(strXRefName.find(L"."));

        name = strXRefName + L"_" + name;

        OdString repairedName;

        if( OdDbSymUtil::repairSymbolName(repairedName, name, dst) == eOk && !repairedName.isEmpty() )
          name = repairedName;
...
OdDbObjectId DgnImporter::getDgnContinuousLineStyle() const
{
  return m_idDgnContinuous;
}

void         DgnImporter::setDgnContinuousLineStyle(const OdDbObjectId& idDgnContinuous)
{
  m_idDgnContinuous = idDgnContinuous;
}
...
void DgnImporter::copyLineStyles(OdDgDatabase* src, OdDbDatabase* dst)
{
  ...
  OdDbObjectId idContinuous = dst->getLinetypeContinuousId();

  OdString strDgnContinuousName;

  if( !idContinuous.isNull() )
  {
    OdDbLinetypeTableRecordPtr pDbContinuousLT = idContinuous.openObject(OdDb::kForRead);
    strDgnContinuousName = pDbContinuousLT->getName() + L"_DGN";
  }
  ...
  for (OdDgElementIteratorPtr i = lst->createIterator(); !i->done();  i->step())
  {
    ...
    else
    {
      ...
      if (lt_d->getName() == strDgnContinuousName)
        m_idDgnContinuous = lt_d->id();
    }
  }
}
...
OdDgViewPtr getActualDgnView(OdDgModel* m, OdInt8 iViewIndex)
{
  OdDgViewPtr pRet;

  if (!m || iViewIndex < 0 || iViewIndex > 7)
    return pRet;

  OdDgDatabasePtr pDgnDb = m->database();

  if( !pDgnDb.isNull() && (pDgnDb->getActiveModelId() == m->elementId()) )
  {
    OdDgElementId idActiveVG = pDgnDb->getActiveViewGroupId();

    if( !idActiveVG.isNull() )
    {
      OdDgViewGroupPtr pActiveVG = idActiveVG.openObject(OdDg::kForRead);
      
      if (!pActiveVG.isNull())
      {
        OdDgViewPtr pActiveView = pActiveVG->getAt((OdUInt32)iViewIndex).openObject(OdDg::kForWrite);

        if( pActiveView->getModelId() == m->elementId() )
        {
          pRet = pActiveView;
        }
      }
    ...
    if( pRet.isNull() )
    {
      OdDgElementId idRecomendedVG = pDgnDb->recommendActiveViewGroupId();

      if (!idRecomendedVG.isNull())
      {
        OdDgViewGroupPtr pActiveVG = idRecomendedVG.openObject(OdDg::kForRead);
      ...
      if (!pVG.isNull() && pVG->getModelId() == m->elementId())
      {
        ...
        for (; !pViewIter->done(); pViewIter->step())
        {
          ...
          if (!pView.isNull() && (pView->getIndex() == (OdUInt8)(iViewIndex)))
          {
            ...
            OdDgnImportViewFlags newFlags;
            newFlags.init(pView);
            OdDgnImportContext::claerViewFlags();
            OdDgnImportContext::pushViewFlags(newFlags);
...
bool DgnImporter::setActiveModelView( OdDgModel* m, OdInt8 iViewIndex )
{
  ...
  {
    OdDgViewPtr pView = getActualDgnView(m, iViewIndex);
...
{
  // Set global line style scale

  double dActualLineTypeScale = 1.0;
  bool bRestoreGlobalLineScale = m && !OdDgnImportContext::getGlobalLineScale(dActualLineTypeScale);

  if(bRestoreGlobalLineScale)
  {
    dActualLineTypeScale = m->getLinestyleScale();

    if( m->getCompoundLineScaleFlag() )
    {
      if (OdZero(dActualLineTypeScale))
        dActualLineTypeScale = 1.0;

      double dAnnotationScale = m->getAnnotationScale();

      if (OdZero(dAnnotationScale))
        dAnnotationScale = 1.0;

      dActualLineTypeScale *= dAnnotationScale;
    }
    else if (m->getAnnotationLineScaleFlag())
      dActualLineTypeScale = m->getAnnotationScale();

    if (OdZero(dActualLineTypeScale))
      dActualLineTypeScale = m->database()->getLineStyleScale();

    if (OdZero(dActualLineTypeScale))
      dActualLineTypeScale = 1.0;

    OdDgnImportContext::pushGlobalLineScale(dActualLineTypeScale);
  }
  ...
  setModelLevelMask( m, iViewIndex );
	...
	if( m )
		updateLayerVisibilityByLevelMask( m->database() );
...
OdDbObjectId DgnImporter::copyLayout(OdDgModel* m, OdDbDatabase* pDb)
{
  ...
  OdDbSetPlotSettingsPEPtr pSetPlotSettings = OdDbSetPlotSettingsPE::cast(dbLayout);

  OdDgModel::UnitDescription paperUnits;
  paperUnits.m_numerator   = 1000.0;
  paperUnits.m_denominator = 1.0;

  OdDgModel::UnitDescription masterUnits;
  sm->getMasterUnit(masterUnits);

  double dMasterUnitsToPaperUnitsScale = masterUnits.m_denominator / masterUnits.m_numerator * paperUnits.m_numerator / paperUnits.m_denominator;

  if( !sm->getSheetPaperName().isEmpty() )
    pSetPlotSettings->setStyleSheet( dbLayout, sm->getSheetPaperName() );

  pSetPlotSettings->setPlotPaperSize( dbLayout, sm->getSheetWidth() * dMasterUnitsToPaperUnitsScale, sm->getSheetHeight() * dMasterUnitsToPaperUnitsScale );
  pSetPlotSettings->setMargins( dbLayout, sm->getSheetLeftMargin() * dMasterUnitsToPaperUnitsScale, sm->getSheetRightMargin() * dMasterUnitsToPaperUnitsScale,
                                sm->getSheetTopMargin() * dMasterUnitsToPaperUnitsScale, sm->getSheetBottomMargin() * dMasterUnitsToPaperUnitsScale );

  OdDbPlotSettings::PlotRotation curRotation = OdDbPlotSettings::k0degrees;

  if( OdZero(sm->getSheetRotation() - OdaPI2) )
    curRotation = OdDbPlotSettings::k90degrees;
  else if( OdZero(sm->getSheetRotation() - OdaPI) )
    curRotation = OdDbPlotSettings::k180degrees;
  else if( OdZero(sm->getSheetRotation() - 3*OdaPI2) )
    curRotation = OdDbPlotSettings::k270degrees;

  double dModelScale = sm->getAnnotationScale();

  pSetPlotSettings->setPlotRotation( dbLayout, curRotation );

  OdGePoint2d ptOffset = sm->getSheetOffset() * -1.0 * dMasterUnitsToPaperUnitsScale / dModelScale;

  if( curRotation == OdDbPlotSettings::k90degrees )
  {
    OdGeVector2d vrOffset = ptOffset.asVector();
    vrOffset.rotateBy(OdaPI2);
    ptOffset.y = sm->getSheetHeight() * dMasterUnitsToPaperUnitsScale + vrOffset.y;
    ptOffset.x = -vrOffset.x;
  }
  else if( curRotation == OdDbPlotSettings::k180degrees )
  {
    OdGeVector2d vrOffset = ptOffset.asVector();
    vrOffset.rotateBy(OdaPI);
    ptOffset.x = sm->getSheetWidth() * dMasterUnitsToPaperUnitsScale - vrOffset.x;
    ptOffset.y = sm->getSheetHeight() * dMasterUnitsToPaperUnitsScale - vrOffset.y;
  }
  else if( curRotation == OdDbPlotSettings::k270degrees )
  {
    OdGeVector2d vrOffset = ptOffset.asVector();
    vrOffset.rotateBy(3*OdaPI2);
    ptOffset.x = sm->getSheetWidth() * dMasterUnitsToPaperUnitsScale + vrOffset.x;
    ptOffset.y = - vrOffset.y;
  }

  pSetPlotSettings->setOrigin( dbLayout, ptOffset );

  pSetPlotSettings->setPlotPaperUnits( dbLayout, OdDbPlotSettings::kMillimeters );

  pSetPlotSettings->setScaleFactor( dbLayout, dMasterUnitsToPaperUnitsScale / dModelScale );

  OdDbPlotSettingsValidator* pv = pDb->appServices()->plotSettingsValidator();

  if (pv)
    pv->setCustomPrintScale(dbLayout, dMasterUnitsToPaperUnitsScale, 1.0);
//   // TODO: units
...
void scaleBlockDefinition(OdDbBlockTableRecord* pBlock, double dScale)
{
  if (!pBlock || OdZero(dScale - 1.0) )
    return;

  OdGeMatrix3d matScale = OdGeMatrix3d::scaling(dScale);

  OdDbObjectIteratorPtr pIter = pBlock->newIterator();

  for(; !pIter->done(); pIter->step())
  {
    OdDbEntityPtr pEnt = pIter->entity(OdDb::kForWrite);

    if( !pEnt.isNull() )
      pEnt->transformBy(matScale);
  }
}

void DgnImporter::copyBlocks(OdDgDatabase* pDg, OdDbDatabase* pDb, bool bDimTermBlocksOnly, bool bSkipDimTermBlocks )
{
  ...
  std::set<OdDgElementId> setDimTerminatorBlockIds;
  std::set<OdDgElementId> setSkipBlockIds;
  ...
  for (; !i->done(); i->step())
  {
    ...
    bool bDimTermitatorBlock = name.getLength() > 1 && name.getAt(0) == L'_';

    if( bDimTermitatorBlock )
      setDimTerminatorBlockIds.insert(scd->elementId());

    if( bSkipDimTermBlocks && bDimTermitatorBlock)
      continue;

    if (bDimTermBlocksOnly && !bDimTermitatorBlock )
      continue;
  ...
  for (i->start(); !i->done(); i->step())
  {
    ...
    std::set<OdDgElementId>::const_iterator pSkipIter = setSkipBlockIds.find(i->item());

    if( pSkipIter != setSkipBlockIds.end() )
      continue;

    std::set<OdDgElementId>::const_iterator pDimTermIter = setDimTerminatorBlockIds.find(i->item());

    bool bDimTermitatorBlock = pDimTermIter != setDimTerminatorBlockIds.end();
    ...
    if( !idBlock.isNull() )
    {
      ...
      if(bDimTermitatorBlock)
      {
        OdGeExtents3d extBlock;
        block->getGeomExtents(extBlock);
        double dWidth  = extBlock.maxPoint().x - extBlock.minPoint().x;
        double dHeight = extBlock.maxPoint().y - extBlock.minPoint().y;

        double dDimTermScale = dWidth > dHeight ? dWidth : dHeight;
        scaleBlockDefinition(block, 1.0 / dDimTermScale);
        OdDgnImportContext::addScaledBlock(block->objectId(), dDimTermScale);
      }
    }
  }
}

void DgnImporter::copyDatabaseDefaults(OdDgDatabase* pDg, OdDbDatabase* pDb)
{
  // Color

  OdCmColor defaultColor;
  OdUInt32  uDefaultColorIndex = pDg->getActiveColorIndex();
  setDwgColorByDgnIndex(pDg, defaultColor, uDefaultColorIndex);
  pDb->setCECOLOR(defaultColor);

  // Line type

  OdDbObjectId idDbLineType;
  OdUInt32 uLineStyleId = pDg->getActiveLineStyleEntryId();
  setDwgLineTypeIdByDgnLineTypeIndex(pDb, pDg, uLineStyleId, idDbLineType);

  if(!idDbLineType.isNull())
    pDb->setCELTYPE(idDbLineType);

  // Line weight

  OdUInt32 uLineWeight = pDg->getActiveLineWeight();
  OdDb::LineWeight extWeight = odDgnImportLineweightMapping(uLineWeight);
  pDb->setCELWEIGHT(extWeight);

  // Level

  OdUInt32 uLevelId = pDg->getActiveLevelEntryId();
  OdDgLevelTablePtr pLevelTable = pDg->getLevelTable(OdDg::kForRead);
  OdDgElementId idLevel = pLevelTable->getAt(uLevelId);

  OdDbObjectId layerId;

  if( !idLevel.isNull() )
    layerId = OdDgnImportContext::getObjectId(idLevel);
  ...
  OdDgMultilineStyleTableRecordPtr pMLineDefault = pMLineStyleTable->getActiveMultilineStyle();

  if (!pMLineDefault.isNull())
  {
    OdDbObjectId dimMLineId = OdDgnImportContext::getObjectId(pMLineDefault->elementId());

    if (!dimMLineId.isNull())
      pDb->setCMLSTYLE(dimMLineId);
  }

  // Text style

  OdDgTextStyleTablePtr pTextStyleTable = pDg->getTextStyleTable(OdDg::kForRead);
  OdDgTextStyleTableRecordPtr pTextDefault = pTextStyleTable->getDefaultData();
  OdDgElementId idTextStyle;

  if( !pTextDefault.isNull() )
  {
    idTextStyle = pTextStyleTable->getAt(pTextDefault->getName());

    if (!idTextStyle.isNull())
    {
      OdDbObjectId dbTextStyleId = OdDgnImportContext::getObjectId(idTextStyle);

      if( !dbTextStyleId.isNull() )
        pDb->setTEXTSTYLE(dbTextStyleId);
    }
  }
}
...
void DgnImporter::copyTextStyles(OdDgDatabase* pDg, OdDbDatabase* pDb)
{
  ...
  if (!pDg->getDefaultModelId().isNull())
    pDefaultModel = pDg->getDefaultModelId().openObject(OdDg::kForRead);
  else if (!pDg->getActiveModelId().isNull())
    pDefaultModel = pDg->getActiveModelId().openObject(OdDg::kForRead);
  else
  {
    OdDgModelTablePtr pModelTable = pDg->getModelTable();
  ...
  if (!pDefaultModel.isNull())
    dUORsToModelScale = pDefaultModel->convertUORsToWorkingUnits(1.0);
  ...
  for (OdDgElementIteratorPtr i = pDg->getTextStyleTable()->createIterator(); !i->done(); i->step())
  {
    ...
    dbTs->setTextSize(giStyle.textSize()*dUORsToModelScale);
    ...
    if (giStyle.isShxFont())
    {
      OdString fileName = giStyle.ttfdescriptor().fileName();

      if (fileName.isEmpty())
      {
        dbTs->setFileName(giStyle.ttfdescriptor().typeface());
      }
  ...
  OdDgTextStyleTableRecordPtr pDefaultTextStyle = pDg->getTextStyleTable()->getDefaultData();

  if( !pDefaultTextStyle.isNull() && !OdZero(pDefaultTextStyle->getTextHeight()) )
  {
    if (!pDefaultModel.isNull())
      pDb->setTEXTSIZE(pDefaultTextStyle->getTextHeight()*dUORsToModelScale);
  }
}
                                                ...
                                                double dTextWidth )
{
  ...
  pFontRec = OdDgFontTable::getFont(pDgDimStyle->database(), uFontId);
  ...
  if( idRet.isNull() )
  {
    idRet = OdDgnImportContext::createStyleForDbText(pDb, pDgDimStyle->database(), dimTextStyle, false );
  }
                                              ...
                                              OdDbDatabase* pDb )
{
  ...
  pDbDimStyle->setDimcen(pDgDimStyle->getCenterMarkSize() * dUORtoMasterScaleFactor);
  ...
  if( !pTextStyle->getAt(pDgDimStyle->getTextStyleEntryId()).isNull() )
  {
    ...
    dTextHeight = pDimTextStyle->getTextHeight() * dUORtoMasterScaleFactor;
    dTextWidth  = pDimTextStyle->getTextWidth() * dUORtoMasterScaleFactor;
  }

  if( pDgDimStyle->getTextHeightOverrideFlag() )
    dTextHeight = pDgDimStyle->getTextHeight() * dUORtoMasterScaleFactor;

  if( pDgDimStyle->getTextWidthOverrideFlag() )
    dTextWidth = pDgDimStyle->getTextWidth() * dUORtoMasterScaleFactor;
  ...
  if( bDisplayTolerance )
  {
    ...
    double dLowTol  = pDgDimStyle->getLowerToleranceValue() * dUORtoMasterScaleFactor;
    double dHighTol = pDgDimStyle->getUpperToleranceValue() * dUORtoMasterScaleFactor;
...
void DgnImporter::copyDimStyles(OdDgDatabase* pDg, OdDbDatabase* pDb)
{
  ...
  for(; !pIter->done(); pIter->step() )
  {
    ...
    if( strName.isEmpty() )
    {
      ...
      pDbDimStyle->setName(L"Standard");
      
      OdDbObjectId idDefaultDimStyle = pDb->getDIMSTYLE();

      if( !idDefaultDimStyle.isNull() )
      {
        OdDbDimStyleTableRecordPtr pDefaultDimStyle = idDefaultDimStyle.openObject(OdDb::kForWrite);
        OdString strDefName = pDefaultDimStyle->getName();
        pDefaultDimStyle->copyFrom(pDbDimStyle);
        pDefaultDimStyle->setName(strDefName);
      }
      else
      {
        pDbDimStyleTable->add(pDbDimStyle);
        pDb->setDIMSTYLE(pDbDimStyle->id());
      }
...
void DgnImporter::setLayoutViewport(OdDgDatabase* pDg, const OdDbObjectId& idLayout, const OdDgModelPtr& pBaseModel)
{
  OdInt32 uViewIndex = _properties->get_ImportViewIndex();

  OdDgViewGroupPtr viewGroup;

  if (!pDg->getActiveViewGroupId().isNull())
  {
    viewGroup = pDg->getActiveViewGroupId().openObject(OdDg::kForRead);

    if (viewGroup.isNull() || viewGroup->getModelId() != pBaseModel->elementId())
      viewGroup = NULL;
  }

  OdDgViewGroupTablePtr pViewGroupTable = pDg->getViewGroupTable(OdDg::kForRead);

  if (viewGroup.isNull() && !pViewGroupTable.isNull() && !pBaseModel.isNull())
  {
  ...
  if (!viewGroup.isNull())
  {
    ...
    while (!iterator->done())
    {
      ...
      {
        OdDbViewportPtr pViewport = idViewport.openObject(OdDb::kForWrite);
        OdAbstractViewPEPtr pView = OdAbstractViewPE::cast(pViewport);
        copyViewport(pDgView, pView, pViewport);
      }
      break;
    }
    ...
    if (!bCreateActiveViewport)
    {
      ...
      if (!idViewport.isNull())
      {
        OdDbViewportPtr pViewport = idViewport.openObject(OdDb::kForWrite);
        OdAbstractViewPEPtr pView = OdAbstractViewPE::cast(pViewport);
        copyViewport(pDgView0, pView, pViewport);
      }
    }
  }
}
...
extern bool getActualXRefLevelMask(const OdDgReferenceAttachmentHeader* pXRef, OdUInt32 uViewNumber, OdDgLevelMaskPtr& pLevelMask);
...
void DgnImporter::setModelLevelMask( OdDgModel* m, OdInt8 iViewIndex )
{
  if( m && (iViewIndex >= 0) && (iViewIndex < 8) )
  {
    ...
    if( !OdDgnImportContext::getActiveXRefId().isNull() )
    {
      OdDgReferenceAttachmentHeaderPtr pActiveXRef = OdDgnImportContext::getActiveXRefId().openObject(OdDg::kForRead);
      OdDgnImportContext::removeLastXRef();

      OdDgLevelMaskPtr pLevelMask;
      getActualXRefLevelMask(pActiveXRef, iViewIndex, pLevelMask);

      OdDgnImportContext::restoreLastXRef();

      if (!pLevelMask.isNull())
      {
        OdDgnImportContext::pushLevelMask(pLevelMask, m->database()->getLevelTable(OdDg::kForRead), true);
    ...
    if( !bLevelMaskAdded )
    {
      OdDgViewPtr pActualView = getActualDgnView(m, iViewIndex);

      if( !pActualView.isNull() )
      {
        OdDgElementIteratorPtr pLMIter = pActualView->createIterator();
