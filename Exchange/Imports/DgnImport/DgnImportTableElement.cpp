...
OdDgElementId getElementModelId(OdDgElement* pElm)
{
  if( pElm->isKindOf(OdDgModel::desc()) )
    return pElm->id();
  else if (!pElm->ownerId().isNull())
  {
    OdDgElementPtr pOwner = pElm->ownerId().openObject(OdDg::kForRead);
    return getElementModelId(pOwner);
  }
...
double getEmptyContentTextSize(OdDgTableElement* pDgTable, OdUInt32 uRowIndex, OdUInt32 uColumnIndex)
{
  double dRet = 1.0;

  if( !pDgTable )
  {
    ODA_ASSERT_ONCE(!"Null table element ptr.");
    return dRet;
  }

  OdDgTableCellElementPtr pCell = pDgTable->getCell(uRowIndex, uColumnIndex);
  OdDgTableRowElementPtr  pRow  = pDgTable->getRow(uRowIndex);
  dRet = pRow->getHeight();
  ...
  switch( pCell->getCellMarginType() )
  {
  ...
  if( (dRet - dTopMargin - dBottomMargin) > 0 )
    dRet = dRet - dTopMargin - dBottomMargin;

  bool bUseBodyStyle = true;

  OdDgDatabasePtr pDb = pDgTable->database();
  double dTextSizeFromStyle = 0;

  switch( pCell->getTableCellType() )
  {
    case OdDgTableCellElement::kTitleRow:
    {
      if(pDgTable->getTitleRowTextStyle() != (OdUInt32)(-1))
      {
        OdDgElementId idTextStyle = pDb->getTextStyleTable()->getAt(pDgTable->getTitleRowTextStyle());

        if (!idTextStyle.isNull())
        {
          OdDgTextStyleTableRecordPtr pStyle = idTextStyle.openObject(OdDg::kForRead);
          dTextSizeFromStyle = pStyle->getTextHeight();
          bUseBodyStyle = false;
        }
      }
    } break;

    case OdDgTableCellElement::kHeaderRow:
    {
      if (pDgTable->getHeaderRowTextStyle() != (OdUInt32)(-1))
      {
        OdDgElementId idTextStyle = pDb->getTextStyleTable()->getAt(pDgTable->getHeaderRowTextStyle());
    ...
    case OdDgTableCellElement::kFooterRow:
    {
      if (pDgTable->getFooterRowTextStyle() != (OdUInt32)(-1))
      {
        OdDgElementId idTextStyle = pDb->getTextStyleTable()->getAt(pDgTable->getFooterRowTextStyle());
    ...
    case OdDgTableCellElement::kFirstColumn:
    {
      if (pDgTable->getFirstColumnTextStyle() != (OdUInt32)(-1))
      {
        OdDgElementId idTextStyle = pDb->getTextStyleTable()->getAt(pDgTable->getFirstColumnTextStyle());
    ...
    case OdDgTableCellElement::kLastColumn:
    {
      if (pDgTable->getLastColumnTextStyle() != (OdUInt32)(-1))
      {
        OdDgElementId idTextStyle = pDb->getTextStyleTable()->getAt(pDgTable->getLastColumnTextStyle());
  ...
  if (bUseBodyStyle)
  {
    if (pDgTable->getBodyTextStyle() != (OdUInt32)(-1))
    {
      OdDgElementId idTextStyle = pDb->getTextStyleTable()->getAt(pDgTable->getBodyTextStyle());
  ...
  OdDgElementId idModel = getElementModelId(pDgTable);

  if( !idModel.isNull() )
  {
    OdDgModelPtr pModel = idModel.openObject(OdDg::kForRead);
    dTextSizeFromStyle = pModel->convertUORsToWorkingUnits(1.0);
  }

  if ((dTextSizeFromStyle > 0) && (dTextSizeFromStyle < dRet))
    dRet = dTextSizeFromStyle;
...
void setCorrectSizeForTextNode(OdDgTextNode3d* pDgCellContent)
{
  if (!pDgCellContent)
    return;

  double dMaxTextSize = 0.0;

  OdDgElementIteratorPtr pIter = pDgCellContent->createIterator();

  for(; !pIter->done(); pIter->step() )
  {
    OdDgElementPtr pItem = pIter->item().openObject(OdDg::kForRead);

    if (pItem->isKindOf(OdDgText3d::desc()))
    {
      OdDgText3dPtr pText3d = pItem;

      double dTextSize = pText3d->getHeightMultiplier();

      if( dMaxTextSize < dTextSize )
        dMaxTextSize = dTextSize;
    }
  }

  if( !OdZero(dMaxTextSize) )
    pDgCellContent->setHeightMultiplier(dMaxTextSize);
}
...
void OdDgnTableElementImportPE::subImportElement(OdDgElement* e, OdDbBlockTableRecord* owner)
{
  ...
  for( i = 0; i < (OdInt32)pDgTable->getRowCount(); i++ )
    for( j = 0; j < (OdInt32)pDgTable->getColumnCount(); j++ )
    {
      ...
      setCorrectSizeForTextNode(pDgCellContent);

      if( pDgCellContent.isNull() )
      {
        pDbTable->setTextHeight( i, j, getEmptyContentTextSize(pDgTable,i,j) );
      }
      else
      {
