...
OdDgLevelTablePtr mergeLevelTables(const OdDgLevelTable* pNativeLevelTable, const OdDgLevelTable* pXRefLevelTable)
{
  OdDgLevelTablePtr pRet = OdDgLevelTable::createObject();
  pRet->setDatabaseDefaults(pXRefLevelTable->database());

  OdDgElementIteratorPtr pIter = pNativeLevelTable->createIterator();

  for (; !pIter->done(); pIter->step())
  {
    OdDgLevelTableRecordPtr pNativeLevel = pIter->item().openObject(OdDg::kForRead);

    if( pNativeLevel.isNull() )
      continue;

    OdDgElementId idXRefLevel = pXRefLevelTable->getAt(pNativeLevel->getName());

    OdDgLevelTableRecordPtr pResultLevel;

    if (idXRefLevel.isNull())
      pResultLevel = pNativeLevel->clone();
    else
    {
      OdDgLevelTableRecordPtr pXRefLevel = idXRefLevel.openObject(OdDg::kForRead);

      if(pXRefLevel.isNull())
        pResultLevel = pNativeLevel->clone();
      else
        pResultLevel = pXRefLevel->clone();
    }

    pRet->add(pResultLevel, false);
  }
...
OdDgLevelTablePtr getActualXRefLevelTable(const OdDgReferenceAttachmentHeader* pXRef, OdDgDatabase* pDgnDb )
{
  OdDgLevelTablePtr pNativeLevelTable = pDgnDb->getLevelTable(OdDg::kForRead);
  OdDgLevelTablePtr pRet = pNativeLevelTable;

  OdDgReferenceAttachmentHeaderPtr pBaseXRef = pXRef;

  if (OdDgnImportContext::getXRefNestedDepth() > 0)
  {
    OdArray<OdDgElementId> arrXRefPathId;
    ...
    for (OdUInt32 k = 1; k < OdDgnImportContext::getXRefNestedDepth(); k++)
    {
      ...
      if (!idPathXRef.isNull())
      {
        ...
        arrXRefPathId.push_back(idPathXRef);
      }
      else
      {
        arrXRefPath.push_back((OdUInt64)(0));
        arrXRefPathId.push_back(OdDgElementId());
      }
    }

    arrXRefPath.push_back((OdUInt64)(pXRef->elementId().getHandle()));
    arrXRefPathId.push_back(pXRef->elementId());

    OdUInt32 uPathSize = arrXRefPath.size();

    for (OdUInt32 i = 0; i < uPathSize; i++)
    {
      OdDgElementId idBaseXRef = arrXRefPathId[0];

      if (idBaseXRef.isNull())
        continue;

      pBaseXRef = idBaseXRef.openObject(OdDg::kForRead);

      if( arrXRefPathId.size() == 1 )
      {
        if (!pBaseXRef->getLevelTable().isNull())
        {
          pRet = pBaseXRef->getLevelTable();
          break;
        }
      }
      else
        ...
        for (; !pXRefIter->done(); pXRefIter->step())
        {
          ...
          if (!pChild.isNull() && pChild->isKindOf(OdDgReferenceOverride::desc()))
          {
            ...
            if (arrPath.size() == arrXRefPath.size())
            {
              ...
              if (bCorrectOverride)
              {
                if (!pXRefOverride->getLevelTable().isNull())
                  pRet = pXRefOverride->getLevelTable();
                break;
              }
            }
          }
      ...
      arrXRefPath.removeFirst();
      arrXRefPathId.removeFirst();
    }
  }
  else
  {
    if (!pXRef->getLevelTable().isNull())
      pRet = pXRef->getLevelTable();
  }

  if( pRet != pNativeLevelTable )
  {
    pRet = mergeLevelTables(pNativeLevelTable, pRet);

    OdDgnImportContext::setOverridedLevelTable(pRet->elementId());

    std::map< OdDgElementId, OdDgElementId > mapLevelOverrides;

    OdDgElementIteratorPtr pLevelIter = pNativeLevelTable->createIterator();

    for(; !pLevelIter->done(); pLevelIter->step() )
    {
      OdDgLevelTableRecordPtr pNativeLevel = pLevelIter->item().openObject(OdDg::kForRead);

      if( !pNativeLevel.isNull() )
      {
        OdDgElementId idLevelOverride = pRet->getAt(pNativeLevel->getName());

        if( !idLevelOverride.isNull() )
        {
          mapLevelOverrides[pNativeLevel->elementId()] = idLevelOverride;
        }
      }
    }

    if( mapLevelOverrides.size() > 0 )
    {
      OdDgnImportContext::pushLevelTableOverride(mapLevelOverrides);
      OdDgnImportContext::setUseLevelOverrideFlag(true);
    }
    else
    {
      OdDgnImportContext::setUseLevelOverrideFlag(false);
    }
  }
  else
  {
                   ...
                   )
{
  ...
  if( bCorrectFile )
  {
    ...
    if( !pXRefDef.isNull() )
    {
      ...
      if (!pXRefOwnerModel.isNull())
      {
        matTransform = OdGeMatrix3d::translation(-(pXRef->getReferenceOrigin() + pXRefOwnerModel->getInsertionBase().asVector()).asVector() );
      }
      else
      {
  ...
  {
    OdString msg;
    msg.format(OD_T("%ls is missing"), xRefFileName.isEmpty() ? databaseShortName.c_str() : xRefFileName.c_str());
    //pDgnDb->appServices()->warning(msg);
  }
}
                      ...
                      )
{
  ...
  if( !idDgnDef.isNull() )
  {
    ...
    if (!pModel.isNull())
    {
      matTransform = OdGeMatrix3d::translation(-(pXRef->getReferenceOrigin() + pModel->getInsertionBase().asVector()).asVector() * dModelMeasuresConversion);
    }
    else
    {
...
bool isEqualLevelTables(OdDgDatabase* pDb, OdDgLevelTable* pXRefLevelTable )
{
  bool bRet = true;

  OdDgLevelTablePtr pLevelTable = pDb->getLevelTable();

  OdDgElementIteratorPtr pIter = pXRefLevelTable->createIterator();

  for( ; !pIter->done(); pIter->step() )
  {
    OdDgLevelTableRecordPtr pLevel = pIter->item().openObject(OdDg::kForRead);

    if( pLevel.isNull() )
      continue;

    OdDgElementId idNativeLevel = pLevelTable->getAt(pLevel->getName());

    if( idNativeLevel.isNull() )
    {
      bRet = false;
      break;
    }

    OdDgLevelTableRecordPtr pNativeLevel = idNativeLevel.openObject(OdDg::kForRead);
    ...
    if (pLevel->getIsDisplayedFlag() != pNativeLevel->getIsDisplayedFlag())
    {
      bRet = false;
      break;
    }

    if (pLevel->getIsFrozenFlag() != pNativeLevel->getIsFrozenFlag())
    {
      bRet = false;
      break;
    }

    if (pLevel->getElementColorIndex() != pNativeLevel->getElementColorIndex())
    {
      bRet = false;
      break;
    }

    if (pLevel->getElementLineStyleEntryId() != pNativeLevel->getElementLineStyleEntryId())
    {
      bRet = false;
      break;
    }

    if (pLevel->getElementLineWeight() != pNativeLevel->getElementLineWeight())
    {
      bRet = false;
      break;
    }
    ...
    {
      if (pLevel->getUseOverrideColorFlag() != pNativeLevel->getUseOverrideColorFlag())
      {
        bRet = false;
        break;
      }
      ...
      if (pLevel->getUseOverrideLineStyleFlag() != pNativeLevel->getUseOverrideLineStyleFlag())
      {
        bRet = false;
        break;
      }

      if (pLevel->getOverrideLineStyleEntryId() != pNativeLevel->getOverrideLineStyleEntryId())
      {
        bRet = false;
        break;
      }

      if (pLevel->getUseOverrideLineWeightFlag() != pNativeLevel->getUseOverrideLineWeightFlag())
      {
        bRet = false;
        break;
      }

      if (pLevel->getOverrideLineWeight() != pNativeLevel->getOverrideLineWeight())
      {
        bRet = false;
        break;
      }
                   ...
                   )
{
  ...
  OdInt32 uViewNumber = -1;

  if (!OdDgnImportContext::getActiveView().isNull())
    uViewNumber = OdDgnImportContext::getActiveView()->getIndex();

  OdDgnImportViewFlags xRefFlags;
  xRefFlags.init(pXRef, (OdUInt32)(uViewNumber), OdDgnImportContext::getActualViewFlags());
  OdDgnImportContext::pushViewFlags(xRefFlags);

  bool bOldUseLevelOverrides = OdDgnImportContext::getUseLevelOverrideFlag();
  OdDgElementId idOldLevelOverride = OdDgnImportContext::getOverridedLevelTable();

  OdDgLevelTablePtr pXRefLevelTable = getActualXRefLevelTable(pXRef, pModel->database());

  bool bEqualLevelTables = isEqualLevelTables(pModel->database(), pXRefLevelTable);
  OdDgLevelTablePtr pLevelTableForXRefRegistration = bEqualLevelTables ? pModel->database()->getLevelTable(OdDg::kForRead) : pXRefLevelTable;

  bool bUseLevelOverrides = OdDgnImportContext::getUseLevelOverrideFlag();
  ...
  if( uXRefImportMode == 2 && OdDgnImportContext::isLevelMask() )
  {
    ...
    bool bShowNewLevels = false;

    if (pXRef->getNewLevelDisplayMode() != OdDgReferenceAttachmentHeader::kConfigVariable)
      bShowNewLevels = pXRef->getNewLevelDisplayMode() == OdDgReferenceAttachmentHeader::kAlways;
    else
      bShowNewLevels = OdDgnImportContext::getShowNewXRefLevelsFlag();

    if (!pXRefDb.isNull() && pXRefLevelTable.isNull() )
      OdDgnImportContext::pushLevelMask(pLevelMask, pXRefDb->getLevelTable(OdDg::kForRead), bShowNewLevels);
    else
      OdDgnImportContext::pushLevelMask(pLevelMask, pXRefLevelTable, bShowNewLevels);

    strLevelMaskExt = OdDgnImportContext::getLevelMaskString();
  }

  bool bAllowToEraseBlockDef = false;

  OdDbObjectId idBlockDefinition = OdDgnImportContext::getDwgModelBlockId(strFilename, pModel->elementId(), pLevelTableForXRefRegistration, strLevelMaskExt );

  if( idBlockDefinition.isNull() ) // Import model of x-ref as block definition
  {
    bAllowToEraseBlockDef = true;
    ...
    if( !idBlockDefTable.isNull() )
    {
      ...
      OdDgnImportContext::pushXRef(strFilename, pModel->elementId(), pLevelTableForXRefRegistration, pXRef->elementId(), pXRef->getNestDepth(), strLevelMaskExt );
      ...
      if( pDb && ( ((pDb->getOriginalFileVersion() > 7) && (OdDgnImportContext::getXRefNestedDepth() < 2)) || 
                     pDb->getColorTable(OdDg::kForRead).isNull()) )
      {
      ...
      if( (uXRefImportMode == 1) && (pModel->database() != pOwnerModel->database() ) && strGroupName.isEmpty() && bEqualLevelTables ) // Retain X-Ref
      {
        if( pImport )
        {
          ...
          if( !bXRefPresent )
          {
            ...
            importer->properties()->putAt( L"ImportDgTextsAsDbMTexts", OdRxVariantValue(OdDgnImportContext::isImportDgTextAsDbMText()));
            importer->properties()->putAt( L"CropXRefExtraClipByExtents", OdRxVariantValue(OdDgnImportContext::getCropXRefExtraClipByExtentsFlag()));
            importer->properties()->putAt( L"ShowNewXRefLevels", OdRxVariantValue(OdDgnImportContext::getShowNewXRefLevelsFlag()));
            ...
            OdDgElementId idActiveXRef = OdDgnImportContext::getActiveXRefId();
            OdDgnImportContext::setActiveXRefId(pXRef->elementId());
            ...
            OdDgnImportContext::setActiveXRefId(idActiveXRef);
      ...
      else
      {
        ...
        if( pImport )
        {
          ...
          pImport->copyLayers(pXRefLevelTable, pDwgDb, true, pDgnDb->getFilename());
        ...
        if( pImport )
        {
          ...
          else
          {
            ...
            OdDgnImportContext::clearBlockForSharedCellOverrideMap();
          }
      ...
      OdDgnImportContext::addDwgModelBlockId(strFilename, pModel->elementId(), pLevelTableForXRefRegistration, strLevelMaskExt, idBlockDefinition);
    }
  }

  if (bUseLevelOverrides)
  {
    OdDgnImportContext::popLevelTableOverride();
    OdDgnImportContext::setUseLevelOverrideFlag(bUseLevelOverrides);
    OdDgnImportContext::setOverridedLevelTable(idOldLevelOverride);
  }
  ...
  if( !idBlockDefinition.isNull() )
  {
    try
    {
      matTransform = OdGeMatrix3d::translation(-(pXRef->getReferenceOrigin() + pModel->getInsertionBase().asVector()).asVector()
    ...
    {
      if( bAllowToEraseBlockDef )
        idBlockDefinition.openObject(OdDb::kForWrite)->erase(true);
    }
  }
}
...
bool OdDgReferenceAttachmentImportPE::allowImportElement(OdDgElement* e)
{
  ...
  if( !OdDgnImportContext::isImportInvisible() && OdDgnImportContext::isLevelMask() )
  {
    ...
    {
      OdDgElementId levelId = OdDgnImportContext::getLevelOverride(pXRef->getLevelId());
...
bool isExtentsIntoClipBoundary(const OdGePoint2dArray& extentsPts, const OdGePoint2dArray& arrClipBoundary)
{
  OdGeExtents2d extBase;
  OdGeExtents2d extClipBoundary;
  extBase.addPoints(extentsPts);
  extClipBoundary.addPoints(arrClipBoundary);

  if (!extClipBoundary.contains(extBase))
    return false;
  ...
  for( OdUInt32 i = 0; i < extentsPts.size() - 1; i++ )
  {
    OdGeLineSeg2d segExt(extentsPts[i], extentsPts[i + 1]);

    for( OdUInt32 j = 0; j < arrClipBoundary.size() - 1; j++ )
    {
      OdGeLineSeg2d segBoundary(arrClipBoundary[j], arrClipBoundary[j + 1]);

      if( segExt.intersectWith(segBoundary, ptIsect) )
      {
        bRet = false;
        break;
      }
  ...
  if( bRet )
  {
    ...
    OdGeLineSeg2d segCheck(extBase.center(), arrClipBoundary[0]);

    for (OdUInt32 j = 0; j < extentsPts.size() - 1; j++)
    {
      OdGeLineSeg2d segExt(extentsPts[j], extentsPts[j + 1]);

      if (segExt.intersectWith(segCheck, ptIsect))
      {
        bRet = true;
        break;
      }
...
class OdDgnImportXRefBlockExtentsCalculator : public OdGiExtCalc
{
public:
  OdDgnImportXRefBlockExtentsCalculator();
  virtual ~OdDgnImportXRefBlockExtentsCalculator() {};

  OdGiRegenType regenType() const;
  void draw(const OdGiDrawable* pDrawable);

private:
  OdGiRegenType m_uCurType;
};

//---------------------------------------------------------------------------------------------------

OdDgnImportXRefBlockExtentsCalculator::OdDgnImportXRefBlockExtentsCalculator()
{
  m_uCurType = kOdGiForExtents;
}

//---------------------------------------------------------------------------------------------------

OdGiRegenType OdDgnImportXRefBlockExtentsCalculator::regenType() const
{
  return m_uCurType;
}
...
void OdDgnImportXRefBlockExtentsCalculator::draw(const OdGiDrawable* pDrawable)
{
  if (pDrawable && pDrawable->isKindOf(OdDbText::desc()))
  {
    OdDbTextPtr pDbText = pDrawable;
    OdString strText = pDbText->textString();

    if((strText.find(L"%%o") != -1) || (strText.find(L"%%u") != -1) || (strText.find(L"%%O") != -1) || (strText.find(L"%%U") != -1))
      m_uCurType = kOdGiStandardDisplay;
  }
  else if (pDrawable && pDrawable->isKindOf(OdDbMText::desc()))
  {
    OdDbMTextPtr pDbText = pDrawable;
    OdString strText = pDbText->contents();

    if ((strText.find(L"\\O") != -1) || (strText.find(L"||L") != -1) || (strText.find(L"\\o") != -1) || (strText.find(L"||l") != -1))
      m_uCurType = kOdGiStandardDisplay;
  }

  OdGiExtCalc::draw(pDrawable);
...
OdGeExtents3d getDwgBlockExtents(OdDbBlockReference* pBlockRef)
{
  OdGeExtents3d extRet;

  OdStaticRxObject<OdGiContextForDbDatabase> giContext;
  giContext.setDatabase(pBlockRef->database(), false);

  OdStaticRxObject<OdDgnImportXRefBlockExtentsCalculator> drawObject;
  drawObject.setContext(&giContext);
  drawObject.draw(pBlockRef);
  drawObject.getExtents(extRet);

  if( extRet.isValidExtents() )
  {
    OdGeVector3d vrOffset = extRet.diagonal() / 1000.0;
    extRet.addPoint(extRet.minPoint() - vrOffset);
    extRet.addPoint(extRet.maxPoint() + vrOffset);
  }
...
void OdDgReferenceAttachmentImportPE::subImportElement(OdDgElement* e, OdDbBlockTableRecord* owner )
{
  ...
  if( pXRef->isSelfReference() && (OdDgnImportContext::getXRefNestedDepth() > 0) && 
      OdDgnImportContext::getXRefId(OdDgnImportContext::getXRefNestedDepth()-1) == pXRef->elementId() 
    )
  {
    return;
  }
  ...
  if( !pBlockReference.isNull() )
  {
    ...
    {
      OdDbObjectId layerId = OdDgnImportContext::getObjectId(OdDgnImportContext::getLevelOverride(pXRef->getLevelId()));
    ...
    if (useClipBoundary && (clipBoundary.m_Points.size() == 5))
    {
      // Calculate block extents clipping points
      OdGeMatrix3d toClipSpace = matTransform * clipBoundary.m_xInverseBlockRefXForm * clipBoundary.m_xToClipSpace.inverse();
      toClipSpace.invert();

      OdGeExtents3d extBlock = getDwgBlockExtents(pBlockReference);

      if( extBlock.isValidExtents() )
      {
        extBlock.transformBy(toClipSpace);

        OdGePoint2dArray arrBlockExtBoundary;
        OdGePoint2d pt0 = extBlock.minPoint().convert2d();
        OdGePoint2d pt2 = extBlock.maxPoint().convert2d();
        OdGePoint2d pt1 = pt0;
        pt1.x = pt2.x;
        OdGePoint2d pt3 = pt0;
        pt3.y = pt2.y;
        arrBlockExtBoundary.push_back(pt0);
        arrBlockExtBoundary.push_back(pt1);
        arrBlockExtBoundary.push_back(pt2);
        arrBlockExtBoundary.push_back(pt3);
        arrBlockExtBoundary.push_back(pt0);

        // Clip boundary more then extents
        if( OdDgnImportContext::getCropXRefExtraClipByExtentsFlag() && isExtentsIntoClipBoundary(arrBlockExtBoundary, clipBoundary.m_Points))
          clipBoundary.m_Points = arrBlockExtBoundary;
      }
    }
