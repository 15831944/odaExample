...
void setActiveUnits(const OdDgModelPtr& pModel, OdDbDatabase* pDb)
{
  OdDgModel::UnitMeasure masterUnitsEnum = pModel->getMasterUnit();
  OdDb::UnitsValue insUnit = OdDb::kUnitsInches;

  switch( masterUnitsEnum )
  {
    case OdDgModel::kMegameters: insUnit = OdDb::kUnitsUndefined; break;
    case OdDgModel::kKilometers: insUnit = OdDb::kUnitsKilometers; break;
    case OdDgModel::kMeters: insUnit = OdDb::kUnitsMeters; break;
    case OdDgModel::kDecimeters: insUnit = OdDb::kUnitsDecimeters; break;
    case OdDgModel::kCentimeters: insUnit = OdDb::kUnitsCentimeters; break;
    case OdDgModel::kMillimeters: insUnit = OdDb::kUnitsMillimeters; break;
    case OdDgModel::kMicrometers: insUnit = OdDb::kUnitsMicrons; break;
    case OdDgModel::kNanometers: insUnit = OdDb::kUnitsNanometers; break;
    case OdDgModel::kMiles: insUnit = OdDb::kUnitsMiles; break;
    case OdDgModel::kYards: insUnit = OdDb::kUnitsYards; break;
    case OdDgModel::kFeet: insUnit = OdDb::kUnitsFeet; break;
    case OdDgModel::kInches: insUnit = OdDb::kUnitsInches; break;
    case OdDgModel::kMils: insUnit = OdDb::kUnitsMils; break;
    case OdDgModel::kUSSurveyFeet: insUnit = OdDb::kUnitsUSSurveyFeet; break;
    case OdDgModel::kMicroinches: insUnit = OdDb::kUnitsMicroinches; break;
    default: insUnit = OdDb::kUnitsUndefined; break;
  }

  pDb->setINSUNITS(insUnit);
}
...
OdDgnImport::ImportResult DgnImporter::import()
{
  ...
  OdDgnImportContext::setCropXRefExtraClipByExtentsFlag(_properties->get_CropXRefExtraClipByExtents());
  ...
  OdDgDatabasePtr dgn = _properties->get_DgnDatabase();

  if( !OdDgnImportContext::getInternalImportFlag() && dgn.isNull() )
  {
    _services->cleanupLineStyleDefRscMap();
    _services->cleanupRscFontMap();

    if (!_properties->get_MS_SYMBRSRC().isEmpty())
      _services->setMS_SYMBRSRC(const_cast<OdStringArray&>(_properties->get_MS_SYMBRSRC()));

    if (!_properties->get_dgnFastFont().isEmpty())
      _services->setFAST_FONT(_properties->get_dgnFastFont());

    _services->setMS_REF_NEWLEVELDISPLAY(_properties->get_ShowNewXRefLevels());
  }

  OdDgnImportContext::setShowNewXRefLevelsFlag(_properties->get_ShowNewXRefLevels());

  if( _properties->get_EnableDgnFileMTLoading() )
    _services->setMtMode(1);
  else
    _services->setMtMode(0);

  try
  {    
    if( dgn.isNull() )
      dgn = _services->readFile(_properties->get_DgnPath());
    ...
    OdDgnImportContext::setImportDgTextAsDbMText( _properties->get_ImportDgTextsAsDbMTexts() );

    // Get dgn model to import it to dwg model space.

    OdDgModelPtr pModelSpaceModel;
    OdDgModelPtr pActiveSpaceModel;
    OdDgElementId idModelSpaceModel;

    bool modelSpaceFound = false;
    bool bSetActivePaperSpaceLayout = false;
    ...
    if( bImportActiveModel )
    {
      ...
      if( !idModelSpaceModel.isNull() )
      {
        ...
        if( !pModelSpaceModel.isNull() && pModelSpaceModel->isKindOf(OdDgSheetModel::desc()) && _properties->get_ImportPaperSpaceModels() )
        {
          OdUInt32 nDesignModels = 0;

          for (OdDgElementIteratorPtr mi = dgn->getModelTable()->createIterator(); !mi->done(); mi->step())
          {
            OdDgModelPtr m = mi->item().safeOpenObject();

            if( !m.isNull() && !m->isKindOf(OdDgSheetModel::desc()))
              nDesignModels++;
          }

          if (nDesignModels == 1)
          {
            modelSpaceFound = false;
            idModelSpaceModel = NULL;
            bSetActivePaperSpaceLayout = true;
          }
        }
      }
    }
    ...
    copyLayers(dgn->getLevelTable(OdDg::kForRead), pDb); // Should be called after line styles, because depends from id's
    ...
    copyBlocks(dgn, pDb, true, false);
    copyDimStyles(dgn, pDb);
    copyBlocks(dgn, pDb, false, true);
    copyDatabaseDefaults(dgn, pDb);
    setActiveUnits(pModelSpaceModel, pDb);

    pDb->setLWDISPLAY( true );
    pDb->setXCLIPFRAME(0);

		setLevelVisibilityByLevelMaskMode( dgn );
    ...
    if( !modelSpaceFound || _properties->get_ImportPaperSpaceModels() )
    {
      for( OdDgElementIteratorPtr mi = dgn->getModelTable()->createIterator(); !mi->done(); mi->step())
      {
        ...
        {
          OdDbObjectId idLayout = copyLayout(m, pDb);

          if( !isInitialized && bRemoveEmptyLayouts && (pDb->appServices()->layoutManager()->countLayouts(pDb) > 2) )
          {
            pDb->appServices()->layoutManager()->deleteLayout(pDb, OD_T("Layout1"));
            pDb->appServices()->layoutManager()->deleteLayout(pDb, OD_T("Layout2"));
            bRemoveEmptyLayouts = false;
          }

          if (bSetActivePaperSpaceLayout && (m == pActiveSpaceModel))
          {
            pDb->appServices()->layoutManager()->setCurrentLayout(pDb, idLayout);
...
void DgnImporter::updateLayerVisibilityByLevelMask( const OdDgDatabase* pDg )
{
	...
	for (; !pLevelIter->done(); pLevelIter->step())
	{
		...
		OdDbObjectId idLayer = OdDgnImportContext::getObjectId(pLevelIter->item());

		if (idLayer.isNull())
			continue;

		OdDbLayerTableRecordPtr pLayer = idLayer.openObject(OdDb::kForWrite);

		if( (pLevel->getEntryId() != 0) && (pLevel->getEntryId() != 0x40))
		{
			bool bLayerFrozen = !OdDgnImportContext::checkLevelMask(pLevel->getEntryId(), !pLayer->isFrozen());

     if( !pLayer->isFrozen() || (OdDgnImportContext::getXRefNestedDepth() != 0) )
			  pLayer->setIsFrozen(bLayerFrozen);
		}
	}
}
...
void DgnImporter::setLevelVisibilityByLevelMaskMode( OdDgDatabase* pDg )
{
	if (!OdDgnImportContext::getInternalImportFlag())
	{
		if (!_properties->get_ImportPaperSpaceModels())
			OdDgnImportContext::setLevelVisibilityByLevelMask(true);
		else
		{
			bool bAllowToSetLevelVisibilityByLevelMask = true;

			for (OdDgElementIteratorPtr mi = pDg->getModelTable()->createIterator(); !mi->done(); mi->step())
			{
				OdDgModelPtr m = mi->item().safeOpenObject();

				if (m->isKindOf(OdDgSheetModel::desc()))
				{
					bAllowToSetLevelVisibilityByLevelMask = false;
					break;
				}
			}

			OdDgnImportContext::setLevelVisibilityByLevelMask(bAllowToSetLevelVisibilityByLevelMask);
		}
	}
}
...
void DgnImporter::setImportPEToElements( bool bExplodeTextNode )
{
  ...
  static OdStaticRxObject<OdDgnProxyGraphicsElementImportPE> proxyElementImp;
  OdDgProxyGraphicsElement::desc()->addX(OdDgnImportPE::desc(), &proxyElementImp);
  OdDgComplexProxyGraphicsElement::desc()->addX(OdDgnImportPE::desc(), &proxyElementImp);
  OdDgExtendedGraphicsElement::desc()->addX(OdDgnImportPE::desc(), &proxyElementImp);
}
...
void DgnImporter::removeImportPEFromElements()
{
  ...
  OdDgProxyGraphicsElement::desc()->delX(OdDgnImportPE::desc());
  OdDgComplexProxyGraphicsElement::desc()->delX(OdDgnImportPE::desc());
  OdDgExtendedGraphicsElement::desc()->delX(OdDgnImportPE::desc());
}
...
ODRX_DECLARE_PROPERTY( dgnFastFont )
...
ODRX_DECLARE_PROPERTY( ImportDgTextsAsDbMTexts )
ODRX_DECLARE_PROPERTY( CropXRefExtraClipByExtents )
ODRX_DECLARE_PROPERTY( EnableDgnFileMTLoading )
ODRX_DECLARE_PROPERTY( ShowNewXRefLevels )

ODRX_DEFINE_PROPERTY(DgnPath, DgnProperties, getString)
ODRX_DEFINE_PROPERTY_OBJECT(DgnDatabase, DgnProperties,  get_DgnDatabase, put_DgnDatabase, OdDgDatabase)
...
ODRX_DEFINE_PROPERTY(ImportDgTextsAsDbMTexts, DgnProperties, getBool)
...
ODRX_DEFINE_PROPERTY(dgnFastFont, DgnProperties, getString)
...
ODRX_DEFINE_PROPERTY(CropXRefExtraClipByExtents, DgnProperties, getBool)
ODRX_DEFINE_PROPERTY(EnableDgnFileMTLoading, DgnProperties, getBool)
ODRX_DEFINE_PROPERTY(ShowNewXRefLevels, DgnProperties, getBool)
...
ODRX_BEGIN_DYNAMIC_PROPERTY_MAP( DgnProperties );
  ...
  ODRX_GENERATE_PROPERTY( ImportDgTextsAsDbMTexts )
  ...
  ODRX_GENERATE_PROPERTY( dgnFastFont )
  ...
  ODRX_GENERATE_PROPERTY( CropXRefExtraClipByExtents )
  ODRX_GENERATE_PROPERTY( EnableDgnFileMTLoading )  
  ODRX_GENERATE_PROPERTY( ShowNewXRefLevels )
