...
OdDbLinetypeTableRecordPtr DgnLSImporter::copyLinetype(OdDgLineStyleTableRecord *pLS)
{
  ...
  if (pLS->getRefersToElementFlag())
  {
    ...
    if (pLS->getUnitsType() == OdDg::kLsMasterUnits)
      m_fUORPerMaster = 1.0;
  ...
  if (!bIsInternal)
  {
    ...
    {
      if (m_pLTTable->getAt(name) == m_pLTTable->database()->getLinetypeContinuousId())
      {
        name += L"_DGN";
      }

      if (m_pLTTable->has(name))
...
OdDbObjectId OdDgnImportLineStyleImpl::importLineStyle( OdDgDatabase* pDgnDb, OdRxObject* pDwgDbObj, const OdString& strLineStyleName )
{
  OdDbObjectId retVal;
  ...
  OdDbDatabase* pDwgDb = (OdDbDatabase*)(pDwgDbObj);
