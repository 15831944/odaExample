...
//-----------------------------------------------------------------------------------------------
  ...
  extern void importDimensionSegment(OdDgElement* e, OdUInt32 uSegmentNum, const OdGeVector3d& vrNormal, 
    double dElevation, OdDbBlockTableRecord* pDimBlock);
...
OdDbObjectId createTextStyleFromDimension(OdDgDimension* pDim, OdDbDatabase* pDb )
{
  ...
  pFontRec = OdDgFontTable::getFont(pDim->database(), txtInfo.getFontEntryId());
  ...
  if(!pFontRec.isNull() && (pFontRec->getType() != kFontTypeTrueType) && !pDgDimPE.isNull() && (pDgDimPE->getTextType( pDim ) == OdDgDimTool::kVertical) )
  {
  ...
  if( idRet.isNull() )
  {
    idRet = OdDgnImportContext::createStyleForDbText(pDb, pDim->database(), dimTextStyle, true, true );
  }
...
OdString getDgnFontName( OdDgDatabase* pDgnDb, OdUInt32 uFontEntryId, bool& bTTF )
{
  ...
  OdDgFontTableRecordPtr pFont = OdDgFontTable::getFont(pDgnDb, uFontEntryId);
...
OdDbObjectId getTerminatorSymbolCellId( OdUInt32 uFontId, OdChar chSymbol, OdDgDatabase* pDgnDb, OdDbDatabase* pDwgDb )
{
  ...
  if( !pBlockTable.isNull() && pBlockTable->getAt( strBlockName ).isNull() )
  {
    ...
    pBlock->setOrigin(OdGePoint3d(0.0, 0.0, 0.0));
    ...
    textStyle.setTextSize(0.0);

    OdDbObjectId idTextStyle = OdDgnImportContext::getStyleForDbText( pDwgDb, textStyle );

    if( idTextStyle.isNull() )
    {
      idTextStyle = OdDgnImportContext::createStyleForDbText( pDwgDb, pDgnDb, textStyle, false );
    }
...
void createDimensionGeometryBlock( OdDgDimension* pDgDim, OdUInt32 uPointNum, const OdGeVector3d& vrNormal, 
                                   OdDbDimension* pDbDim, OdDbDatabase* pDb, OdDbBlockTableRecord* owner)
{
  ...
  if( !pBlockTable.isNull() )
  {
    ...
    importDimensionSegment( pDgDim, uPointNum, vrNormal, pDbDim->elevation(), pDimGeometryBlock );
                                                      ...
                                                      )
{
  ...
  if( OdDgnImportContext::getActualViewFlags().getUseLevelSymbologyFlag() )
  {
    ...
    applyLevelSymbologyOverrides(OdDgnImportContext::getLevelOverride(pDim->getLevelId()), uTextColorIndex, uTmp1, uTmp2, bLineStyleIsFromLevelDatabase);
  }
...
//     pDbDim->setTextRotation( dTextAngle );
  ...
  bool     bDimStyleIsFromLevelDatabase = false;

  OdUInt32 iExtColor  = iDimColor;
  OdUInt32 iExtWeight = iDimWeight;
  OdUInt32 iExtStyle  = iDimStyle;
  bool     bExtStyleIsFromLevelDatabase = false;

  OdDgElementId idDimLevel = OdDgnImportContext::getLevelOverride(pDim->getLevelId());
  ...
  if( OdDgnImportContext::getActualViewFlags().getUseLevelSymbologyFlag() )
  {
    ...
    applyLevelSymbologyOverrides(idDimLevel, iExtColor, iExtWeight, iStyle, bExtStyleIsFromLevelDatabase);
    iExtStyle = iStyle;
    iStyle = iDimStyle;
    applyLevelSymbologyOverrides(idDimLevel, iDimColor, iDimWeight, iStyle, bDimStyleIsFromLevelDatabase);
    iDimStyle = iStyle;
  }
  ...
  if( bExtStyleIsFromLevelDatabase )
    setDwgLineTypeIdByDgnLineTypeIndex(pDb, idDimLevel.database(), iExtStyle, lineTypeId);
  else
  ...
  if( bDimStyleIsFromLevelDatabase)
    setDwgLineTypeIdByDgnLineTypeIndex(pDb, idDimLevel.database(), iDimStyle, lineTypeId);
  else
  ...
  double dArrowWidth = geom.getTerminatorHeight() > geom.getTerminatorWidth() ? geom.getTerminatorWidth() : geom.getTerminatorHeight();

  if (OdZero(dArrowWidth))
    dArrowWidth = dArrowSize;
                                 ...
                                 )
{
  ...
  createDimensionGeometryBlock( pDim, iPoint - 1, vrNormal, pDbDim, owner->database(), owner);
  ...
  void importSizeArrowDimension( OdDgElement* e, OdDbBlockTableRecord* owner )
  {
    ...
    for( OdUInt32 i = 1; i < numPts; i++ )
    {
      ...
      if( pDgDim->isKindOf(OdDgDimSingleLocation::desc()) )
      {
        ...
        double dStackOffset = dDimLineStackOffset;

        if (dStackOffset > 0 && tHeight > dStackOffset)
          dStackOffset = tHeight + txtInfo.getHeight();
        ...
        {
          if (i > 1)
            bUseStackOffset = pDgDimPE->getUseStackOffset(pDgDim, ptBase, ptEndIsect, ptDimEnd, vrDimNormal, dDimWidth, tWidth, tHeight, dStackOffset);
          else
            bUseStackOffset = false;
        }
      ...
      else
      {
        ...
        {
          if (i > 1)
            bUseStackOffset = pDgDimPE->getUseStackOffset(pDgDim, ptCurIsect, ptEndIsect, ptDimEnd, vrDimNormal, dDimValue, tWidth, tHeight, dStackOffset);
          else
            bUseStackOffset = false;
        }
...
//-----------------------------------------------------------------------------------------------
                                            ...
                                            )
  {
    ...
    createDimensionGeometryBlock( pDim, uSegmentNum, vrNormal, pAngDim, owner->database(), owner);
...
//-----------------------------------------------------------------------------------------------
                                                     ...
                                                     OdDbBlockTableRecord* owner )
  {
    ...
    createDimensionGeometryBlock( pDim, 0, vrNormal, pDbDim, owner->database(), owner);
  }
...
//-----------------------------------------------------------------------------------------------
  ...
  void importDiameterRadiusDimension( OdDgElement* e, OdDbBlockTableRecord* owner, bool bDiameterDimension )
  {
    ...
    createDimensionGeometryBlock( pDim, 0, vrNormal,pDbDimBase, owner->database(), owner);
  }
...
//       vrDimYInc *= -1;
    ...
    for( OdUInt32 i = 0; i < nPts; i++ )
    {
      ...
      createDimensionGeometryBlock( pDim, i, vrNormal, pDbDim, owner->database(), owner);
    }
  }
...
//-----------------------------------------------------------------------------------------------
  ...
  void OdDgnCenterMarkDimensionImportPE::subImportElement(OdDgElement* e, OdDbBlockTableRecord* owner)
  {
    ...
    if( !pBlockTable.isNull() )
    {
      ...
      importDimensionSegment( pDim, 0, vrNormal, 0.0, pDimGeometryBlock );

      OdDgDimRotation quatDim(pDim);

      OdGeMatrix3d matTransform = quatDim.getMatrix().transpose() * OdGeMatrix3d::translation( pDim->getPoint(0).getPoint().asVector() );
...
//-----------------------------------------------------------------------------------------------
  ...
  OdDbObjectId getDimNoteInternalMText( OdDgDimNote* pDim, OdDbBlockTableRecord* owner )
  {
    ...
    if( pImporter && pDim )
    {
      ...
      OdArray<OdDgElementPtr> pTextNodeArr;
      ...
      for( OdUInt32 k = 0; k < arrDimExplode.size(); k++ )
      {
        ...
        {
          pTextNodeArr.push_back(arrDimExplode[k]);
        }
      }

      if( pTextNodeArr.size() > 0 )
      {
        OdDgGraphicsElementPtr pGrText;

        if( pTextNodeArr.size() == 1 )
        {
          pGrText = pTextNodeArr[0];
          OdUInt32 uOldColor  = pGrText->getColorIndex();
          pGrText->setPropertiesFrom( pDim );
          pGrText->setColorIndex( uOldColor );
        }
        else
        {
          OdDgTextNode3dPtr pTextNode = OdDgTextNode3d::createObject();
          pTextNode->setDatabaseDefaults(pDim->database());

          for( OdUInt32 k = 0; k < pTextNodeArr.size(); k++ )
          {
            if( pTextNodeArr[k]->getElementType() == OdDgElement::kTypeText )
            {
              pGrText = pTextNodeArr[k];

              if( pTextNodeArr[k]->isKindOf(OdDgText3d::desc()) )
              {
                OdDgText3dPtr pText3d = pTextNodeArr[k];
                pText3d->setCrCount(1);

                if( k == 0 )
                {
                  pTextNode->setOrigin( pText3d->getOrigin() );
                  pTextNode->setRotation( pText3d->getRotation() );
                }
              }
              else if( pTextNodeArr[k]->isKindOf(OdDgText2d::desc()) )
              {
                OdDgText2dPtr pText2d = pTextNodeArr[k];
                pText2d->setCrCount(1);

                if( k == 0 )
                {
                  OdGePoint2d ptOrigin = pText2d->getOrigin();
                  OdGeMatrix3d matRotation = OdGeMatrix3d::rotation( pText2d->getRotation(), OdGeVector3d::kZAxis);
                  OdGeQuaternion quatRot;
                  quatRot.set(matRotation);

                  pTextNode->setOrigin( OdGePoint3d(ptOrigin.x, ptOrigin.y, 0.0) );
        ...
        if( !pBlockTable.isNull() )
        {
          ...
          ((OdDgnImportPE*)pGrText->queryX(OdDgnImportPE::desc()))->subImportElement(pGrText, pDimGeometryTmpBlock);
                                                          ...
                                                          )
{
  ...
  else
  {
    switch( iAlignment )
    {
      ...
      case OdDgDimNote::kAlignUnderline:
      {
        retVal = OdDbMLeaderStyle::kAttachmentAllLine;
      } break;
    }
  }
...
static OdDbObjectId findMleaderStyle(OdDbDatabase* pDwgDb, 
                                     OdDbMLeaderStyle::ContentType type = OdDbMLeaderStyle::kNoneContent,
                                     double dLeaderLength = 0.0,
                                     double dTextHeight = 0.0, // for kMTextContent only
                                     double dLandingGap = 0.0, // for kMTextContent only
                                     bool bCreateIfMissing = true)
{
  ODA_ASSERT_ONCE(pDwgDb && dLeaderLength >= 0.0 && dTextHeight >= 0.0);
  ...
  for (OdDbDictionaryIteratorPtr pMStyleIter = pMLeaderStyleDic->newIterator();
       idMLStyle.isNull() && !pMStyleIter->done(); pMStyleIter->next() )
  {
    OdDbMLeaderStylePtr pCurMLeaderStyle = OdDbMLeaderStyle::cast(pMStyleIter->getObject(OdDb::kForRead));
    ODA_ASSERT_ONCE(pCurMLeaderStyle.get())
    if (pCurMLeaderStyle.isNull())
      continue;

    if (   pCurMLeaderStyle->contentType() == type 
        && OdEqual(pCurMLeaderStyle->doglegLength(), dLeaderLength))
    {
      switch (type)
      {
      case OdDbMLeaderStyle::kMTextContent:
        if (!OdEqual(pCurMLeaderStyle->textHeight(), dTextHeight))
          break;
  ...
  if (idMLStyle.isNull() && bCreateIfMissing)
  {
    ...
    if (type == OdDbMLeaderStyle::kMTextContent)
    {
      pMLeaderStyle->setTextHeight(dTextHeight);
      pMLeaderStyle->setLandingGap(dLandingGap);
    }
...
//-----------------------------------------------------------------------------------------------
  ...
  void OdDgnNoteDimensionImportPE::subImportElement(OdDgElement* e, OdDbBlockTableRecord* owner)
  {
    ...
    OdDgDimNote::NoteTerminatorType iNoteTermType = OdDgDimNote::kTtnArrow;
    ...
    iNoteTermType  = pDim->getTerminator();
    bSplineFit     = pDim->getSplineFit();

    OdDgDimTool::TerminatorType iTermType = (OdDgDimTool::TerminatorType )(iNoteTermType);

    if( iNoteTermType == OdDgDimNote::kTtnNote )
      iTermType = OdDgDimTool::kTtArrow;

    iTextRot = pDim->getTextRotation();
    if (iTextRot != OdDgDimTextInfo::kHorizontal)
    {
    ...
    OdDbMLeaderStyle::TextAngleType textAngleType = OdDbMLeaderStyle::kAlwaysRightReadingAngle; // = OdDbMLeaderStyle::kInsertAngle;
    switch (iTextRot)
    {
    case OdDgDimTextInfo::kHorizontal:
    case OdDgDimTextInfo::kVertical:
      textAngleType = OdDbMLeaderStyle::kHorizontalAngle;
    ...
    //  //if (pDim->getLeaderLength(dLeaderLength))
    //  //{
    //  //  if (OdZero(dDimLeaderLength))
    //  //    dLeaderLength = 0;
    //  //  else
    //  //    dLeaderLength = dDimLeaderLength;
    //  //}
    ...
    bool bSetZeroDglLength = OdZero(dLeaderLength);
    ...
    OdDbMTextPtr pMText;
    if (!idMText.isNull())
      pMText = OdDbMText::cast(idMText.openObject(OdDb::kForWrite));
    if (pMText.get() && iTextRot == OdDgDimTextInfo::kInline)
    {
      OdGeVector3d vTextDir = pMText->direction();
      if (vTextDir.isParallelTo(OdGeVector3d::kXAxis))
        iTextRot = OdDgDimTextInfo::kHorizontal;
      else if (vTextDir.isParallelTo(OdGeVector3d::kYAxis))
        iTextRot = OdDgDimTextInfo::kVertical; // DGN-2569
      switch (iTextRot)
      {
      case OdDgDimTextInfo::kHorizontal:
      case OdDgDimTextInfo::kVertical:
        textAngleType = OdDbMLeaderStyle::kHorizontalAngle;
        dLeaderLength = txtInfo.getWidth()*2.0;
        bSetZeroDglLength = OdZero(dLeaderLength);
        break;
      }
    }
    ...
    if (   iTextRot == OdDgDimTextInfo::kHorizontal && geom.getUseMargin() 
        && pMText.get() && !bSplineFit && uFrameType == OdDgDimNote::kFrameNone // TODO for other criteria instead frame type
        && dLeaderLength > dLeftMargin && dLeftMargin > 0)
    {
      OdString sText = pMText->contents();
      if (!sText.isEmpty() && sText.find(L"\\P") < 0)
      {
        dLeaderLength = dLeftMargin; // = geom.getTextMargin(); // right low mleader of DgnImportDimensions_12808_RAV 4,12
        bSetZeroDglLength = OdZero(dLeaderLength);
      }
    }
    // unused //pDim->getLeftMargin(dLeftMargin);
    ...
    double dLandingGap = geom.getTextMargin() / textInfo.getHeight();
    pDim->getLeftMargin(dLandingGap);
    pMLeader->setLandingGap(dLandingGap * textInfo.getHeight());
    
    OdDbMLeaderStyle::ContentType contentType = OdDbMLeaderStyle::kNoneContent;
    OdGePoint3d ptLoc;

    if( !idMText.isNull() )
    {
      pMText = idMText.safeOpenObject(OdDb::kForWrite);
      pMText->setWidth(0);
      ...
      curPlane.project(pMText->location(), ptLoc);

      double dTextHeight = pMText->textHeight();
      OdDbObjectId idMLStyle = findMleaderStyle(pDwgDb, OdDbMLeaderStyle::kMTextContent,
                                                dLeaderLength, dTextHeight);
      pMLeader->setMLeaderStyle(idMLStyle);
      pMLeader->setContentType(contentType = OdDbMLeaderStyle::kMTextContent);
      pMLeader->setTextAngleType(textAngleType);

      pMLeader->setTextHeight(dTextHeight);

      if (pDim->getSplineFit()) // || iTextRot == OdDgDimTextInfo::kVertical)
      {
        //dLeaderLength = 0;
    ...
    {
      OdDbObjectId idMLStyle = findMleaderStyle(pDwgDb, OdDbMLeaderStyle::kBlockContent);
      pMLeader->setMLeaderStyle(idMLStyle);
      pMLeader->setContentType(contentType = OdDbMLeaderStyle::kBlockContent );
      pMLeader->setBlockContentId( idBlock );
    }
    else
    {
      OdDbObjectId idMLStyle = findMleaderStyle(pDwgDb, OdDbMLeaderStyle::kNoneContent);
      pMLeader->setMLeaderStyle(idMLStyle);
      pMLeader->setContentType(contentType = OdDbMLeaderStyle::kNoneContent);
    ...
    if (numPts > 1)
    {
      OdGePoint3d ptFirst, ptLast, pt;
      curPlane.project(arrDimPts.first(), ptFirst);
      curPlane.project(arrDimPts.last(), ptLast);

      if (pMText.get())
      {
        while (numPts > 2 && OdZero(dLeaderLength))
        {
          curPlane.project(arrDimPts[numPts - 2], pt);
          if (pt == ptLast)
          {
            numPts--;
            continue;
          ...
          if (OdEqual(pt.y, ptLast.y))
          {
            // ignore textLocation in this case 
            // (it should be set automatic by last point
            //  with direction by previous point)
            textAngleType = OdDbMLeaderStyle::kHorizontalAngle;
            dLeaderLength = fabs(ptLast.x - pt.x);
            if (!bTextFrame)
              dLeaderLength += txtInfo.getWidth()*2.0; // problem with DgnImportNoteDims_DGN1712_RAV 0
            bSetZeroDglLength = false; // == OdZero(dLeaderLength);
            curPlane.project(arrDimPts[--numPts - 1], ptLast);
          }
          break;
        }
      ...
      ODA_ASSERT_ONCE(bSetZeroDglLength == OdZero(dLeaderLength) || pDim->getSplineFit());

      if (pMText.get() && textAngleType == OdDbMLeaderStyle::kHorizontalAngle)
      {
        pMLeader->setMText(pMText);
        pMLeader->addLeaderLine(ptLast, uLeaderLineIndex);

        if (!OdEqual(pMLeader->doglegLength(uLeaderLineIndex), dLeaderLength))
          pMLeader->setDoglegLength(uLeaderLineIndex, dLeaderLength);

        pMLeader->getLastVertex(uLeaderLineIndex, pt);
        if (pt != ptLast)
          pMLeader->setLastVertex(uLeaderLineIndex, ptLast);

        ODA_ASSERT_ONCE(pMLeader->getLastVertex(uLeaderLineIndex, pt) == eOk);
        ODA_ASSERT_ONCE(pt == ptLast);

        for (int idx = numPts - 2; idx >= 0; idx--)
        {
          curPlane.project(arrDimPts[idx], pt);
          if (idx == (numPts - 2))
          {
            ODA_ASSERT_VAR(int num = 0; OdGePoint3d ptFirst;)
            ODA_ASSERT_ONCE_X(MLEADER,    pMLeader->numVertices(uLeaderLineIndex, num) == eOk && num == 2
                                       && pMLeader->getFirstVertex(uLeaderLineIndex, ptFirst) == eOk && ptFirst == ptLast);

            pMLeader->setFirstVertex(uLeaderLineIndex, pt); // DGN-2569
          }
          else
            pMLeader->addFirstVertex(uLeaderLineIndex, pt);
        }
      }
      else
      {
        pMLeader->addLeaderLine(ptFirst, uLeaderLineIndex);

        if (!OdEqual(pMLeader->doglegLength(uLeaderLineIndex), dLeaderLength))
          pMLeader->setDoglegLength(uLeaderLineIndex, dLeaderLength);

        for (int idx = 1; idx < (int)numPts; idx++) //for( int k = numPts-2; k >= 0; k-- )
        {
          curPlane.project(arrDimPts[idx], pt);
          pMLeader->addLastVertex(uLeaderLineIndex, pt);
        }
        ...
        if (pMText.get())
        {
          ...
          ODA_ASSERT_ONCE(bSetZeroDglLength);
          ...
          //if (bSetZeroDglLength)
          {
            pMLeader->setEnableLanding(false);
            pMLeader->setEnableDogleg(false);
          }

          pMLeader->getTextLocation(pt);
          if (pt != ptLoc) //ODA_ASSERT_ONCE(pt == ptLoc);
            pMLeader->setTextLocation(ptLoc);
        }
      }
      ...
      //if (pMText.get() && numPts > 1)
      //{
      //  if (textAngleType != OdDbMLeaderStyle::kHorizontalAngle)
      //  {
      //    pMLeader->getTextLocation(pt);
      //    if (pt != ptLoc)
      //      pMLeader->setTextLocation(ptLoc);
  ...
  if (contentType != OdDbMLeaderStyle::kNoneContent)
  {
    //OdGePoint3d ptOldTextPos;
    //if( pMLeader->contentType() != OdDbMLeaderStyle::kBlockContent )
    //  pMLeader->getTextLocation(ptOldTextPos);

    OdDgDimNote::NoteAttachmentAlignment leftAlign  = pDim->getNoteLeftAttachmentAlignment();
    OdDgDimNote::NoteAttachmentAlignment rightAlign = pDim->getNoteRightAttachmentAlignment();

    if( !bExternalContext )
    {
      if( leftAlign == OdDgDimNote::kAlignMiddle && (pDim->getTextLocation() == OdDgDimTextInfo::kTextAbove) )
        leftAlign =OdDgDimNote::kAlignUnderline;

      if( rightAlign == OdDgDimNote::kAlignMiddle && (pDim->getTextLocation() == OdDgDimTextInfo::kTextAbove) )
        rightAlign =OdDgDimNote::kAlignUnderline;
    }

    pMLeader->setTextAttachmentType( convertTextAttachment( leftAlign, bMultiRowMText ), OdDbMLeaderStyle::kLeftLeader );
    pMLeader->setTextAttachmentType( convertTextAttachment( rightAlign, bMultiRowMText ), OdDbMLeaderStyle::kRightLeader );
    pMLeader->setTextAttachmentType( convertTextAttachment( leftAlign, bMultiRowMText ), OdDbMLeaderStyle::kTopLeader );
    pMLeader->setTextAttachmentType( convertTextAttachment( rightAlign, bMultiRowMText ), OdDbMLeaderStyle::kBottomLeader );

    //if( pMLeader->contentType() != OdDbMLeaderStyle::kBlockContent )
    //  pMLeader->setTextLocation( ptOldTextPos );
  }

    if (pDim->getSplineFit())
      pMLeader->setLeaderLineType(OdDbMLeaderStyle::kSplineLeader); // pMLeader->setLeaderLineType(uLeaderLineIndex, OdDbMLeaderStyle::kSplineLeader);

    if (pMLeader->objectId().isNull())
      owner->appendOdDbEntity( pMLeader );
    if (pMText.get() && bSetZeroDglLength)
    {
      //double dDeltaWidth = pMLeader->mtext()->actualWidth();
      pMText = NULL; 
      // next works for DgnInport in ODE
      // TODO: but it gets incorrect save DWG result in DgnImportDimensions_12808_RAV 13
      // (it is strange but mtext()->actualWidth() returns differ value)
      pMLeader->downgradeOpen(); pMLeader->upgradeOpen(); // force recompute (is needed via setTextAttachmentType etc)
    }
