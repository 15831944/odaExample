...
inline
bool isNestedPersistent(const OdGiDrawableDesc *pCurPath)
{
  while (pCurPath) // skip transients
  {
    if (pCurPath->persistId && GETBIT(pCurPath->nDrawableFlags, OdGiDrawable::kDrawableUsesNesting) &&
        pCurPath->pParent   && GETBIT(pCurPath->pParent->nDrawableFlags, OdGiDrawable::kDrawableIsCompoundObject))
      break;
    pCurPath = pCurPath->pParent;
  }
  return pCurPath != NULL;
}
...
inline
OdUInt64 ExGsDecodeHex(const char *pIn, OdUInt32 nLen = 16)
{ OdUInt64 nVal = 0;
  if (nLen > 16) nLen = 16;
  OdUInt32 nProc = 16 - nLen;
  OdUInt8 nCurProc = 0;
  while (nProc < nLen)
  { OdUInt8 val = 0;
    switch ((char)*pIn)
    ...
    if (!GETBIT(nProc, 1))
      ...
      nVal |= OdUInt64(nCurProc | val) << OdUInt64((16 - (nProc + 1)) << 2), nCurProc = 0;
    nProc++; pIn++;
  }
  return nVal;
}

// Check that line is degenerate inside view space

inline
int isLineParallelDegenerate(const OdGePoint3d &pt1, const OdGePoint3d &pt2,
                             ...
                             bool bLinesRepModeEnabled = false)
{
  ...
  const OdGeVector3d line(pt2 - pt1);
  const OdGeVector2d projLine(xVec.dotProduct(line), yVec.dotProduct(line));
  //if (projLine.isZeroLength()) // Optimized
  if (((projLine.x * projLine.x) + (projLine.y * projLine.y)) <= 1.e-20)
...
{
  public:
    static void getExtents(const OdGsViewImpl& view, OdGsOverlayId nOverlay, OdGeExtents3d& ext, bool bVisible = false)
    {
      view.getExtents(nOverlay, ext, bVisible);
    }
};
