...
ODRX_DEFINE_PROPERTY_METHODS2(LinesRepMode, ExGsDirectXDevice, ExGsDirectXDevice, isLinesRepModeEnabled, enableLinesRepMode, getBool);
...
ODRX_DEFINE_PROPERTY_METHODS2(Logger, ExGsDirectXDevice, ExGsDirectXDevice, logger, setLogger, getIntPtr);
...
const OdString D3ResToStr(HRESULT res)
{
  switch (res)
  {
  case D3DERR_WRONGTEXTUREFORMAT: return OD_T("D3DERR_WRONGTEXTUREFORMAT");
  case D3DERR_UNSUPPORTEDCOLOROPERATION: return OD_T("D3DERR_UNSUPPORTEDCOLOROPERATION");
  case D3DERR_UNSUPPORTEDCOLORARG: return OD_T("D3DERR_UNSUPPORTEDCOLORARG");
  case D3DERR_UNSUPPORTEDALPHAOPERATION: return OD_T("D3DERR_UNSUPPORTEDALPHAOPERATION");
  case D3DERR_UNSUPPORTEDALPHAARG: return OD_T("D3DERR_UNSUPPORTEDALPHAARG");
  case D3DERR_TOOMANYOPERATIONS: return OD_T("D3DERR_TOOMANYOPERATIONS");
  case D3DERR_CONFLICTINGTEXTUREFILTER: return OD_T("D3DERR_CONFLICTINGTEXTUREFILTER");
  case D3DERR_UNSUPPORTEDFACTORVALUE: return OD_T("D3DERR_UNSUPPORTEDFACTORVALUE");
  case D3DERR_CONFLICTINGRENDERSTATE: return OD_T("D3DERR_CONFLICTINGRENDERSTATE");
  case D3DERR_UNSUPPORTEDTEXTUREFILTER: return OD_T("D3DERR_UNSUPPORTEDTEXTUREFILTER");
  case D3DERR_CONFLICTINGTEXTUREPALETTE: return OD_T("D3DERR_CONFLICTINGTEXTUREPALETTE");
  case D3DERR_DRIVERINTERNALERROR: return OD_T("D3DERR_DRIVERINTERNALERROR");
  case D3DERR_NOTFOUND: return OD_T("D3DERR_NOTFOUND");
  case D3DERR_MOREDATA: return OD_T("D3DERR_MOREDATA");
  case D3DERR_DEVICELOST: return OD_T("D3DERR_DEVICELOST");
  case D3DERR_DEVICENOTRESET: return OD_T("D3DERR_DEVICENOTRESET");
  case D3DERR_NOTAVAILABLE: return OD_T("D3DERR_NOTAVAILABLE");
  case D3DERR_OUTOFVIDEOMEMORY: return OD_T("D3DERR_OUTOFVIDEOMEMORY");
  case D3DERR_INVALIDDEVICE: return OD_T("D3DERR_INVALIDDEVICE");
  case D3DERR_INVALIDCALL: return OD_T("D3DERR_INVALIDCALL");
  case D3DERR_DRIVERINVALIDCALL: return OD_T("D3DERR_DRIVERINVALIDCALL");
  case D3DERR_WASSTILLDRAWING: return OD_T("D3DERR_WASSTILLDRAWING");
  case D3DOK_NOAUTOGEN: return OD_T("D3DOK_NOAUTOGEN");
  }
...
#endif
  SETBIT_1(m_flags, kDynamicHighlight | kHighlightTwoPass);
...
void ExGsDirectXDevice::createContext()
{
  ...
  HRESULT createDeviceRes = m_pD3D->CreateDevice(D3DADAPTER_DEFAULT, devType, m_hWnd,
    ...
    &d3dpp, &m_pD3Ddevice);
  logResult(OD_T("ExGsDirectXDevice::createContext CreateDevice"), createDeviceRes);
  if(FAILED(createDeviceRes) )
  {
    // some videocards (e.g. Intel 82915G/82910GL) doesn't support hardware vertexprocessing
    createDeviceRes = m_pD3D->CreateDevice(D3DADAPTER_DEFAULT, devType, m_hWnd,
      D3DCREATE_FPU_PRESERVE | D3DCREATE_SOFTWARE_VERTEXPROCESSING |
      ((m_vectPerfData.enableParallelVectorization()) ? D3DCREATE_MULTITHREADED : 0),
      &d3dpp, &m_pD3Ddevice);
    logResult(OD_T("ExGsDirectXDevice::createContext CreateDevice"), createDeviceRes);
    DXError::check(createDeviceRes);
  }
...
void ExGsDirectXDevice::logResult(const OdString& sMethod, HRESULT res) const
{
  if (m_pLogger)
  {
    m_pLogger->logMessage(sMethod + OD_T(" returned ") + D3ResToStr(res));
    if (res != D3D_OK)
      m_pLogger->logError(sMethod + OD_T(" returned ") + D3ResToStr(res));
  }
}
...
void ExGsDirectXDevice::onSize(const OdGsDCRect& outputRect)
{
  ...
  if(m_bContextValid)
  {
    ...
    if (m_pLogger)
      m_pLogger->logMessage(OD_T("ExGsDirectXDevice::onSize TestCooperativeLevel returned ") + D3ResToStr(coLevel));

    if (coLevel == D3DERR_DRIVERINTERNALERROR)
    {
      if (m_pLogger)
        m_pLogger->logError(OD_T("ExGsDirectXDevice::onSize TestCooperativeLevel returned D3DERR_DRIVERINTERNALERROR"));
      DXError::fail(coLevel);
    }
    else if (coLevel == D3DERR_DEVICELOST)
    {
      if (m_pLogger)
        m_pLogger->logWarning(OD_T("ExGsDirectXDevice::onSize TestCooperativeLevel returned D3DERR_DEVICELOST"));
      return; // Can't render anything in Lost Device state
    }

    // Get the current desktop display mode
    D3DDISPLAYMODE d3ddm;
    HRESULT adapterDisplayModeResult = m_pD3D->GetAdapterDisplayMode(D3DADAPTER_DEFAULT, &d3ddm);
    logResult(OD_T("ExGsDirectXDevice::onSize GetAdapterDisplayMode"), coLevel);
    DXError::check(adapterDisplayModeResult);
    ...
    d3dpp.EnableAutoDepthStencil = TRUE;
    d3dpp.AutoDepthStencilFormat = D3DFMT_D24S8;//D3DFMT_D16;
    HRESULT resetResult = m_pD3Ddevice->Reset(&d3dpp);
    logResult(OD_T("ExGsDirectXDevice::onSize Reset"), resetResult);
...
void ExGsDirectXDevice::update(OdGsDCRect* pUpdatedRect)
{
  ...
  else
  {
    ...
    switch (coLevel)
    {
      case D3DERR_DRIVERINTERNALERROR:
        DXError::fail(coLevel);
        if (m_pLogger)
          m_pLogger->logError(OD_T("ExGsDirectXDevice::update TestCooperativeLevel returned D3DERR_DRIVERINTERNALERROR"));
      break;
      case D3DERR_DEVICELOST:
        if (m_pLogger)
          m_pLogger->logWarning(OD_T("ExGsDirectXDevice::update TestCooperativeLevel returned D3DERR_DEVICELOST"));
        return; // Can't render anything in Lost Device state
      case D3DERR_DEVICENOTRESET:
        onSize(m_outputRect);
        if (m_pLogger)
          m_pLogger->logError(OD_T("ExGsDirectXDevice::update TestCooperativeLevel returned D3DERR_DEVICENOTRESET"));
      break;
    };
  }
  ...
  if (!supportPartialUpdate())
  {
    ...
    HRESULT clearResult = m_pD3Ddevice->Clear( 0L, NULL, D3DCLEAR_TARGET, bgc, 1.0f, 0L );
    logResult(OD_T("ExGsDirectXDevice::update Clear"), clearResult);
    HRESULT beginSceneResult =  m_pD3Ddevice->BeginScene();
    logResult(OD_T("ExGsDirectXDevice::update BeginScene"), beginSceneResult);
  }
  ExGsBaseDirectXDevice::update(pUpdatedRect);
  if (!supportPartialUpdate())
  {
    HRESULT endSceneResult = m_pD3Ddevice->EndScene();
    logResult(OD_T("ExGsDirectXDevice::update EndScene"), endSceneResult);
    if (!userGiContext()->regenAbort())
    {
      HRESULT presentResult = m_pD3Ddevice->Present(NULL, NULL, NULL, NULL);
      logResult(OD_T("ExGsDirectXDevice::update Present"), presentResult);
    }
  }
}
...
void ExGsDirectXDevice::updateScreen()
{
  if(!isValid())
  {
    ...
    HRESULT beginSceneRes = p3DD->BeginScene();
    logResult(OD_T("ExGsDirectXDevice::updateScreen BeginScene"), beginSceneRes);
    ...
    if(i)
    {
      ...
      HRESULT setTransformRes = p3DD->SetTransform(D3DTS_PROJECTION, &kD3DIdentityMatrix);
      logResult(OD_T("ExGsDirectXDevice::updateScreen SetTransform D3DTS_PROJECTION"), setTransformRes);
      DXError::check(setTransformRes);
      setTransformRes = p3DD->SetTransform(D3DTS_WORLD, &kD3DIdentityMatrix);
      logResult(OD_T("ExGsDirectXDevice::updateScreen SetTransform D3DTS_WORLD"), setTransformRes);
      ...
      do
      {
        ...
        HRESULT drawPrimitiveUPRes = p3DD->DrawPrimitiveUP(D3DPT_TRIANGLEFAN, 2, triData, sizeof(OdD3DVertexCol));
        logResult(OD_T("ExGsDirectXDevice::updateScreen DrawPrimitiveUP"), drawPrimitiveUPRes);
      }
      while(--i);
    ...
    {
      HRESULT clearRes = p3DD->Clear( 0L, NULL, D3DCLEAR_TARGET, bgc, 1.0f, 0L );
      logResult(OD_T("ExGsDirectXDevice::updateScreen Clear"), clearRes);
    }
    
    ExGsBaseDirectXDevice::updateScreen();
    
    // End the scene
    HRESULT endSceneRes = p3DD->EndScene();
    logResult(OD_T("ExGsDirectXDevice::updateScreen EndScene"), endSceneRes);
  }
  // Present the backbuffer contents to the display
  if (!userGiContext()->regenAbort())
  {
    HRESULT presentRes = m_pD3Ddevice->Present(NULL, NULL, NULL, NULL);
    logResult(OD_T("ExGsDirectXDevice::updateScreen Present"), presentRes);
  }
}
...
bool ExGsDirectXDevice::isLinesAntiAliasingSupported() const
{
  ...
  if (m_pD3Ddevice)
  {
    ...
    HRESULT getDeviceCapsRes = m_pD3Ddevice->GetDeviceCaps(&devCaps);
    logResult(OD_T("ExGsDirectXDevice::isLinesAntiAliasingSupported GetDeviceCaps"), getDeviceCapsRes);
...
OdIntPtr ExGsDirectXDevice::logger() const
{
  return (OdIntPtr)(&m_pLogger);
}

void ExGsDirectXDevice::setLogger(OdIntPtr pPtr)
{
  OdLoggerBase* pLogger = (OdLoggerBase*)(pPtr);
...
{
  m_pGeometry->endMetafile(pMetafile, m_nMetafileSize, !baseDevice()->getUpdateManager().isNull());
...
void ExGsDirectXView::playMetafile(const OdRxObject* pMetafile)
{
  switch( metafilePlayMode() )
  {
    case kMfDisplay:
      resetFading(isFaded());
      m_pGeometry->playMetafile(pMetafile, highlightingPass());
      m_pGeometry->present();
    break;
    case kMfSelect:
    case kMfExtents:
...
{ const bool isTransformForSelect = dynamicSubEntHlt() && ( m_pSelectProc || (&(m_pSelectXfm->output().destGeometry()) != &(OdGiEmptyGeometry::kVoid)) );
...
const OdGeMatrix3d &ExGsDirectXView::metafileTransform() const
{
  if (!m_metafileTransfStack.isEmpty())
    return OdGsBaseVectorizer::metafileTransform();
  return mfTransformStack().metafileTransform();
}

void ExGsDirectXView::xformByMetafileTransform(OdGeMatrix3d &mtx, bool bInverse) const
{
  if (mfTransformStack().hasStack())
    mtx.preMultBy((bInverse) ? mfTransformStack().inverseMetafileTransform() : mfTransformStack().metafileTransform());
}

void ExGsDirectXView::xformByMetafileTransform(OdGeVector3d &vec, bool bInverse) const
{
  if (mfTransformStack().hasStack())
    vec.transformBy((bInverse) ? mfTransformStack().inverseMetafileTransform() : mfTransformStack().metafileTransform());
}
...
void ExGsDirectXView::beginViewVectorization()
{
  ...
  beginLightsAccumulation(true, false, GETBIT(vectorizingFlags(), kVectorizing));
}
...
void ExGsDirectXView::renderTypeOnChange(OdGsModel::RenderType renderType)
{
  ...
  if (view().mode() != k2DOptimized) // 2D mode have always disabled Z-buffer
  {
    ...
    if (!stateManager.GetDevice())
      return; // #BIM-3321 : in case if viewExtents called before udate() with enabled overlays.
                            ...
                            const OdGiVertexData* vertexData)
{
  ...
  if (!m_pGeometry->twoSidesMode()) m_pModelToEyeProc->initGeometryClipStatus();
  OdGsBaseVectorizer::shell(numVertices, vertexList, faceListSize, faceList, edgeData, faceData, vertexData);
  if (!m_pGeometry->twoSidesMode() && (m_pModelToEyeProc->getGeometryClipStatus() == 1))
    m_pGeometry->setShmClipped(true);
  m_pGeometry->setTwoSidesMode(true);
}

void ExGsDirectXView::mesh(OdInt32 numRows,
                           OdInt32 numColumns,
                           const OdGePoint3d* vertexList,
                           ...
                           const OdGiVertexData* pVertexData)
{
  if ((view().mode() >= kFlatShaded) && ::odExDiscardBackfaces(*this, device()->discardBackFaces()) &&
      (!currentHighlightBranch() || device()->discardBackFaces()) &&
      ((pFaceData && pFaceData->normals()) || (pVertexData && pVertexData->normals())))
    m_pGeometry->setTwoSidesMode(false);
  if (!m_pGeometry->twoSidesMode()) m_pModelToEyeProc->initGeometryClipStatus();
  OdGsBaseVectorizer::mesh(numRows, numColumns, vertexList, pEdgeData, pFaceData, pVertexData);
  if (!m_pGeometry->twoSidesMode() && (m_pModelToEyeProc->getGeometryClipStatus() == 1))
    m_pGeometry->setShmClipped(true);
  m_pGeometry->setTwoSidesMode(true);
}

void ExGsDirectXView::pline(const OdGiPolyline &polyline, OdUInt32 fromIndex, OdUInt32 numSegs)
{
  m_pGeometry->plineCalcStarted();
  OdGsBaseVectorizer::pline(polyline, fromIndex, numSegs);
  m_pGeometry->plineCalcEnded();
}
...
{
  OdGsBaseMaterialVectorizer::addPointLight(traits);
  D3DLIGHT light;
...
{
  OdGsBaseMaterialVectorizer::addSpotLight(traits);
  D3DLIGHT light;
  memset(&light, 0, sizeof(light));
...
{
  OdGsBaseMaterialVectorizer::addDistantLight(traits);
  D3DLIGHT light;
  memset(&light, 0, sizeof(light));
...
void ExGsDirectXView::resetFading(bool bFlag)
{
  if ((m_bFading != bFlag) || (bFlag && (m_nCurFading != (OdUInt8)fadingIntensity())))
  {
    ...
    if (bFlag)
    {
      ...
      const double zScale = zFar - zNear;
      const double zMid = zNear + zScale * 0.5;
      const double coef = (m_nCurFading = (OdUInt8)fadingIntensity()) * 0.01;
      double fogNear = zNear - zScale * coef * 255;
      double fogFar = zFar + zScale * (1.0 - coef) * 255;
      if (zMid < 0.0)
        fogNear = -fogNear, fogFar = -fogFar;
      const DWORD bkgndColor = device()->getPaletteBackground();
      const FLOAT Start = odmax(odmin((FLOAT)fogNear, 1e20f), -1e20f);
      const FLOAT End   = odmin(odmax((FLOAT)fogFar, -1e20f),  1e20f);
      ...
      g_pDevice->SetRenderState(D3DRS_FOGSTART, *(const DWORD*)(&Start));
      g_pDevice->SetRenderState(D3DRS_FOGEND,   *(const DWORD*)(&End));
    }
    else
    {
...
{
  OdArray<ExGsDirectXDevice::RasterImageEntry> &riEntry = device()->rasterImageCache();
...
void ExGsDirectXView::setupDisplayOpts(bool bEnable)
{
  ...
  if (userGiContext() && GETBIT(userGiContext()->antiAliasingMode(), 1) && GETBIT(view().antiAliasingMode(), 1) &&
...
void ExGsDirectXView::renderBackground(bool secondPass)
{
  ...
  if (viewportObjectId() != NULL)
  {
    ...
    if (mode() != OdGsView::k2DOptimized)
    {
      ...
      geometry().draw_noColorMod(false);
    }
...
void ExGsDirectXView::tfcPlayMetafile(const OdGeMatrix3d &xForm, const OdRxObject *pMetafile) const
{
  ...
  if (dynamicSubEntHlt() && (m_pSelectProc || (&(pThis->m_pSelectXfm->output().destGeometry()) != &(OdGiEmptyGeometry::kVoid))))
    pThis->playMetafile(pMetafile);
  else
...
{
  return GETBIT(device()->useLutPalette(), 1);
}

bool ExGsDirectXView::useLutPaletteColor() const
{
  return GETBIT(device()->useLutPalette(), 2);
}

int ExGsDirectXView::checkLineTopology(const OdGePoint3d &pt1, const OdGePoint3d &pt2) const
{
  return ::isLineParallelDegenerate(pt1, pt2, xVector(), upVector(), eyeVector(), device()->isLinesRepModeEnabled());
}
...
OdGiRasterImagePtr ExGsDirectXDevice::snapshotRegion(const OdGsDCRect &area, bool /*bCrop*/) const
{
  ...
  HRESULT stretchRectRes = m_pD3Ddevice->StretchRect(pSurfFrom, &sourceRect, pSurfTo, &destRect, D3DTEXF_NONE);
  logResult(OD_T("ExGsDirectXDevice::snapshotRegion StretchRect"), stretchRectRes);
  ODA_VERIFY(stretchRectRes == D3D_OK);
  pRT2->GetSurfaceLevel(0, &pSurfTo2);
  HRESULT getRenderTargetDataRes = m_pD3Ddevice->GetRenderTargetData(pSurfTo, pSurfTo2);
  logResult(OD_T("ExGsDirectXDevice::snapshotRegion GetRenderTargetData"), getRenderTargetDataRes);
  ODA_VERIFY(getRenderTargetDataRes == D3D_OK);
