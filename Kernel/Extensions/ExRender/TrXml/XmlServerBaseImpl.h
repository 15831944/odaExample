...
#ifndef ODTRXMLSERVERBASEIMPL
#define ODTRXMLSERVERBASEIMPL
...
*/
class OdTrXmlServerBaseImpl : public OdTrXmlServer, protected OdTrVisUniqueID::DefUIDTransformer
{
protected:
  int m_nIdent;
  ProcessingMode m_procMode;
...
public:
  OdTrXmlServerBaseImpl(OdDbBaseDatabasePE *pDbPE = NULL)
    : m_nIdent(0)
    , m_procMode(kNormalMode)
    , m_pDbPE(NULL)
    , m_pszTmpBuf(NULL)
    , m_lenTmpBuf(0)
  {
  ...
  void setProcessingMode(ProcessingMode procMode)
  { 
    m_procMode = procMode;
  }
  void setDatabasePE(OdDbBaseDatabasePE *pDbPE = NULL)
  {
    m_pDbPE = pDbPE;
  }
                      ...
                      OdInt32 enAdv = -1)
  {
    ...
    ODA_ASSERT_ONCE(!pId || data <= OdUInt64(100));

    OdTrXmlServer::DropId(pTagName, data, pId, enAdv); // DropUInt64(pTagName, data);
  }
  ...
  virtual void DropUInt8s(const char* pTagName, OdUInt32 nData, const OdUInt8* pData)
  {
    ...
    ODFMTFORBEG(8)
      index ? ",%u" : "%u" , (unsigned)pData[index]
    ODFMTEND()
    ident(pTagName, m_sTmpBuf.c_str());
  }
  ...
  virtual void DropDoubles(const char* pTagName, OdUInt32 nData, const double *pData)
  {
    ...
    double val, offs[3];
    offs[0] = ceil(pData[0]) - 1.0; offs[1] = ceil(pData[1]) - 1.0; offs[2] = ceil(pData[2]) - 1.0; // by first

    DropDouble3("ArrayOffset", offs[0], offs[1], offs[2]);

    ODA_ASSERT_ONCE(nData && pData);
    ODFMTFORBEGEX(32, val = pData[index] - offs[index % 3])
      //index ? ",%g" : "%g" , pData[index] - offs[index % 3]
      ODFLTFMT("%s", val, ), index ? "," : "", val
    ODFMTEND()
    ident(pTagName, m_sTmpBuf.c_str());
  ...
  virtual void DropDbOwning(const char *pTagName, OdTrVisOwning owningId)
  {
    ...
    if (owningId == kTrVisNullId)
      return;

    m_sTmpBuf.empty();
    append(m_sTmpBuf, OdTrVisIdToPtr(OdDbStub, owningId));
    ident(pTagName, m_sTmpBuf);
  }

  virtual ProcessingMode ClientProcessingMode() const
  {
    return m_procMode;
  }
};
