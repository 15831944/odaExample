  ...
  if (pId && data > OdUInt64(100)) // is not an address
  {
    ...
    m_pFiler->wrAnsiString(m_sTmpBuf);
           ...
           && (   !odStrCmpA(pTagName, "TextureID")
               ...
               || !odStrCmpA(pTagName, "OverlayID"))) // Overlay Id can be null (it means "main" default overlay)
  {
...
void OdGlesBinaryServerImpl::DropUInt8s(const char *pTagName, OdUInt32 nData, const OdUInt8 *pData)
{
  if (!wrPathKey(pTagName))
    return;
  m_pFiler->wrUInt32(nData);
  for (OdUInt32 idx = 0; idx < nData; idx++)
    m_pFiler->wrUInt8(pData[idx]);
}

void OdGlesBinaryServerImpl::DropUInt16s(const char *pTagName, OdUInt32 nData, const OdUInt16 *pData)
{
  if (!wrPathKey(pTagName))
    return;
  m_pFiler->wrUInt32(nData);
...
void OdGlesBinaryServerImpl::DropUInt32s(const char *pTagName, OdUInt32 nData, const OdUInt32 *pData)
{
  if (!wrPathKey(pTagName))
    return;
  m_pFiler->wrUInt32(nData);
  for (OdUInt32 idx = 0; idx < nData; idx++)
    m_pFiler->wrUInt32(pData[idx]);
}
...
void OdGlesBinaryServerImpl::DropDoubles(const char* pTagName, OdUInt32 nData, const double *pData)
{
  ODA_ASSERT_ONCE(nData && !(nData % 3) && pData); // test
  if (  odStrCmpA(pTagName, "ArrayData")
      || !nData || (nData % 3)
  ...
  m_pFiler->wrUInt32(nData);
  for (OdUInt32 idx = 0; idx < nData; idx++)
    m_pFiler->wrDouble(pData[idx] - offs[idx % 3]);
}
...
void OdGlesBinaryServerImpl::DropDbOwning(const char *pTagName, OdTrVisOwning owningId)
{
  ...
  if (m_useFakeDbStub)
  {
    m_pFiler->wrAnsiString(OdAnsiString().format("%" PRIX64, OdTrVisIdToPtr(OdFakeDbStub, owningId)->handle));
    return;
  }
