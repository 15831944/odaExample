...
static bool func_DbOwning(OdXmlGlesParser* pThis, TiXmlElement* elem, OdEnPathKey pk, const OdAnsiString& asPath)
{
  OdAnsiString asHandle = elem->GetText();
  if (pThis->m_pLoader->DropDbOwning(pk, asHandle))
    return true;
  pThis->m_sError = L"error inside OdXmlGlesLoader::DropDbOwning";
  return false;
}
...
static bool func_BinaryStream(OdXmlGlesParser* pThis, TiXmlElement* elem, OdEnPathKey pk, const OdAnsiString& asPath)
{
  OdAnsiString asValue = elem->GetText();
  OdBinaryData data;
  bool isBase64 = (asValue.findOneOf("GHIJKLMNOPQRSTUVWXYZghijklmnopqrstuvwxyz+/") >= 0);
  if (isBase64)
...
static bool func_UInts8(OdXmlGlesParser* pThis, TiXmlElement* elem, OdEnPathKey pk, const OdAnsiString& asPath)
{
  OdUInt8Array data;
  OdAnsiString asValue = elem->GetText();
  ...
  if (pThis->m_pLoader->DropUInts8(pk, data))
    return true;
  pThis->m_sError = L"error inside OdXmlGlesLoader::DropUInts8";
  return false;
}
...
static bool func_UInts32(OdXmlGlesParser* pThis, TiXmlElement* elem, OdEnPathKey pk, const OdAnsiString& asPath)
{
  OdUInt32Array data;
  OdAnsiString asValue = elem->GetText();
  char* pszBuf = const_cast<char*>(asValue.c_str());
  ...
  if (pThis->m_pLoader->DropUInts32(pk, data))
    return true;
  pThis->m_sError = L"error inside OdXmlGlesLoader::DropUInt32";
  return false;
}
...
static bool func_UInt32ArrayType(OdXmlGlesParser* pThis, TiXmlElement* elem, OdEnPathKey pk, const OdAnsiString& asPath)
{
  ...
  const OdTrVisArrayWrapper &arrayType = pThis->m_pLoader->activeArrayType();
  ...
  case OdTrVisArrayWrapper::Type_Depth:
    if (arrayType.m_uData == 0) pThis->m_funcArray = func_UInts8;
    else if (arrayType.m_uData == 1) pThis->m_funcArray = func_Floats;
    else if (arrayType.m_uData == 2) pThis->m_funcArray = func_Floats;
    else if (arrayType.m_uData == 3) pThis->m_funcArray = func_Doubles;
    else { pThis->m_sError = L"func_UInt32ArrayType error: unsupported array format"; return false; }
  break;
  case OdTrVisArrayWrapper::Type_Index:
    if (arrayType.m_uData == 0) pThis->m_funcArray = func_UInts8;
    else if (arrayType.m_uData == 1) pThis->m_funcArray = func_UInts16;
    else if (arrayType.m_uData == 2) pThis->m_funcArray = func_UInts32;
    else { pThis->m_sError = L"func_UInt32ArrayType error: unsupported array format"; return false; }
  break;
