                                   ...
                                   bool bIntAsUnsigned = false)
{
  ...
  case OdVariant::kInt64: // == OdVariant::kUInt64
    // bIntAsUnsigned
    sValue.format(L"%" PRIu64W, vValue.getUInt64());
    break;
...
public:
  ...
  bool save(OdStreamBufPtr pBuf,
            OdUInt32 maxFileSize = 50 * 1024 * 1024,
            OdString* psErrMessage = NULL) const
  {
    OdBinaryData data;
    for (pBuf->rewind(); !pBuf->isEof();)
      data.push_back(pBuf->getByte());
    pBuf->rewind();
    return save(data, maxFileSize, psErrMessage);
  }
};
