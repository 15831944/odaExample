...
void OdGlesServerBaseImpl::DropDbOwning(const char* pTagName, OdTrVisOwning owningId)
{
  if (!m_useFakeDbStub || (owningId == kTrVisNullId))
    OdTrXmlServerBaseImpl::DropDbOwning(pTagName, owningId);
  else
    ident(pTagName, OdAnsiString().format("%" PRIX64, OdTrVisIdToPtr(OdFakeDbStub, owningId)->handle).c_str()); // ident(pTagName, (const char*) pDbStub);
...
void OdGlesServerBaseImpl::DropBinaryStream(const char* pTagName, const OdUInt8* pData, OdUInt32 nData)
{
  if (m_bEncodeTextureHex)
    OdTrXmlServerBaseImpl::DropBinaryStream(pTagName, pData, nData);
  else
  {
