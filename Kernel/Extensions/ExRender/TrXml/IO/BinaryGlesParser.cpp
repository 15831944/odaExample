...
static bool func_DbOwning(OdBinaryGlesParser* pThis, OdUniversalReadFiler* pFiler, OdEnPathKey pk)
{
...
static bool func_UInts8(OdBinaryGlesParser* pThis, OdUniversalReadFiler* pFiler, OdEnPathKey pk)
{
  OdUInt32 nData = pFiler->rdUInt32();
  ODA_ASSERT_ONCE(nData);
  OdUInt8Array data;
...
static bool func_UInts32(OdBinaryGlesParser* pThis, OdUniversalReadFiler* pFiler, OdEnPathKey pk)
{
  OdUInt32 nData = pFiler->rdUInt32();
  ODA_ASSERT_ONCE(nData);
  OdUInt32Array data;
  ...
  for (OdUInt32 idx = 0; idx < nData; idx++)
  {
    OdUInt32& iVal = data[idx]; 
    iVal = pFiler->rdUInt32();
  }
  if (pThis->m_pLoader->DropUInts32(pk, data))
    return true;
  pThis->m_sError = L"error inside OdXmlGlesLoader::DropUInts32";
  return false;
}
