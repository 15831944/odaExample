                                  ...
                                  OdGiConveyorContext *pDrawContext, TextInfo &textOutInfo, void *pSessionId)
{
  ...
  OdCharArray srcArray;
  { // getTextAsDByte
    bool bFirstChar = true;
    OdChar ch = pIter->nextChar();
    OdCharacterProperties currProps = pIter->currProperties();
    while ( ch )
    {
      srcArray.push_back(ch);
      if (!bRaw)
      {
        if ((textFlags.isUnderlined() != currProps.bUnderlined) ||
          (textFlags.isOverlined() != currProps.bOverlined) ||
          (textFlags.isStriked() != currProps.bStriked))
        {
          textFlags.setUnderlined(currProps.bUnderlined);
          textFlags.setOverlined(currProps.bOverlined);
          textFlags.setStriked(currProps.bStriked);
          if (bFirstChar)
          {
      ...
      ch = pIter->nextChar();
      currProps = pIter->currProperties();
    }
  }

  if (pFont->isSupportReorder())
  {
    textFlags.setGlyph(true);

    OdCharArray dstArray;
    OdGeVector2dArray offsets;
    OdIntArray visualFromLogical;
    OdIntArray logicalFromVisual;
    pFont->getGlyphIndexes(srcArray, dstArray, &visualFromLogical, NULL, &offsets);
    for (OdUInt32 k = 0; k < dstArray.size(); ++k)
    {
      OdGeVector3d offset;
      if (k < offsets.size())
        offset = OdGeVector3d(offsets[k].x, offsets[k].y, 0.0);

      if (offset != OdGeVector3d::kIdentity)
        return false;

      if  ( !dstArray[k] ) // No glyph found.
        return false;
  ...
  {
    for (OdUInt32 i = 0; i < srcArray.size(); i++)
    {
      procCharacter(pFont, textOutInfo.m_fontKey, *fontCache, srcArray[i], textFlags, pSessionId);
      textOutInfo.m_textString.push_back(srcArray[i]);
    }
  }
