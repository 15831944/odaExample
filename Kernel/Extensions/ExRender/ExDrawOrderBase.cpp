...
OdDrawOrderBaseDevice::~OdDrawOrderBaseDevice()
{
  releaseMetafileContext();
}

void OdDrawOrderBaseDevice::releaseMetafileContext()
{
  ...
  for (GsViewOverlayDataContainer::Iterator oIt = m_overlayData.newIterator(); !oIt.done(); oIt.next())
  {
    ...
    for (OdUInt32 nDrawable = 0; nDrawable < nDrawablesIn; nDrawable++)
    {
      ...
      else
      {
          ...
          case DrawableInfo::kContainer:
            {
              ...
              if (!pNode) // Could be in case if loaded from GsState
                pNode = static_cast<OdGsContainerNode*>(m_drawables.at(nDrawable).m_pGsModel->drawableGsNode(drawableAt(m_drawables[nDrawable])));
              ...
              if (pDrawables[nDrawable].m_pGsModel->hasChangedLayers() ||
                ...
                || !sameSortedArrays( this->m_frozenLayers, this->m_lastFrozenLayers )
                )
              {
                drawableInfo.m_flags = kFullRegeneration;
                if( bAllowInheritanceNotification )
                ...
                bViewChanges || (drawableInfo.m_flags == kPartialRegeneration))
              {
                ...
                || m_refModels[ (OdGsModel*)pDrawables[ nDrawable ].m_pGsModel.get() ].bHidden )
                drawableInfo.m_flags = kStateModified;
              if( drawableInfo.m_flags )
              {
                m_refModels[ (OdGsModel*)pDrawables[ nDrawable ].m_pGsModel.get() ].bHighlighted = false;
                m_refModels[ (OdGsModel*)pDrawables[ nDrawable ].m_pGsModel.get() ].bHidden = false;
              }
            }
          break;
          case DrawableInfo::kPersistent:
            {
              ...
              //if (!pNode)
              const bool bCheckValid = isCheckValid();
              setCheckValid(false);
              if (!isValid())
                drawableInfo.m_flags = kFullRegeneration;
              setCheckValid(bCheckValid);
                if( bAllowInheritanceNotification )
                  pBaseDevice->setCheckDrawOrderInheritance( true );
              //else
        ...
        drawableInfo.m_nLink = nDrawable; // #VIS-1435
      }
      nDrawableOut++;
    }
    if (pOrder->m_drawableInfo.size() > nDrawableOut)
...
void OdDrawOrderBaseView::MapBasedComparer::setOriginal( const OdUInt64* pList, OdUInt32 size, OdUInt32 offset )
{
  m_map.clear();
  if( !pList || size == 0 ) return;
  for( OdUInt32 i = 0; i < size; ++i )
  {
    m_map[ pList[ i + offset ] ] = Options( i );
  }
}
...
void OdDrawOrderBaseView::listCmp( OdUInt64Vector& originalList,  OdUInt64Vector& newList, OdUInt32 nDrawInfo, DrawableInfo* drawInfo, ActionSequence& result  )
{
  result.clear();
  OdUInt32 origFrom = 0;
  for (OdUInt32 nPrevDrw = 0; nPrevDrw < nDrawInfo; nPrevDrw++)
    origFrom += drawInfo[nPrevDrw].m_nLen; // Compute actual number of entries in list before our data
  OdUInt32 lenOrig = drawInfo[nDrawInfo].m_nLen;
  OdUInt32 lenNew = newList.size();
  m_mbCmp.setOriginal( originalList.asArrayPtr(), lenOrig, origFrom );
      ...
      for (GsViewOverlayDataContainer::Iterator oIt = m_overlayData.newIterator(); !oIt.done(); oIt.next())
      {
        ...
        if( isContentCompatibleOverlay( pView, pOrder, oIt.overlayId() ) )
        {
          if( isMetafilesCompatibleOverlays( oIt.overlayId(), pView ) )
          {
            ...
            //Re-refer all that refer this to the pView
            {
              for( int j = 0; j < device()->numViews(); ++j )
              {
                if( i == j ) continue;
                OdDrawOrderBaseView* pView1 = static_cast<OdDrawOrderBaseView*>( device()->viewAt( j ) );
                if( pView1 == this ) continue;
                for( GsViewOverlayDataContainer::Iterator oIt1 = pView1->m_overlayData.newIterator(); !oIt1.done(); oIt1.next() )
                {
                  DrawOrderOverlayData* pOrder1 = static_cast<DrawOrderOverlayData*>( oIt1.overlayData() );
                  if( pOrder1 == pOrder ) continue;
                  if( pOrder1->m_pConnectedView == this )
                  {
                    pOrder1->m_pConnectedView = pView;
                    pView1->onMetafileOrderInheritance( oIt1.overlayId(), pView );
                    pOrder1->m_bBreakInheritance = false;
                  }
                }
              }
            }
  ...
  for (GsViewOverlayDataContainer::Iterator oIt = m_overlayData.newIterator(); !oIt.done(); oIt.next())
  {
    ...
    for (OdUInt32 nDrawInfo = 0; nDrawInfo < nDrawables; nDrawInfo++)
    {
      if (drawInfo[nDrawInfo].m_flags != 0)
      {
        ...
        if ((drawInfo[nDrawInfo].m_flags != kStateModified) && !bForPartialUpdate)
        {
#ifdef CMP_LIST
    ...
    for (OdUInt32 n = 0; n < m_drawables.size(); n++)
    {
	    ...
	    if ((sceneGraph->isPersistent() && (holder.m_drawableId == sceneGraph->id())) || (drawableAt(holder).get() == sceneGraph))
      {
        if (!holder.m_pGsModel.isNull())
        {
          ...
          if (mRef.nRefs <= 1)
            onGsModelLinkErased(holder.m_pGsModel.get());
          else
            mRef.nRefs--;
        }
        break;
...
void OdDrawOrderBaseView::onGsModelLinkAdded(OdGsModel *pModel)
{
  pModel->addModelReactor(this);
}

void OdDrawOrderBaseView::onGsModelLinkErased(OdGsModel *pModel)
{
  pModel->removeModelReactor(this);
  m_refModels.erase(pModel);
}

bool OdDrawOrderBaseView::onHighlight(OdGsModel *pModel, const OdGiPathNode& /*path*/, bool /*bDoIt*/, OdUInt32 & /*nStyle*/, const OdGsView* /*pView*/)
{
  m_refModels[pModel].bHighlighted = true;
  return true;
}

bool OdDrawOrderBaseView::onHighlight(OdGsModel *pModel, const OdGiPathNode& /*path*/, const OdGsMarker* /*pMarkers*/, OdUInt32 /*nMarkers*/, bool /*bDoIt*/,
                                      OdUInt32 & /*nStyle*/, const OdGsView* /*pView*/)
{
  m_refModels[pModel].bHighlighted = true;
  return true;
}

bool OdDrawOrderBaseView::onHide(OdGsModel *pModel, OdGsNode *pNode, bool &bHidden, bool &bSelectable, const OdGsView* /*pView*/)
{ OdDbStub *pPersId;
  if (pPersId = pNode->underlyingDrawableId())
  { OdDbStub *pContainer = ::odgsDbGetOwner(pPersId);
    for (GsViewOverlayDataContainer::Iterator oIt = m_overlayData.newIterator(); !oIt.done(); oIt.next())
    {
      DrawOrderOverlayData* pOrder = static_cast<DrawOrderOverlayData*>(oIt.overlayData());
      OdUInt32 nDrawables = pOrder->m_drawableInfo.size();
      ...
      for (OdUInt32 nDrawable = 0; nDrawable < nDrawables; nDrawable++)
      {
        if ((drawInfo[nDrawable].m_drawableId == pPersId) || 
            (pContainer && (drawInfo[nDrawable].m_drawableId == pContainer)))
        {
...
bool OdDrawOrderBaseView::onHide(OdGsModel *pModel, const OdGiPathNode& path, const OdGsMarker* pMarkers, OdUInt32 nMarkers, bool& bHidden,
                                 bool &bSelectable, const OdGsView* /*pView*/)
{
  m_refModels[ pModel ].bHidden = true;
  return true;
}
...
#if 0 // Experimental fix of multi-view container nodes invalidation. Do not delete.
bool OdDrawOrderBaseView::onInvalidate(OdGsModel *pModel, OdGsView* pView)
{
  if (!device()->drawOrderForPartialUpdate())
  {
    for (GsViewOverlayDataContainer::Iterator oIt = m_overlayData.newIterator(); !oIt.done(); oIt.next())
    {
      DrawOrderOverlayData* pOrder = static_cast<DrawOrderOverlayData*>(oIt.overlayData());
      ...
      for (OdUInt32 nDrawInfo = 0; nDrawInfo < nDrawables; nDrawInfo++)
      {
        if ((drawInfo[nDrawInfo].m_type == DrawableInfo::kContainer) &&
            (drawInfo[nDrawInfo].m_pGsModel == pModel))
        {
          OdUInt32 nDrawablesIn = m_drawables.size();
          const DrawableHolder *pDrawables = m_drawables.getPtr();
          for (OdUInt32 nDrawableIn = 0; nDrawableIn < nDrawablesIn; nDrawableIn++)
          {
            if (((pDrawables[nDrawableIn].m_drawableId == drawInfo[nDrawInfo].m_drawableId) ||
                (pDrawables[nDrawableIn].m_pDrawable.get() == drawInfo[nDrawInfo].m_pDrawable)) &&
                (pDrawables[nDrawableIn].m_pGsModel.get() == drawInfo[nDrawInfo].m_pGsModel))
            {
              if (pDrawables[nDrawableIn].m_pGsRoot)
              { OdGsContainerNode *pNode = static_cast<OdGsContainerNode*>(pDrawables[nDrawableIn].m_pGsRoot);
                for (int nView = 0; nView < device()->numViews(); ++nView)
                { OdDrawOrderBaseView* pView = static_cast<OdDrawOrderBaseView*>(device()->viewAt(nView));
                  if (pView == this) continue;
                  for (GsViewOverlayDataContainer::Iterator oIt2 = m_overlayData.newIterator(); !oIt2.done(); oIt2.next())
                  { DrawOrderOverlayData* pOrder2 = static_cast<DrawOrderOverlayData*>(oIt2.overlayData());
                    OdUInt32 nDrawables2 = pOrder2->m_drawableInfo.size();
                    DrawableInfo *drawInfo2 = pOrder2->m_drawableInfo.asArrayPtr();
                    for (OdUInt32 nDrawInfo2 = 0; nDrawInfo2 < nDrawables2; nDrawInfo2++)
                    {
                      if ((drawInfo2[nDrawInfo2].m_drawableId == drawInfo[nDrawInfo].m_drawableId) &&
                          (drawInfo2[nDrawInfo2].m_pDrawable == drawInfo[nDrawInfo].m_pDrawable) &&
                          (drawInfo2[nDrawInfo2].m_pGsModel == drawInfo[nDrawInfo].m_pGsModel))
                      {
                        pNode->invalidate(NULL, pView, kVpAllProps);
                        drawInfo2[nDrawInfo2].m_flags = kFullRegeneration;
                      }
                    }
                  }
                }
...
bool OdDrawOrderBaseView::isMetafilesCompatibleOverlays( OdGsOverlayId overId1, const OdDrawOrderBaseView* pView2 )
{
  ...
  for( OdUInt32 i = 0; i < m_drawables.size(); ++i )
  {
    ...
    if( !m_drawables[ i ].m_pGsRoot ) return false; // #VIS-627: Transient drawable behavior is unpredictable.
...
{
  //#VIS-627 Since onOverlayDeactivated comes after actual deactivetion
  //there is no sence to find it in list
...
void OdDrawOrderBaseMetafile::clear()
{
  if (!m_pContext.isNull() && m_pContext->m_pDevice && isFilled())
    m_pContext->m_pDevice->onDeleteRequest(onMetafileDeleteRequestProc, this);
  m_pContext = NULL; m_pLayer = NULL;
  OdDrawOrderMetafileBase::clear();
}
...
void OdDrawOrderBaseVectorizer::beginMetafile(OdRxObject* pMetafile)
{
  ...
  if (gsWriter().currentGeomPortion())
    pMf->m_pLayer = gsWriter().currentGeomPortion()->m_pLayer;
  else if (gsWriter().gsModel() && m_effectiveLayerId) // ???
    pMf->m_pLayer = gsLayerNode(m_effectiveLayerId, gsWriter().gsModel());
  else
    pMf->m_pLayer = NULL;
  pMf->setVisible(true);
  ...
  pMf->setRefIndex((pMf->isHighlighted()) ? currentSelectionStyle() : 0);
  pMf->setFaded(GETBIT(m_fadingFlags, kFfRefEdit));
...
void OdDrawOrderBaseVectorizer::endMetafile(OdRxObject* pMetafile)
{
  ...
  if (pMf->isHighlighted() && pMf->hasRefIndex())
    device()->onMetafileSelectionStyleChanged(pMetafile, pMf->refIndex());
...
void OdDrawOrderBaseVectorizer::playMetafile(const OdRxObject* pMetafile)
{
  ...
  //Metafile visibility check
  bool bMetafileVisible = true;
  ...
  if (!m_bDisableOffLayersVisibility && pMf->m_pLayer)
  {
    ...
    {
      bMetafileVisible = !pMf->isVisible();
    }
  }
  //2) Since TrVecDevice uses "SuppressHide", it had to check EntityNode visibility
  if( m_MfPlayNode )
  {
    bMetafileVisible &= !m_MfPlayNode->isHidden();
  }
  if( pMf->isVisible() != bMetafileVisible )
  {
    pMf->setVisible( bMetafileVisible );
    device()->onMetafileVisibilityChanged( pMf, bMetafileVisible );
  }
  ...
  { // Don't process highlighting for nested metafiles
    ...
    const OdUInt32 selStyleIndex = (isHighlighted()) ? currentSelectionStyle() : 0;
    if (pMf->refIndex() != selStyleIndex)
    { pMf->setRefIndex(selStyleIndex);
      device()->onMetafileSelectionStyleChanged(pMf, selStyleIndex);
    }
  }
