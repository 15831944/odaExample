...
static void textureBlit_Nearest_4bpp_Transparent(const OdUInt8 *pTexScanLines, const ODCOLORREF *pTexPalette,
                                                 OdUInt32 texScanLineLength, int u, int v, OdUInt8 *bgr)
{
  OdUInt8 nColorIndex = ((u % 2) & 0x01) ?  ((pTexScanLines + texScanLineLength * v)[u / 2] & 0x0F) :
                                           (((pTexScanLines + texScanLineLength * v)[u / 2] & 0xF0) >> 4);
...
static void textureBlit_Nearest_8bpp_Transparent(const OdUInt8 *pTexScanLines, const ODCOLORREF *pTexPalette,
                                                 OdUInt32 texScanLineLength, int u, int v, OdUInt8 *bgr)
{
  OdUInt8 nColorIndex = (pTexScanLines + texScanLineLength * v)[u];
  if (ODGETALPHA(pTexPalette[nColorIndex]))
...
OdGsBitmapVectorizeDevice::blitFunc OdGsBitmapVectorizeDevice::chooseBlitFuncFor() const
{
  switch (m_rasterBitDepth)
  {
    ...
    case kRIBD16Clr:
    return (m_bRasterTransparency) ? textureBlit_Nearest_4bpp_Transparent : textureBlit_Nearest_4bpp;
    case kRIBD256Clr:
    return (m_bRasterTransparency) ? textureBlit_Nearest_8bpp_Transparent : textureBlit_Nearest_8bpp;
    case kRIBD24Bpp:
...
void OdGsBitmapVectorizeDevice::setRasterImage(const OdGiRasterImage *pImage, bool bTransparent)
{
  ...
  switch (pImage->colorDepth())
  {
    case 1: m_rasterBitDepth = kRIBDMono; m_bRasterTransparency = bTransparent && (pImage->transparentColor() >= 0); break;
    case 4: m_rasterBitDepth = kRIBD16Clr; m_bRasterTransparency = bTransparent && (pImage->transparentColor() >= 0); break;
    case 8: m_rasterBitDepth = kRIBD256Clr; m_bRasterTransparency = bTransparent && (pImage->transparentColor() >= 0); break;
...
void OdGsBitmapVectorizeView::beginViewVectorization()
{
  ...
  m_bLinesAntiAlias = GETBIT(giContext().antiAliasingMode(), 1) && GETBIT(view().antiAliasingMode(), 1) && (view().mode() == k2DOptimized);
  // initialize transparency
  m_bDrawTransparency = view().isPlotTransparency() || !giContext().isPlotGeneration();
  // initialize optional color converter
  m_colorConverter.setColorConverterCallback(device()->getColorConverter());
...
void OdGsBitmapVectorizeView::onTraitsModified()
{
  ...
  if (m_colorConverter.colorConverterCallback())
  {
    if (m_colorConverter.colorConverterCallback()->convertBackgroundColors() ||
        !GETBIT(effectiveTraits().drawFlags(), OdGiSubEntityTraits::kDrawNoPlotstyle))
      m_curColor = m_colorConverter.convert(m_curColor);
  }
}
...
void OdGsBitmapVectorizeView::triangleOut( const OdInt32* p3Vertices, const OdGeVector3d* /*pNormal*/ )
{
  ...
  else if (vertexData() && vertexData()->trueColors())
  {
    ...
    device()->draw_gouraud_triangle(
      (int)OdRound(vertexDataList()[p3Vertices[0]].x), (int)OdRound(vertexDataList()[p3Vertices[0]].y),
      m_colorConverter.convert(OD_OPAQUE(ODTOCOLORREF(vertexData()->trueColors()[p3Vertices[0]]))),
      (int)OdRound(vertexDataList()[p3Vertices[1]].x), (int)OdRound(vertexDataList()[p3Vertices[1]].y),
      m_colorConverter.convert(OD_OPAQUE(ODTOCOLORREF(vertexData()->trueColors()[p3Vertices[1]]))),
      (int)OdRound(vertexDataList()[p3Vertices[2]].x), (int)OdRound(vertexDataList()[p3Vertices[2]].y),
      m_colorConverter.convert(OD_OPAQUE(ODTOCOLORREF(vertexData()->trueColors()[p3Vertices[2]]))), ODGETALPHA(m_curColor));
  }
  else
  {
                                          ...
                                          double fade)
{
  ...
  else if (pImage->colorDepth() <= 8 && transparency && pImage->transparentColor() >= 0)
  {
    bTransparentBitonal = true;
  }
...
inline ODCOLORREF convertBackgroundColor(OdColorConverterCallbackCaching &colorConverter, ODCOLORREF color)
{
  if (colorConverter.colorConverterCallback() && colorConverter.colorConverterCallback()->convertBackgroundColors())
...
{
  device()->draw_point((int)OdRound(point.x), (int)OdRound(point.y),
                       ::convertBackgroundColor(m_colorConverter, OD_OPAQUE(ODTOCOLORREF(color))));
}
...
{
  device()->draw_line((int)OdRound(pt1.x), (int)OdRound(pt1.y), (int)OdRound(pt2.x), (int)OdRound(pt2.y),
                      ::convertBackgroundColor(m_colorConverter, OD_OPAQUE(ODTOCOLORREF(color))));
}
...
void OdGsBitmapVectorizeView::directRenderOutputTriangleFlat(const OdGePoint3d &pt1, const OdGePoint3d &pt2, const OdGePoint3d &pt3,
                                                             const OdCmEntityColor &color)
{
  device()->draw_triangle((int)OdRound(pt1.x), (int)OdRound(pt1.y), (int)OdRound(pt2.x), (int)OdRound(pt2.y), (int)OdRound(pt3.x), (int)OdRound(pt3.y),
                          ::convertBackgroundColor(m_colorConverter, OD_OPAQUE(ODTOCOLORREF(color))));
}
...
{
  device()->draw_gouraud_triangle((int)OdRound(pt1.x), (int)OdRound(pt1.y), ::convertBackgroundColor(m_colorConverter, OD_OPAQUE(ODTOCOLORREF(clr1))),
                                  (int)OdRound(pt2.x), (int)OdRound(pt2.y), ::convertBackgroundColor(m_colorConverter, OD_OPAQUE(ODTOCOLORREF(clr2))),
                                  (int)OdRound(pt3.x), (int)OdRound(pt3.y), ::convertBackgroundColor(m_colorConverter, OD_OPAQUE(ODTOCOLORREF(clr3))));
}
