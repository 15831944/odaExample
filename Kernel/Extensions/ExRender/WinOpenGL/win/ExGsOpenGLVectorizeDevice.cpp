...
static void emulateWindowPos2i(long int hOffset, long int vOffset, long int width, long int height)
{ // Emulate glWindowPos2i through glRasterPos2i API
  GLdouble modelMatrix[4 * 4], projMatrix[4 * 4], viewport[4];
  ...
  ::glLoadIdentity();
  ::glViewport(hOffset, vOffset, width, height);
  ::glRasterPos2s(-1, -1);
...
void ExGsOpenGLVectorizeDevice::update(OdGsDCRect* pRect)
{
  ...
  if (isClearScreenEnabled() && !isCompositionUpdateEnabled())
  {
    ...
    if (supportPartialUpdate() && m_bDoubleBufferEnabled)
    {
      ...
      long int vertSv = windowHeight() - (vOffset() + height());
      if (hOffset() || vertSv)
      {
        if (isExtensionSupported(OpenGLExtension_ARB_WinPos))
          ((OdGLFn_WindowPos2i_ARB)getExtensionFunc(OpenGLExtFunc_ARB_WindowPos2i))(hOffset(), vertSv);
        else
          ::emulateWindowPos2i(hOffset(), vertSv, width(), height());
      }
      ::glCopyPixels(hOffset(), vertSv, width(), height(), GL_COLOR);
    }
  }
...
long int ExGsOpenGLVectorizeDevice::windowHeight() const
{
  if (m_hWnd)
  { RECT cliRect; ::GetClientRect(m_hWnd, &cliRect);
    return cliRect.bottom - cliRect.top;
  }
  return OdGsOpenGLVectorizeDevice::windowHeight();
}
