...
ODRX_DECLARE_PROPERTY(GDITransparencyMode)
ODRX_DECLARE_PROPERTY(GDIRasterMode)
...
ODRX_BEGIN_DYNAMIC_PROPERTY_MAP(ExGsGDIVectorizeDevice);
  ...
  ODRX_GENERATE_PROPERTY(GDITransparencyMode)
  ODRX_GENERATE_PROPERTY(GDIRasterMode)
...
ODRX_DEFINE_PROPERTY_METHODS(GDITransparencyMode,       ExGsGDIVectorizeDevice, getTransparencyMode,          setTransparencyMode,             getUInt8 );
ODRX_DEFINE_PROPERTY_METHODS(GDIRasterMode,             ExGsGDIVectorizeDevice, getRasterMode,                setRasterMode,                   getUInt8 );
...
void ExGsGDIVectorizeDevice::onSize(const OdGsDCRect& outputRect)
{
  ...
  if(m_hbmScreen) // substitute the bitmap
  {
    ...
    if(m_hbmScreen)
    {
      ODA_VERIFY(::DeleteObject(::SelectObject(m_hOffscreenDC->getHDC(), m_hbmScreen)));
      initDrawDcOffset();
    }
    else // no space for double buffer - turning it off
    {
      m_hOffscreenDC.reset(new HDCStore(hdc(), getTransparencyMode(), getRasterMode() ));
      m_hOffscreenDC->setSizes( width(), height() );
    }
  }
}
...
HDC ExGsGDIVectorizeDevice::createDrawDc()
{
  ...
  {
    m_hOffscreenDC.reset(new HDCAutoDestroyStore(::CreateCompatibleDC(hdc()), getTransparencyMode(), getRasterMode() ));
    int w = width();
    int h = height();
    int v_min = m_outputRect.m_min.x;
    int v_max = m_outputRect.m_max.x;
    if( m_outputRect.m_min.x > m_outputRect.m_max.x )
    {
      v_min = m_outputRect.m_max.x;
      v_max = m_outputRect.m_min.x;
    }
    if( v_min != 0 ) w = v_max;
    v_min = m_outputRect.m_min.y;
    v_max = m_outputRect.m_max.y;
    if( m_outputRect.m_min.y > m_outputRect.m_max.y )
    {
      v_min = m_outputRect.m_max.y;
      v_max = m_outputRect.m_min.y;
    }
    if( v_min != 0 ) h = v_max;
    m_hOffscreenDC->setSizes( w, h );
    if(m_hOffscreenDC.get())
    {
      ...
      if(m_hbmScreen)
      {
        ::SelectObject( m_hOffscreenDC->getHDC(), m_hbmScreen);
        initDrawDcOffset();
      }
      else
      {
        m_hOffscreenDC.reset();
      }
    }
  }
  if(!m_hOffscreenDC.get())
...
void ExGsGDIVectorizeDevice::flushDrawDc()
{
  ...
  {
    initDrawDcOffset(false);
    ::BitBlt(hdc(), hOffset(), vOffset(), width(), height(), drawDc()->targetDC(), 0, 0, SRCCOPY);
    initDrawDcOffset();
  }
}

void ExGsGDIVectorizeDevice::initDrawDcOffset(bool bSet)
{
#ifndef _WIN32_WCE
  const int offsetX = (bSet) ? hOffset() : 0, offsetY = (bSet) ? vOffset() : 0;
  ::SetWindowOrgEx(m_hOffscreenDC->getHDC(), offsetX, offsetY, NULL);
#endif
}
...
void ExGsGDIVectorizeDevice::beginPaint()
{
  ...
  ExGiGDIGeometry::setHiddenGeometry( false );
  ExGiGDIGeometry::setSuppressHighlight(false);

  if(invalidRects().size())
    ExGiGDIGeometry::pushViewportClip(invalidRects().size(), invalidRects().getPtr());

  if(m_bClearScreen && !isValid())
    drawDc()->rectangle( m_outputRect.m_min.x, m_outputRect.m_max.y, m_outputRect.m_max.x, m_outputRect.m_min.y);
}
...
void ExGDIVectorizeView::beginViewVectorization()
{
  ...
  // setting draw context and deviation to simplifier
  OdGeDoubleArray devs(5, 1);
  for (int nDeviation = 0; nDeviation < 5; nDeviation++)
    devs.push_back(handleDeviationMultiplier((OdGiDeviationType)nDeviation, .5));
...
void ExGDIVectorizeView::endViewVectorization()
{
  ...
  setAuxData(OdGiAuxiliaryDataPtr());
...
void ExGDIVectorizeView::pushClipBoundary(OdGiClipBoundary* pBoundary)
{
  ...
  OdGsBaseVectorizer::pushClipBoundary(pBoundary);
...
void ExGDIVectorizeView::pushClipBoundary(OdGiClipBoundary* pBoundary, OdGiAbstractClipBoundary* pClipInfo)
{
  ...
  OdGsBaseVectorizer::pushClipBoundary(pBoundary, pClipInfo);
...
struct RecHide : OdGiGeometryMetafile::Record
{
  bool m_bState;
...
void ExGDIVectorizeView::onTraitsModified()
{
  ...
  if(m_pMetafiler->metafile())
  {
    if (!pDevice->supportDynamicSubhighlight())
    {
      ...
      RecHide* pRHide = NULL;
      if( m_bCurHiddenState != isHidden() )
      {
        m_pMetafiler->flush();
        m_pMetafiler->add( pRHide = new RecHide );
        pRHide->m_bState = m_bCurHiddenState = isHidden();
      }
      if (m_pMetafiler->saveTraits(effectiveTraits(), byBlockTraits()))
      {
        if (pRHLT != NULL || pRHide != NULL)
        {
  ...
  {
    const OdGsStateBranch *pBranch = getCurrentHLTBranch();
    if (pBranch && !pBranch->markersEmpty() && pBranch->id() == currentGiPath()->persistentDrawableId())
    {
           ...
           bDrawDragging) // needn't to apply plotstyles when dragging
  {
    ...
    if(bDrawDragging)
    {
      ...
      const ODCOLORREF bkRgb = pDevice->getPaletteBackground();
      rgb ^= bkRgb;
      if ((rgb & 0xFFFFFF) == (bkRgb & 0xFFFFFF))
        rgb = ~rgb; // To make geometry visible if it's color inverted
  ...
  pDevice->setHiddenGeometry( isHidden() );
  pDevice->setSuppressHighlight((OdUInt8(highlightingPass()) & 2) | (GETBIT(highlightingPass(), isHighlighted() ? 1 : 2) ? 1 : 0));
  pDevice->updatePenNBrush(bDrawSelection);

  pDevice->draw_transparency( effectiveTraits().transparency() );
}
...
bool ExGDIVectorizeView::pushPaletteOverride(const OdGiPalette* pOverride)
{
  ...
  if (m_pMetafiler->metafile())
  {
    ...
    return OdGsBaseVectorizer::pushPaletteOverride(pOverride);
}
...
struct RecOwnerDrawDc : OdGiGeometryMetafile::Record
{
  ...
  void play(OdGiConveyorGeometry*, OdGiConveyorContext* pCtx) const
  {
    ...
    {
      OdGeMatrix3d x = pView->worldToDeviceMatrix() * pView->metafileTransform();
...
void ExGDIVectorizeView::beginMetafile(OdRxObject* pMetafile)
{
  ...
  {
    OdGiPalettePtr pCurPalette = OdGiPalette::createDynamic();
    int iNumColors = 0;
    const OdUInt32* pCurRGBData = getPalette(&iNumColors);
    pCurPalette->install(pCurRGBData);
    pushPaletteOverride(pCurPalette);
  }
}
...
void ExGDIVectorizeView::pushMetafileTransform(const OdGeMatrix3d& mtx, OdUInt32 uFlags)
{
  if (GETBIT(uFlags, kSharedRefUpdate) ||
      (!m_bInexactSelection && GETBIT(uFlags, kSharedRefSelect)))
  {
...
const OdGeMatrix3d &ExGDIVectorizeView::metafileTransform() const
{
  if (!m_metafileTransfStack.isEmpty())
    return OdGsBaseVectorizer::metafileTransform();
  return m_mfTransformStack.metafileTransform();
}
                              ...
                              const OdGiTextStyle* pStyle)
{
  ...
  if (device()->useTextOut())
  {
    ...
    if (m_bMetafile 
      ...
      && !(!raw && OdCharMapper::containsSpecialSequence(sMsg, pStyle->getFont(), ulRanges))
  ...
  if (device()->useTtfCache() && pStyle->isTtfFont())
  {
    ...
    else
    {
      ...
      { OdGiGeometryMetafilePtr pSavedMf = m_pMetafiler->metafile();
        m_pMetafiler->flush();
        OdGeMatrix3d m2w(m_pModelToEyeProc->modelToWorldTransform());
        switch (m_pMetafiler->coordinatesType())
        {
          case OdGiMetafiler::kEye: m2w.preMultBy(view().worldToEyeMatrix()); break;
          case OdGiMetafiler::kDevice: m2w.preMultBy(worldToDeviceMatrix()); break;
          default: break;
        }
        ...
        else
        {
          ...
          { // Text doesn't clipped, vectorize to cache
            if (!device()->ttfFontsCache()->processText(origin, m2w * u, m2w * v,
                msg, length, raw, pStyle, drawContext(), ttfInfo, this))
            {
              ...
              secondaryOutput().destGeometry().textProc(origin, m2w * u, m2w * v, msg, length, raw, pStyle);
            m_pMetafiler->exchangeMetafile(pSavedMf);
          }
        }
      }
...
OdGiRasterImagePtr ExGsGDIVectorizeDevice::snapshotImageParams() const
{
  ...
  HDC hDc = const_cast<ExGsGDIVectorizeDevice*>(this)->drawDc()->targetDC();
  if (!hDc)
