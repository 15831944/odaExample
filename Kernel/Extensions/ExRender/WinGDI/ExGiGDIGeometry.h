...
private:
  ...
  ExGiGDIDC*              m_hTargetDC;
  HDC safeTargetDC() const
  {
    if( !m_hTargetDC ) return (HDC)NULL;
    return m_hTargetDC->targetDC();
  }
...
protected:
  ...
  void renderGradientTriangles( ExGiGDIDC* hdc );
public:
  /** \details
  ...
  */
  ...
  /** \details
    ...
    \param transp [in]  Transparency.
  */
  void draw_transparency( OdCmTransparency transp );
  /** \details
...
protected:
  ...
  /** Hidden geometry flag */
  bool              m_bHiddenGeometry;
  OdUInt8           m_suppressByHlt;
public:
  ExGiGDIGeometry();
  ~ExGiGDIGeometry();
  ...
  */
  void setSelection(bool drawSelection) 
  { 
  ...
  */
  bool isHiddenGeometry(OdUInt8 sFlag = 1) const { return m_bHiddenGeometry || GETBIT(m_suppressByHlt, sFlag); }
  /** \details
    Specifies taht geometry should be hidden or not.
    \param bHidden [in] Hidden flag value.
  */
  void setHiddenGeometry( bool bHidden ) { m_bHiddenGeometry = bHidden; }
  /** \details
    Enable geometry painting suppression by highlighting sign.
    \param nSuppress [in] Geometry suppression flags.
  */
  void setSuppressHighlight(OdUInt8 nSuppress) { m_suppressByHlt = nSuppress; }

  /** \details
    Initializes the paint process for this Vectorizer Device object.
    \param targetDC [in]  Handle to the target device context.
  */
  void beginPaint(ExGiGDIDC* targetDC);
  /** \details
  ...
  */
  ...
  /** \details
    ...
    \remarks
    The visualization framework uses this function to visualize Ole2Frame entities. 
    
    \note
    ownerDrawDc is not conveyor primitive, so the ODA visualization framework performs no processing 
