                             ...
                             , public OdTtfFontsCacheCallback
{
  ...
  {
    ExGiGDIDC* m_hdc;
    ExGiGDIDC *createDC(OdUInt8 nTranspMode, OdUInt8 nRasterMode)
    {
      ExGiGDIDC::ExGDIDCTransparencyMode mode = ExGiGDIDC::kNoTransparency;
      if( nTranspMode == ExGiGDIDC::kDirectTransparency ) mode = ExGiGDIDC::kDirectTransparency;
      else if( nTranspMode == ExGiGDIDC::kMirrorTransparency ) mode = ExGiGDIDC::kMirrorTransparency;
      if (nRasterMode && mode != ExGiGDIDC::kMirrorTransparency) mode = ExGiGDIDC::kRasterTransparency;
      return m_hdc = new ExGiGDIDC( mode );
    }
  public:
    //HDC& operator=(const HDC &hdc) { m_hdc = hdc; return m_hdc; }
    ExGiGDIDC* getExDC() const { return m_hdc; }
    HDC getHDC() const { return m_hdc->targetDC(); }
    HDCStore( OdUInt8 nTranspMode, OdUInt8 nRasterMode ) 
    {
      createDC(nTranspMode, nRasterMode);
    }
    HDCStore( HDC hdc, OdUInt8 nTranspMode, OdUInt8 nRasterMode )
    { 
      createDC(nTranspMode, nRasterMode)->setDC( hdc );
    }
    virtual ~HDCStore() { delete m_hdc; };
    void setSizes( int w, int h ) { m_hdc->setSizes( w, h ); }
  };
  ...
  public:
    ...
    ExGiGDIDC* getExDC() { return HDCStore::getExDC(); }
    HDC getHDC() const { return HDCStore::getHDC(); }
    HDCAutoDestroyStore(HDC hdc, OdUInt8 nTranspMode, OdUInt8 nRasterMode ) : HDCStore(hdc, nTranspMode, nRasterMode) { }
    virtual ~HDCAutoDestroyStore() 
    {
      ::DeleteDC( getHDC() );
    };
    void setSizes( int w, int h ) { HDCStore::setSizes( w, h ); }
  };

  void beginPaint();
  void endPaint();
  POINTArray              m_dcPoints;
  std::unique_ptr<HDCStore> m_hOffscreenDC;
  HBITMAP                 m_hbmScreen;
  bool                    m_bClearScreen;
  ...
  void deleteDrawDc();
  void flushDrawDc();
  void initDrawDcOffset(bool bSet = true);
protected:
  ...
  OdUInt8 m_nTransparencyMode, m_nRasterMode;
public:
  /** \details
    Returns the offscreen device context for the Vectorizer Device object.
  */
  inline ExGiGDIDC* drawDc() { return (m_hOffscreenDC.get()) ? (m_hOffscreenDC->getExDC()) : NULL; }
  ...
  void setColorConverter(const OdColorConverterCallback *pCC)
  {
    ...
    ExGiGDIGeometry::m_colorConverter.setColorConverterCallback(getColorConverter());
  }
...
public:
  ...
  */
  ...
  OdUInt8 getTransparencyMode() const { return m_nTransparencyMode; }
  void setTransparencyMode( OdUInt8 nMode ) { m_nTransparencyMode = nMode; }

  OdUInt8 getRasterMode() const { return m_nRasterMode; }
  void setRasterMode(OdUInt8 nMode) { m_nRasterMode = nMode; }
protected:
...
class ExGDIVectorizeView : public OdGsBaseVectorizeViewMT
{
  ...
  bool                           m_bCurHiddenState; // !dynamicSubEntHltEnabled
...
public:
  ...
  */
  ...
  const OdGsStateBranch* getCurrentHLTBranch() const
  {
