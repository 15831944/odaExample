                                        ...
                                        const OdGiVertexData* pVertexData)
{
  if(m_grFillPrAddr)
  {
    ...
    const bool bConvertColors = m_colorConverter.colorConverterCallback() && (m_colorConverter.colorConverterCallback()->convertBackgroundColors() ||
                                                                              !GETBIT(drawContext()->effectiveTraits().drawFlags(), OdGiSubEntityTraits::kDrawNoPlotstyle));
    ...
    for(OdInt32 i = 0; i < nbVertex; i ++)
    {
      ...
      ODCOLORREF color;
      if (!m_bGlobalAlpha)
        color = ODTOCOLORREF(pVertColors[i]);
      else
        color = OdMergeRGBAlpha(m_clrFadeColor, ODTOCOLORREF(pVertColors[i]), (OdUInt8)m_nGlobalAlpha);
      if (bConvertColors)
        color = m_colorConverter.convert(color);
      m_triVertexes[i].Red   = unsigned short(ODGETRED(color))   << 8;
      m_triVertexes[i].Green = unsigned short(ODGETGREEN(color)) << 8;
      m_triVertexes[i].Blue  = unsigned short(ODGETBLUE(color))  << 8;
...
void ExGiGDIGeometry::endGradientFill( ExGiGDIDC* hdc)
{
  ...
  if (!m_bGradientAsBitmap && !m_bGradientAsPolys)
  {
    ...
    int devType = ::GetDeviceCaps(hdc->targetDC(), TECHNOLOGY);
...
#endif
    {
      ((GRADFILLPROC)m_grFillPrAddr)( hdc->targetDC(), m_triVertexes.asArrayPtr(), m_triVertexes.size(),
        m_triangles.asArrayPtr(), m_triangles.size(), GRADIENT_FILL_TRIANGLE);
      if( hdc->isMirrorMode() )
      {
        ( (GRADFILLPROC)m_grFillPrAddr )( hdc->memoryDC(), m_triVertexes.asArrayPtr(), m_triVertexes.size(),
          m_triangles.asArrayPtr(), m_triangles.size(), GRADIENT_FILL_TRIANGLE );
      }
    }
#ifdef OD_ENABLE_POSTSCRIPT_PRINTING
...
#define OD_MULT_X_3(x) (((x) << 1) + (x)) // Optimized multiplication by 3 ((x * 3) == ((x << 1) + x))
      ...
      for (i = 0; i < trisSize; i += PSLimit)
      {
        ...
        ((GRADFILLPROC)m_grFillPrAddr)(hdc->targetDC(), triVerts, OD_MULT_X_3(nTris),
          pTriOut, nTris, GRADIENT_FILL_TRIANGLE);
        if( hdc->isMirrorMode() )
        {
          ( (GRADFILLPROC)m_grFillPrAddr )( hdc->memoryDC(), triVerts, OD_MULT_X_3( nTris ),
            pTriOut, nTris, GRADIENT_FILL_TRIANGLE );
        }
      }
      delete []pTriOut;
...
//#define OD_CLUSTERIZER_DEBUG
          ...
          pPixelsBuf = pPixels;
#else
          HGDIOBJ hPrevPen = hdc->selectObject( ::GetStockObject(NULL_PEN));
          HGDIOBJ hPrevBrush;
          if (m_dcBrushClrPrAddr)
            hPrevBrush = hdc->selectObject( ::GetStockObject(DC_BRUSH));
#endif
          ...
          struct Clusterizer {
            ...
            Clusterizer( ExGiGDIDC* hDc, const SIZE &minLim, OdUInt8 threshold, OdUInt8 *pPixels, const SIZE &sizeIm, DCBRUSHCLRPROC dcProc)
              ...
              for (OdUInt32 nY = 0; nY < (OdUInt32)m_sizeIm.cy; nY++)
              {
                for (OdUInt32 nX = 0; nX < (OdUInt32)m_sizeIm.cx; nX++, pScan += 4)
                {
                  if (pScan[3] == 0) // First pixel for square
                  {
                    ...
                    if (m_dcProc)
                    {
                      m_dcProc(m_hDc->targetDC(), RGB(minColor[2] + ((maxColor[2] - minColor[2]) >> 1),
                                          minColor[1] + ((maxColor[1] - minColor[1]) >> 1),
                                          minColor[0] + ((maxColor[0] - minColor[0]) >> 1)));
                      if( m_hDc->isMirrorMode() )
                      {
                        m_dcProc( m_hDc->memoryDC(), RGB( minColor[ 2 ] + ( ( maxColor[ 2 ] - minColor[ 2 ] ) >> 1 ),
                          minColor[ 1 ] + ( ( maxColor[ 1 ] - minColor[ 1 ] ) >> 1 ),
                          minColor[ 0 ] + ( ( maxColor[ 0 ] - minColor[ 0 ] ) >> 1 ) ) );
                      }
                    ...
                    else
                    {
                      HBRUSH hBrush = ::CreateSolidBrush(RGB(minColor[2] + ((maxColor[2] - minColor[2]) >> 1),
                                                             minColor[1] + ((maxColor[1] - minColor[1]) >> 1),
                                                             minColor[0] + ((maxColor[0] - minColor[0]) >> 1)));
                      HGDIOBJ hPrevBrush = m_hDc->selectObject( (HGDIOBJ)hBrush);
                      m_hDc->polygon( apt, 4);
                      m_hDc->selectObject( hPrevBrush);
          ...
          hdc->selectObject( hPrevPen);
          if (m_dcBrushClrPrAddr)
            hdc->selectObject( hPrevBrush);
#endif
          delete[] pPixelsBuf;
        }
      }
      ...
      { // Typically doesn't supported by printers. But works with XPS.
        HDC hMemDC = ::CreateCompatibleDC(hdc->targetDC());
        if (!hMemDC) hMemDC = ::CreateCompatibleDC(NULL); // Possible for EMF
        HBITMAP hMemBitmap = ::CreateCompatibleBitmap( hdc->targetDC(), bmpSize.cx, bmpSize.cy);
        HBITMAP hPrevMemBitmap = (HBITMAP)::SelectObject(hMemDC, (HGDIOBJ)hMemBitmap);
        ...
        ::SetBkColor(hMemDC, RGB(0, 0, 0));
        ::BitBlt(hMaskDC, 0, 0, bmpSize.cx, bmpSize.cy, hMemDC, 0, 0, SRCCOPY);
        hdc->maskBlt( minLim.cx, minLim.cy, bmpSize.cx, bmpSize.cy, hMemDC, 0, 0, hMaskBitmap, 0, 0, 0xCCAA0020);
        ::SelectObject(hMaskDC, (HGDIOBJ)hPrevMaskBitmap);
        ::SelectObject(hMemDC, (HGDIOBJ)hPrevMemBitmap);
        ::DeleteObject((HGDIOBJ)hMaskBitmap);
        ::DeleteObject((HGDIOBJ)hMemBitmap);
...
void ExGiGDIGeometry::beginPaint( ExGiGDIDC* targetDC )
{
  ...
  m_hOldBrush = m_hTargetDC->selectObject( ::CreateBrushIndirect( &m_logBrush ) );  //::SelectObject(hTargetDC, ::CreateBrushIndirect(&m_logBrush));
#ifndef _WIN32_WCE
  m_hOldPen = m_hTargetDC->selectObject( createCurrentPen() ); //::SelectObject(hTargetDC, createCurrentPen());
#else
  m_hOldPen = m_hTargetDC->selectObject( ::CreatePen( m_dwPenStyle, m_dwWidth, m_penBrush.lbColor ) );  //::SelectObject(hTargetDC, ::CreatePen(m_dwPenStyle, m_dwWidth, m_penBrush.lbColor));
#endif
  ...
  m_hTargetDC->setBkColor( m_paletteColors[ 0 ] );

  m_oldBkMode = TRANSPARENT;
  //::SetBkMode(hTargetDC, m_oldBkMode);
  m_hTargetDC->setBkMode( m_oldBkMode );
}

void ExGiGDIGeometry::endPaint()
{
  draw_globalAlpha( false );
  if( m_hTargetDC )
    ::DeleteObject( m_hTargetDC->selectObject( m_hOldPen ) );
  if(m_logBrush.lbStyle == BS_PATTERN)
    ::DeleteObject((HBITMAP)m_logBrush.lbHatch); // delete old bitmap
  if( m_hTargetDC )
    ::DeleteObject( m_hTargetDC->selectObject( m_hOldBrush ) );
...
static void applyLwdPolylineHighlighting(ExGiGDIDC* hdc, OdInt32 nPts, const OdGePoint3d* points, ODCOLORREF color)
{
  HGDIOBJ hOldPen = hdc->selectObject( ::CreatePen(PS_DOT, 1, color));
  ...
  ::DeleteObject(hdc->selectObject(hOldPen));
}
...
public:
  inline GDIObjStack()
    : m_hdc(0)
  { }
  inline GDIObjStack( ExGiGDIDC* hdc, HGDIOBJ hObj)
    : m_hdc(0)
  {
    pushObject(hdc, hObj);
  }
  inline void pushObject( ExGiGDIDC* hdc, HGDIOBJ hObj)
  {
    ODA_ASSERT(m_hdc==0);
    m_hdc = hdc;
    m_hOldObj = hdc->selectObject(hObj);
  }
  inline void popObject()
  {
    if(m_hdc)
    {
      m_hdc->selectObject(m_hOldObj);
      m_hdc = 0;
    }
  }
  inline ~GDIObjStack()
  ...
  if(nPts > 0)
  {
    if (!m_bExtLwdStyle || m_dwWidth <= 1 || nPts == 1)
    {
      ...
      if(nPts > 1)
      {
        ...
        m_hTargetDC->moveToEx( x, y, 0 );
        ++points;
        while(--nPts)
        {
          x = OdTruncateToLong(points->x);
          y = OdTruncateToLong(points->y);
          m_hTargetDC->lineTo( x, y );
          ++points;
        }
        if(!isExtPen())
        {
        ...
        if (m_dwWidth > 1 && m_oldLinetype == kLtpForSelection)
          applyLwdPolylineHighlighting( m_hTargetDC, nStore, points - nStore, m_paletteColors[0]);
      }
      else
      {
        if (!m_bPointsLwd || m_dwWidth <= 1)
        {
          m_hTargetDC->setPixel(x, y, m_penBrush.lbColor);
        }
        else
        {
          // Draw as filled circle
          m_hTargetDC->moveToEx(x, y, NULL); m_hTargetDC->lineTo(x, y);
        }
      }
    }
    else
    {
      { // #9277 : extended lineweight styles support
        ...
        HGDIOBJ prevPen = m_hTargetDC->selectObject( ( itlThinPen ) ? itlThinPen : thinPen() ); //::SelectObject(hdc, (itlThinPen) ? itlThinPen : thinPen());
        ...
        if ((m_logBrush.lbStyle != BS_SOLID) || m_logBrush.lbHatch)
        {
          ...
          prevBrush = m_hTargetDC->selectObject( itlBrush ); //::SelectObject(hdc, itlBrush);
        }
        class OdExGDIExtendedLineWeightStyleClient : public OdExExtendedLineWeightStyleClient
        {
          ...
          HGDIOBJ capPen;
          public:
            OdExGDIExtendedLineWeightStyleClient(double m_lwd, OdPs::LineEndStyle capStyle, OdPs::LineJoinStyle joinStyle, ExGiGDIDC* _hdc, HGDIOBJ hPen)
            ...
            virtual bool elsPolygonOut(const OdGePoint3d *pPoints, OdUInt32 nPoints)
            {
              ...
              hdc->polygon(dcPoints, (int)nPoints);
              return true;
            }
            virtual bool elsCircleOut(const OdGePoint3d &pPoint, const OdGeVector3d & /*normal*/, double /*radius*/)
            {
              ...
              hdc->moveToEx(x, y, NULL); hdc->lineTo(x, y);
              return true;
            }
        } _exlwd_renderer((double)m_dwWidth, m_lwdCapStyle, m_lwdJoinStyle, m_hTargetDC, prevPen);
        _exlwd_renderer.process(points, nPts);
        if (itlBrush)
          ::DeleteObject( m_hTargetDC->selectObject(prevBrush));
        m_hTargetDC->selectObject(prevPen);
        if (itlThinPen)
      ...
      if (m_oldLinetype == kLtpForSelection)
        applyLwdPolylineHighlighting( m_hTargetDC, nPts, points, m_paletteColors[0]);
    }
  }
}
                                   ...
                                   OdGsMarker /*lBaseSubEntMarker*/)
{
  ...
  if(pExtrusion && ((OdGeVector2d*)pExtrusion)->lengthSqrd() > .25)
  {
    ...
    for(OdInt32 i=1; i<nPts; ++i)
    {
      ...
      m_hTargetDC->moveToEx( X, Y, 0);
      m_hTargetDC->lineTo( X2, Y2);
      m_hTargetDC->lineTo( X2e, Y2e);
      m_hTargetDC->lineTo( Xe, Ye);
...
{
  if( isHiddenGeometry() ) return true;
  ...
  if (nPoints > 0)
  {
    GDIObjStack _pen(m_hTargetDC, (HGDIOBJ)m_hExPen);
    if (nPoints > 1)
    {
      ...
      m_hTargetDC->polyline( pOut, (int)nPoints);
    }
    else
    {
      // Draw as filled circle
...
void ExGiGDIGeometry::polygonOut(OdInt32 nPts, const OdGePoint3d* points, const OdGeVector3d*)
{
  ...
  if( m_hTargetDC->isAlphaMode() && nPts > 3 )
  {
    OdInt32Array faces;
    ...
    shellProc( nPts, points, faces.size(), faces.asArrayPtr() );
    return;
  }
  ::toDevice(nPts, points, m_dcPoints);
  m_hTargetDC->polygon( m_dcPoints.getPtr(), nPts);
}
...
void ExGiGDIGeometry::shellFaceOut(OdInt32 faceListSize, const OdInt32* pFaceList, const OdGeVector3d* pNormal)
{
  ...
  if( isHiddenGeometry() ) return;
  if( m_hTargetDC->isAlphaMode() )
  {
    if( faceListSize != 4 || pFaceList[ 0 ] != 3 )
    {
...
#ifndef _WIN32_WCE
    m_hTargetDC->polyPolygon( m_dcPoints.getPtr(), (int*)counts.getPtr(), counts.size());
#else
    CDC *pDc = CDC::FromHandle( m_hTargetDC->targetDC() );
    pDc->PolyPolygon( (struct tagPOINT *)m_dcPoints.getPtr(), (int*)counts.getPtr(), counts.size());
#endif
  }
  else // no holes
  {
    m_hTargetDC->polygon( m_dcPoints.getPtr(), *pFaceList);
  }
...
#else
  if (::GetDeviceCaps(m_hTargetDC->targetDC(), TECHNOLOGY) == DT_METAFILE || m_hTargetDC->isAlphaMode() )
  {
    // Old Windows metafiles don't support GDI paths, so goto shell branch
    // Also currently we do not support transparency for GDI paths
    OdGiConveyorGeometry::ttfPolyDrawProc(nbVertex, pVertexList, faceListSize, pFaceList, pBezierTypes, pFaceData);
  }
  else
  {
    ...
    int prevFillMode = ::GetPolyFillMode(m_hTargetDC->targetDC() );
    if (prevFillMode != WINDING)
      m_hTargetDC->setPolyFillMode( WINDING );
    m_hTargetDC->beginPath();
    m_hTargetDC->polyDraw( m_dcPoints.getPtr(), (const BYTE*)pBezierTypes, nbVertex);
    m_hTargetDC->endPath();
    m_hTargetDC->strokeAndFillPath();
    if (prevFillMode != WINDING)
      m_hTargetDC->setPolyFillMode( prevFillMode );
  }
#endif
}
...
class PresentedPointsWrap
{
  ExGiGDIDC* m_pDC;
public:
  PresentedPointsWrap( ExGiGDIDC* pDC )
  {
    m_pDC = pDC;
    pDC->clearPresentedPoints();
  }
  ~PresentedPointsWrap()
  {
    m_pDC->clearPresentedPoints();
  }
};
...
void ExGiGDIGeometry::shellProc(OdInt32 nbVertex,
                                ...
                                const OdGiVertexData* pVertexData)
{
  PresentedPointsWrap __wrap( m_hTargetDC );
#ifndef _WIN32_WCE
  // Gradient filling
...
class TextCharacterExtraSaver
{
  ...
  bool m_bSet;
  public:
    TextCharacterExtraSaver( ExGiGDIDC* hDc, int nExtra)
      : m_hDc(hDc)
      , m_nExtra(0)
      , m_bSet(false)
    {
      if (nExtra != 0)
      {
        m_nExtra = ::GetTextCharacterExtra(hDc->targetDC());
        hDc->setTextCharacterExtra(nExtra);
        m_bSet = true;
      }
    }
    ...
    {
      if (m_bSet)
        m_hDc->setTextCharacterExtra( m_nExtra);
    }
};
...
class TextAlignSaver
{
  ...
  bool m_bSet;
  public:
    TextAlignSaver( ExGiGDIDC* hDc, UINT nAlign)
      : m_hDc(hDc)
      , m_nAlign(0)
      , m_bSet(false)
    {
      m_nAlign = ::GetTextAlign(hDc->targetDC());
      if (m_bSet = (nAlign != m_nAlign))
        hDc->setTextAlign( nAlign);
    }
    ~TextAlignSaver()
    {
      if (m_bSet)
        m_hDc->setTextAlign( m_nAlign );
    }
};
...
OdString ExGiGDIGeometry::textProcExEmbedRawData(const OdString &msg, const OdIntArray &ulFlags, int *length, bool *raw)
{
  if (ulFlags.isEmpty())
    return msg;
  return OdCharMapper::embedTextRawData(msg, ulFlags, length, raw);
}
...
static void drawTextProcExScore(ExGiGDIDC* hdc, HGDIOBJ hThinPen, const OdGePoint3d& position, const OdIntArray &ulFlags,
  ...
  for (int nFlag = 0; nFlag < nFlags; nFlag++, pFlags += 2)
  {
    ...
    if (nPos > nPrevPos)
    {
      ...
      if (nPrevFlags)
      {
        ...
        TEXTMETRIC tm; ::GetTextMetrics(hdc->targetDC(), &tm);
        ...
        { // Xform should be added here, elsewhere text metrics will differ from original
          ...
          hdc->setWorldTransform(pXform);
        }
        if (GETBIT(nPrevFlags, 1))
        { // Underline
          const int y = yBase + tm.tmDescent;
          hdc->moveToEx( xFrom, y, NULL);
          hdc->lineTo( xTo, y);
        }
        ...
        { // Xform should be removed here, elsewhere text metrics will differ from original
          hdc->modifyWorldTransform( pXform, MWT_IDENTITY);
          hdc->setGraphicsMode( GM_COMPATIBLE);
        }
      }
      nPrevPos = nPos;
      nAccum += nLen;
...
//   }
  LOGFONT lFont;
  if ( ::getLogFont( xDir, yDir, *pTextStyle, twisted, &lFont, m_hTargetDC->targetDC(), &drawContext()->giContext() ) )
  {
    if ( HFONT hFont = ::CreateFontIndirect( &lFont ) )
    {
      HGDIOBJ hOldFont = m_hTargetDC->selectObject( hFont );
      ...
      // AMark: cr5603
      m_hTargetDC->setTextColor(m_penBrush.lbColor); // Set text color yet, because SetTextColor() not always called in draw_color()
      ...
      DWORD flags = ::GetFontLanguageInfo(m_hTargetDC->targetDC());
      flags &= ~(GCP_LIGATE|GCP_USEKERNING); // Acad doesn't use kerning or ligatures
      DWORD size = ::GetCharacterPlacementW( m_hTargetDC->targetDC(), sMsg, length, length, &res, flags);
      if ( ( size & 0xffff ) == 0  ) // #10937 
      {
        m_hTargetDC->selectObject( hOldFont );
        ::DeleteObject(hFont);
        ::drawTextThroughVectorizer(this, position, u, v, sMsg, length, ulFlags, pTextStyle);
        return;
      }
      ...
      m_hTargetDC->setBkMode( TRANSPARENT );
      TextAlignSaver _tas(m_hTargetDC, TA_BASELINE | TA_LEFT);
      ...
      if (twisted)
      {
        ...
        m_hTargetDC->setGraphicsMode( GM_ADVANCED );
        m_hTargetDC->setWorldTransform( &xForm );
        m_hTargetDC->extTextOutW( 0, 0, outFlags, 0, strOut, sizeMsg, res.lpDx);
        m_hTargetDC->modifyWorldTransform( &xForm, MWT_IDENTITY );
        m_hTargetDC->setGraphicsMode( GM_COMPATIBLE );
        if (!ulFlags.isEmpty())
          ::drawTextProcExScore( m_hTargetDC, (ttfProcessingPenOverride()) ? thinPen() : NULL, OdGePoint3d::kOrigin, ulFlags, res.lpDx, pTextStyle, &xForm);
      }
      else
      {
        try
        {
          m_hTargetDC->extTextOutW( ROUND(position.x), ROUND(position.y), outFlags, 0, strOut, sizeMsg, res.lpDx);
          if (!ulFlags.isEmpty())
            ::drawTextProcExScore(m_hTargetDC, (ttfProcessingPenOverride()) ? thinPen() : NULL, position, ulFlags, res.lpDx, pTextStyle);
        }
        catch (const OdError &)
...
void ExGiGDIGeometry::pushViewportClip(const OdGsDCPoint* pPoints, const int *nrcCounts, int nCount)
{
  if( m_hTargetDC )
  {
    m_clipStack.push_back( m_hTargetDC->saveDC() );
    m_hTargetDC->beginPath();
    m_hTargetDC->polyPolygon( ( CONST POINT* )pPoints, ( CONST INT* )nrcCounts, nCount );
    m_hTargetDC->endPath();
    m_hTargetDC->selectClipPath( RGN_AND );
  }
  else
  {
    HDC dc = safeTargetDC();
    m_clipStack.push_back( ::SaveDC( dc ) );
    ::BeginPath( dc );
    ::PolyPolygon( dc, ( CONST POINT* )pPoints, ( CONST INT* )nrcCounts, nCount );
    ::EndPath( dc );
...
void ExGiGDIGeometry::pushViewportClip(const OdGsDCPoint& min, const OdGsDCPoint& max)
{
  if( !m_hTargetDC )
  {
    m_clipStack.push_back( ::SaveDC( safeTargetDC() ) );
    ::IntersectClipRect( safeTargetDC(), min.x, max.y, max.x, min.y );
  }
  else
  {
    m_clipStack.push_back( m_hTargetDC->saveDC() );
    m_hTargetDC->intersectClipRect( min.x, max.y, max.x, min.y );
  }
}
...
void ExGiGDIGeometry::pushViewportClip(OdUInt32 nNumRects, const OdGsDCRect* rects)
{
  ...
  {
    m_clipStack.push_back( m_hTargetDC->saveDC() );
    int pfm = m_hTargetDC->setPolyFillMode( WINDING );
    m_hTargetDC->beginPath();
    m_hTargetDC->polyPolygon( ( CONST POINT* )points.getPtr(), ( CONST INT* )counts.getPtr(), counts.size() );
    m_hTargetDC->endPath();
    m_hTargetDC->selectClipPath( RGN_AND );
    m_hTargetDC->setPolyFillMode( pfm );
  }
#else
  // This WCE implementation sets clip region to extents of union of clipping rects.
  CDC *pDc = CDC::FromHandle( safeTargetDC() );
...
void ExGiGDIGeometry::popViewportClip()
{
  if(!m_clipStack.empty())
  {
    ...
    HPEN hPen = (HPEN)::GetCurrentObject( safeTargetDC(), OBJ_PEN );
    HBRUSH hBrush = (HBRUSH)::GetCurrentObject( safeTargetDC(), OBJ_BRUSH );

    if( !m_hTargetDC )
    {

      ::RestoreDC( safeTargetDC(), m_clipStack.last() );

      ::SelectObject( safeTargetDC(), hPen );
      ::SelectObject( safeTargetDC(), hBrush );
    }
    else
    {
      m_hTargetDC->restoreDC( m_clipStack.last() );
      m_hTargetDC->selectObject( hPen );
      m_hTargetDC->selectObject( hBrush );
    }
...
inline void PAINT_RECT(int left, int top, int right, int bottom, OdUInt8 blue,
                       OdUInt8 green, OdUInt8 red, ExGiGDIDC* hdc)
{
  RECT rect;
  rect.left   = left;
  rect.top    = top;
...
class StretchBltModeSaver
{
  ...
  int m_nNewMode;
  public:
    StretchBltModeSaver( ExGiGDIDC* hDc, int nMode)
      : m_hDc(hDc), m_nPrevMode(nMode), m_nNewMode(nMode)
    {
      m_nPrevMode = ::GetStretchBltMode(hDc->targetDC());
      if (m_nPrevMode != m_nNewMode)
        hDc->setStretchBltMode(nMode);
    }
    ~StretchBltModeSaver()
    {
      if (m_nNewMode != m_nPrevMode)
        m_hDc->setStretchBltMode( m_nPrevMode);
    }
...
void ExGiGDIGeometry::rasterImageProc(const OdGePoint3d& origin,
                                      ...
                                      double fade)
{
  if(isDragging() || isHiddenGeometry(2))
    return ;
...
#ifndef _WIN32_WCE
  StretchBltModeSaver _sbms( m_hTargetDC, (pImg->colorDepth() > 1) ? COLORONCOLOR : StretchBltModeSaver::monoImageMode(transparency, m_paletteColors[0]));
#endif
  ...
  if( u.isCodirectionalTo(OdGeVector3d::kXAxis) && v.isCodirectionalTo(-OdGeVector3d::kYAxis) )
  {
       ...
       (fabs(OdRound(corners[2].y) - OdRound(corners[0].y)) <= 1) )
    {
      ...
      {
        PAINT_RECT(0, 0, chColorTop, chColorLeft, blue, green, red, m_hTargetDC );
      }
      if(getScreenPixel(dWidth, 0, screen2UV, imgWidth, imgHeight, blue, green, red, pImg))
      {
        PAINT_RECT(chColorTop, 0, dWidth, chColorLeft, blue, green, red, m_hTargetDC );
      }
      if(getScreenPixel(0, dHeight, screen2UV, imgWidth, imgHeight, blue, green, red, pImg))
      {
        PAINT_RECT(0, chColorLeft, chColorTop, dHeight, blue, green, red, m_hTargetDC );
      }
      if(getScreenPixel(dWidth, dHeight, screen2UV, imgWidth, imgHeight, blue, green, red, pImg))
      {
        PAINT_RECT(chColorTop, chColorLeft, dWidth, dHeight, blue, green, red, m_hTargetDC );
      }
    }
    else
    {
      ...
      if((pImg->colorDepth() <= 8) && transparency && (pImg->transparentColor() >= 0))
      {
        ...
        const long rasterImageDim[4] = { OdTruncateToLong(origin.x), OdTruncateToLong(origin.y - v.length() * imgHeight),
                                         OdTruncateToLong(u.length() * imgWidth), OdTruncateToLong(v.length() * imgHeight) };
        ExGiGDIDC::RasterTrpContext *pTrpCtx = m_hTargetDC->beginTransparentRasterProc(rasterImageDim[0], rasterImageDim[1],
                                                                                       rasterImageDim[2], rasterImageDim[3]);

        OdUInt8 *pPixels;
        HBITMAP hBmpMask = CreateDIBSection(NULL, pHeader, DIB_RGB_COLORS, (void**)&pPixels, NULL, 0);
        pImg->scanLines(pPixels, 0, imgHeight);
        HDC maskDC = CreateCompatibleDC( m_hTargetDC->targetDC() );
        if (maskDC)
        ...
        else
        {
          ...
          m_hTargetDC->stretchDIBits(rasterImageDim[0], rasterImageDim[1], rasterImageDim[2], rasterImageDim[3],
        ...
        pImg->scanLines(pPixels, 0, imgHeight);
        HDC patchedDC = CreateCompatibleDC( m_hTargetDC->targetDC() );
        if (patchedDC)
        ...
        if (maskDC && patchedDC)
        {
          m_hTargetDC->stretchBlt(rasterImageDim[0], rasterImageDim[1], rasterImageDim[2], rasterImageDim[3],
        ...
        m_hTargetDC->endTransparentRasterProc(pTrpCtx, rasterImageDim[0], rasterImageDim[1], rasterImageDim[2], rasterImageDim[3]);
      }
      ...
      else
      {
        ...
        HDC sourceDC = CreateCompatibleDC(m_hTargetDC->targetDC());
        if (sourceDC)
        {
          // We're in raster device
          SelectObject(sourceDC, hBmp);
          m_hTargetDC->stretchBlt(
  ...
  else
  {
    ...
    if((srcColDepth <= 8) && transparency && (pImg->transparentColor() >= 0))
    {
      // First, get the color that is transparent
      int transpColor(pImg->transparentColor());
      const OdUInt32 dstColDepth((srcColDepth < 4) ? 4 : srcColDepth);

      OdUInt32 maskScLnSz = OdGiRasterImage::calcBMPScanLineSize(boundRectWidth, 1);
      OdUInt32 pixScLnSz  = OdGiRasterImage::calcBMPScanLineSize(boundRectWidth, dstColDepth);
      ...
      OdUInt32 pixBlackIndex((srcColDepth < 4) ? 2 : OdUInt32(transpColor)); // need to not forget to initialize
      ...
      for(i = boundRectY; i < boundRectY + boundRectHeight; i ++)
      {
        for(j = boundRectX; j < boundRectX + boundRectWidth; j ++)
        {
          ...
          if(x >= 0 && y >= 0 && x < imgWidth && y < imgHeight )
          {
            ...
            OdUInt8* pixPtr = (OdUInt8*)pixels.asArrayPtr() + dstY * pixScLnSz + dstX / ((dstColDepth >> 2) ^ 3);
            ...
            if(index == (OdUInt32)transpColor)
            {
              ...
              if (GETBIT(dstColDepth, 8))
                pixPtr[0] = pixBlackIndex;
              else
              { // 4bpp is minimal supported color depth
      ...
      ExGiGDIDC::RasterTrpContext *pTrpCtx = m_hTargetDC->beginTransparentRasterProc(boundRectX, boundRectY,
                                                                                     boundRectWidth, boundRectHeight);

      // now creating bitmaps
      BITMAPINFO* pBmpH;
      OdUInt8Array headerMemCont;
      headerMemCont.resize(sizeof(BITMAPINFOHEADER) + OdUInt32(1 << dstColDepth)*sizeof(ODCOLORREF));
      ...
      // Patched pixels
      pBmpH->bmiHeader.biBitCount = (OdUInt16)dstColDepth;
      ...
      if (sourceDC)
      {
        ...
        m_hTargetDC->bitBlt( boundRectX, boundRectY,
      ...
      else
      {
        ...
        pBmpH->bmiColors[maskWhiteIndex].rgbRed   = 255;
        pBmpH->bmiHeader.biBitCount = 1;
        m_hTargetDC->stretchDIBits( boundRectX, boundRectY,
      ...
      m_hTargetDC->endTransparentRasterProc(pTrpCtx, boundRectX, boundRectY, boundRectWidth, boundRectHeight);
    }
    ...
    else
    {
      ...
      void* pBuf = NULL;
      OdExGiGDIBytesReflector refl( &pBuf );

      HBITMAP hBmp = m_hTargetDC->createDIBSection( pBoundRectH, DIB_RGB_COLORS, refl, NULL, 0 );

      if(pBuf)
        refl.fillBy( pixels.asArrayPtr(), boundRectHeight*dstScLnSz);
      HDC sourceDC = CreateCompatibleDC(m_hTargetDC->targetDC());
      if (sourceDC)
      {
        SelectObject(sourceDC, hBmp);
      ...
      {
        // Probably we're inside metafile (#7066)
        m_hTargetDC->setDIBitsToDevice( boundRectX, boundRectY, boundRectWidth, boundRectHeight,
          0, 0, 0, boundRectHeight, refl, pBoundRectH, DIB_RGB_COLORS);
      }
#endif
      DeleteObject(hBmp);
      m_hTargetDC->deleteMirrorObject( ExGiGDIDC::kDIBSectionBitmap );
    }
  }
...
//    else
    ...
    if(hEmf)
    {
      ...
      m_hTargetDC->fillRect( &rect, ::GetSysColorBrush(COLOR_WINDOW) );
      m_hTargetDC->playEnhMetaFile( hEmf, (LPRECT)&rect);
      popViewportClip();
...
static bool drawOwnerDrawDcThroughMemDc(const OdGiSelfGdiDrawable* pDrawable,
                                        ...
                                        ExGiGDIDC* hTargetDc, const OdGsDCRect &rect)
{
  RECT clipRect;
  ...
  HDC hMemDc = ::CreateCompatibleDC(hTargetDc->targetDC());
  ...
  if (clipWidth > limit || clipHeight > limit)
  {
    ...
    while (clipStepY < clipHeight)
    {
      ...
      while (clipStepX < clipWidth)
      {
        ...
        {
          POINT origR;
          ::GetViewportOrgEx( hTargetDc->memoryDC(), &origR );
          ::SetViewportOrgEx( hTargetDc->memoryDC(), -actRect.left, -actRect.top, NULL );
          pDrawable->draw( context, (void*)hTargetDc->memoryDC(), rect );
          ::SetViewportOrgEx( hTargetDc->memoryDC(), origR.x, origR.y, NULL );
        }
        hTargetDc->bitBlt( actRect.left, actRect.top, actRect.right - actRect.left, actRect.bottom - actRect.top,
                 hMemDc, actRect.left, actRect.top, SRCCOPY);
        hTargetDc->moveToEx( actRect.left, actRect.top, NULL);
        hTargetDc->lineTo( actRect.right, actRect.bottom);
        clipStepX += limit;
      }
      clipStepY += limit;
    }
  ...
  else
  {
    ...
    {
      HBITMAP hMem = ::CreateCompatibleBitmap(hTargetDc->targetDC(), clipWidth, clipHeight);
      HGDIOBJ hPrevBmp = ::SelectObject(hMemDc, (HGDIOBJ)hMem);
      ...
      if( bReDrawInMem )
      {
        POINT origR;
        ::GetViewportOrgEx( hTargetDc->memoryDC(), &origR );
        ::SetViewportOrgEx( hTargetDc->memoryDC(), -clipRect.left, -clipRect.top, NULL );
        pDrawable->draw( context, (void*)hTargetDc->memoryDC(), rect );
        ::SetViewportOrgEx( hTargetDc->memoryDC(), origR.x, origR.y, NULL );
      }
      hTargetDc->bitBlt( clipRect.left, clipRect.top, clipWidth, clipHeight,
    ...
    else
    {
      ...
      {
        HBITMAP hMem = ::CreateCompatibleBitmap(hTargetDc->targetDC(), repWidth, repHeight);
        HGDIOBJ hPrevBmp = ::SelectObject(hMemDc, (HGDIOBJ)hMem);
        ...
        {
          ::GetViewportOrgEx( hTargetDc->memoryDC(), &origR );
          ::SetViewportOrgEx( hTargetDc->memoryDC(), -crpLeft, -crpTop, NULL );
        }
        ...
        if( bReDrawInMem )
        {
          pDrawable->draw( context, (void*)hTargetDc->memoryDC(), repRect );
          ::SetViewportOrgEx( hTargetDc->memoryDC(), origR.x, origR.y, NULL );
        }
        hTargetDc->stretchBlt( clipRect.left, clipRect.top, clipWidth, clipHeight,
                                  ...
                                  bool /*bAllowClipping*/)
{
  ...
  if (m_hTargetDC)
  {
    ...
    {
      pDrawable->draw( context, (void*)m_hTargetDC->targetDC(), rect );
      if( m_hTargetDC->isMirrorMode() )
      {
        pDrawable->draw( context, (void*)m_hTargetDC->memoryDC(), rect );
      }
    }
    popViewportClip();
  }
...
void ExGiGDIGeometry::draw_color(COLORREF rgb)
{
  ...
  if (m_colorConverter.colorConverterCallback())
  {
    if (m_colorConverter.colorConverterCallback()->convertBackgroundColors() ||
        !GETBIT(drawContext()->effectiveTraits().drawFlags(), OdGiSubEntityTraits::kDrawNoPlotstyle))
      rgb = m_colorConverter.convert(rgb);
  }
  if(m_penBrush.lbColor != rgb)
  {
    ...
    if(m_hTargetDC) // in some cases m_hTargetDC is not set yet when this function called
      m_hTargetDC->setTextColor( rgb);
  }
}

void ExGiGDIGeometry::draw_transparency( OdCmTransparency transp )
{
  if( m_hTargetDC == NULL ) return;
  OdUInt8 alpha = transp.alpha();
  m_hTargetDC->setAlpha( alpha, m_penBrush.lbColor );
}
...
void ExGiGDIGeometry::updatePenNBrush(bool bForSelection)
{
  if( !m_bPenValid  )
  {
    if ( m_hTargetDC )
    {
      ...
      if(bForSelection)
      {
        ::DeleteObject( m_hTargetDC->selectObject( ::CreatePen(m_dwPenStyle, m_dwWidth,
          m_penBrush.lbColor)));
      }
      else
      {
        ::DeleteObject( m_hTargetDC->selectObject( createCurrentPen()));
      }
#else
      ::DeleteObject( m_hTargetDC->selectObject(
...
#ifndef _WIN32_WCE
    if ( m_hTargetDC )
      ::DeleteObject( m_hTargetDC->selectObject( ::CreateBrushIndirect(&m_logBrush)));
#endif
    m_bBrushValid = true;
  }
}
...
void ExGiGDIGeometry::renderGradientTriangles( ExGiGDIDC* hdc )
{
#ifndef _WIN32_WCE
  // Draw gradients as bitmap
  // Compute size of gradient
...
//    ::SetWindowOrgEx(hMemDC, 0, 0, NULL);
    if( !m_hTargetDC->isAlphaMode() )
    {
      ...
      if( hdc->isMirrorMode() )
      {
        m_hTargetDC->stretchBitsMemDC( minLim.cx, minLim.cy, bmpSize.cx, bmpSize.cy, pPixelsBuf, bmpSize.cx, BI_RGB, 32, 0, 0, bmpSize.cx, bmpSize.cy, 0 );
      }
      ( (ALPHABLENDPROC)m_alphaBlendPrAddr )( hdc->targetDC(), minLim.cx, minLim.cy, bmpSize.cx, bmpSize.cy, hMemDC, 0, 0, bmpSize.cx, bmpSize.cy, bf );
    }
    else
    {
      m_hTargetDC->alphaStretchBits( minLim.cx, minLim.cy, bmpSize.cx, bmpSize.cy, pPixelsBuf, bmpSize.cx, BI_RGB, 32, 0, 0, bmpSize.cx, bmpSize.cy, 0 );
    }
