...
void GsDIBSection::computeTransparency(const OdUInt8 *pPixelData, ODCOLORREF bkColor)
{ OdUInt8 *pSecData = (OdUInt8*)m_pBits;
  const OdUInt32 nWidth = getDibWidth(), nHeight = getDibHeight(), nScanSize = scanLineSize();
  PixImageDescIn blackBg = { { pSecData + 0, pPixelData + 1, pSecData + 2, pPixelData + 3 }, 4, (OdInt32)nScanSize };
  PixImageDescIn whiteBg = { { pPixelData + 0, pSecData + 1, pPixelData + 2, pSecData + 3 }, 4, (OdInt32)nScanSize };
  PixImageDescOut outImg = { { pSecData + 0, pSecData + 1, pSecData + 2, pSecData + 3 }, 4, (OdInt32)nScanSize };
