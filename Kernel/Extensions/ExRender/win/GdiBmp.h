...
public:
  ...
  {
    OdSmartPtr<GDI_BMP> pRes = OdRxObjectImpl<GDI_BMP>::createObject();
    pRes->m_infoHeader = m_infoHeader;

    //m_pBits points to m_hDC, which will be destroyed when drawWithGdiBmp will end
    //so we need to copy data into buffer and remap m_pBits pointer to it
    OdUInt32 srcScanLineSz = scanLineSize()*pixelHeight();
    pRes->m_bits.resize(srcScanLineSz);
    scanLines(pRes->m_bits.asArrayPtr(), 0, pixelHeight());
    pRes->m_pBits = (OdUInt8*)pRes->m_bits.asArrayPtr();
