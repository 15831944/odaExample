...
public:  
  ...
  */
  ...
  /** \details
    Compute transparency onto generated pixel data.
  */
  void computeTransparency(const OdUInt8 *pPixelData, ODCOLORREF bkColor);
...
public:
  ...
  */
  ...
  { const ODCOLORREF bkColor = T::getBackgroundColor();
    const bool bTransparentBackground = (T::m_nColorDepth == 32) && (ODGETALPHA(bkColor) < 255);
    m_dibSection.createDib(T::width(), T::height(), T::m_nColorDepth);
    if (bTransparentBackground)
      T::setBackgroundColor(ODRGBA(255, 0, 255, 255));
    T::update(pUpdatedRect);
    if (bTransparentBackground)
    { T::setBackgroundColor(ODRGBA(0, 255, 0, 255));
      if (m_trpData.size() != (m_dibSection.getDibHeight() * m_dibSection.scanLineSize()))
        m_trpData.resize(m_dibSection.getDibHeight() * m_dibSection.scanLineSize());
      ::memcpy(m_trpData.asArrayPtr(), m_dibSection.scanLines(), m_trpData.size());
      T::update(pUpdatedRect);
      m_dibSection.computeTransparency(m_trpData.getPtr(), bkColor);
      T::setBackgroundColor(bkColor);
    }
  }
