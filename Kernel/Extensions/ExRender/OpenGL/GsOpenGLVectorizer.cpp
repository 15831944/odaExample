...
void OdGsOpenGLVectorizeDevice::setGLViewport(long int xOffset, long int yOffset, long int width, long int height)
{ long int hOffset = windowHeight() - (vOffset() + this->height());
  ::glViewport(xOffset, yOffset - vOffset() + hOffset, width, height);
}

void OdGsOpenGLVectorizeDevice::clear()
{
  // OpenGL Normalized device coordinates: lower-left(-1,-1):upper-right(1,1)
  setGLViewport(hOffset(), vOffset(), width(), height());
  ...
  if(supportPartialUpdate() && n)
  {
    ...
    */
    ...
    do
    {
      ...
      {
        static_cast<GLfloat>(pInvRect->m_min.x), static_cast<GLfloat>(pInvRect->m_min.y),
        static_cast<GLfloat>(pInvRect->m_min.x), static_cast<GLfloat>(pInvRect->m_max.y),
        static_cast<GLfloat>(pInvRect->m_max.x), static_cast<GLfloat>(pInvRect->m_max.y),
        static_cast<GLfloat>(pInvRect->m_max.x), static_cast<GLfloat>(pInvRect->m_min.y)
      };
...
const char *OdGsOpenGLVectorizeDevice::getExtensionNameFromList(OdGsOpenGLVectorizeDevice::OpenGLExtension extIdx) const
{
  ...
  */
  switch (extIdx)
  {
    ...
    case OpenGLExtension_ARB_WinPos:
...
void *OdGsOpenGLVectorizeDevice::getExtensionFuncPtrFromList(OpenGLExtFunc funcIdx) const
{
  ...
  switch (funcIdx)
  {
    ...
    break;
    case OpenGLExtFunc_ARB_WindowPos2i:
      pName = "glWindowPos2iARB";
    break;
#if defined(ODA_WINDOWS)
...
#define appendDynamicLwLinesMacro \
  ...
  inline void play(OdGsOpenGLVectorizeView *pOGLView, bool bHighlighted, OdOpenGLCheckMarkIface *pCheckMark)
  {
    ...
    for (OdUInt32 ip = 0; ip < m_primCache.size(); ip++)
    {
      ...
      if (pCheckMark)
      {
        OdUInt8 nStates = pCheckMark->metafileCheckMarker((OdOpenGLGsMarker)pce.m_gsMark, bHighlighted);
        bHighlighted = GETBIT( nStates, OdOpenGLCheckMarkIface::kHighlightStateChanged );
...
void OdGsOpenGLVectorizeView::mesh(OdInt32 numRows,
                                   OdInt32 numColumns,
                                   const OdGePoint3d* vertexList,
                                   ...
                                   const OdGiVertexData* pVertexData)
{
  ...
  if ((view().mode() >= kFlatShaded) && ::odExDiscardBackfaces(*this, device()->discardBackFaces()) &&
      ...
      (forceAlternativeHlt() || (!currentHighlightBranch() || device()->discardBackFaces())) &&
#endif
      ((pFaceData && pFaceData->normals()) || (pVertexData && pVertexData->normals())) && !isOutputSuppressed())
  {
    if (m_pCurMetafile == NULL)
      onTraitsModified(); // AMark fix #3736: Support for discardBackFaces in OpenGL GS is broken
    // ::glLightModeli(GL_LIGHT_MODEL_TWO_SIDE, GL_FALSE);
...
const OdGeMatrix3d &OdGsOpenGLVectorizeView::metafileTransform() const
{
...
//#endif
    ...
    if (bFlag)
    {
      ...
      const double coef = (m_nCurFading = (OdUInt8)fadingIntensity()) * 0.01; // [0-1]
      ...
      ::glFogf(GL_FOG_START, odmax(odmin((GLfloat)fogNear, 1e20f), -1e20f));
      ::glFogf(GL_FOG_END,   odmin(odmax((GLfloat)fogFar, -1e20f),  1e20f));
...
bool clip1d(const double offset, double& start, double& len, const double& clip_len, double& scale, double& trans)
{
  double c0 = offset;
  double c1 = offset + clip_len;
  ODA_ASSERT(c0 < c1);
  double x0;
  double x1;
  if(len > 0.)
...
void OdGsOpenGLVectorizeView::loadViewport()
{
  ...
  */
  ...
  const double windowHOffset = device()->hOffset(), windowVOffset = device()->vOffset();
  const double windowWidth = device()->width(), windowHeight = device()->height();

  double sx, tx, sy, ty;
  bool bFrontFaceCW = false;
  if(clip1d(windowHOffset, left,   width,  windowWidth, sx, tx) && clip1d(windowVOffset, bottom, height, windowHeight, sy, ty))
  {
    if(sx < 0. || sy < 0.)
    {
      if(sx < 0.)
  ...
  {
    // screen to GL:
    device()->setGLViewport(ROUND(windowHOffset), ROUND(windowVOffset), ROUND(windowWidth), ROUND(windowHeight));

    // #17083 : inversion will be included in screenMatrix, so we must skip it here
    x(0, 0) = windowWidth;
    x(1, 1) = -windowHeight;
    x(0, 3) = windowHOffset;
    x(1, 3) = windowVOffset + windowHeight;
    // eof #17083
    x(2,2) = 2.0; // Z-mapping
    x.invert();
    ...
    if(n)
    {
      ...
      for( i = 0; i < n; ++i )
      {
        ...
        {
          static_cast<GLfloat>(pInvRect->m_min.x), static_cast<GLfloat>(pInvRect->m_min.y), 1.9f,
          static_cast<GLfloat>(pInvRect->m_min.x), static_cast<GLfloat>(pInvRect->m_max.y), 1.9f,
          static_cast<GLfloat>(pInvRect->m_max.x), static_cast<GLfloat>(pInvRect->m_max.y), 1.9f,
          static_cast<GLfloat>(pInvRect->m_max.x), static_cast<GLfloat>(pInvRect->m_min.y), 1.9f
        };
      ...
      if(m) // combine stencil and dept buffers to stencil buffer
      {
        ...
        const GLfloat fltVerts[4 * 3] =
        {
            static_cast<GLfloat>(lowerLeft.x), static_cast<GLfloat>(lowerLeft.y), 0.0f,
            static_cast<GLfloat>(lowerLeft.x), static_cast<GLfloat>(upperRight.y), 0.0f,
            static_cast<GLfloat>(upperRight.x), static_cast<GLfloat>(upperRight.y), 0.0f,
            static_cast<GLfloat>(upperRight.x), static_cast<GLfloat>(lowerLeft.y), 0.0f
        };
        ::oglEsDrawBuffer(3, fltVerts, GL_TRIANGLE_FAN, 4);
#endif
      }
  ...
  device()->setGLViewport(ROUND(left), ROUND(bottom), ROUND(width), ROUND(height));
                                         ...
                                         const OdGiVertexData* pVertexData)
{
  ...
  if(pVertexData)
  {
    ...
    if(pColors)
    {
      ...
      if (m_pColorConverter && (m_bConvertBackgroundColor || !m_bNoColorMod))
      { OdColorConverterCallbackCaching csh(m_pColorConverter);
        for (int i = 0; i < nbVertex; ++i)
        { const ODCOLORREF resColor(csh.convert(ODTOCOLORREF((*pColors))));
          m_colors.append(ODGETRED(resColor));
          m_colors.append(ODGETGREEN(resColor));
          m_colors.append(ODGETBLUE(resColor));
          if (clrNComponents == 4)
                                                 ...
                                                 const OdGiEdgeData* pEdgeData, const OdGiFaceData* pFaceData)
{
      ...
      !(vertexData() && vertexData()->trueColors()) )
  {
    if (hasPlineStarted())
    {
      if (effectiveTraits().transparency().alpha() == 0xFF)
...
void OdGsOpenGLVectorizeView::generateThinWires(bool bPline, OdInt32 faceListSize, const OdInt32 *pFaceList,
                                                const OdGiEdgeData *pEdgeData, const OdGiFaceData *pFaceData)
{ OdDb::LineWeight lw = lineWeight();
  ...
  if (bPline)
    OdGiPlineContourCalc<OdGsBaseMaterialViewMT>::dropPlineCalcShellWires(plineCalcStripsEnabled());
  else
...
void OdGsOpenGLVectorizeView::dropPlineCalcShellWires(bool /*bStrips*/)
{
                                            ...
                                            double fade)
{
  ...
  else
  {
    ...
    if (!GETBIT(giContext().imageSelectionBehavior(), OdGiContext::kImageHighlight))
      highlight(false);
    OdGsBaseVectorizer::rasterImageDc(origin, u, v, pImg, uvBoundary, 
      numBoundPts, transparency, brightness, contrast, fade);
  }
                                          ...
                                          double fade)
{
  ...
  {
#ifdef GL_EXT_texture_compression_s3tc
    if ((srcColDepth == 1) && device()->isExtensionSupported(OdGsOpenGLVectorizeDevice::OpenGLExtension_EXT_Dxt))
    {
    ...
    if (srcColDepth > 1 && transparency && pImage->transparentColor() < 0)
      internalformat = format = GL_RGB; // Fix problem with invalid color format
  }
  ...
  if (!isTextureLoaded(pImage, transparency, traitsCol, bSecColor, secCol, brightness, contrast, fade, pScanLines, bufSize, pPalData, lutSize))
  {
    ...
    else if(srcColDepth <= 8 && transparency && pImage->transparentColor() >= 0)
    {
      bTransparentBitonal = true;
    }
#ifdef GL_ARB_fragment_program
  ...
  {
    bTransparentBitonal = (srcColDepth <= 8 ) && transparency && ((srcColDepth == 1) || (pImage->transparentColor() >= 0));
  ...
  OdGeMatrix3d xWorld2UV;
  OdGeVector3d imgNormal = u.crossProduct(v);
  if (!OdZero(imgNormal.normalizeGetLength(), 1.e-300))
    xWorld2UV.setCoordSystem(origin, u, v, imgNormal).invert();
...
void OdGsOpenGLVectorizeView::drawViewportFrame()
{
  if (isViewportBorderVisible())
  {
    ...
    device()->setGLViewport(baseDevice()->hOffset(), baseDevice()->vOffset(), baseDevice()->width(), baseDevice()->height());
    ::glDisable(GL_LIGHTING);
    ::glDisable(GL_TEXTURE_2D);
    ::glDisable(GL_LINE_STIPPLE);
#ifndef OD_OGL_ALTERNATIVEHLT
    ...
    OdGeMatrix3d x;
    OdGeMatrix3d vptMtx;
    { const OdGsDCRect &dcrc(baseDevice()->outputRect());
      x(0, 0) = double(dcrc.m_max.x - dcrc.m_min.x);
      x(1, 1) = double(dcrc.m_max.y - dcrc.m_min.y);
      x(0, 3) = double(dcrc.m_min.x);
      x(1, 3) = double(dcrc.m_min.y);
      x.invert();
