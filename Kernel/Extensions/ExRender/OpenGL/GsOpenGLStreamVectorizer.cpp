      ...
      case kMfDisplay:
        {
          ...
          pOGLView->m_metaRead.setHighlightingPass(pOGLView->highlightingPass());
                                          ...
                                          const OdGiVertexData* vertexData)
{
  ...
  if (m_metaWrite.isActive())
  {
    ...
    if (m_bBackfacesDiscarded) m_pModelToEyeProc->initGeometryClipStatus();
    OdGsBaseVectorizer::shell(nVertices, vertexList, faceListSize, faceList, edgeData, faceData, vertexData);
    if (m_bBackfacesDiscarded && (m_pModelToEyeProc->getGeometryClipStatus() == 1))
      m_bShmClipped = true;
...
void OdGsOpenGLStreamVectorizeView::mesh(OdInt32 numRows,
                                         OdInt32 numColumns,
                                         const OdGePoint3d* vertexList,
                                         ...
                                         const OdGiVertexData* pVertexData)
{
  ...
  if (m_metaWrite.isActive())
  {
    ...
    OdGsOpenGLVectorizeView::mesh(numRows, numColumns, vertexList, pEdgeData, pFaceData, pVertexData);
}
                                                  ...
                                                  OdInt32 nPointSize)
{
  ...
  {
    const bool bMarkers = pSubEntMarkers && m_bDynamicSubEntHlt && !(isNestedPersistent(drawableDesc()) && !device()->supportBlocks());
...
void OdGsOpenGLStreamVectorizeView::onTraitsModified()
{
  ...
  if (m_bDynamicSubEntHlt && m_metaWrite.isActive())
  {
    ...
    if (gsWriter().gsModel())
    {
      ...
      {
        if (isNestedPersistent(drawableDesc()) && !device()->supportBlocks())
...
{
  if( (currentHighlightBranch() != NULL) && !currentHighlightBranch()->markersEmpty() ) return true;
  return (currentVisibilityBranch() != NULL) && !currentVisibilityBranch()->markersEmpty();
}

OdUInt8 OdGsOpenGLStreamVectorizeView::metafileCheckMarker(OdOpenGLGsMarker gsMarker, bool bPrevState)
{
  OdUInt8 res = bPrevState ? 1 : 0;
  const OdGsStateBranch* pVisBranch = currentVisibilityBranch();
  if( pVisBranch && !pVisBranch->markersEmpty() )
  {
    if(OdGsStateBranch::isValidMarker((OdGsMarker)gsMarker))
      SETBIT( res, OdOpenGLMetafileReaderCallback::kVisibilityStateChanged, pVisBranch->hasMarker( (OdGsMarker)gsMarker ) );
    else 
      SETBIT( res, OdOpenGLMetafileReaderCallback::kVisibilityStateChanged, false );
  }
  const OdGsStateBranch* pBranch = currentHighlightBranch();
  if (pBranch && !pBranch->markersEmpty())
  {
    if (OdGsStateBranch::isValidMarker((OdGsMarker)gsMarker))
      SETBIT( res, OdOpenGLMetafileReaderCallback::kHighlightStateChanged, pBranch->hasMarker( (OdGsMarker)gsMarker ) );
    else 
      SETBIT( res, OdOpenGLMetafileReaderCallback::kHighlightStateChanged, false );
  }
  return res;
}
...
void OdGsOpenGLStreamVectorizeView::beginMetafile(OdRxObject* pMetafile)
{
  ...
  if (!m_pCurMetafile)
  {
    ...
    if (isHighlighted() && (!currentHighlightBranch() || currentHighlightBranch()->markersEmpty()))
...
void OdGsOpenGLStreamVectorizeView::endMetafile(OdRxObject* pMetafile)
{
  ...
  OdOpenGLStreamList *pMf = static_cast<OdOpenGLStreamList *>(pMetafile);
  pMf->end();
  OdGsBaseMaterialVectorizer::endMetafile(pMetafile);
  if (!validateMetafileExtents())
    pMf->clear();
  m_nMetafileSize = pMf->size();
  if( device()->updateManager() )
  {
    for( OdUInt32 i = 0; i < pMf->arrayElementsSize(); ++i )
    {
      m_nMetafileSize += pMf->arrayElement( i ).m_uSize;
    }
  }
  if (m_bShmClipped && m_nMetafileSize)
    updateShmClipping(pMf);
}

void OdGsOpenGLStreamVectorizeView::updateShmClipping(OdRxObject *pMf)
{
  const OdUInt32 chunkSize[OdOpenGLMetaRecType_NTypes] =
  {
    0 /* OdOpenGLMetaRecType_Empty */, 1 /* OdOpenGLMetaRecType_EnableOpt */, 1 /* OdOpenGLMetaRecType_DisableOpt */,
    4 /* OdOpenGLMetaRecType_Color */, sizeof(OdUInt32) + 1 /* OdOpenGLMetaRecType_EnableArray */,
    1 /* OdOpenGLMetaRecType_DisableArray */, (sizeof(GLint) << 1) + 1 /* OdOpenGLMetaRecType_DrawArrays */,
    sizeof(GLint) + 1 + sizeof(OdUInt32) /* OdOpenGLMetaRecType_DrawElements */, 1 /* OdOpenGLMetaRecType_CullFace */,
    sizeof(OdUInt8) * 2 /* OdOpenGLMetaRecType_LStipple */, sizeof(OdUInt8) * 2 /* OdOpenGLMetaRecType_PStipple */,
#ifndef OD_OGL_ENABLEFLOATCONVERSION
    sizeof(OdGePoint3d) /* OdOpenGLMetaRecType_VPoint */, sizeof(OdGePoint3d) << 1 /* OdOpenGLMetaRecType_VLine */,
#else
    (sizeof(GLfloat) << 1) + sizeof(GLfloat) /* OdOpenGLMetaRecType_VPoint */,
    (sizeof(GLfloat) << 2) + (sizeof(GLfloat) << 1) /* OdOpenGLMetaRecType_VLine */,
#endif
    sizeof(OdInt32) /* OdOpenGLMetaRecType_IPoint */, sizeof(OdInt32) * 2 /* OdOpenGLMetaRecType_ILine */,
    1 /* OdOpenGLMetaRecType_HLRStencil */, 1 /* OdOpenGLMetaRecType_EnableShading */, 1 /* OdOpenGLMetaRecType_DisableShading */,
    sizeof(OdUInt32) /* OdOpenGLMetaRecType_Material */, 0 /* OdOpenGLMetaRecType_UserEntry */,
    sizeof(OdOpenGLMetafileReaderCallback::RasterImageEntry) + sizeof(OdUInt32) + sizeof(bool) /* OdOpenGLMetaRecType_InitTexture */,
    0 /* OdOpenGLMetaRecType_UninitTexture */, sizeof(OdOpenGLGsMarker) /* OdOpenGLMetaRecType_SelectionMarker */,
    sizeof(OdUInt32) /* OdOpenGLMetaRecType_EnableMarkerArray */, 0 /* OdOpenGLMetaRecType_DisableMarkerArray */,
    1 /* OdOpenGLMetaRecType_SelectionFlags */, sizeof(OdUInt8) + sizeof(OdInt16) /* OdOpenGLMetaRecType_Lineweight */,
    sizeof(OdUInt8) * 3 /* OdOpenGLMetaRecType_Linestyle */, 0 /* OdOpenGLMetaRecType_TtfText */
  };
  OdOpenGLFlatMetafileContainer *pDataCont = static_cast<OdOpenGLStreamList *>(pMf);
  OdUInt8 *pMemPtr = const_cast<OdUInt8*>(pDataCont->memoryPtr());
  const OdUInt8 *pMemPtrReadFor = pMemPtr + pDataCont->size();
  while (pMemPtr < pMemPtrReadFor)
  {
    ...
    switch (recType)
    {
      case OdOpenGLMetaRecType_EnableOpt:
        if (*pMemPtr == OdOpenGLAttribute_CullFace)
          *(pMemPtr - 1) = (OdUInt8)OdOpenGLMetaRecType_DisableOpt;
        OD_OGL_RDR_SEEKINC(pMemPtr);
      break;
      ...
      case OdOpenGLMetaRecType_UserEntry:
        if (m_metaRead.callback())
        { const OdUInt8 *pMemPtr2 = pMemPtr;
          m_metaRead.callback()->metafileProcessUserEntry(pDataCont, pMemPtr2);
          pMemPtr += pMemPtr2 - pMemPtr;
        }
      break;
      ...
      break;
      default:
        OD_OGL_RDR_SEEKFWD(pMemPtr, chunkSize[recType]);
    }
  }
}
...
void OdGsOpenGLStreamVectorizeView::beginViewVectorization()
{
  ...
  m_metaRead.enableLinesAntiAliasing(GETBIT(giContext().antiAliasingMode(), 1) && GETBIT(view().antiAliasingMode(), 1) && (view().mode() == k2DOptimized));
