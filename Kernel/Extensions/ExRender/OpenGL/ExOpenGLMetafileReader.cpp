...
void OdOpenGLMetafileReader::PrevState::setDefaults()
{
  ...
  { // #9643 optimization (loop was opened, some properties doesn't require direct initialization)
    ...
    bSuppressHighlighted = bSuppressUnhighlighted = false;
  }
  GLint glInt;
...
OdUInt8 *OdOpenGLMetafileReader::PrevState::ColorConverter::convertColorsArray(const void *pInput, OdUInt32 nSize)
{
  if (m_colorArray.size() < nSize)
    m_colorArray.resize(nSize);
  OdUInt8 *pOutColorData = m_colorArray.asArrayPtr();
  const OdUInt8 *pInColorData = (const OdUInt8 *)pInput;
  for (OdUInt32 nColor = 0; nColor < nSize; nColor += OD_OGL_NCOLORCOMPONENTS)
  { const ODCOLORREF outColor = convert(ODRGB(pInColorData[nColor], pInColorData[nColor + 1], pInColorData[nColor + 2]));
    pOutColorData[nColor] = ODGETRED(outColor); pOutColorData[nColor + 1] = ODGETGREEN(outColor); pOutColorData[nColor + 2] = ODGETBLUE(outColor);
#if (OD_OGL_NCOLORCOMPONENTS == 4)
    pOutColorData[nColor + 3] = pInColorData[nColor + 3];
#endif
  }
  return pOutColorData;
}
...
void OdOpenGLMetafileReader::play(const OdBaseMetafileContainer *pContainer, bool bHighlighted, bool bCheckMark)
{
  ...
  SETBIT(inprocCtx.bFlags, kSuppressHlted, prevState.suppressHighlighted(bHighlighted));
  ...
  while (pMemPtr < pMemPtrReadFor)
  {
    ...
    switch (recType)
    {
      ...
      case OdOpenGLMetaRecType_Color:
        if (prevState.m_colorConverter.colorConverterCallback() && (prevState.m_bProcessBackgroundColors || !prevState.bNoColorModState))
        {
          const ODCOLORREF color = prevState.m_colorConverter.convert(ODRGBA(pMemPtr[0], pMemPtr[1], pMemPtr[2], pMemPtr[3]));
      ...
      case OdOpenGLMetaRecType_EnableArray:
        {
          ...
          const void *pData = pDataCont->m_ArrayElements.getPtr()[nArray].m_pData;
          if ((type == OdOpenGLMetaRecArrayType_Color) && prevState.m_colorConverter.colorConverterCallback() &&
              (prevState.m_bProcessBackgroundColors || !prevState.bNoColorModState))
            pData = prevState.m_colorConverter.convertColorsArray(pData, pDataCont->m_ArrayElements.getPtr()[nArray].m_uSize);
          gGLArrayProto[type](gGLArrayNVals[type], gGLArrayType[type],
                              strideSize, /*pMemPtr + sizeof(OdUInt32)*/ pData);
      ...
      case OdOpenGLMetaRecType_DrawArrays:
        {
          ...
          if (!inprocCtx.pMarks)
          {
            if( !GETBIT(inprocCtx.bFlags, kHiddenGeometry | kSuppressHlted) )
            {
...
#define OGL_CHECK_MARK(getMarker) \
          OdUInt8 newState = m_pCallback->metafileCheckMarker(getMarker, bHighlighted); \
          if (GETBIT( newState, OdOpenGLMetafileReaderCallback::kHighlightStateChanged ) != bHighlighted) \
          { \
            bHighlighted = GETBIT( newState, OdOpenGLMetafileReaderCallback::kHighlightStateChanged ); \
            SETBIT(inprocCtx.bFlags, kSuppressHlted, prevState.suppressHighlighted(bHighlighted)); \
          ...
          SETBIT(inprocCtx.bFlags, kHiddenGeometry, GETBIT( newState, OdOpenGLMetafileReaderCallback::kVisibilityStateChanged ) );
            ...
            do
            {
              if (inprocCtx.nMarks)
                ...
                ctDiff = count - first;
              if (ctDiff && !GETBIT(inprocCtx.bFlags, kHiddenGeometry | kSuppressHlted))
              {
                if (!GETBIT(inprocCtx.bFlags, kLineweightProc | kLinesAntiAlias) || !m_pCallback)
...
#undef OD_GL_INDEX_TYPE
      ...
      case OdOpenGLMetaRecType_VPoint:
        if (!GETBIT(inprocCtx.bFlags, kSuppressGeom | kHiddenGeometry | kSuppressHlted))
        {
      ...
      case OdOpenGLMetaRecType_InitTexture:
        {
          ...
          if (m_pCallback)
          {
            ...
            if (bHighlighted && !prevState.bForceAlternativeHlt && !prevState.bImageContentsHlt)
            {
              m_pCallback->metafileFillStipple(OdOpenGLFillPatt::kFillPattSel, 0);
              SETBIT(inprocCtx.bFlags, kSuppressHlted, prevState.suppressHighlighted(false));
            }
#endif
          }
        }
      ...
      case OdOpenGLMetaRecType_UninitTexture:
        if (m_pCallback)
        {
          ...
          {
            m_pCallback->metafileFillStipple(OdOpenGLFillPatt::kFillPattSel, 1);
            SETBIT(inprocCtx.bFlags, kSuppressHlted, prevState.suppressHighlighted(true));
          }
#endif
        }
      break;
      ...
      case OdOpenGLMetaRecType_TtfText:
        {
          if (m_pCallback && !GETBIT(inprocCtx.bFlags, kSuppressGeom | kHiddenGeometry | kSuppressHlted))
          {
...
void OdOpenGLMetafileReader::drawArrayLwd(GLenum mode, GLint first, GLsizei count, bool bHighlighted, const GLvoid *pArray)
{
  ...
  else
  {
    ...
    if (((bHighlighted || lnStpl) && bIsLine) && !prevState.bForceAlternativeHlt)
    {
      ...
      if (prevState.m_colorConverter.colorConverterCallback() && prevState.m_bProcessBackgroundColors)
        bkgndColor = prevState.m_colorConverter.convert(bkgndColor);
      GLfloat curColor[4];
