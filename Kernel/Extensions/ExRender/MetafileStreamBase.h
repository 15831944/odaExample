...
class OdFlatMetafileContainer : public OdBaseMetafileContainer
{
  ...
  protected:
    OdUInt8 *m_pData;
    OdUInt32 m_uLength;
    OdUInt32 m_revPartLow; // Low-data reserved part
  public:
    OdFlatMetafileContainer() : m_pData(NULL), m_uLength(0), m_revPartLow(0) { }
    ...
    struct Gap
    { OdUInt32 m_from, m_to;
      Gap *m_pNextGap;
      Gap() : m_from(0), m_to(0), m_pNextGap(NULL) { }
    };
    virtual OdFlatMetafileContainer *createMe() const { return new OdFlatMetafileContainer; }
    // Clone stream data with gaps elimination
    virtual OdFlatMetafileContainer *clone(const Gap *pGaps = NULL) const;
...
inline OdFlatMetafileContainer *OdFlatMetafileContainer::clone(const Gap *pGaps) const
{ OdFlatMetafileContainer *pClone = createMe();
  const Gap defGap; if (!pGaps) pGaps = &defGap;
  OdUInt32 nGapsSpace = pGaps->m_to - pGaps->m_from;
  if (pGaps->m_pNextGap)
  {
    const Gap *pGap = pGaps->m_pNextGap;
    do
    { nGapsSpace += pGap->m_to - pGap->m_from;
      pGap = pGap->m_pNextGap;
    } while (pGap);
  }
  const OdUInt32 dataSize = size();
  OdFlatMetafileContainerWriter writer(pClone, dataSize - nGapsSpace, 1);
  const Gap *pGap = pGaps;
  OdUInt32 nCurPos = 0;
  do // Gaps is sorted anyway
  { const OdUInt32 nEndPos = (pGap) ? pGap->m_from : dataSize;
    if (nEndPos > nCurPos)
      writer.write(m_pData + nCurPos, nEndPos - nCurPos);
    nCurPos = (pGap) ? pGap->m_to : dataSize;
    if (pGap) pGap = pGap->m_pNextGap;
  } while (nCurPos < dataSize);
  return pClone;
}
