  ...
  , protected OdDrawOrderBaseDeviceMCCallback
{
  ...
  public:
    ...
    virtual void onMetafileSelectionStyleChanged(OdRxObject *pMetafile, OdUInt32 nStyle) { }
  ...
  , protected OdGsBaseModelReactor
{
  protected:
    enum DrawablesFlags
    {
      kStateModified   = (1 << 0), // display list doesn't changed, required only state (highlight, visibility) check
    ...
    struct ModelRef { OdUInt32 nRefs; bool bHighlighted; bool bHidden; ModelRef() : nRefs( 0 ), bHighlighted( false ), bHidden( false ) { } };
  ...
  public:
    ...
    virtual bool onHide(OdGsModel *pModel, const OdGiPathNode& path, const OdGsMarker* pMarkers, OdUInt32 nMarkers, bool& bHidden,
  ...
  protected:
    virtual void onGsModelLinkAdded(OdGsModel *pModel);
    virtual void onGsModelLinkErased(OdGsModel *pModel);
    ...
    public:
      MapBasedComparer();
      ~MapBasedComparer();
      void setOriginal( const OdUInt64* pList, OdUInt32 size, OdUInt32 offset = 0 );
...
struct OdDrawOrderMetafileBase : public OdRxObject
{
  enum MetafileFlagsBase
  {
    ...
    kLastFlag = (kIsArray << 1)
  };

  OdIntPtr m_pPtr;
  OdUInt16 m_flags;
  OdUInt16 m_reserved; // 2byte padding which could be useful somewhere
  OdUInt32 m_refIndex;

  void setIsArray(bool bSet) { SETBIT(m_flags, kIsArray, bSet); }
  bool isArray() const { return GETBIT(m_flags, kIsArray); }

  void setRefIndex(OdUInt32 refIndex) { m_refIndex = refIndex; }
  OdUInt32 refIndex() const { return m_refIndex; }
  bool hasRefIndex() const { return m_refIndex != 0; }
  ...
  void clear() { m_pPtr = 0; m_flags = m_reserved = 0; m_refIndex = 0; }
};
...
struct OdDrawOrderBaseMetafile : public OdDrawOrderMetafileBase
{
  enum MetafileFlags
  {
    kFilled      = (OdDrawOrderMetafileBase::kLastFlag << 0), // Marks completed metafile (endMetafile called)
    ...
    kEmpty       = (OdDrawOrderMetafileBase::kLastFlag << 5), // Marks metafiles which doesn't contain any visible geometry
  ...
  bool isEmpty() const { return GETBIT(m_flags, kEmpty); }
  void setEmpty() { SETBIT_1(m_flags, kEmpty); }
