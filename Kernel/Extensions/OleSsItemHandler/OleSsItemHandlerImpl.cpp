...
#if defined(OLESS_WIN_GDI_SUPPORT)
...
#if defined(OLESS_WIN_GDI_SUPPORT) && defined(ODA_WINDOWS)
#include "Gi/GiRasterWrappers.h"
#endif
#include "OdCharMapper.h"

//#if defined(_DEBUG) && defined(ODA_WINDOWS) && !defined(_WINRT)
//#define PATH_TO_SAVE_INTERNAL_DATA L"e:/_Oda/data/_ole/-core-17059 Reading problem in embedded OLE images in DWG file/save"
//#define PATH_TO_SAVE_INTERNAL_DATA L"e:/_Oda/data/_ole/-core-17010 OleSsItemHandler can not load OLE entity/save"
//#define PATH_TO_SAVE_INTERNAL_DATA L"e:/_Oda/data/_ole/-core-13785 Can not get OLE"
//#endif

#ifdef PATH_TO_SAVE_INTERNAL_DATA
#include "../Kernel/Extensions/ExRender/TrXml/IO/CloudTools.h"
static void saveIntertnalData(const OdBinaryData& data, const OdString& sPref, const OdString& sSfx)
{ 
  if (!OdFilePathName(PATH_TO_SAVE_INTERNAL_DATA).existsDir())
    return;
  OdString path;
  for (int idx = 0; OdFilePathName(path.format(L"%ls/%ls%04d%ls", 
                                               PATH_TO_SAVE_INTERNAL_DATA, sPref.c_str(), idx, sSfx.c_str())).exists();)
    idx++;
  OdStreamBufPtr io = ::odrxSystemServices()->createFile(path, Oda::kFileWrite, Oda::kShareDenyNo, Oda::kCreateAlways);
  if (io.get())
    io->putBytes(data.asArrayPtr(), data.size());
}
#else
#  define saveIntertnalData(data, sPref, sSfx)
#endif
...
public:
  ...
  void inFileds(OdStreamBufPtr pStream)
  {
    ...
    if (m_isNativePBrush)
    {
      ODA_ASSERT_ONCE(m_lenUnknown10 == 2 || m_lenUnknown10 == 10);
      m_data.resize(len - m_lenUnknown10); // -sizeof(m_unknown10));
    }
    else
      m_data.resize(len);
    ...
    {
      ODA_ASSERT_ONCE(m_lenUnknown10 == 2 || m_lenUnknown10 == 10);
      pStream->getBytes(m_unknown10, m_lenUnknown10); // sizeof(m_unknown10));
    }
  }
  ...
  void outFileds(OdStreamBufPtr pStream)
  {
    ...
    {
      ODA_ASSERT_ONCE(m_lenUnknown10 == 2 || m_lenUnknown10 == 10);
      len += m_lenUnknown10; // += sizeof(m_unknown10);
    }
    ...
    {
      ODA_ASSERT_ONCE(m_lenUnknown10 == 2 || m_lenUnknown10 == 10);
      pStream->putBytes(m_unknown10, m_lenUnknown10); // sizeof(m_unknown10));
    }
  }
};
...
static OdString getExtByContent(OdBinaryData dataImage)
{
  ...
  saveIntertnalData(dataImage, L"preview_", sExt); // PATH_TO_SAVE_INTERNAL_DATA
  return sExt;
}
...
void OdOleSsItemHandlerImpl::setCompoundDocument(OdUInt32 nSize, OdStreamBuf& stream)
{
  ...
  else
  {
    ...
    saveIntertnalData(m_buff, L"storage.", L""); // PATH_TO_SAVE_INTERNAL_DATA
    ...
    if (pRootStorage && (pStorage = getStorage(pRootStorage)) != NULL)
    {
      ...
      if ((pStrm = openStorageStream(pStorage, L"\01CompObj")) != NULL)
      {
        ...
        if (compObj.m_asThird == "StaticDib") // compObj.m_asFirst == "Picture (Device Independent Bitmap)"
          nOffsetInContents = 0;
        if (!compObj.m_asFirst.isEmpty())
        {
          // CORE-15682
          //OdCharArray buf;
          //OdCharMapper::utf8ToUnicode(compObj.m_asFirst.c_str(), compObj.m_asFirst.getLength(), buf);
          //m_sUserType = buf.getPtr(); // strange but it does not work
          m_sUserType = compObj.m_asFirst;
          int pos = m_sUserType.trimLeft().trimRight().reverseFind(' '),
              pos0 = m_sUserType.find(' ');
          if (pos > 0 && pos > pos0)
            m_sUserType = compObj.m_asFirst.left(pos);
        }
      }
      ...
      if ((pStrm = openStorageStream(pStorage, L"\01Ole10Native")) != NULL) // ole_past_bmp.dwg 
      {
        ...
        OdStreamBufPtr pStream = OdFlatMemStream::createNew(data.asArrayPtr(), data.size());
        if (m_type == kEmbedded)
          native.m_lenUnknown10 = 2; // CORE-17010
        native.inFileds(pStream);
        dataImageNative = native.m_data;
        saveIntertnalData(dataImageNative, L"imagenative_", L".bmp"); // PATH_TO_SAVE_INTERNAL_DATA
      }
      ...
      if (pRasSvcs.get())
      {
        try
        {
          if (dataImageNative.size())
            m_pImage = pRasSvcs->loadRasterImage(
              OdFlatMemStream::createNew(dataImageNative.asArrayPtr(), dataImageNative.size()));
          if (m_pImage.isNull() && dataImageContent.size())
            m_pImage = pRasSvcs->loadRasterImage(
              OdFlatMemStream::createNew(dataImageContent.asArrayPtr(), dataImageContent.size()));
          if (m_pImage.isNull() && dataImagePress.size())
            m_pImage = pRasSvcs->loadRasterImage(
              OdFlatMemStream::createNew(dataImagePress.asArrayPtr(), dataImagePress.size()));
        }
        catch (OdError&) // err
        {
          ODA_FAIL_ONCE();
                                  ...
                                  const OdGsDCRect& rect) const
{
  ...
  // draw m_pImage on HDC for Windows // CORE-14810 for OdaMfcApp in Win CGI //see also CPreviewGiRasterImage::OnPaint()

  CDC* pDC = CDC::FromHandle((HDC) hdc);
  CDC& dc = *pDC; // CPaintDC dc(pWnd);
  CRect cliArea;
  cliArea.SetRect(rect.m_min.x, rect.m_max.y, rect.m_max.x, rect.m_min.y); // left, top, right, bottom
...
#endif // of else !defined(OLESS_WIN_GDI_SUPPORT) || !defined(ODA_WINDOWS)
