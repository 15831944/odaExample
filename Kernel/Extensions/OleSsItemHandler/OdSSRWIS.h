...
inline size_t OdSSRWIS_Read(void* in_pv, size_t in_size, size_t in_num, SSRWIS* in_pIS)
{
  ...
  OdUInt32 sz = (OdUInt32)(in_size * in_num);
  if (OdFlatMemStream* pMemStream = OdFlatMemStream::cast(pStream).get())
  {
    OdUInt32 lft = (OdUInt32)pMemStream->left();
    if (lft < sz)
    {
      ::memset(&((OdUInt8*)in_pv)[lft], 0, sz - lft);
      sz = lft; // CORE-13785
    }
  }
  pStream->getBytes(in_pv, sz);
  return in_num;
}
