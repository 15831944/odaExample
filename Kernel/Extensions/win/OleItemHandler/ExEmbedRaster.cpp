...
#if 1
bool exGetEmbedData(const OdOleItemHandler*,
                    OdGiRasterImagePtr*, 
                    OdBinaryData*) // = NULL
{
  return false;
...
#else
// TODO for CORE-13060
bool exGetEmbedData(const OdOleItemHandler* pHandler,
                    OdGiRasterImagePtr* ppImage, 
                    OdBinaryData* pData) // = NULL
{
  return false; // TODO
  ...
  pHandler->getCompoundDocument(*pStream.get());
  OdUInt32 nSize = (OdUInt32) pStream->length();
  ODA_ASSERT_ONCE(nSize);
  if (!nSize)
    return false;
  pStream->rewind();
...
//#endif
  ...
  OdBinaryData data;
  for (pStream->rewind(); !pStream->isEof();)
    data.push_back(pStream->getByte());
  pStream->rewind();
  ODA_ASSERT_ONCE(nSize == OdUInt32(data.size()));
  OdStreamBufPtr io = ::odrxSystemServices()->createFile(sStoragePathName,
                                                         Oda::kFileWrite, Oda::kShareDenyNo, Oda::kCreateAlways);
  ODA_ASSERT_ONCE(io.get());
  if (!io.get())
    return false;
  io->putBytes(data.asArrayPtr(), nSize);

  IStoragePtr pStorage;
  //if (StgCreateDocfile(sStoragePathName.c_str(), STGM_WRITE | STGM_SHARE_EXCLUSIVE | STGM_CREATE, 0, &pStorage) != S_OK)
  if (StgOpenStorage(sStoragePathName.c_str(), 0, STGM_READ | STGM_SHARE_DENY_READ, 0, 0, &pStorage) != S_OK)
  {
    ODA_FAIL_ONCE(); // TODO
    return false;
  }
