...
static int numCPUs(int &nPhysicalCores)
{
  ...
  Glpi = (LPFN_GLPI)::GetProcAddress(
	  GetKernelModule(), "GetLogicalProcessorInformation"
  );
#else
  Glpi = (LPFN_GLPI)::GetProcAddress(
...
class ExceptionHandler
{
  ...
  public:
    ...
    ExceptionType detach()
    {
      if(m_pException)
      {
        std::exception_ptr p = m_pException;
        m_pException = nullptr;
        return p;
      }
...
public:
  ...
  void process()
  {
    if(m_pException)
    {
      std::exception_ptr e = m_pException;
      m_pException = nullptr;
...
class ExceptionHandler
{
  ...
  public:
    ...
    ExceptionType detach()
    {
      ...
      return OdError((OdErrorContext*)NULL);
    }
};
...
{
  protected:
    ExceptionHandler::ExceptionType m_exception;
  public:
    MultiExceptionHandler() : m_exception((OdErrorContext*)NULL) {}
    void add(const ExceptionHandler::ExceptionType& error)
    {
      if (error.context())
        m_exception = error;
    }
    void process()
    {
      if (m_exception.context())
      {
        ExceptionHandler::ExceptionType error = m_exception;
        m_exception.setContext(NULL);
        throw error;
      }
    }
...
public:
  ...
  public:
    ...
    OdTP::ExceptionHandler::ExceptionType detachException() {
      return exception.detach();
    }
  ...
  static OdApcThreadImpl *find( OdApcThreadImplPtrVector& aThreads, unsigned threadId)
  {
    for(unsigned i = 0; i < aThreads.size(); ++i)
    {
      if((*aThreads[i])->getId() == threadId)
        return getPtr(aThreads[i]);
    }
    return NULL;
  }
...
public:
  ...
  protected:
    OdInt32 numEntries, numIncreased;
    OdMutex readyMutex;
    ...
    enum MtQueueRuntimeFlags
    {
      ...
      kWaitUnlockThread  = (kMtQueueLastFlag << 3)  // We wait any single thread unlocking, but not finalize our current queue usage
    };
    ...
    OdTP::MultiExceptionHandler m_exceptions;

  public:
    MTQueue() {
      numEntries = numIncreased = 0;
    ...
    void setBusy() {
      ...
      if ( !numEntries ) {
        ...
        if (GETBIT(m_nFlags, kMtQueueForceTopLevel) || frmwrk->isMainThread())
        {
    ...
    void wait() {
      ...
      OdApcThreadImpl* pExceptionThread = NULL;
      {
        ...
        if ( !numEntries ) {
          ...
          // It even have to raise exceptions, that holds
          if (GETBIT(m_nFlags, kFinalizeThreading)) {
            pExceptionThread = frmwrk->finalizeThreading();
            SETBIT_0(m_nFlags, kFinalizeThreading); // Clear finalization flag
                                                    // for next execution
          }
          if (pExceptionThread) {
            pExceptionThread->processException();
          }
          m_exceptions.process();
          return;
        }
      }
      // Top-level task
    ...
    void callIncreaseDecrease(bool bInc, bool bTopJob, unsigned topJobId)
    {
      ...
      if (bInc)
      {
        ...
        numIncreased += nThreads;
      }
      else
      {
        m_completedThreads.clear();
        numIncreased -= nThreads;
        ODA_ASSERT(!numIncreased);
      ...
      if (!m_completedThreads.isEmpty())
      {
        if (!GETBIT(m_nFlags, kThreadsLocked))
        {
          ...
          for (OdUInt32 nThread = 0; nThread < nThreads; nThread++)
          {
            ...
            OdApcThreadImpl *pThisThread = find(m_queueThreads, pThreads[nThread]);
            if(pThisThread)
              m_exceptions.add(pThisThread->detachException());
        ...
        else
        {
          ...
          if (nThreads == m_queueThreads.size() && !GETBIT(m_nFlags, kWaitUnlockThread))
            callIncreaseDecrease(false, bTopJob, (bTopJob) ? m_completedThreads.last() : 0u);
          for(unsigned i = 0; i < m_queueThreads.size(); ++i)
            m_exceptions.add(getPtr(m_queueThreads[i])->detachException());
        }
      }
    }
    ...
    OdApcThreadImpl* readyThread() {
      ...
      else
      {
        ...
        if (m_readyQueueThreads.isEmpty())
          ...
          if (m_readyQueueThreads.size() == m_queueThreads.size() && !numIncreased)
          {
    ...
    inline void addEntryPoint( QueueEntry* queueEntry ) {
      ...
      for (;;)
      { OdSaveStateFlag(OdUInt32, kWaitUnlockThread, ssWaitUnlockFlag, m_nFlags, true); // #CORE-14902
        thread = readyThread();
  ...
  void executeMainThreadAction( MainThreadFunc mtFunc, void *pArg ) {
    if (odThreadsCounter() && (!m_pTopLevelTask || (odGetCurrentThreadId() != m_pTopLevelTask->m_mainThreadId)))
    {
...
#else
	  DWORD reserved = 0;
	  AttachConsoleFuncPtr pAttachConsole = (AttachConsoleFuncPtr)::GetProcAddress(::LoadPackagedLibrary(OdString("kernel32"), reserved), "AttachConsole");
#endif
...
public:
  OdRxThreadPoolImpl()
    : m_nCPUs(0), m_nPhysicalCores(0), m_pTopLevelTask(NULL), m_mainThreadId(0) {
    m_mainThreadId = OD_TP_ID_TO_UINT(OdTP::currentThreadId());
  }
  ...
  unsigned int getCurrentThreadId() const { return OD_TP_ID_TO_UINT(OdTP::currentThreadId()); }
