...
protected:
  ...
  void outCharacter(void *data, OdGiConveyorGeometry* pGeom, const OdTextProperties& textFlags);
...
public:
  ...
  static void  getLogicalIndicesFromVisual(const OdIntArray& visualFromLogical,
              unsigned int numVisual,
              OdIntArray& logicalFromVisual);
  ...
  virtual OdString getSubstituteFontByChar(OdChar unicodeChar, void* pSubstitutedFontData = NULL) const
  {
...
protected:
  ...
  OdResult getBBox(OdChar character, OdGeBoundBlock3d& bbox, OdGePoint2d& advance, bool bGlyph = false);
  OdResult createOutlineAndDraw(OdChar character,
    ...
    OdTextProperties textFlags,
    bool bDraw = false);
    OdUInt32 getAvailableCharsImpl(OdCharArray& retArray);
    bool hasCharacterImpl(OdChar character);
    void OdTrueTypeFontWin::getGlyphIndexesImpl(const OdCharArray& srcArray, OdCharArray& glyphs, OdIntArray* visualOrderFromLogical = NULL,
      OdBoolArray* isLTR = NULL, OdGeVector2dArray* offsets = NULL);

    //Is a copy of OdTrueTypeFont::hasCharacter.Added to exclude deadlock in thread - safe mode.When upgrading to C++ 11, it is recommended to removeand use
    //  std::recursive_mutex.
    bool hasCharacterBaseClassCopy(OdChar character);
public:
  ...
  virtual bool isSupportExtendedLayouting();
