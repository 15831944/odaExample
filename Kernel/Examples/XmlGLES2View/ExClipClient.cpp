...
class ExClipTransient : public OdTrRndLocalTransient
{
  public:
    ExClipTransient() {}
...
class ExClipCurveTransient : public ExClipPolygonTransient
{
  ...
  public:
    ...
    struct NurbsDef
    {
      int m_nDegree; bool m_bRational, m_bPeriodic;
      OdGeKnotVector m_knots;
      OdGePoint3dArray m_ctrlPts;
      OdGeDoubleArray m_weights;

      NurbsDef() : m_nDegree(0), m_bRational(false), m_bPeriodic(false) {}
      void getDefinition(const OdGeNurbCurve3d* pCurve)
      { pCurve->getDefinitionData(m_nDegree, m_bRational, m_bPeriodic, m_knots, m_ctrlPts, m_weights); }
      OdGeNurbCurve3d *createCurve() const
      { return new OdGeNurbCurve3d(m_nDegree, m_knots, m_ctrlPts, m_weights, m_bPeriodic); }
    };
    ...
    virtual void save(OdStreamBuf *pStream) const
    {
      ...
      switch (type)
      {
        ...
        case OdGe::kNurbCurve3d:
          {
            NurbsDef nd; nd.getDefinition(static_cast<const OdGeNurbCurve3d*>(m_pCurve.get()));
            pStream->putBytes(&nd.m_nDegree, sizeof(int));
            { const OdUInt32 nKnots = (OdUInt32)nd.m_knots.logicalLength(); pStream->putBytes(&nKnots, sizeof(OdUInt32));
              if (nKnots) pStream->putBytes(nd.m_knots.getPtr(), sizeof(double) * nKnots); }
            { const OdUInt32 nCtrlPts = nd.m_ctrlPts.logicalLength(); pStream->putBytes(&nCtrlPts, sizeof(OdUInt32));
              if (nCtrlPts) pStream->putBytes(nd.m_ctrlPts.getPtr(), sizeof(OdGePoint3d) * nCtrlPts); }
            { const OdUInt32 nWeights = nd.m_weights.logicalLength(); pStream->putBytes(&nWeights, sizeof(OdUInt32));
              if (nWeights) pStream->putBytes(nd.m_weights.getPtr(), sizeof(double) * nWeights); }
            pStream->putByte((nd.m_bPeriodic) ? 1 : 0);
          }
        break;
      }
    }
    virtual void load(OdStreamBuf *pStream)
    {
      ...
      OdGe::EntityId type = OdGe::kObject;
      ...
      switch (type)
      {
        ...
        case OdGe::kNurbCurve3d:
          {
            NurbsDef nd;
            pStream->getBytes(&nd.m_nDegree, sizeof(int));
            { OdUInt32 nKnots = 0; pStream->getBytes(&nKnots, sizeof(OdUInt32));
              if (nKnots) pStream->getBytes(nd.m_knots.setLogicalLength((int)nKnots).asArrayPtr(), sizeof(double) * nKnots); }
            { OdUInt32 nCtrlPts = 0; pStream->getBytes(&nCtrlPts, sizeof(OdUInt32));
              if (nCtrlPts) pStream->getBytes(nd.m_ctrlPts.setLogicalLength(nCtrlPts).asArrayPtr(), sizeof(OdGePoint3d) * nCtrlPts); }
            { OdUInt32 nWeights = 0; pStream->getBytes(&nWeights, sizeof(OdUInt32));
              if (nWeights) pStream->getBytes(nd.m_weights.setLogicalLength(nWeights).asArrayPtr(), sizeof(double) * nWeights); }
            nd.m_bPeriodic = pStream->getByte() != 0;
            m_pCurve = nd.createCurve();
          }
        break;
      }
    }
...
class ExClipMarkerTransient : public ExClipTransient
{
  ...
  public:
    ...
    virtual void draw(OdTrVisMetafileBuilderPtr mfBuilder, const OdTrRndLocalRenderer* pRenderer)
    {
      ...
      unitVec.transformBy(pRenderer->coordinateSpaceTransform( mfBuilder->viewportId(), mfBuilder->overlayId(), OdTrRndLocalRenderer::kScreenSpace, OdTrRndLocalRenderer::kEyeSpace));
...
static const OdUInt32 ExCmdElem_CreateNurbs    = 114;
static const OdUInt32 ExCmdElem_ClipAsPlanes   = 115;
static const OdUInt32 ExCmdElem_LinesClip      = 116;
static const OdUInt32 ExCmdElem_RaysClip       = 117;
static const OdUInt32 ExCmdElem_SegmentsClip   = 118;
static const OdUInt32 ExCmdElem_CurveClip      = 119;
static const OdUInt32 ExCmdElem_PolygonClip    = 120;
static const OdUInt32 ExCmdElem_PolygonGhost   = 121;
static const OdUInt32 ExCmdElem_CurveGhosts    = 122;
static const OdUInt32 ExCmdElem_SegmentGhosts  = 123;
static const OdUInt32 ExCmdElem_RayGhosts      = 124;
static const OdUInt32 ExCmdElem_LineGhosts     = 125;
static const OdUInt32 ExCmdElem_ClearPolygon   = 126;
static const OdUInt32 ExCmdElem_ClearClip      = 127;
static const OdUInt32 ExCmdElem_ClearResult    = 128;
static const OdUInt32 ExCmdElem_Save           = 129;
static const OdUInt32 ExCmdElem_ClearAll       = 130;
...
static struct ExClipRenderClient : public AppClient
{
  ...
  void init()
  {
    ...
    vpDef.setDefault();
    vpDef.setViewportVisible(true);
    rendition()->onViewportAdded(100, vpDef);
  ...
  virtual void onCreate(AppHandle appHandle)
  {
    ...
    extBar.createButton(ExCmdElem_CreateNurbs)->setText(L"Create Nurbs");
    ...
    extBar.createButton(ExCmdElem_PolygonGhost)->setText(L"Ghost Polygons");
    extBar.createButton(ExCmdElem_CurveGhosts)->setText(L"Ghost Curves");
    extBar.createButton(ExCmdElem_SegmentGhosts)->setText(L"Ghost Line Segments");
    extBar.createButton(ExCmdElem_RayGhosts)->setText(L"Ghost Rays");
    extBar.createButton(ExCmdElem_LineGhosts)->setText(L"Ghost Lines");
    extBar.createPlaceholder();
  ...
  virtual void onLMouseButtonUp(AppHandle /*appHandle*/, int x, int y)
  {
    ...
    pt.transformBy(renderer()->coordinateSpaceTransform(viewer()->activeViewportId(), kTrVisMainOverlayId, OdTrRndLocalRenderer::kScreenSpace, OdTrRndLocalRenderer::kWorldSpace));
    if (EditTransient.isNull())
    {
  ...
  virtual void onExtBar(AppHandle appHandle, ExtBar::Element *pElem)
  {
    switch (pElem->elementId())
    {
      ...
      case ExCmdElem_3dViewBack: case ExCmdElem_3dViewRight: case ExCmdElem_3dViewBottom:
        {
          OdTrVisViewportData *pVp = renderer()->getViewportData(viewer()->activeViewportId());
          OdTrVisViewParamsDef &viewParams = pVp->viewParams();
          switch (pElem->elementId())
          {
            case ExCmdElem_3dViewTop:    viewParams.m_viewBasis.setZAxis( OdGeVector3d::kZAxis); viewParams.m_viewBasis.setYAxis( OdGeVector3d::kYAxis); break;
            case ExCmdElem_3dViewFront:  viewParams.m_viewBasis.setZAxis( OdGeVector3d::kYAxis); viewParams.m_viewBasis.setYAxis( OdGeVector3d::kZAxis); break;
            case ExCmdElem_3dViewLeft:   viewParams.m_viewBasis.setZAxis( OdGeVector3d::kXAxis); viewParams.m_viewBasis.setYAxis( OdGeVector3d::kZAxis); break;
            case ExCmdElem_3dViewBack:   viewParams.m_viewBasis.setZAxis(-OdGeVector3d::kYAxis); viewParams.m_viewBasis.setYAxis(-OdGeVector3d::kZAxis); break;
            case ExCmdElem_3dViewRight:  viewParams.m_viewBasis.setZAxis(-OdGeVector3d::kXAxis); viewParams.m_viewBasis.setYAxis(-OdGeVector3d::kZAxis); break;
            case ExCmdElem_3dViewBottom: viewParams.m_viewBasis.setZAxis(-OdGeVector3d::kZAxis); viewParams.m_viewBasis.setYAxis(-OdGeVector3d::kYAxis); break;
          }
          viewParams.m_viewBasis.setOrigin(OdGePoint3d::kOrigin);
          viewParams.m_viewTarget = viewParams.m_viewBasis.origin() + viewParams.m_viewBasis.zAxis();
          viewer()->markViewportModified(viewer()->activeViewportId());
        }
      break;
      case ExCmdElem_CreateCircles:
      case ExCmdElem_CreateEllipses:
      case ExCmdElem_CreateNurbs:
        {
          for (OdUInt32 nTrans = 0; nTrans < Transients.size(); nTrans++)
          {
            ...
            if (pTrans->transientType() == ExClipTransient::kPolygonTransient)
            {
                  ...
                  (pPoly->contourType() == ExClipPolygonTransient::kPolygonHole))
              {
                ...
                if (pElem->elementId() != ExCmdElem_CreateNurbs)
                {
                ...
                {
                  pCurve = new OdGeNurbCurve3d(pPoly->pointsArray());
                  if (pPoly->contourType() == ExClipPolygonTransient::kPolygonContour)
                    static_cast<OdGeNurbCurve3d*>(pCurve)->makeClosed();
                }
      ...
      case ExCmdElem_PolygonGhost:
      case ExCmdElem_CurveGhosts:
      case ExCmdElem_SegmentGhosts:
      case ExCmdElem_RayGhosts:
      case ExCmdElem_LineGhosts:
        {
          ExClip::ClipSpace clipSpace;
          clipSpace.setGhostsOutputEnable((pElem->elementId() >= ExCmdElem_PolygonGhost) && (pElem->elementId() <= ExCmdElem_LineGhosts));
                  ...
                  (pPoly->contourType() == ExClipPolygonTransient::kClipHole))
              {
                ...
                if (!ExtBar::CheckElement::cast(getExtBar(appHandle).getElement(ExCmdElem_ClipAsPlanes))->isChecked())
                {
                  if (!pClipPoly)
                  {
                    ...
                    clipSpace.setGhostsGeneration(pClipPoly, clipSpace.ghostsOutputEnabled());
                  }
                ...
                else
                {
                  ...
                  while (nVert1 < poly.numPoints())
                  {
                    ...
                    clipSpace.setGhostsGeneration(pClipPlane, clipSpace.ghostsOutputEnabled());
          ...
          {
            const ExClipTransient *pTrans = Transients.getPtr()[nTrans];
            if ((pElem->elementId() != ExCmdElem_CurveClip) && (pElem->elementId() != ExCmdElem_CurveGhosts))
            {
              if (pTrans->transientType() == ExClipTransient::kPolygonTransient)
              {
                const ExClipPolygonTransient *pPoly = ExClipPolygonTransient::cast(pTrans);
                if ((pPoly->contourType() == ExClipPolygonTransient::kPolygonContour) ||
                    (pPoly->contourType() == ExClipPolygonTransient::kPolygonHole))
                {
                  if (!(pElem->elementId() == ExCmdElem_PolygonClip) && !(pElem->elementId() == ExCmdElem_PolygonGhost))
                  {
                    ExClip::ClipIntervalChain ivals, ghostIvals;
                    OdGePoint3dArray points = pPoly->pointsArray();
                    switch (pElem->elementId())
                    {
                      case ExCmdElem_LinesClip: case ExCmdElem_LineGhosts:
                      case ExCmdElem_RaysClip: case ExCmdElem_RayGhosts:
                        {
                          for (OdUInt32 nSeg = 0; nSeg < points.size() - 1; nSeg++)
                          {
                            ...
                            if ((((pElem->elementId() == ExCmdElem_LinesClip) || (pElem->elementId() == ExCmdElem_LineGhosts)) ?
                                clipSpace.clipLine(ptOrigin, ptDirection, ivals, &ghostIvals) :
                                clipSpace.clipRay(ptOrigin, ptDirection, ivals, &ghostIvals)) || !ghostIvals.empty())
                            {
                              if (clipSpace.ghostsOutputEnabled()) { ivals.clear(); ivals.moveFrom(ghostIvals); }
                      ...
                      break;
                      case ExCmdElem_SegmentsClip: case ExCmdElem_SegmentGhosts:
                        if (clipSpace.clipLineSegment(points.size() - 1, points.getPtr(), ivals, &ghostIvals) || !ghostIvals.empty())
                        {
                          if (clipSpace.ghostsOutputEnabled()) { ivals.clear(); ivals.moveFrom(ghostIvals); }
                          ExClip::ClipInterval *pIval = ivals.first();
                          while (pIval)
            ...
            { // Curve clip
              if (pTrans->transientType() == ExClipTransient::kCurveTransient)
              {
                ...
                {
                  ExClip::ClipIntervalChain ivals, ghostIvals;
                  if (clipSpace.clipCurve(pCrv->curve(), ivals, &ghostIvals) || !ghostIvals.empty())
                  {
                    if (clipSpace.ghostsOutputEnabled()) { ivals.clear(); ivals.moveFrom(ghostIvals); }
                    ExClip::ClipInterval *pIval = ivals.first();
                    while (pIval)
          ...
          { // Check that AABB algorithm works
            ...
            int needClip = clipSpace.checkAABBClip(exts, ExClip::ClipSpace::kCheckExact | ((clipSpace.ghostsOutputEnabled()) ? ExClip::ClipSpace::kCheckGhosts : 0));
            // Do polygon clipping
            ExClip::PolygonChainChain ghostChain, *pGhostChain = NULL;
            if (pElem->elementId() == ExCmdElem_PolygonGhost) pGhostChain = &ghostChain;
            if (((needClip == ExClip::ClipSpace::kCheckResClip) || (needClip == ExClip::ClipSpace::kCheckResGhost)) && clipSpace.clipPolygon(polyChain, pGhostChain))
            {
              if (pGhostChain) polyChain.clear(), polyChain.moveFrom(ghostChain);
