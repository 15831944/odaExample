...
AnimationHost *createAnimationHost(HWND hWnd); // Defined in AnimationHost.cpp
      ...
      case IDC_MENU_OPENANIMATION:
        {
          ...
          if (GetOpenFileName(&ofn) == FALSE)
            break;
          if (id != IDC_MENU_OPENANIMATION)
            CurAppClient->onOpen(::makeAppHandle(paintWindow()), OdString(fileName));
          else
            CurAppClient->onOpenAnimation(::makeAppHandle(paintWindow()), OdString(fileName), ::createAnimationHost(hwnd));
...
void AppClientRegistrar::updateClientMenu()
{
  ...
  ::EnableMenuItem(::GetMenu(ViewerWnd::mainWindow()), IDC_MENU_OPENANIMATION, MF_BYCOMMAND |
    ((CurAppClient->isSupportAnimation(::makeAppHandle(ViewerWnd::paintWindow()))) ? MF_ENABLED : MF_GRAYED));
}
...
int PASCAL _tWinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPTSTR /*lpszCmdLine*/, int nCmdShow) {
  ...
  ::odrxInitialize(&sysServices); // Initialize dynamic loader
  ...
  ::odrxUninitialize(); // Uninitialize dynamic loader
