...
{
  PDFResultEx res = startUndoRecord(pParams);
  if(res != exOk)
    return res;

  res = m_ParamsHolder.init(pParams);
  if (res != exOk)
    return res;
  ...
  const bool linearize = GETBIT(m_ParamsHolder.getParams().exportFlags(), PDFExportParams::kLinearized);
  const bool use_qpdf = linearize || 
    ((!m_ParamsHolder.getParams().userPassword().isEmpty() || !m_ParamsHolder.getParams().ownerPassword().isEmpty()) && 
      m_ParamsHolder.getParams().version() > PDFExportParams::kPDFv1_2);
  m_ParamsHolder.document().setUseQPDF(use_qpdf);
...
PDFResultEx CPdfExportImpl::exportFile()
{
  ...
  OdStreamBufPtr qpdfOutput;
  const PDFVersion pdf_version = ver2ver(m_ParamsHolder.getParams().version());
  const bool linearize = GETBIT(m_ParamsHolder.getParams().exportFlags(), PDFExportParams::kLinearized);
  const bool use_qpdf = linearize || 
    ((!m_ParamsHolder.getParams().userPassword().isEmpty() || !m_ParamsHolder.getParams().ownerPassword().isEmpty()) && 
      pdf_version.Version() > PDFVersion::kPDFv1_2);

  if (use_qpdf)
  {
    qpdfOutput = OdMemoryStream::createNew();
    outputStream->setStreamBuf(qpdfOutput);
  }
  else
  {
  ...
  m_ParamsHolder.document().Export(outputStream, pdf_version);

  if (use_qpdf)
  {
    OdStreamBufPtr output = m_ParamsHolder.getParams().output();

    PDFExportParams::PDFAccessPermissionsFlags access_permissions = m_ParamsHolder.getParams().accessPermissionFlags();
    TD_PDF_HELPER_FUNCS::PDFAccessPermissionsParams access_params;
    access_params.AllowExtract = GETBIT(access_permissions, PDFExportParams::kAllowExtract);
    access_params.AllowAssemble = GETBIT(access_permissions, PDFExportParams::kAllowAssemble);
    access_params.AllowAnnotateAndForm = GETBIT(access_permissions, PDFExportParams::kAllowAnnotateAndForm);
    access_params.AllowFormFilling = GETBIT(access_permissions, PDFExportParams::kAllowFormFilling);
    access_params.AllowModifyOther = GETBIT(access_permissions, PDFExportParams::kAllowModifyOther);
    access_params.AllowPrintAll = GETBIT(access_permissions, PDFExportParams::kAllowPrintAll);
    access_params.AllowPrintLow = GETBIT(access_permissions, PDFExportParams::kAllowPrintLow);

    TD_PDF_HELPER_FUNCS::QPDFHelper::process(qpdfOutput, output, linearize, m_ParamsHolder.getParams().userPassword(), m_ParamsHolder.getParams().ownerPassword(),
      access_params, pdf_version);
  }
