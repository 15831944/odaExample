...
void PdfExportParamsHolder::showAllLayers(OdDbBaseDatabasePtr& pDb)
{
  for(OdRxIteratorPtr it = OdDbBaseDatabasePEPtr(pDb)->layers(pDb); !it->done(); it->next())
  {
...
void PdfExportParamsHolder::showAllLayers()
{
  ...
  {
    for(OdUInt32 i = 0; i < m_Params.databases().size(); ++i)
    {
      pDb = m_Params.databases()[i];
      showAllLayers(pDb);
    }
  }
  else
  {
    pDb = m_Params.database();
    showAllLayers(pDb);
  }
}
...
PDFResultEx PdfExportParamsHolder::init(const PDFExportParams &pParams)
{
  ...
  if (m_Params.version() < PDFExportParams::currentVersion && GETBIT(m_Params.exportFlags(), PDFExportParams::kMeasuring))
    m_Params.setVersion(PDFExportParams::currentVersion);
  ...
  if(m_Params.version() < PDFExportParams::kPDFv1_6 && (GETBIT(m_Params.exportFlags(), PDFExportParams::kMeasuring) || m_Params.watermarks().size()))
    m_Params.setVersion(PDFExportParams::kPDFv1_6);

  if (m_Params.getPRCMode() != PDFExportParams::kDisabled)
  {
    m_Params.setUseGsCache(false); //using GsCache with PRC is not implemented yet
    ...
    if (m_Params.version() < PDFExportParams::currentVersion)
      m_Params.setVersion(PDFExportParams::currentVersion);

    PdfExportServiceInterfacePtr pExportIFace = getPdfExportService();
    if (pExportIFace.isNull())
    {
      return exPdfExportServiceMissed;
    }
  }

  if (m_Params.database() == 0 && !m_Params.databases().size())
    return exNullDatabase;

  if (m_Params.output().isNull())
    return exNullOutputStream;

  if (m_Params.hatchDPI() < 1 || m_Params.colorImagesDPI() < 1 || m_Params.bwImagesDPI() < 1)
  ...
  if (m_Params.layouts().isEmpty())
  {
    ...
    m_Params.addLayout(OdDbBaseLayoutPEPtr(activeLayout)->name(activeLayout));
  ...
  else
  {
    ...
    for(OdUInt32 nPageIndx = 0; nPageIndx < m_Params.layouts().size(); ++nPageIndx)
    {
      if(m_Params.databases().size())
      {
        OdDbBaseDatabasePtr pDb = m_Params.databases()[nPageIndx];
        m_Params.setDatabase(pDb);
      }
  ...
  if (GETBIT(m_Params.exportFlags(), PDFExportParams::kZoomToExtentsMode) == 0)
  {
    ...
    for(OdUInt32 nPageIndx = 0; nPageIndx < m_Params.layouts().size(); ++nPageIndx)
    {
      ...
      PagesetupToLayout pagesetupToLayout(m_Params.database(), nPageIndx);
      OdRxObjectPtr pLayout = OdDbBaseDatabasePEPtr(m_Params.database())->getLayout( m_Params.database(), m_Params.layouts()[nPageIndx] );
  ...
  //Check PDF page size limits
  for (OdUInt32 indx = 0; indx < m_Params.pageParams().size(); ++indx)
  {
    if (m_Params.pageParams()[indx].getPaperWidth() > 14400 || m_Params.pageParams()[indx].getPaperHeight() > 14400)
      return exInvalidPageParams;
  }
