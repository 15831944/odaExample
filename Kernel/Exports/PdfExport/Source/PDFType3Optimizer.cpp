...
#include "../DevInclude/DbRoot/OdShxFont.h"
...
void PDFType3Optimizer::addUnicodeChar(const OdGiTextStyle& pTextStyle, PDFType3FontPtr pFont, OdChar pUnicode, OdAnsiString &text, OdBool isInBigFont, OdChar pRealUnicode)
{
...

  {
    ...
    if (!pElem->hasCharacter(pUnicode, singleByteCode))
    {
      OdGePoint2d adv;
      ...
      if (!pElem->AddUChar(pUnicode, pRealUnicode, ShxGeom.getCharWidth(), pStream, singleByteCode))
      {
...
bool PDFType3Optimizer::checkTextScale(const OdGiTextStyle& pTextStyle, const OdString &sUnicode, const OdBoolArray &isInBigFont)
{
  if (sUnicode.isEmpty())
    return true;
  ...
  OdShxTextData shxTd(TD_PDF_HELPER_FUNCS::isShx(pOdFont));
  OdShxTextData shxBigTd(pBigFont && TD_PDF_HELPER_FUNCS::isShx(pBigFont));
  ...
  for (int f = 0; f < sUnicode.getLength(); ++f)
  {
    OdChar ch = sUnicode.getAt(f);
    if (isInBigFont[f])
    {
      ODA_ASSERT(pBigFont);
      ODA_VERIFY(static_cast<OdShxFont*>(pBigFont)->drawCharacterImp(ch, adv, &emptyGeom, textFlags, &shxBigTd) == ::eOk);
      if (shxBigTd.hasScale())
        return false;
    }
    else
    {
      ODA_VERIFY(static_cast<OdShxFont*>(pOdFont)->drawCharacterImp(ch, adv, &emptyGeom, textFlags, &shxTd) == ::eOk);
      if (shxTd.hasScale())
        return false;
    }
  }
...
void PDFType3Optimizer::addUnicodeText(const OdGiTextStyle& pTextStyle, PDFType3FontPtr pFont, const OdUInt16Array &pUnicode, OdAnsiString &text, const OdBoolArray &isInBigFont)
{
  ...
  {
    OdAnsiString tmp;
    addUnicodeChar(pTextStyle, pFont, pUnicode[f], tmp, isInBigFont[f], pUnicode[f]);
    text += tmp;
  }
}
...
void PDFType3Optimizer::Optimize()
{
  ...
  while(pIt != pE)
  {
    ...
    for(f=0; f<nNumChars; ++f)
    {
      ...
      charPairs[f].nUnicodeCode = pIt->MappedUnicodeChars[f];
    }
