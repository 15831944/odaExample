...
protected:
  /** \details
    Shows all the layers for the database.

    \remarks
    Layers are determined in m_Params member variable.
    */
  void showAllLayers(OdDbBaseDatabasePtr& pDb);
...
class PagesetupToLayout
{
public:
  PagesetupToLayout(OdDbBaseDatabase* db, OdUInt32 pageIdx);
  ~PagesetupToLayout();
  void saveGsModel()
  {
    m_savedGsModel = SavedGsModelData(OdGiDrawable::cast(m_db));
  }
private:
  OdDbBaseDatabase* m_db;
  SavedGsModelData m_savedGsModel;
};
