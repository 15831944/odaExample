...
public:
  ...
  OdInt32 get_ColorPolicy() { return _colorPolicy; }
  void put_ColorPolicy(OdInt32 c) { _colorPolicy = (ColorPolicy)c; }
  double get_MinimalWidth() { return _minimalWidth; }
  void put_MinimalWidth(double w) { _minimalWidth = w; }
  double get_PlotLineweights() { return _plotLineweights; }
  void put_PlotLineweights(double w) { _plotLineweights = w; }
};
...
ODRX_DECLARE_PROPERTY(ColorPolicy)
ODRX_DECLARE_PROPERTY(MinimalWidth)
ODRX_DECLARE_PROPERTY(PlotLineweights)
...
ODRX_DEFINE_PROPERTY(ColorPolicy, SvgProperties, getInt32)
ODRX_DEFINE_PROPERTY(MinimalWidth, SvgProperties, getDouble)
ODRX_DEFINE_PROPERTY(PlotLineweights, SvgProperties, getBool)
...
ODRX_BEGIN_DYNAMIC_PROPERTY_MAP( SvgProperties );
  ...
  ODRX_GENERATE_PROPERTY(ColorPolicy)
  ODRX_GENERATE_PROPERTY(MinimalWidth)
  ODRX_GENERATE_PROPERTY(PlotLineweights)
...
static void convertColorToGrayscale(ODCOLORREF& rgb)
{
  OdUInt8 gray = (OdUInt8)((30 * ODGETRED(rgb) + 59 * ODGETGREEN(rgb) + 11 * ODGETBLUE(rgb)) / 100);
  rgb = ODRGBA(gray, gray, gray, ODGETALPHA(rgb));
}

static void applyColorPolicy(ODCOLORREF& color, OdInt32 cp)
{
  if (cp == kMono)
  {
    if (ODGETRED(color) != 255 || ODGETGREEN(color) != 255 || ODGETBLUE(color) != 255)
      color = ODRGBA(0, 0, 0, ODGETALPHA(color));
  }
  else if (cp == kGrayscale)
  {
...
public:
  ...
  bool isUseHLR() const { return _properties->get_UseHLR(); }
  ...
  void setCurrTraits(bool fill, TextMode textMode = kTxtNone, bool forceCreateGroup = false)
  {
        ...
        if (d)
        {
          ...
          if (!hColl.isNull() && hColl->count() > 0)
          {
            ...
            if (_hyperlink != h)
            {
              ...
              {
                svg::replaceXmlSpecials(_hyperlink);
    ...
    if (!_properties->get_PlotLineweights() || (lw == 0 && textMode != kTxtTtf))
    ...
    applyColorPolicy(color, _properties->get_ColorPolicy());
    applyColorPolicy(color2, _properties->get_ColorPolicy());
         ...
         (fill && ((fillContour != _fillContour) || (fillContour && _color2 != color2))) )
    {
      ...
      if ( _fill ) 
      {
        if ( fillContour )
        {
          ...
          setLineWidth(_currentLineWeight);
        }
        else
        {
          _currentNode->addAttribute( svg::Fill( _color ) );
          _currentNode->addAttribute( svg::Stroke( _color ) );
          setLineWidth(_bShellMode ? _currentLineWeight * 4 : _currentLineWeight); // Widen lineweight in shell mode, to mask border gaps between fills
        }
        if (_alpha != 255)
        {
  ...
  {
    _currentNode->addAttribute(OD_T("stroke-width"), odmax(_properties->get_MinimalWidth(), w));
  }

  virtual void plineProc(const OdGiPolyline& lwBuf, const OdGeMatrix3d* pXform, OdUInt32 fromIndex, OdUInt32 numSegs)
  {
    bool bCreatePolyline = true;
    double dPLineWidth = lwBuf.getConstantWidth();

    if (!OdZero(lwBuf.thickness()))
      bCreatePolyline = false;
    else
    {
      bool bInitWidth = !OdZero(dPLineWidth);

      for (OdUInt32 i = 0; i < lwBuf.numVerts(); i++)
      {
        if (!OdZero(lwBuf.getBulgeAt(i)))
        {
          bCreatePolyline = false;
          break;
        }
        ...
        lwBuf.getWidthsAt(i, dStartWidth, dEndWidth);
        ...
        if (!OdZero(dStartWidth) || !OdZero(dEndWidth))
        {
          if (OdEqual(dStartWidth, dEndWidth))
          {
            ...
            {
              bInitWidth = true;
              dPLineWidth = dStartWidth;
            }
            else if (!OdEqual(dStartWidth, dPLineWidth))
            {
              bCreatePolyline = false;
              break;
            }
    ...
    if (bCreatePolyline)
    {
      ...
      if (!OdZero(dPLineWidth))
      {
        double dScaleCorrection = 1.0;

        if (pXform)
        {
          double dXScale = fabs(pXform->getCsXAxis().length());
          double dYScale = fabs(pXform->getCsYAxis().length());
          double dZScale = fabs(pXform->getCsZAxis().length());

          if (OdZero(dZScale))
            dScaleCorrection = (dXScale + dYScale + dZScale) / 3.0;
          else
            dScaleCorrection = (dXScale + dYScale) / 2.0;
        }

        _currentNode->removeAttribute(OD_T("stroke-width"));
        setLineWidth(dPLineWidth * dScaleCorrection);

      }

      OdGePoint3dArray arrVertices;
      OdGeMatrix3d matTransform = OdGeMatrix3d::planeToWorld(lwBuf.normal());

      if (pXform)
        matTransform.preMultBy(*pXform);

      for (OdUInt32 i = 0; i < lwBuf.numVerts(); i++)
      {
        OdGePoint2d ptVertex2d;
        lwBuf.getPointAt(i, ptVertex2d);
        OdGePoint3d ptVertex(ptVertex2d.x, ptVertex2d.y, lwBuf.elevation());
        ptVertex.transformBy(matTransform);

        arrVertices.push_back(ptVertex);
      }

      if (lwBuf.isClosed() && !arrVertices.first().isEqualTo(arrVertices.last()))
        arrVertices.push_back(arrVertices.first());

      if (xml::Node* pPolylineNode = _currentNode->addChild(new xml::Node(OD_T("polyline"))))
      {
        pPolylineNode->addAttribute(svg::Points(arrVertices.size(), arrVertices.asArrayPtr()));
      ...
      Od2dExportDevice::plineProc(lwBuf, pXform, fromIndex, numSegs);
  }
  ...
  void gouraud_triangle(OdGePoint3d* pts, OdCmEntityColor* cols)
  {
    ...
    {
      // average flat color triangle
      xml::Node* use = _currentNode->addChild(L"use");
      use->addAttribute(L"xlink:href", pathRef);
      ODCOLORREF color = ODRGB(int((cols[0].red() + cols[1].red() + cols[2].red()) / 3), int((cols[0].green() + cols[1].green() + cols[2].green()) / 3), int((cols[0].blue() + cols[1].blue() + cols[2].blue()) / 3));
      applyColorPolicy(color, _properties->get_ColorPolicy());
      use->addAttribute(L"fill", svg::formatColor(color));
    }
    else
    {
  ...
  {
    if (OdZero(sweepAngle))
    {
      OdGeCircArc3d arc(center, normal, startVector, radius, 0., sweepAngle);
      OdGePoint3d pp[2] = { arcType == kOdGiArcSector ? center : arc.startPoint(), arc.startPoint() };
      polylineOut(2, pp);
    }
    ...
    OdGiArcType arcType = kOdGiArcSimple, const OdGeVector3d* pExtrusion = 0)
  {
    ...
    correctArc.orthogonalizeAxes( OdGeContext::gTol, ec );
    if ( ec != OdGe::kOk 
    ...
    const OdGeVector3d* pExtrusion = 0)
  {
    ...
    {
      bool bUseArray = m_curDashArray.m_bUseDashArray;
      m_curDashArray.m_bUseDashArray = false;

      OdGiGeometrySimplifier::textProc( position, u, v, msg, length, raw, pTextStyle, pExtrusion );

      m_curDashArray.m_bUseDashArray = bUseArray;
      return;
    }
...
//
  ...
  OdSmartPtr<OdGiBitonalRasterTransformer> process2ColorsImage(const OdGiRasterImage* pImg, ColorPolicy cp, bool bTransparency)
  {
    ...
    OdInt16 primaryColor = traits.trueColor().colorIndex();
//    bool bTransparency = (bool)(traits.drawFlags() & OdGiSubEntityTraits::kDrawContourFill);
    ODCOLORREF background = bTransparency ? m_logPalette[traits.secondaryTrueColor().colorIndex()] : _properties->get_BackGround();
    ODCOLORREF fgColor = m_logPalette[primaryColor];
    applyColorPolicy(background, cp);
    applyColorPolicy(fgColor, cp);
    pBitTrans->setOriginal(pImg, fgColor, background, bTransparency);
  ...
  virtual void rasterImageProc(
    ...
    double fade = 0.0 )
  {
    if (!svg::isValidImage( pImg ) || _properties->get_ImageBase().isEmpty()) 
      return;

    OdDbBaseDatabase* pDb = drawContext()->giContext().database();
    OdDbBaseDatabasePEPtr pDbPE( pDb );

    ColorPolicy cp = (ColorPolicy)_properties->get_ColorPolicy();
    OdString path;
    if (pImg->imageSource() == OdGiRasterImage::kFromFile && !pImg->sourceFileName().isEmpty() && cp == kNoPolicy && pImg->numColors() != 2)
    {
    ...
    else
    {
      ...
      OdRxRasterServicesPtr pRasSvcs = odrxDynamicLinker()->loadApp(RX_RASTER_SERVICES_APPNAME);
      OdGiRasterImagePtr converted = pImg;
      if (pImg->numColors() == 2 || cp == kMono)
        converted = process2ColorsImage(pImg, cp, transparency);
      else if (cp == kGrayscale)
      {
        OdSmartPtr<OdGiRasterImageWrapper> pWrapper = OdRxObjectImpl<OdGiGrayscaleRasterTransformer>::createObject();
        pWrapper->setOriginal(converted);
        converted = pWrapper;
      }

      if ( !pRasSvcs.get() || !pRasSvcs->saveRasterImage( converted, _properties->get_ImageBase() + path) )
        return;
    }
...
xml::Node* OdSvgDevice::createGradient(OdString& id, const OdCmEntityColor& col1, const OdCmEntityColor& col2, const OdGePoint3d& p1, const OdGePoint3d& p2)
{
  ...
  ODCOLORREF color1 = ODRGB(col1.red(), col1.green(), col1.blue());
  ODCOLORREF color2 = ODRGB(col2.red(), col2.green(), col2.blue());
  applyColorPolicy(color1, _properties->get_ColorPolicy());
  applyColorPolicy(color2, _properties->get_ColorPolicy());
  stop1->addAttribute(L"stop-color", svg::formatColor(color1));
  stop1->addAttribute(L"stop-opacity", L"1");
  xml::Node* stop2 = grad->addChild(L"stop");
  stop2->addAttribute(L"offset", L"1");
  stop2->addAttribute(L"stop-color", svg::formatColor(color2));
...
// vladimirv, reported: OdaJira:CORE-12436
      ...
      OdInt32 colorPolicy = pCmdCtx->userIO()->getKeyword(L"Color policy (Default|Monochrome|Grayscale) <Default>", L"Default Monochrome Grayscale", 0, 0);
      dev->properties()->putAt(L"ColorPolicy", OdRxVariantValue(colorPolicy));
