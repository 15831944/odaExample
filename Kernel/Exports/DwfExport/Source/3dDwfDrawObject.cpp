...
{
  // ::odgsInitialize(); // Moved to C3dDwfExportGsInitializer
}

Od3dDwfDevice::~Od3dDwfDevice()
{
  // ::odgsUninitialize(); // Moved to C3dDwfExportGsInitializer
}
            ...
            const OdGeVector3d* pExtrusion)
{
  ...
  bool bPreviousState = WriteToTheSameSegment( OdZero(dRotation) );
  {
    ...
    else
    {
      ...
      if (OdZero(dY) && !OdZero(v.z))
      {
        yDir = OdGeVector2d(v.x, v.z); // CORE-11268
        dY = yDir.length();
      }
      if (!OdZero( fSlant ))
      {
      ...
      // CORE-11268 //if ( !OdZero(dRotation) )
      {
        TK_Matrix& m = getRootSubpart()->getModellingMatrixHandler();
        OdGeMatrix3d gm;

        // CORE-16531 //gm.setToRotation(dRotation, OdGeVector3d::kZAxis, position);
        OdGeVector3d vX = u.normal(), vY = v.normal(), vZ = u.crossProduct(v).normalize();
        OdGeMatrix3d w3DTransform;
        w3DTransform.setCoordSystem(OdGePoint3d::kOrigin, vX, vY, vZ);
        gm.setToTranslation(-position.asVector());
        gm.preMultBy(w3DTransform);
        gm.preMultBy(OdGeMatrix3d::translation(position.asVector()));
