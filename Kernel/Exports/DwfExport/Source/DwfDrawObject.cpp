...
//  ptPosition3d.transformBy(objectToDeviceMatrix());
  ...
  if (bBox && pFont)
  {
    ...
    if (!f->isShxFont()) 
       // CORE-14760 In other case we have incorrect text height in Design Review // && !OdZero(pFont->m_rotation) && !OdZero(pFont->m_width_scale - 1))
       // It looks like Design Review ignore height of font
       // and uses pBounds to get height of TTF font. Therefore the next is critical
    {
    ...
    OdGsDCPoint ll, ur;
    screenRect(ll, ur);
    if (isPolygonOutOfRect2d(4, points, toGePoint2d(ll), toGePoint2d(ur)))
    {
      bSkip = true;
    }
    else
    {
      std::unique_ptr<WT_Font> wtFont(device()->classFactory()->Create_Font());
      ...
      //if (device()->getExportImpl()->getParams().format() == DW_XPS)
      //{
      //  const wchar_t* pFontPath = static_cast<WT_XAML_File*>(device()->getWtFile())->resolveFontUri(fontName);
      //  static_cast<WT_XAML_File*>(device()->getWtFile())->registerFontUri(fontName, pFontPath);
      //}
  ...
  if (!bSkip)
  {
    if (bASCII)
    {
      std::unique_ptr<WT_Text> wtText(device()->classFactory()->Create_Text( 
...
void OdDwfView::postprocess_Url(const OdGiDrawable* pDrawable)
{
  ...
  ODA_ASSERT_VAR(if (g_UrlList.empty()))
  {
    ODA_ASSERT_VAR(OdString sName = pDrawable->isA()->name());
    ODA_ASSERT(sName == L"AcDbBlockTableRecord"); // is possible for some entities
  }
}
