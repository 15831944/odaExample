...
class C3dDwfExportGsInitializer
{
  public:
    C3dDwfExportGsInitializer() 
    {
      ::odgsInitialize();
    }
    ~C3dDwfExportGsInitializer()
...
bool C3dDwfExportImpl::run()
{
  try
  {
    ...
    C3dDwfExportGsInitializer gsInitializer; // Wraps Od3dDwfDevice to prevent Gs module unloading before all Gs-related classes release
    ...
    std::unique_ptr<DWFPackagePublisher> oPublisher(ext.iCompare(L"dwfx") == 0 ?
