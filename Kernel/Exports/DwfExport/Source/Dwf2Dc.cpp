...
#define DWF_EXPORT_BITONAL_IMAGE_QUALITY_OFF // TODO comment for experiments of CORE-16506
...
bool OdDwfDevice::isEnabledToAddRenderDevice(const OdArray<OdGsViewPtr>& views, int indexView) const
{
  ODA_ASSERT_ONCE_X(TDWF, m_dwf);
  if (!m_dwf)
    return Od2dExportDevice::isEnabledToAddRenderDevice(views, indexView);
  int numView = views.size();
  ODA_ASSERT_ONCE_X(TDWF, indexView >= 0 && indexView < numView);

  // core-14820
  for (int idx = 0; idx < numView; idx++)
  {
    OdGsView* pView = const_cast<OdGsView*>(views[idx].get());
    if (!pView->isVisible())
      continue;
    if (pView->mode() < OdGsView::kHiddenLine)
      continue;
    return true; 
  }
  return false;
...
void OdDwfDevice::dc_polyline(OdUInt32 nPts, const OdGePoint2d* pPoints)
{
  ...
  if (nPts == 1)
  {
    ...
    std::unique_ptr<WT_Polyline> line(m_pWhipFactory->Create_Polyline(2, pWtPoints, WD_False));
  ...
  else
  {
    ...
    for (int j = 0; j <= N; ++j)
    {
      ...
      std::unique_ptr<WT_Polyline> line(m_pWhipFactory->Create_Polyline(K, pWtPoints + (j * WD_MAXIMUM_POINT_SET_SIZE), WD_False));
      line->serialize(*getWtFile());
    }
    delete [] pWtPoints;
  }
...
void OdDwfDevice::dc_polygon(OdUInt32 nPoints, const OdGePoint2d* pPoints)
{
  ...
  std::unique_ptr<WT_Polygon> polygon(m_pWhipFactory->Create_Polygon(nPoints, pWtPoints, WD_False));
  polygon->serialize(*getWtFile());

  delete [] pWtPoints;
}

void OdDwfDevice::triangleOut(const OdInt32* vertices, const OdGeVector3d* normals)
{
  if (vertexData()) // CORE-14500
  {
    Od2dExportDevice::triangleOut(vertices, normals);
    return;
  }

  WT_File& file = *getWtFile();
  WT_Rendition& rend = file.desired_rendition();
  const WT_Color& colorOut = rend.color(); // m_color;
  const WT_RGBA32& rgbaOut = colorOut.rgba(); // contains WD_PREFERRED_RGB32 b, g, r, a
  OdCmEntityColor valueOut(ODGETBLUE(rgbaOut.m_whole),
                           ODGETGREEN(rgbaOut.m_whole),
                           ODGETRED(rgbaOut.m_whole));
  OdCmEntityColor value; // = m_effTraits.trueColor(); // = m_pTraits->trueColor();
#if !defined(NDEBUG)
  value = m_effTraits.trueColor(); 
  ODA_ASSERT_ONCE_X(TDWF, !value.isByBlock() && !value.isByLayer()) // test
  if (!value.isByColor())
  {
    if (!value.isByACI()) // CORE-14030 incorrect color in DWF export result of sample.dwg
      value = m_pTraits->secondaryTrueColor(); // for DGN only
    //else // TODO
    ODA_ASSERT_ONCE_X(TDWF, value.isByACI());
    OdUInt16 clrIndex = value.colorIndex();
    ODCOLORREF rgb = getColor(clrIndex);
    value.setRGB(ODGETRED(rgb), ODGETGREEN(rgb), ODGETBLUE(rgb));
  }
  if (value != valueOut)
  {
    // non-default plot style sheet (monochrome.ctb, greyscale.ctb  ...) is using 
    ODA_ASSERT_ONCE_X(TDWF, valueOut.red() == valueOut.green() && valueOut.red() == valueOut.blue());
  }
#endif
  //  better use valueOut than via style sheet name or advanced kMONO like in PDF
  value = valueOut; // CORE-15950 DWF Export Incorrect Colors 

  // CORE-14500 Workaround for Design Review zooming problem

  OdInt32 cntVertexData = vertexDataCount();
  const OdGePoint3d* pVertexList = vertexDataList();

  OdArray<OdCmEntityColor> colors;
  colors.resize(cntVertexData, value);
  OdGiVertexData data;
  data.setTrueColors(colors.getPtr());
  setVertexData(cntVertexData, pVertexList, &data);

  Od2dExportDevice::triangleOut(vertices, normals);

  setVertexData(cntVertexData, pVertexList, NULL); // restore
}
...
void OdDwfDevice::dc_circle(const OdGeEllipArc3d& ellipse)
{
  ...
  std::unique_ptr<WT_Outline_Ellipse> wtEllipse(m_pWhipFactory->Create_Outline_Ellipse(wtCenter, wt_iMajor, wt_iMinor, wtStart, wtEnd, wtTilt));
                                           ...
                                           const OdCmEntityColor* pColors)
{
  ...
  for(int i = 0; i < faceListSize;)
  {
    ...
    {
      // CORE-14500 Workaround for Design Review zooming problem
      ODCOLORREF rgb = ODRGBA(color.red(), color.green(), color.blue(), 255);
      dc_color(rgb);
      std::unique_ptr<WT_Polytriangle> gpt(m_pWhipFactory->Create_Polytriangle(3, vP, WD_True));
      gpt->serialize(*getWtFile());
      continue;
    }
    std::unique_ptr<WT_Gouraud_Polytriangle> gpt(m_pWhipFactory->Create_Gouraud_Polytriangle(3, vP, rgba, WD_True));
...
class AlphaComponentDetector : public OdGiRasterImageWrapper
{
  ...
  ODCOLORREF m_alphaColor; 
public:
  AlphaComponentDetector() : m_alphaColor(ODRGB(0, 0, 0)) { }
  ...
  void setOriginal(const OdGiRasterImage* pOrig)
  {
    ...
    m_pOrig2 = pOrig;
    OdUInt32 len = pOrig->scanLineSize() * pOrig->pixelHeight();
    m_prevData.resize(len);
    pOrig->scanLines(m_prevData.asArrayPtr(), 0, pOrig->pixelHeight());
  }
  void setAlphaColor(ODCOLORREF alphaColor)
  {
  ...
  const OdUInt8* scanLines() const
  {
    ...
    {
      OdUInt8 clr[3];
      if (m_pOrig2->pixelFormat().isBGR() || m_pOrig2->pixelFormat().isBGRA())
        clr[2] = ODGETRED(m_alphaColor), clr[1] = ODGETGREEN(m_alphaColor), clr[0] = ODGETBLUE(m_alphaColor);
      else if (m_pOrig2->pixelFormat().isRGB() || m_pOrig2->pixelFormat().isRGBA())
        clr[0] = ODGETRED(m_alphaColor), clr[1] = ODGETGREEN(m_alphaColor), clr[2] = ODGETBLUE(m_alphaColor);
      else
      {
        ODA_FAIL_ONCE();
      }
      ODA_ASSERT_ONCE(!ODGETALPHA(m_alphaColor));
      ...
      for (OdUInt32 y = 0; y < h; y++)
      {
        ...
        for (OdUInt32 x = 0; x < w; x++, pCur += 3, pPrev += 3, pMerge += 4)
        {
          ...
          pMerge[1] = pPrev[1];
          pMerge[2] = pPrev[2];
          // CORE-13428 //pMerge[3] = (pPrev[0] != pCur[0] || pPrev[1] != pCur[1] || pPrev[2] != pCur[2]) ? 0 : 255; 
          if (pMerge[0] != clr[0] || pMerge[1] != clr[1] || pMerge[2] != clr[2])
          {
            pMerge[3] = 255;
            continue;
          }
          pMerge[0] = pMerge[1] = pMerge[2] = pMerge[3] = 0;
        }
      }
    }
...
static OdGiRasterImagePtr convertImage(const OdGiRasterImage* pImage,
                                       bool bConvertForQualityTexture = false,
                                       OdUInt32 maxResolution = 0,
                                       enum OdRxRasterServices::ImageType toType = OdRxRasterServices::kPNG)
{
  OdGiRasterImagePtr pRes;
  OdRxRasterServicesPtr pSvcs = odrxDynamicLinker()->loadApp(RX_RASTER_SERVICES_APPNAME).get();
  ODA_ASSERT_ONCE(pSvcs.get());
  if (pSvcs.isNull())
    return pRes;

  int idxTransparentColor = -1;
  if (bConvertForQualityTexture)
    idxTransparentColor = pImage->transparentColor();
//#if !defined(DWF_EXPORT_BITONAL_IMAGE_QUALITY_OFF)
  if (pImage->colorDepth() == 1 && idxTransparentColor >= 0)
  {
    // CORE-16506
    // see also OdGsBitmapVectorizeView::initTexture

    ODCOLORREF fgColor = pImage->color(idxTransparentColor ? 0 : 1),
               bgColor = pImage->color(idxTransparentColor);
    if (false)
      bgColor = ODRGBA(ODGETRED(bgColor), ODGETGREEN(bgColor), ODGETBLUE(bgColor), 255);

    bool transparency = (idxTransparentColor >= 0);
    bool bTransparentBitonal = false;
    if (transparency)
      bTransparentBitonal = true; // without it result is OK too
    OdSmartPtr<OdGiBitonalRasterTransformer> pBitRast = OdRxObjectImpl<OdGiBitonalRasterTransformer>::createObject();

    pBitRast->setOriginal(pImage, fgColor, bgColor, bTransparentBitonal);
    ODA_ASSERT_ONCE(idxTransparentColor == pBitRast->transparentColor() || !bTransparentBitonal);

    OdGiRasterImagePtr pBitonalImg = pBitRast;
    pImage = pBitonalImg;

    OdUInt32 pxlWidth = pImage->pixelWidth(),
             pxlHeight = pImage->pixelHeight();
    //if (false)
    {
      if (!maxResolution)
        maxResolution = 2000;
      while (odmax(pxlWidth, pxlHeight) > maxResolution)
      {
        pxlWidth /= 2;
        pxlHeight /= 2;
      }
    ...
    pDesc->setPixelWidth(pxlWidth);
    pDesc->setPixelHeight(pxlHeight);
    ...
    pRes = pImage->convert(true, // bConvertPalettedToRGB
                           50, 50, 0, // brightness, contrast, fade, 
                           bgColor, false, false, false, // bFlipX, bFlipY, bRotate90,
                           pDesc);
    ODA_ASSERT_ONCE(pRes->transparentColor() == -1); // unfortunately it is so (instead idxTransparentColor)
    if (toType != OdRxRasterServices::kJPEG)
      return pRes;
    pImage = pRes;
  }
//#endif // !defined(DWF_EXPORT_BITONAL_IMAGE_QUALITY_OFF)

  const OdUInt32* pFlagsChain = NULL;
  const OdUInt32 flagsChain[3] = { OdRxRasterServices::kJpegQuality, 100, 0 };
  if (toType == OdRxRasterServices::kJPEG)
    pFlagsChain = flagsChain;
  OdMemoryStreamPtr pStreamBuf = OdMemoryStream::createNew();
  if (!pSvcs->convertRasterImage(pImage, toType, pStreamBuf, pFlagsChain))
  {
    ODA_FAIL_ONCE();
    return pRes;
  }
  ...
  pStreamBuf->rewind();
  pRes = pSvcs->loadRasterImage(pStreamBuf);
  ODA_ASSERT_ONCE(pRes.get());
  if (idxTransparentColor >= 0 && pRes->transparentColor() < 0)
  {
    OdExGiRasterImage* pExRes = static_cast<OdExGiRasterImage*>(pRes.get());
    pExRes->setTransparentColor(idxTransparentColor);
  }
                                  ...
                                  double fade)
{
  ...
  OdUInt32 maxResolution = getExportImpl()->getParams().maxRasterResolution();
  OdDbBaseDatabase* pRxDb = getExportImpl()->getParams().database();
  OdDbBaseDatabasePEPtr pDbPe(pRxDb);
  OdGiRasterImagePtr pImg;
  OdUInt32 colorDepth = pImage->colorDepth();
  int idxBackgroundColor = -1, idxTransparentColor = -1;
  if (transparency)
  {
    idxTransparentColor = pImage->transparentColor();
    // problem with DWF2dExport_Image8bpp_CORE14342_YM 0 1 // idxBackgroundColor = idxTransparentColor;
  }
  ODCOLORREF backgroundColor = ODRGB(0, 0, 0);
  if (colorDepth <= 8 && idxTransparentColor >= 0 && idxBackgroundColor >= 0)
    backgroundColor = pImage->color(idxTransparentColor); // (idxBackgroundColor); // CORE-11627
  if (transparency && colorDepth == 1 && idxTransparentColor < 0)
  {
    // CORE-11627
    pImg = pImage->clone();
    idxBackgroundColor = idxTransparentColor = OdGiRasterImage_adjustTransparentColor(pImg);
    if (idxTransparentColor >= 0)
    {
      pImage = pImg.get();
      colorDepth = pImg->colorDepth();
      backgroundColor = pImage->color(idxTransparentColor); // (idxBackgroundColor);
#if !defined(DWF_EXPORT_BITONAL_IMAGE_QUALITY_OFF)
      // CORE-16506 // Possible TODO with new idea
      // also we have low quality render problem with bitonal images.
      // use command BMPOUTBG for attachment to reproduce it.
      // (Andrew says we draw it like ACAD with skipping columns & lines)
      // But now it looks for me like problem is in a texture size 
      // limitation on GL level (1024). But render for pRaster uses only 
      // fragment of it. Therefore the result is not good.
      pImg = convertImage(pImage, true, maxResolution); // workaround for it
      if (pImg.get())
      {
        pImage = pImg.get();
        colorDepth = pImg->colorDepth();
        ODA_ASSERT_ONCE(!pImage->numColors());
      }
#endif
    }
    ODA_ASSERT_VAR(else)
  ...
  OdRxRasterServicesPtr pSvcsToSv;
  if (false)
    pSvcsToSv = odrxDynamicLinker()->loadApp(RX_RASTER_SERVICES_APPNAME).get();
  static std::map<OdString, int> s_map_idx;
  OdString sFileName = pDbPe->getFilename(pRxDb);
  if (s_map_idx.find(sFileName) == s_map_idx.end()) s_map_idx[sFileName] = 0;
  int& idxFile = s_map_idx[sFileName];
  if (pSvcsToSv.get())
  {
    OdString sFileNameByRef = OdString(sFileName);
    pSvcsToSv->saveRasterImage(
      pImage,
      OdString_replace(sFileNameByRef, L".dwg",
        OdString().format(L"_tmp_%02d_pImage.png", idxFile++).c_str()));
  }
#endif

  // CORE-14342 //  transparency = false;          
  ODA_ASSERT_VAR(if (   transparency
                     && !(   colorDepth == 32 // see #8290 Background color for monochrome rasters support // amarkovich  12/30/2010 4:03:10 PM
                          || (colorDepth <= 8 && pImage->transparentColor() >= 0)))) // CORE-14342
    ODA_ASSERT_ONCE_X(TDWF, colorDepth == 1 || colorDepth == 24); // transparency = false;          
  ...
  OdGsDevicePtr pDevice = pDbPe->appServices(pRxDb)->gsBitmapDevice(NULL, pRxDb, OdDbBaseHostAppServices::kFor2dExportRender);
  if (pDevice.isNull())
    throw OdError(OD_T("Can't create Bitmap Device for exporting RasterImage to DWF."));
  ...
  if (!maxResolution)
    maxResolution = 2000; //CORE-13428 // maxResolution = 800;
  while (odmax(pixelWidthNew, pixelHeightNew) > maxResolution)
  {
    pixelWidthNew /= 2;
    pixelHeightNew /= 2;
  }
  ...
  //ODCOLORREF backgroundColor = ODRGB(0, 0, 0);
  if (idxTransparentColor >= 0 && idxBackgroundColor >= 0)
  {
    if (colorDepth <= 8)
      backgroundColor = pImage->color(idxBackgroundColor); // CORE-11627
  }
  else
  if (   !pImage->paletteDataSize() // CORE-13779
      && OdGiRasterImage_isColorPresent(pImage, backgroundColor))
  {
    OdGiRasterImage_isColorPresent(pImage, backgroundColor, true); // CORE-11493 
    backgroundColor = ODRGB(255, 255, 255); // CORE-11493 // = ODRGB(255, 0, 255); // magenta //ODRGB(255,255,255);
    //if (OdGiRasterImage_isColorPresent(pImage, backgroundColor))
    //{
    //  ODA_FAIL_ONCE(); // TODO
    //}
  }

  pDevice->setBackgroundColor(backgroundColor); //ODRGB(0,0,0));

  OdUInt32 numCols = pImage->numColors();
  OdUInt8Array imgPalette;
  imgPalette.resize(pImage->paletteDataSize());

  //if(numCols != 2 || idxTransparentColor >= 0)
  //{
  ...
  if (cd == 32)
    cd = 24;
  pDevice->properties()->putAt(OD_T("BitPerPixel"), OdRxVariantValue(OdUInt32(cd)));
  ...
  ODA_ASSERT_VAR(cd = pRaster->colorDepth();)

  if (colorDepth == 1 && transparency)
  {
    // CORE-16434
    // renew idxBackgroundColor & idxTransparentColor in result of render.
    // it is needed to compensate kernel changes in v 21.5
    int idxTrans = pRaster->transparentColor();
    if (idxTransparentColor != idxTrans)
    {
      // OdGiRasterImage_adjustTransparentColor works with OdExGiRasterImage images only
      // but pReaster contains ExGsBitmapDevice image (pRaster->clone() is useless to fix it)
      pImg = convertImage(pRaster);
      if (pImg.get())
      {
        //int colorDpth = pImg->colorDepth(); // maybe changed
        //ODA_ASSERT_ONCE(colorDpth == 1);
        int idxTrans = pImg->transparentColor();
        if (idxTrans < 0)
        {
          idxBackgroundColor = idxTransparentColor = OdGiRasterImage_adjustTransparentColor(pImg);
          pRaster = pImg;
  ...
  ODA_ASSERT_VAR(if (pSvcsToSv.get()))
  {
    ODA_ASSERT_VAR(OdString tempFileName = pDbPe->getFilename(pRxDb);)
      ODA_ASSERT_VAR(OdString sFileNameByRef = OdString(sFileName);)
    ODA_ASSERT_VAR(
      pSvcsToSv->saveRasterImage(
        pRaster, 
        OdString_replace(sFileNameByRef, L".dwg",
                         OdString().format(L"_tmp_%02d_pRaster.png", idxFile++).c_str()));)
  }

  if (transparency && colorDepth == 32)
  {
    ...
    pDet->setAlphaColor(backgroundColor); // CORE-13428
    pDevice->setBackgroundColor(backgroundColor); // ODRGB(255, 255, 255));
    dbctx->setPlotGeneration(false);
    pDevice->update();
    dbctx->setPlotGeneration(true);
    pRaster = pDet;
    ...
    ODA_ASSERT_VAR(if (pSvcsToSv.get()))
    {
      ...
      ODA_ASSERT_VAR(
        pSvcsToSv->saveRasterImage(
          pDet, 
          OdString_replace(sFileNameByRef, L".dwg",
                           OdString().format(L"_tmp_%02d_pDet.png", idxFile++).c_str()));)
    }
  }
    ...
    ODA_ASSERT_VAR(if (pSvcsToSv.get()))
    {
      ...
      ODA_ASSERT_VAR(
        pSvcsToSv->saveRasterImage(
          pMask, 
          OdString_replace(sFileNameByRef, L".dwg",
                           OdString().format(L"_tmp_%02d_pMask.png", idxFile++).c_str()));)
    }
  }

  dc_raster_image(pRaster, pMask, imgExts, transparency,
                  //idxTransparentColor >= 0 ? idxTransparentColor : 0); // CORE-11627
                  idxBackgroundColor >= 0 ? idxBackgroundColor : 0); // CORE-11627
...
static WT_Color_Map* getWTColorMap(int outClrDepth, OdGiRasterImage* pRaster, WT_Color_Map& wtColorMap, 
                                   bool bBitonalTransp, OdUInt32 idxTransparentColor, WT_File& wtfile)
{
  if (outClrDepth == 32)
    return WD_Null;
  WT_RGBA32 pMap[256];
  ...
  for (OdUInt32 i = 0; i < numColors ; i++)
  {
    ...
    pMap[i] = WT_RGBA32(ODGETRED(col), ODGETGREEN(col), 
      ODGETBLUE(col), (i==idxTransparentColor && bBitonalTransp) ? 0 : 255);       // Add new color
  }
...
void OdDwfDevice::dc_raster_image(OdGiRasterImagePtr pRaster, 
                                  ...
                                  bool bBitonalTransp,
                                  OdUInt32 idxTransparentColor) // = 0
{
  if (IsNonprocessRun()) return;
  push_Url();
  OdUInt32 imageHeight = pRaster->pixelHeight();
  ...
  if (getExportImpl()->getParams().RGBToJpeg() && imageOutFormat == WT_Image::RGB)
  {
    ...
    if (!pRasSvcs.isNull())
    {
      ...
      if (pRasSvcs->convertRasterImage( pRaster, OdRxRasterServices::kJPEG, pStreamBuf))
      {
        ...
        std::unique_ptr<WT_Image> wtImage(m_pWhipFactory->Create_Image((WT_Unsigned_Integer16)imageHeight,(WT_Unsigned_Integer16)imageWidth,
  ...
  else if (imageOutFormat == WT_Image::RGBA && getExportImpl()->getParams().format() == DW_XPS)
  {
    ...
    if (!pRasSvcs.isNull())
    {
      ...
      if (pRasSvcs->convertRasterImage( bgraImage, OdRxRasterServices::kPNG, pStreamBuf))
      {
        ...
        std::unique_ptr<WT_PNG_Group4_Image> wtImage(m_pWhipFactory->Create_PNG_Group4_Image((WT_Unsigned_Integer16)imageHeight, (WT_Unsigned_Integer16)imageWidth,
  ...
  else
  {
    ...
    WT_Color_Map* pWtColorMap = getWTColorMap(outClrDepth, pRaster, wtColorMap, bBitonalTransp, idxTransparentColor, *getWtFile());
    OdUInt8Array outScLns;
...
void OdDwfDevice::dc_pushClip(int nrcContours, const int* nrcCounts, const OdGsDCPointArray &nrcPointsIn)
{
  ...
  ODA_ASSERT_ONCE_X(TDWF, nrcContours);
  bool bMerged = !m_nrcClips.empty();
  ...
  if (bMerged)
  {
    ...
    if (!resClipContours.size())
      bMerged = false; // CORE-16738
  }
  if (bMerged)
  {
  ...
  std::unique_ptr<WT_Viewport> viewPort(classFactory()->Create_Viewport(*getWtFile(), "", contour_set));
                      ...
                      const OdGiEdgeData* pEdgeData, const OdGiFaceData* pFaceData, const OdGiVertexData* pVertexData)
{
  ...
  if (m_bDrawShellAsRaster)
  {
    ...
    if (pixelWidthNew < 40 || pixelHeightNew < 40)
    {
      ...
      if (!pixelHeightNew)
      {
        ODA_FAIL_ONCE_X(TDWF); // with export 10_short.dwg(exploded result of CORE-16191) 
        return;
      }
    }
    OdUInt32 maxResolution = getExportImpl()->getParams().maxRasterResolution();
