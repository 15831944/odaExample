...
/************************************************************************/
OdDgElementIdArray selectEntities(OdDgUserIO* pIO)
{
  OdDgElementIdArray entIds;
  OdDgSelectionSetIteratorPtr it = pIO->select(NULL, L"Select entities",OdEd::kSelAllowInactSpaces)->newIterator();
  while (!it->done())
  {
...
//OdDgElementIdArray selectBlocks(OdDgUserIO* pIO)
//{
//  OdDgElementIdArray recIds;
...
//}
//
//OdDb::DuplicateRecordCloning selectDrcType(OdDgUserIO* pIO)
//{
...
//}
//
//OdDgDatabasePtr selectDestDatabase(OdDgUserIO* pIO, OdDgDatabase* pSrcDb)
//{
//  // Select or create a destination database
//  OdDgDatabasePtr pDestDb;
//  try
//  {
...
//}
//
//void saveClonedDatabase(OdDgUserIO* pIO, OdDgDatabase* pDestDb)
//{
//  if (!pDestDb)
//    return;
//
...
void _EXDC_DEEPCLONE_func(OdEdCommandContext* pCmdCtx)
{
  ...
  OdDgUserIO* pIO = pDbCmdCtx->dbUserIO();

  // Select entities to clone
  OdDgElementIdArray entIds = selectEntities(pIO);
  if (!entIds.size())
    return;

  // Select owner record for cloned entities
  OdString recName = pIO->getString(OD_T("Enter name of Module to clone entities <\"Default\">:"), OdEd::kGstNoEmpty, OD_T("Default"));
  OdDgElementId destRecId = pDb->getModelTable()->getAt(recName);

    //OdDbBlockTablePtr(pDb->getBlockTableId().safeOpenObject())->getAt(recName);
  if (destRecId.isNull())
  {
    ODA_FAIL();
    return;
  ...
  // Clone
  OdDgIdMappingPtr pMapping = OdDgIdMapping::createObject();
...
//  std::map<OdDgElementId, OdDgElementIdArray> clonedEnts;
...
//  pMapping->setDestDb(pDb);
//  std::map<OdDgElementId, OdDgElementIdArray>::iterator mapIt = clonedEnts.begin();
...
//  OdDgElementId destRecId = OdDbBlockTablePtr(pDb->getBlockTableId().safeOpenObject())->getAt(recName);
...
//
//  // Select or create a destination database
//  OdDgDatabasePtr pDestDb = selectDestDatabase(pIO, pDb);
//
//  // Clone
//  OdDbIdMappingPtr pMapping = OdDbIdMapping::createObject();
...
//  OdDgElementIdArray objIds = selectBlocks(pIO);
//  if (!objIds.size())
//    return;
//
//  // Select type of DuplicateRecordCloning
...
//    );
//  OdDgDatabasePtr pSrcDb = pDb->appServices()->readFile(fname);
//  if (pSrcDb.get())
//  {
