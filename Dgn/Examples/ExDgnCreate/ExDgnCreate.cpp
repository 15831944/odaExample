...
#endif
{
  ...
  if( argc < 2 )
  {
    //oddgPrintConsoleString(L"ExDgnCreate sample program. Copyright (c) 20019, Open Design Alliance\n");
    oddgPrintConsoleString(L"\tusage: ExDgnCreate <filename> [-DO] [MAT_SS3]");
    oddgPrintConsoleString(L"\n\t-DO disables progress meter output.\n");
    oddgPrintConsoleString(L"\nPress ENTER to continue...\n");
    STD(cin).get();
    return 1;
  }
  STD(cout) << "\nExDgnCreate sample program. Copyright (c) 2019, Open Design Alliance\n";
  ...
  if (argc >= 3)
  {
    ...
    else if( !odStrICmp(argv2, OD_T("MAT_SS3")) )
    {
      bCreateSS3Material = true;
    }
  }

  if(argc >= 4)
  {
    if( !odStrICmp(argv2, OD_T("MAT_SS3")) )
    {
      bCreateSS3Material = true;
    }
  }
  ...
  try
  {
    ...
    OdStringArray arrayRscList;
    OdString sRscFontPath( L"font.rsc" );
    OdString sRscLSylePath( L"lstyle.rsc" );
    OdString sRscPath("C:\\ProgramData\\Bentley\\MicroStation V8i (SELECTseries)\\WorkSpace\\System\\Symb\\font.rsc");
    arrayRscList.append( sRscPath );
    arrayRscList.append( sRscLSylePath );
    ...
    filler.enableCreationOfSS3Materials( bCreateSS3Material );
