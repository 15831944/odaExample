...
//----------------------------------------------------------
//
// OdDgElementCompositeIterator
//
//----------------------------------------------------------

ODRX_CONS_DEFINE_MEMBERS(OdDgElementCompositeIterator, OdDgElementIterator, ABSTRACTIMPL_CONSTR)

void OdDgElementCompositeIterator::addIterator(OdDgElementIteratorPtr pIter)
{
  m_uActualIterator = 0;
  m_pIterArr.push_back(pIter);
}

//===========================================================================

void OdDgElementCompositeIterator::start(bool atBeginning, bool skipErased )
{
  m_uActualIterator = 0;

  if( m_pIterArr.isEmpty() )
    m_pIterArr[0]->start(atBeginning, skipErased);
}

//===========================================================================

bool OdDgElementCompositeIterator::done() const
{
  bool bRet = false;

  if( m_uActualIterator == m_pIterArr.size() - 1 )
    bRet = m_pIterArr.last()->done();
...
OdDgElementId OdDgElementCompositeIterator::item() const
{
  return m_pIterArr[m_uActualIterator]->item();
}

//===========================================================================

void OdDgElementCompositeIterator::step(bool forward, bool skipErased)
{
  if( forward )
  {
    m_pIterArr[m_uActualIterator]->step(forward, skipErased);

    if (m_pIterArr[m_uActualIterator]->done() && m_uActualIterator < m_pIterArr.size() - 1)
    {
      m_uActualIterator++;
      m_pIterArr[m_uActualIterator]->start(true, skipErased);
    }
  }
  else
  {
    OdDgElementId idLast = m_pIterArr[m_uActualIterator]->item();
    m_pIterArr[m_uActualIterator]->step(forward, skipErased);
    OdDgElementId idCur  = m_pIterArr[m_uActualIterator]->item();

    if (idLast == idCur)
    {
      if (m_uActualIterator > 0)
      {
        m_uActualIterator--;
        m_pIterArr[m_uActualIterator]->start(false, skipErased);
      }
    }
  }
}
...
bool OdDgElementCompositeIterator::seek(const OdDgElementId& objectId)
{
  bool bRet = false;

  for (OdUInt32 i = 0; i < m_pIterArr.size(); i++)
  {
    bRet = m_pIterArr[i]->seek(objectId);
...
void OdDgElementDumperPE::dumpData( OdRxObjectPtr pObj, OdExDgnDumper* pDumper ) const
{
  ...
  try
  {
    ...
    pDumper->writeFieldValue("LastModifiedTime", tmp);
  }
  catch (...)
  {
      ...
      switch ( primaryId )
      {
        ...
        case OdDgAttributeLinkage::kFillStyle:
        {
          ...
          if( !pGradient.isNull() )
          {
            ...
            {
              OdDgGradientKey key = pGradient->getKey(k);
        ...
        case OdDgAttributeLinkage::kAssocRegion:
        {
          OdDgAssocRegionLinkagePtr pAssocRgnLinkage = pLinkage;
          pDumper->writeFieldValue( "  Assoc Region Type", pAssocRgnLinkage->getRegionType() );
          pDumper->writeFieldValue( "  Interior Shapes Mode", pAssocRgnLinkage->getFillModeInteriorShapes() );
          pDumper->writeFieldValue( "  Locate Interior Texts", pAssocRgnLinkage->getLocateInteriorTextsFlag() );
          pDumper->writeFieldValue( "  Max Gap", pAssocRgnLinkage->getFillModeMaxGap() );
        }
        break;

        case OdDgAttributeLinkage::kSharedCellFlags:
        {
          OdDgSharedCellFlagsLinkagePtr pCellLinkage = pLinkage;
          pDumper->writeFieldValue("  Annotation Scale Flag", pCellLinkage->getAnnotationScaleFlag());
        }
        break;

        case OdDgAttributeLinkage::kXmlLinkage:
        {
          OdDgXmlLinkagePtr pXmlLinkage = pLinkage;
          pDumper->writeFieldValue( "  Name", pXmlLinkage->getName() );
          pDumper->writeFieldValue( "  Xml Data", pXmlLinkage->getXmlData() );
        }
        break;
        ...
        case OdDgAttributeLinkage::kByteArray:
        {
          ...
          for (OdUInt32 n = 0; n < data.size(); n++)
          {
            OdString strByte;
            strByte.format(L"0x%02X, ", data[n]);
            ...
            {
              OdString strFieldName;
              strFieldName.format(L"Bytes [%d]", uRow);

              if (n == data.size() - 1)
                strValues = strValues.left(strValues.getLength() - 2);
              else
                strValues = strValues.left(strValues.getLength() - 1);

              pDumper->writeFieldValue(strFieldName, strValues);
              strValues.empty();
              uCount = 0;
              uRow++;
            }
        ...
        case OdDgAttributeLinkage::kHatch:
        {
          ...
          OdDgHatchPatternPtr pHatch = pPatternLinkage->getHatchPattern();

          if( !pHatch.isNull() )
          {
            pDumper->writeFieldValue( "  Pattern Type", (OdUInt32)pHatch->getType() );
            if( pHatch->getUseOffsetFlag() )
            {
              OdGePoint3d offset;
              pHatch->getOffset( offset );
              pDumper->writeFieldValue( "  offset", offset );
            }

            if( pHatch->getType() == OdDgHatchPattern::kDWGPattern )
            {
              OdDgDWGHatchPatternPtr pDWGPattern = pHatch;

              pDumper->dumpFieldName(L"  DWG hatch:");

              const OdDgDWGHatchPattern::DWGHatch& hatch = pDWGPattern->getHatch();
              ...
              {
                const DWGHatchLine& hatchLine = hatch[k];
            ...
            else if( pHatch->getType() == OdDgHatchPattern::kSymbolPattern )
            {
              OdDgSymbolHatchPatternPtr pSymPattern = pHatch;
  ...
  if ( xattributes.size() > 0 )
  {
    for (unsigned i = 0; i < xattributes.size(); ++i)
    {
      ...
      if( !pGeomCacheXAttr.isNull() )
      {
        ...
        {
          pDumper->writeFieldValue(OD_T("    "), pGeomCacheXAttr->getAction(i), pElm->database());
        }
      }
      ...
      OdDgElementSchemaXAttributePtr pTableXAttr = OdDgElementSchemaXAttribute::cast(pXAttribute);

      if( !pTableXAttr.isNull() )
      {
        pDumper->writeFieldValue(OD_T("  Table Type"), pTableXAttr->getTableType() );

        OdDgElementSchemaPtr pSchema = pTableXAttr->getSchema( pElm->database() );
        ...
        else
        {
          ...
          {
            case OdDgElementSchema::kSnippedText:
            {
            ...
            case OdDgElementSchema::kPCMCell:
            {
            ...
            case OdDgElementSchema::kPCMCellDef:
            {
            ...
            case OdDgElementSchema::kPrototypeDescription:
            {
            ...
            case OdDgElementSchema::kSchemaPrototype:
            {
            ...
            case OdDgElementSchema::kHeaderPrototype:
            {
            ...
            case OdDgElementSchema::kMergeData:
            {
            ...
            case OdDgElementSchema::kSymbology:
            {
            ...
            case OdDgElementSchema::kEdgeRun:
            {
            ...
            case OdDgElementSchema::kRowData:
            {
            ...
            case OdDgElementSchema::kColumnData:
            {
            ...
            case OdDgElementSchema::kCellData:
            {
            ...
            case OdDgElementSchema::kTableData:
            {
            ...
            case OdDgElementSchema::kPrototypeBasedSchema:
            {
              OdDgPrototypeBasedElementSchemaPtr pPrototypeBasedData = pSchema;
      ...
      OdDgECRelationshipXAttributePtr pRelationshipXAttr = OdDgECRelationshipXAttribute::cast(pXAttribute);

      if (!pRelationshipXAttr.isNull())
      {
        pDumper->writeFieldValue("  Number Of Records", pRelationshipXAttr->getRealtionshipNodeCount() );
        pDumper->writeFieldValueHex("  Flags", pRelationshipXAttr->getFlags() );
        pDumper->writeFieldValue("  - Extra Data Flag", pRelationshipXAttr->getExtDataFlag());
        pDumper->writeFieldValue("  - Extra Data 2 Flag", pRelationshipXAttr->getExtData2Flag());
        pDumper->writeFieldValue("  - Save Size Flag", pRelationshipXAttr->getRecordSizeFlag());
        pDumper->writeFieldValue("  - ECSchema Id Flag", pRelationshipXAttr->getECSchemaIdFlag());
        pDumper->writeFieldValue("  - Object Name Flag", pRelationshipXAttr->getObjectNameStrFlag());

        if (pRelationshipXAttr->getHeaderXAttrId() != 0)
        {
          pDumper->writeFieldValue("  = Header XAttr Id", pRelationshipXAttr->getHeaderXAttrId());
        }
        else
        {
          pDumper->writeFieldValue("  = ECSchema Name", pRelationshipXAttr->getECSchemaName());
          pDumper->writeFieldValue("  = ECSchema Version", pRelationshipXAttr->getECSchemaVersion());
          pDumper->writeFieldValue("  = ECSchema SubVersion", pRelationshipXAttr->getECSchemaSubVersion());

          if( pRelationshipXAttr->getECSchemaIdFlag() )
            pDumper->writeFieldValueHex("  = ECSchema Id", pRelationshipXAttr->getECSchemaId());

          pDumper->writeFieldValue("  = ECRelationshipClass Name", pRelationshipXAttr->getECRelationshipClassName());
          pDumper->writeFieldValue("  = Relationship Type", pRelationshipXAttr->getRelationshipType() == OdDgECRelationshipXAttribute::kSource ? OdString(L"kSource") : OdString(L"kTarget"));
          pDumper->writeFieldValue("  = Handler Name", pRelationshipXAttr->getHandlerName());
        }

        for( OdUInt32 i = 0; i < pRelationshipXAttr->getRealtionshipNodeCount(); i++ )
        {
          OdDgRelationshipNodePtr pCurNode = pRelationshipXAttr->getRelationshipNode(i);

          OdString strRecordNum;
          OdString strRecordData;
          strRecordNum.format(L" -> Relationship Node %d :", i);
          pDumper->writeFieldValue(strRecordNum, pCurNode);
        }
      }
      ...
      OdDgPdfLayerVisibilityDifXAttributePtr pPdfLayerVisibilityXAttr = OdDgPdfLayerVisibilityDifXAttribute::cast(pXAttribute);

      if( !pPdfLayerVisibilityXAttr.isNull() )
      {
        for( OdUInt32 n = 0; n < pPdfLayerVisibilityXAttr->getLayerDifCount(); n++ )
        {
          OdString strLevelName;
          OdUInt32 uLayerIndex = 0;
          bool     bVisible;

          pPdfLayerVisibilityXAttr->getLayerDifStatus(n, uLayerIndex, bVisible );
          strLevelName.format(L"  %d",uLayerIndex);
          pDumper->writeFieldValue( strLevelName, bVisible );
        }
      }

      OdDgAcsXAttributePtr pAcsXAttr = OdDgAcsXAttribute::cast(pXAttribute);

      if( !pAcsXAttr.isNull() )
      {
        pDumper->writeFieldValue(L"  Acs Type", pAcsXAttr->getAcsType() );
        pDumper->writeFieldValue(L"  Acs Origin", pAcsXAttr->getOrigin() );
        pDumper->writeFieldValue(L"  Acs Rotation", pAcsXAttr->getRotation() );
        pDumper->writeFieldValue(L"  Acs View Independent", pAcsXAttr->getViewIndependentFlag() );
        pDumper->writeFieldValueHex(L"  Acs Id", pAcsXAttr->getAcsElementId() );
      }
...
OdString OdDgElementDumperPE::getName( OdRxObjectPtr pObj ) const
{
  ...
  if (pObj->isKindOf(OdDgElement::desc()))
  {
    ...
    if( !pElm->ownerId().isNull() )
      s.format(L"0x%04X - <%ls>", (OdUInt32)(pElm->elementId().getHandle()), pObj->isA()->name().c_str());
    else
...
void OdDgDatabaseDumperPE::dumpData( OdRxObjectPtr pObj, OdExDgnDumper* pDumper ) const
{
  ...
  pDumper->writeFieldValue( "Active level", pDb->getActiveLevelEntryId() );
	...
	if(pDb->getActiveViewGroupId().isNull() )
		pDumper->writeFieldValueHex("Active Viewgroup", OdUInt64(0) );
	else
		pDumper->writeFieldValueHex( "Active Viewgroup", pDb->getActiveViewGroupId().getHandle() );

	if (pDb->recommendActiveViewGroupId().isNull())
		pDumper->writeFieldValueHex("Recommended Viewgroup", OdUInt64(0));
	else
		pDumper->writeFieldValueHex("Recommended Viewgroup", pDb->recommendActiveViewGroupId().getHandle() );
...
void OdDgModelDumperPE::dumpData( OdRxObjectPtr pObj, OdExDgnDumper* pDumper ) const
{
  ...
  // Annotation Scale
  pDumper->writeFieldValue("Propagate annotation scale falg", element->getPropagateAnnotationScaleOffFlag());
  pDumper->writeFieldValue("Annotation scale", element->getAnnotationScale());

  // Sheet settings

  if( element->isKindOf(OdDgSheetModel::desc()) )
  {
    OdDgSheetModelPtr pSheetModel = element;

    pDumper->writeFieldValue( "Paper Name", pSheetModel->getSheetPaperName() );
    pDumper->writeFieldValue( "Paper Width", pSheetModel->getSheetWidth() );
    pDumper->writeFieldValue( "Paper Height", pSheetModel->getSheetHeight() );
    pDumper->writeFieldValue( "Paper Left Margin", pSheetModel->getSheetLeftMargin() );
    pDumper->writeFieldValue( "Paper Right Margin", pSheetModel->getSheetRightMargin() );
    pDumper->writeFieldValue( "Paper Top Margin", pSheetModel->getSheetTopMargin() );
    pDumper->writeFieldValue( "Paper Bottom Margin", pSheetModel->getSheetBottomMargin() );
    pDumper->writeFieldValue( "Paper Rotation", pSheetModel->getSheetRotation() );
    pDumper->writeFieldValue( "Paper Origin", pSheetModel->getSheetOffset() );
    OdGeExtents3d extPaper;
    pSheetModel->getSheetExtents(extPaper);
    pDumper->writeFieldValue("Paper Extents", extPaper);
    //paper unit
    {
      OdDgModel::UnitDescription description = pSheetModel->getSheetUnits();

      OdString unitName = L"Paper unit ";
      pDumper->writeFieldValue( unitName + "Base", OdString( description.m_base == OdDgModel::kMeter ? "kMeter" : "kNone" ) );
      pDumper->writeFieldValue( unitName + "System", toStr(description.m_system) );
      pDumper->writeFieldValue( unitName + "Nominator", description.m_numerator );
      pDumper->writeFieldValue( unitName + "Denominator", description.m_denominator );
...
void OdDgLevelTableRecordDumperPE::dumpData( OdRxObjectPtr pObj, OdExDgnDumper* pDumper ) const
{
  ...
  if( pLevel->hasLevelNameLastModificationTime() )
    pDumper->writeFieldValue("LevelNameLastModificationTime", pLevel->getLevelNameLastModificationTime());
  ...
  pDumper->writeFieldValue( "External", pLevel->getIsExternalFlag());
...
void OdDgMaterialTableDumperPE::dumpData( OdRxObjectPtr pObj, OdExDgnDumper* pDumper ) const
{
  ...
  for( OdUInt32 i = 0; i < pTablePE->getMaterialAssignmentCount(pTable); i++ )
  {
    ...
    for( OdUInt32 j = 0; j < matAssign.getLevelAssignmentCount(); j++ )
    {
      ...
      for( k = 0; k < matAssign.getLevelAssignment(j).getColorRgbAssignmentCount(); k++ )
      {
        ODCOLORREF curColor = matAssign.getLevelAssignment(j).getColorRgbAssignmentByIndex(k);
        OdString strData;
        strData.format(L"{%02x,%02x,%02x},",ODGETRED(curColor), ODGETGREEN(curColor), ODGETBLUE(curColor) );
        strValue += strData;
      }
...
void OdDgFontTableDumperPE::dumpData( OdRxObjectPtr pObj, OdExDgnDumper* pDumper ) const
{
  ...
  OdUInt32Array     arrRscNumbers;
  OdArray<OdString> arrRscNames;

  if( pTable->getLoadedRscFontInfo( arrRscNumbers, arrRscNames ) )
  {
    pDumper->writeFieldValue( L"-----------------", OdString(L"------------------"));
    pDumper->writeFieldValue( L"Rsc Font Name", OdString(L"Rsc Font Id"));
    pDumper->writeFieldValue( L"-----------------", OdString(L"------------------"));

    for( OdUInt32 i = 0; i < arrRscNumbers.size(); i++ )
      pDumper->writeFieldValue( arrRscNames[i], arrRscNumbers[i] );
  }
}
...
void OdDgDimStyleTableRecordDumperPE::dumpData( OdRxObjectPtr pObj, OdExDgnDumper* pDumper ) const
{
  ...
  pDumper->writeFieldValue( "  Stacked fraction flag", element->getStackedFractionFlag());
...
//  pDumper->writeFieldValue( "Secondary sub units", element->getSecondarySubUnits());
  ...
  pDumper->dumpFieldName("----- Note dimension properties -----");
  pDumper->writeFieldValue("Note spline fit flag", element->getNoteSplineFitFlag());
  pDumper->writeFieldValue("Note termitator", element->getNoteTerminator());
  pDumper->writeFieldValue("Note frame type", element->getNoteFrameType());
  pDumper->writeFieldValue("Note frame scale", element->getNoteFrameScale());
  pDumper->writeFieldValue("Note leader length", element->getNoteLeaderLength());
  pDumper->writeFieldValue("Note left margin", element->getNoteLeftMargin());
  pDumper->writeFieldValue("Note lower margin", element->getNoteLowerMargin());
  pDumper->writeFieldValue("Note text rotation", element->getNoteTextRotation());
  pDumper->writeFieldValue("Note hor attach direction", element->getNoteHorizontalAttachmentLeaderDirection());
  pDumper->writeFieldValue("Note edit about", element->getNoteEditAbout());
  pDumper->writeFieldValue("Note justification", element->getNoteJustification());
  pDumper->writeFieldValue("Note left alignment", element->getNoteLeftAttachmentAlignment());
  pDumper->writeFieldValue("Note right alignment", element->getNoteRightAttachmentAlignment());
  pDumper->writeFieldValue("Note enable frame scale flag", element->getEnableNoteFrameScaleFlag());
  pDumper->writeFieldValue("Note inline leader flag", element->getNoteInlineLeaderFlag());
}
...
void OdDgVariablesListPE::dumpData( OdRxObjectPtr pObj, OdExDgnDumper* pDumper ) const
{
  ...
  if( !pList->getItemTypeLibraryId().isNull() )
  {
    ...
    if (!pCurItemType.isNull() && pCurItemType->getPropertyInstanceCount() > 0)
    {
      pDumper->dumpFieldName(L"Item Type Defaults:");
      OdString strPropName = L" - ";
      pDumper->writeFieldValue(strPropName, pCurItemType);
    }
  }
}
...
void OdDgVariableValuesSetPE::dumpData( OdRxObjectPtr pObj, OdExDgnDumper* pDumper ) const
{
  ...
  OdDgECClassInstancePtr pCurItemType = pSet->getItemTypeInstance();
...
void OdDgPrototypeElementPE::dumpData( OdRxObjectPtr pObj, OdExDgnDumper* pDumper ) const
{
  OdDgElementDumperPE::dumpData(pObj, pDumper);
  OdDgPrototypeElementPtr pTablePrototype = pObj;
    ...
    for( OdUInt32 j = 0; j < curSchema.getSchemaItemCount(); j++ )
    {
      ...
      if( !curItem.getProxyDataFlag() )
      {
        switch( curItem.getItemDataType() )
        {
          ...
          case OdDgPrototypeSchemaItem::kFlags64:
          {
            if (j != 0)
              strType.format(L"  - kItemGroup (%d)", j);
            else
...
void OdDgViewGroupDumperPE::dumpData( OdRxObjectPtr pObj, OdExDgnDumper* pDumper ) const
{
  ...
  OdBinaryData displayOrder = element->getViewDisplayOrder();

  OdString strDisplayOrders;

  for (OdUInt32 i = 0; i < 8; i++)
  {
    OdString strOrder;
    strOrder.format(L"%d, ", displayOrder[i]);
    strDisplayOrders += strOrder;
  }

  strDisplayOrders = strDisplayOrders.left(strDisplayOrders.getLength() - 2);

  pDumper->writeFieldValue("View Order", strDisplayOrders);
}
...
void OdDgViewDumperPE::dumpData( OdRxObjectPtr pObj, OdExDgnDumper* pDumper ) const
{
  ...
  pDumper->writeFieldValueHex("Acs Element Id", (OdUInt64)(element->getAcsId().getHandle()) );
  pDumper->writeFieldValue("Acs Type", element->getAcsType() );
  pDumper->writeFieldValue("Acs Origin", element->getAcsOrigin() );
  pDumper->writeFieldValue("Acs Rotation", element->getAcsRotation() );
  pDumper->writeFieldValue("Acs View Independent Flag", element->getAcsViewIndependentFlag() );
}
...
void OdDgTagDefinitionSetDumperPE::dumpData( OdRxObjectPtr pObj, OdExDgnDumper* pDumper ) const
{
  ...
  for( OdUInt32 i = 0; i < j; i++ )
  {
    ...
    if( !pTagDefinitionsSet->getByIndex(i).isNull() && pTagDefinitionsSet->getByIndex(i)->isKindOf(OdDgTagDefinition::desc()) )
    {
      OdDgTagDefinitionPtr pTagDef = pTagDefinitionsSet->getByIndex(i);
      pDumper->dumpFieldName(strFieldName);
      pDumper->writeFieldValue(L"    EntryId", pTagDef->getEntryId());
      pDumper->writeFieldValue(L"    Name", pTagDef->getName());
      pDumper->writeFieldValue(L"    Prompt", pTagDef->getPrompt());
      pDumper->writeFieldValue(L"    Type", pTagDef->getType());
      pDumper->writeFieldValue(L"    Variable Flag", pTagDef->getVariableFlag());
      pDumper->writeFieldValue(L"    Default Flag", pTagDef->getDefaultFlag());
      pDumper->writeFieldValue(L"    Confirm Flag", pTagDef->getConfirmFlag());
      pDumper->writeFieldValue(L"    Display Tag Flag", pTagDef->getDisplayTagFlag());

      switch(pTagDef->getType() )
      { 
        case OdDgTagDefinition::kChar:
          pDumper->writeFieldValue(L"    Default Value", pTagDef->getString()); break;
        case OdDgTagDefinition::kInt16:
          pDumper->writeFieldValue(L"    Default Value", pTagDef->getInt16()); break;
        case OdDgTagDefinition::kInt32:
          pDumper->writeFieldValue(L"    Default Value", pTagDef->getInt32()); break;
        case OdDgTagDefinition::kDouble:
          pDumper->writeFieldValue(L"    Default Value", pTagDef->getDouble()); break;
      }
    }
  }
...
OdString OdDgECSchemaDumperPE::getName(OdRxObjectPtr pObj) const
{
  OdDgECSchemaPtr pSchema = pObj;

  return convertECSchemaItemNameToString(pSchema->getName()) + L"." + pSchema->getVersion();
}

void OdDgECSchemaDumperPE::dumpData(OdRxObjectPtr pObj, OdExDgnDumper* pDumper) const
{
  OdDgElementDumperPE::dumpData(pObj, pDumper);

  OdDgECSchemaPtr pSchema = pObj;

  if( pSchema->getCorruptedDataFlag() )
  {
    pDumper->dumpFieldName("Schema data is corrupted!");
    return;
  }

  pDumper->writeFieldValue("Name", convertECSchemaItemNameToString(pSchema->getName()));
  pDumper->writeFieldValue("Version", pSchema->getVersion());
  pDumper->writeFieldValue("NamespacePrefix", convertECSchemaItemNameToString(pSchema->getNameSpacePrefix()));
  pDumper->writeFieldValue("Description", pSchema->getDescription());
  pDumper->writeFieldValue("DisplayLabel", pSchema->getDisplayName());
  pDumper->dumpFieldName(L" ");
  pDumper->writeFieldValue("===========================", OdString(L"====== Schema References =============="));
  pDumper->dumpFieldName(L" ");

  for( OdUInt32 i = 0; i < pSchema->getReferenceCount(); i++ )
  {
    OdDgECSchemaReference refSchema = pSchema->getReference(i);
    OdString strRefName = convertECSchemaItemNameToString(refSchema.getName());
    OdString strRefPrefix = convertECSchemaItemNameToString(refSchema.getPrefix());
    pDumper->writeFieldValue(L" - " + strRefName, strRefName + L"." + refSchema.getVersion() + L" ( " + strRefPrefix + L" )");
  }

  pDumper->dumpFieldName(L" ");

  if( pSchema->getCustomAttributeCount() > 0 )
  {
    pDumper->writeFieldValue("===========================", OdString(L"====== Schema Custom Attributes =============="));
    pDumper->dumpFieldName(L" ");

    for (OdUInt32 j = 0; j < pSchema->getCustomAttributeCount(); j++)
      pDumper->writeFieldValue(" - ", pSchema->getCustomAttribute(j));
    
    pDumper->dumpFieldName(L" ");
  }

  if( pSchema->getClassCount() > 0 )
  {
    pDumper->writeFieldValue("===========================", OdString(L"====== Classes ======="));

    for (OdUInt32 j = 0; j < pSchema->getClassCount(); j++)
      pDumper->writeFieldValue(" - ", pSchema->getClass(j));
  }
}
...
void OdDgItemTypeLibraryDumperPE::dumpData( OdRxObjectPtr pObj, OdExDgnDumper* pDumper ) const
{
  ...
  pDumper->writeFieldValue( "Name", convertECSchemaItemNameToString(pItemLib->getName()) );
  ...
  else
  {
    ...
    for( OdUInt32 i = 0; i < pItemLib->getItemTypeCount(); i++ )
    {
      ...
      pDumper->writeFieldValue(strName, convertECSchemaItemNameToString(pItem->getName()));
      ...
      if( pItem->getPropertyCount() > 0 )
      {
        ...
        for( OdUInt32 j = 0; j < pItem->getPropertyCount(); j++ )
        {
          ...
          OdString strTypeName = convertECSchemaItemNameToString(pProperty->getTypeName());
          ...
          pDumper->writeFieldValue(strTypeName, convertECSchemaItemNameToString(pProperty->getName()) );

          if( !pProperty->getDescription().isEmpty() )
          {
            pDumper->writeFieldValue(L"         description", convertECSchemaItemNameToString(pProperty->getDescription()));
          }
...
void OdDgXMLCustomSchemaContainerDumperPE::dumpData( OdRxObjectPtr pObj, OdExDgnDumper* pDumper ) const
{
  ...
  pDumper->writeFieldValue( "Name", convertECSchemaItemNameToString(pContainer->getName()) );
...
void OdDgTagDefinitionDumperPE::dumpData( OdRxObjectPtr pObj, OdExDgnDumper* pDumper ) const
{
  ...
  pDumper->writeFieldValue( "Variable Flag", element->getVariableFlag() );
  pDumper->writeFieldValue( "Default Flag", element->getDefaultFlag());
  pDumper->writeFieldValue( "Confirm Flag", element->getConfirmFlag());
  pDumper->writeFieldValue( "Display Tag Flag", element->getDisplayTagFlag());
...
void OdDgColorTableDumperPE::dumpData( OdRxObjectPtr pObj, OdExDgnDumper* pDumper ) const
{
  ...
  if( OdDgColorTable::getExtendedColorCount( pDb ) > 0 )
  {
    pDumper->dumpFieldName("Extended Colors:");

    for( OdUInt32 iColor = 0; iColor < OdDgColorTable::getExtendedColorCount( pDb ); iColor++ )
    {
      OdUInt32 uColorIndex = OdDgColorTable::getExtendedColorIndex(pDb, iColor);
      OdDgCmColor extColor;
      OdDgColorTable::getExtendedColor( pDb, uColorIndex, extColor );

      OdString strColorName;
      OdString strColorValue;
      strColorName.format(OD_T(" - Color 0x%X"), uColorIndex );
      strColorValue.format(OD_T("(%d,%d,%d)"), extColor.red(), extColor.green(), extColor.blue() );

      if( !extColor.colorName().isEmpty() )
        strColorValue = extColor.colorName() + OD_T(" ") + strColorValue;

      pDumper->writeFieldValue(strColorName, strColorValue);
    }
  }
}
...
void OdDgReferenceAttachmentHeaderDumperPE::dumpData( OdRxObjectPtr pObj, OdExDgnDumper* pDumper ) const
{
  ...
  OdDgReferenceAttachCDAPE* pXRefCDAPE = OdDgReferenceAttachCDAPE::cast(element);

  if( pXRefCDAPE )
  {
    OdDgElementId idModelForProps = pXRefCDAPE->getModelIdForPropertyCollections(element);

    pDumper->writeFieldValueHex("ModelId for properties", idModelForProps.isNull() ? OdUInt64(0) : (OdUInt64)(idModelForProps.getHandle()) );
  }
  ...
  pDumper->writeFieldValue( "ManipulateAsElementFlag", element->getManipulateAsElementFlag() );
...
void OdDgCurveElementDumperPE::dumpData( OdRxObjectPtr pObj, OdExDgnDumper* pDumper ) const
{
  OdDgGraphicsElementDumperPE::dumpData(pObj, pDumper);
  ...
  if( pElm->isKindOf( OdDgCurveElement2d::desc()) )
  {
    OdDgCurveElement2dPtr pCurve2d = pElm;
    lsModifiers     = pCurve2d->getLineStyleModifiers();
    dLineStyleScale = pCurve2d->getLineStyleScale();
  }
  else if( pElm->isKindOf( OdDgCurveElement3d::desc()) )
  {
    OdDgCurveElement3dPtr pCurve3d = pElm;
    lsModifiers     = pCurve3d->getLineStyleModifiers();
    dLineStyleScale = pCurve3d->getLineStyleScale();
  }

  pDumper->writeFieldValue( "Line Style Modifiers:", lsModifiers );
  pDumper->writeFieldValue( " - Line Style Scale", dLineStyleScale );
}
...
{
  OdDgCurveElementDumperPE::dumpData(pObj, pDumper);
...
void OdDgRasterFrameDumperPE::dumpData( OdRxObjectPtr pObj, OdExDgnDumper* pDumper ) const
{
  ...
  pDumper->writeFieldValue(L"Page Number", element->getPageNumber() );
  ...
  OdUInt32 nPdfLayers = element->getPdfLayerCount();

  if( nPdfLayers > 0 )
  {
	  pDumper->dumpFieldName(L"Visibility of Pdf Layers:");

	  for (OdUInt32 j = 0; j < nPdfLayers; j++)
	  {
		  OdString strLayerName   = element->getPdfLayerName(j);
		  bool	   bVisibleStatus = element->isPdfLayerVisible(strLayerName);
		  strLayerName = L" - " + strLayerName;
		  pDumper->writeFieldValue(strLayerName, bVisibleStatus);
	  }
  }
}
...
//#pragma MARKMESSAGE("TODO: Examples. OdDgText2d interface was modified.");
  ...
  pDumper->writeFieldValue( "EndOfLine", element->getCrCount() );
  ...
  if( element->getTextIndentationOverrideFlag() )
  {
    pDumper->writeFieldValue( "FirstLineIndentation Override", element->getFirstLineIndentationOverride() );
    pDumper->writeFieldValue( "ParagraphIndentation Override", element->getParagraphIndentationOverride() );

    if( !element->getTabStopArrayOverride().isEmpty() )
      pDumper->writeFieldValue( "TabStopArray Override", element->getTabStopArrayOverride());
  }

  OdString strSpecial;
  OdArray<OdDgTextExtendedProperties::OdDgSpecialSymbol> arrSpecial = element->getSpecialSymbolsAfterText();

  for( OdUInt32 k = 0; k < arrSpecial.size(); k++ )
  {
    if( arrSpecial[k] == OdDgTextExtendedProperties::kTab )
      strSpecial += L"\\t";
    else if( arrSpecial[k] == OdDgTextExtendedProperties::kParagraph )
      strSpecial += L"\\p";
    else if( arrSpecial[k] == OdDgTextExtendedProperties::kNewLine )
      strSpecial += L"\\n";
  }

  if( !strSpecial.isEmpty() )
    pDumper->writeFieldValue( "SpecialSymbolsAfterText", strSpecial );
}
...
void OdDgTextNode2dDumperPE::dumpData( OdRxObjectPtr pObj, OdExDgnDumper* pDumper ) const
{
  ...
  */
  ...
  pDumper->writeFieldValue( "FirstLineIndentation", element->getFirstLineIndentation() );
  pDumper->writeFieldValue( "ParagraphIndentation", element->getParagraphIndentation() );

  if( !element->getTabStopArray().isEmpty() )
    pDumper->writeFieldValue( "TabStopArray", element->getTabStopArray());

  OdString strSpecial;
  OdArray<OdDgTextExtendedProperties::OdDgSpecialSymbol> arrSpecial = element->getTextNodeStartSpecialSymbols();
  ...
  if( !strSpecial.isEmpty() )
    pDumper->writeFieldValue( "SpecialSymbolsBeforeFirstText", strSpecial );
}
...
void OdDgDimensionDumperPE::dumpData( OdRxObjectPtr pObj, OdExDgnDumper* pDumper ) const
{
  ...
  //simple fields
  pDumper->writeFieldValue( "Dim Style Entry Id", element->getDimensionStyleEntryId() );
  ...
  pDumper->writeFieldValue( "AnnotationScaleFlag", element->getAnnotationScaleFlag());
...
void OdDgMeshFaceLoopsDumperPE::dumpData( OdRxObjectPtr pObj, OdExDgnDumper* pDumper ) const
{
  ...
  for( OdUInt32 i = 0; i < nFaces; i++ )
  {
    ...
    strPoints = strPoints.left(strPoints.getLength() - 1);
    pDumper->writeFieldValue(strName, strPoints);
  }
...
void OdDgRasterAttachmentHeaderDumperPE::dumpData( OdRxObjectPtr pObj, OdExDgnDumper* pDumper ) const
{
  ...
  //simple fields
  {
    ...
    pDumper->writeFieldValue( "uVector", u );
    pDumper->writeFieldValue( "vVector", v );
    pDumper->writeFieldValue( "Resolution", element->getResolution() );
		pDumper->writeFieldValue( "rotation", element->getRotation()*180.0 / OdaPI);
  ...
  pDumper->writeFieldValue( "Transparency Flag", element->getTransparencyFlag());
  {
    int   i;
    char  name[ 20 ];
  ...
  pDumper->writeFieldValue( "Raster layer", element->getRasterLayer() );
	...
	ODCOLORREF clrTint, clrHighlight;

	if(element->getTintColor(clrTint) )
		pDumper->writeFieldValueHex(L"Tint color", clrTint);

	pDumper->writeFieldValueHex(L"Foreground color", element->getForegroundColor());
	pDumper->writeFieldValueHex(L"Background color", element->getBackgroundColor());

	if (element->getHighlightColor(clrHighlight))
		pDumper->writeFieldValueHex(L"Highlight color", clrHighlight);
...
void OdDgTagElementDumperPE::dumpData( OdRxObjectPtr pObj, OdExDgnDumper* pDumper ) const
{
  ...
  if( element->get3dFormatFlag() )
    ...
    pDumper->writeFieldValue("Rotation angle", element->getRotation2d());
...
void OdDgTextTableElementDumperPE::dumpData(OdRxObjectPtr pObj, OdExDgnDumper* pDumper) const
{
  OdDgGraphicsElementDumperPE::dumpData(pObj, pDumper);
};
...
void OdDgProxyGraphicsElementDumperPE::dumpData(OdRxObjectPtr pObj, OdExDgnDumper* pDumper) const
{
  OdDgElementDumperPE::dumpData(pObj, pDumper);
  OdDgElementPtr pElm = pObj;
  pDumper->writeFieldValue("Element Type", pElm->getElementType());
  ...
  OdDgProxyGraphicsElementPtr pProxyElm = pObj;
...
OdDgElementIteratorPtr OdDgComplexProxyGraphicsElementDumperPE::createIterator(OdDgElementPtr pElm, bool atBeginning, bool skipDeleted) const
{
  OdDgComplexProxyGraphicsElementPtr pComplElm = pElm;
  return pComplElm->createIterator(atBeginning, skipDeleted);
}

void OdDgComplexProxyGraphicsElementDumperPE::dumpData(OdRxObjectPtr pObj, OdExDgnDumper* pDumper) const
{
  OdDgElementDumperPE::dumpData(pObj, pDumper);
  OdDgComplexProxyGraphicsElementPtr pComplElm = pObj;
