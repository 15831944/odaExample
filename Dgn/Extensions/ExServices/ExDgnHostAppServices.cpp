...
{
  m_iNumThreadsForMTLoading = 0;
  m_iNumThreadsForMTRendering = 0;
}
...
bool OdExDgnHostAppServices::getMtMode(const OdDb::MultiThreadedMode& uMode) const
{
  bool bRet = false;

  if( uMode == OdDb::kSTMode )
  {
    bRet = (OdDgHostAppServices::getMtMode() == 0);
  }
  else
    bRet = (OdDgHostAppServices::getMtMode() & uMode) ? true : false;

  return bRet;
}

void OdExDgnHostAppServices::setMtMode(const OdDb::MultiThreadedMode& uMode, bool bSet)
{
  OdUInt16 uMtMode = OdDgHostAppServices::getMtMode();

  if( uMode == OdDb::kSTMode)
  {
    if( bSet )
      uMtMode = OdDb::kSTMode;
  }
  else
  {
    if( bSet )
  ...
  OdDgHostAppServices::setMtMode(uMtMode);
}

void OdExDgnHostAppServices::setNumThreads(OdDb::MultiThreadedMode mtMode, int numThreads )
{
  switch( mtMode )
  {
    case OdDb::kMTLoading:   m_iNumThreadsForMTLoading   = numThreads; break;
    case OdDb::kMTRendering: m_iNumThreadsForMTRendering = numThreads; break;
  }
}

int  OdExDgnHostAppServices::numThreads(OdDb::MultiThreadedMode mtMode)
{
  int numThrs = 1;
  bool bEnabled = false;
  switch (mtMode)
  {
    case OdDb::kMTLoading:   bEnabled = getMtMode(OdDb::kMTLoading); break;
    case OdDb::kMTRendering: bEnabled = getMtMode(OdDb::kMTRendering); break;
    default:
      ODA_FAIL_ONCE();
  }
  ...
  if (bEnabled)
  {
    if( mtMode == OdDb::kMTLoading && m_iNumThreadsForMTLoading != 0 )
      return m_iNumThreadsForMTLoading;
    else if( mtMode == OdDb::kMTRendering && m_iNumThreadsForMTRendering != 0 )
      return m_iNumThreadsForMTRendering;
    else
      return OdDbBaseHostAppServices::numThreads( mtMode );
  }
...
struct OdDgnUpdateMeterMTParams
{
  OdExDgnHostAppProgressMeter* m_pMeter;
  OdRefCounter                 m_newValue;
};

void OdExDgnHostAppProgressMeter::updateMeterMT(void *pArg)
{
  OdDgnUpdateMeterMTParams* pNewValue = (OdDgnUpdateMeterMTParams*)(pArg);

  if (pNewValue->m_newValue > pNewValue->m_pMeter->m_MeterOld)
  {
    pNewValue->m_pMeter->m_MeterOld = pNewValue->m_newValue;

    double f1 = (double)(pNewValue->m_newValue) / pNewValue->m_pMeter->m_MeterLimit * 100;

#if !defined(_WIN32_WCE) && !defined(__BORLANDC__) // Causing access violation on BB
    oddgPrintConsoleString(L"%lsProgress: %2.2lf%%\n", pNewValue->m_pMeter->m_Prefix.c_str(), f1);
#endif
  }
}
...
void OdExDgnHostAppProgressMeter::meterProgress()
{
  ...
  if (!m_disableOutput)
  {
    ...
    if ((f1 - f2) > 0.7)
    {
      OdDgnUpdateMeterMTParams updateParams;
      updateParams.m_pMeter = this;
      updateParams.m_newValue = m_MeterCurrent;

      odExecuteMainThreadAction(OdExDgnHostAppProgressMeter::updateMeterMT, &updateParams);
    }
  }
}
