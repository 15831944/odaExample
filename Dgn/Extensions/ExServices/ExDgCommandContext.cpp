...
OdDgSelectionSetPtr ExDgCommandContext::box(OdGsView* pGsView, const OdGePoint3d& pt1, OdDbVisualSelection::SubentSelectionMode ssm)
{
  ...
  OdDgSelectionSetPtr pRet;

  OdGsDevice* pDevice = pGsView->device();

  if( pDevice )
  {
    for( OdInt32 k = 0; k < pDevice->numViews(); k++ )
    {
      OdDgSelectionSetPtr pCurSSet = OdDgSelectionSet::select(database(), pDevice->viewAt(k), 2, pts, OdDbVisualSelection::kBox, ssm);
      ...
      if( !pCurSSet.isNull() )
      {
        if( pRet.isNull() )
          ...
          pRet->append( pCurSSet.get() );
      }
    }
  }
  else
    pRet = OdDgSelectionSet::select(database(), pGsView, 2, pts, OdDbVisualSelection::kBox, ssm);
    ...
    try
    {
      try
      {
        ...
        if( pDevice )
        {
          for( OdInt32 k = 0; k < pDevice->numViews(); k++ )
          {
            OdDgSelectionSetPtr pCurSSet = OdDgSelectionSet::select(database(), pDevice->viewAt(k), 1, &pts[0], OdDbVisualSelection::kPoint, ssm);
                                   ...
                                   OdEdRealTracker* pTracker)
{
  ...
  return getReal(prompt, options, defVal, OdDgResBuf::kDxfXdDist, sKeywords, pTracker);
}
    ...
    if (lspMod.isNull())
    {
      ...
      if(s.left(7).makeLower() == OD_T("handent"))
      {
        ...
        if( odSScanf(s, PRIx64W, &handle) )
        {
          ...
          if(s[0]==L')')
          {
            OdDgResBufPtr pRb = OdDgResBuf::newRb(OdDgResBuf::kRtHandle);
