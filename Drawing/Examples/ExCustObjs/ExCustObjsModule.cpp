...
ODRX_DEFINE_DYNAMIC_MODULE(ExCustObjsModule);

extern OdEdCommandPtr pPrevErase;
...
void ExCustObjsModule::initApp()
{
  ...
  pPrevErase = pCommands->lookupCmd(OD_T("ERASE"));
...
#undef CMD_DEF
  ...
  ExCustEntity::rxInit();
  ExSphere::rxInit();
  EmbeddedSolidWrapper::rxInit();
  //odrxGetSchemaManager()->mergeSchema();
}
 
void ExCustObjsModule::uninitApp()
{
  pPrevErase.release();
