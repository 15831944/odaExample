...
ExGsSimpleDevice::ExGsSimpleDevice()
{
  ...
  setBackgroundColor(ODRGB(255, 255, 255));
...
void ExSimpleView::draw(const OdGiDrawable* pDrawable)
{
  ...
  bool bDBRO = false;
  OdDbObjectPtr pObj = OdDbObject::cast(pDrawable);
  if (!pObj.isNull())
    bDBRO = pObj->isDBRO();
  OdString sHandle = bDBRO ? toString( pObj->objectId().getHandle() ) : toString( L"non-DbResident" );
