...
, m_pDumpFilterFunc(NULL)
, m_pBaseVectorizer(NULL)
{
}
...
bool OdGiConveyorGeometryDumper::isEnabledByDumpFiler()
{
  if (m_pDumpFilterFunc && m_pBaseVectorizer)
    return (*m_pDumpFilterFunc)(m_pBaseVectorizer);
  return true;
}

// for example you can use the next to dump only arrawhead geometry of mleader
//static bool dumpFilterToTransmitMLeaderArrowOnly(const OdGiBaseVectorizer* pBaseVectorizer)
//{
//  int marker = pBaseVectorizer->selectionMarker();
//  return (marker >= OdDbMLeader::kArrowMark && marker < OdDbMLeader::kLeaderLineMark);
//}
//
//{ // like in OdGetGeomEx
//  ...
...
//  OdGiDrawObjectForGetGeometryPtr drawObject = OdGiDrawObjectForGetGeometry::createObject(pDwgContext, pConvDumper, pDumper);
//  pConvDumper->setDumpFilter(dumpFilterToTransmitMLeaderArrowOnly, drawObject);
//  pConvDumper->setDrawContext(drawObject->drawContext());
//  drawObject.get()->draw(pMLeader);
//}
//
void OdGiConveyorGeometryDumper::setDumpFilter(OdDumpFilterFunc* pFilter, OdGiBaseVectorizer* pBaseVectorizer)
{
  m_pDumpFilterFunc = pFilter;
  m_pBaseVectorizer = pBaseVectorizer;
}
...
void OdGiConveyorGeometryDumper::plineProc(const OdGiPolyline& lwBuf,
                                 ...
                                 OdUInt32 numSegs)
{
  if (!isEnabledByDumpFiler()) return;
