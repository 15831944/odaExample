...
TD_QT_PRJ_LIST=""
TD_QT_PRJ_LIST="${TD_QT_PRJ_LIST} TD_DgnDb.tx"
...
do
  ...
  # solution for linking with TG_Db.tx 
  # (was moved here from make_tg_qt_xcode to prevent possible incorrect relinking of TD_DgnDb.tx to libTG_Db.dylib via "relink_td_qt_xcode.sh -r")
  if test -f "${TD_CONF_PATH}/TG_Db.tx" ; then 
    if test -f "${TD_CONF_PATH}/libTG_Db.dylib" ; then rm -f "${TD_CONF_PATH}/libTG_Db.dylib" ; fi
    cp -f "${TD_CONF_PATH}/TG_Db.tx" "${TD_CONF_PATH}/libTG_Db.dylib"
  fi
  ...
  if test -f "${TD_CONF_PATH}/libTG_Db.dylib" && test -f "${TD_CONF_PATH}/TG_Db.tx" ; then 
   rm -f "${TD_CONF_PATH}/libTG_Db.dylib"
  fi
