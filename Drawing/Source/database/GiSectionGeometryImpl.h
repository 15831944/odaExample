...
public:
  OdDbStubPath(){}
  OdDbStubPath(OdDbStubPtrArray& ids): m_ids(ids){}
  OdDbStubPath(const OdDbStubPath& c): m_ids(c.m_ids){}
  const OdDbStubPtrArray& get() { return m_ids; }
  int compare(const OdDbStubPath& c) const
  {
      unsigned i = 0;
  ...
  bool operator < (const OdDbStubPath& c) const { return compare(c) < 0; }
...
protected:
    ...
    typedef std::map<OdDbStubPath, MapValue*> _mmapPath;

protected:
     OdGiSectionGeometryImpl* getAtIdMap(const OdDbStub* id, OdGiDrawable& section,
         const OdGiDrawable& drawable, const OdGeMatrix3d& tf, OdGiSectionGeometryManager& manager);
     OdGiSectionGeometryImpl* getAtPathMap(const OdDbStubPath& path, OdGiDrawable& section,
         const OdGiDrawable& drawable, const OdGeMatrix3d& tf, OdGiSectionGeometryManager& manager);
protected:
    _mmapId m_mapId;
    _mmapPath m_mapPath;
