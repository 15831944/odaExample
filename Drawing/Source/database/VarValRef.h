...
class OdVarValRef
{
  const OdResBuf*     m_pRb;
  const OdDbDatabase* m_pDb;
public:
  inline OdVarValRef(const OdResBuf*  pRb, const OdDbDatabase* pDb)
    : m_pRb(pRb), m_pDb(pDb) {}
  ...
  inline operator OdDbDate() const
  {
    OdDbDate res;
  ...
  inline operator OdCmColor() const
  {
    if(m_pRb->restype()==OdResBuf::kRtColor)
      return m_pRb->getColor();
    OdDxfCode::Type type = OdDxfCode::_getType(m_pRb->restype());
    switch (type)
    {
    case OdDxfCode::Integer16:
    case OdDxfCode::Integer32:
      OdInt16 index = m_pRb->getInt16();
      if (index < 0 || index > OdCmColor::MaxColorIndex)
      {
  ...
  inline operator OdCmTransparency() const
  {
    if (m_pRb->restype() != OdResBuf::kRtString)
      throw OdError(eInvalidResBuf);
    OdString sVal = m_pRb->getString().trimLeft().trimRight().makeUpper();
    OdCmTransparency val;
    if (sVal == L"BYLAYER")
      val.setMethod(OdCmTransparency::kByLayer);
    else if (sVal == L"BYBLOCK")
      val.setMethod(OdCmTransparency::kByBlock);
    else if (sVal == L".")
      val.setMethod(OdCmTransparency::kErrorValue);
    else
    {
      if (sVal.isEmpty())
