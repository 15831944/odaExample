...
class OdDbSplineImpl : public OdDbCurveImpl, public OdGeNurbCurve3d
{
  static OdDbSplineImpl* getImpl(const OdDbSpline *pObj)
  { return (OdDbSplineImpl*)OdDbSystemInternals::getImpl(pObj);}
  ...
  OdUInt32 m_Flags1;
  OdGe::OdGeKnotParameterization m_KnotParam;
...
public:
  ...
  OdDbSpline::SplineType type() const;
  OdResult setType(OdDbSpline::SplineType type);

  // acad 2013 specific behavior
  OdResult makeClosedImpl();

  virtual void drawFrame(const OdDbSpline *pSpline, OdGiWorldDraw* pWd) const;
private:
