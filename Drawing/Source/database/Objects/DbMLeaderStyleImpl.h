...
class OdDbMLeaderStyleImpl : public OdDbObjectImpl
{
public:
  static OdDbMLeaderStyleImpl* getImpl(const OdDbMLeaderStyle *pObj)
  { return (OdDbMLeaderStyleImpl*)OdDbSystemInternals::getImpl(pObj); }

public:
  OdString          m_strStyleDescription;
  OdUInt16          m_DrawMLeaderOrder;     // OdDbMLeaderStyle::DrawMLeaderOrderType
  OdUInt16          m_DrawLeaderOrder;      // OdDbMLeaderStyle::DrawLeaderOrderType
  OdUInt16          m_ContentType;          // OdDbMLeaderStyle::ContentType
  OdUInt16          m_LeftAttachment;       // OdDbMLeaderStyle::TextAttachmentType
  OdUInt16          m_RightAttachment;      // OdDbMLeaderStyle::TextAttachmentType
  OdUInt16          m_TextAngleType;        // OdDbMLeaderStyle::TextAngleType
  OdInt16           m_TextAlignmentType;    // OdDbMLeaderStyle::TextAlignmentType
  OdUInt16          m_BlockConnectionType;  // OdDbMLeaderStyle::BlockConnectionType
  ...
  OdInt16           m_TopAttachment;        // OdDbMLeaderStyle::TextAttachmentType
  OdInt16           m_BottomAttachment;     // OdDbMLeaderStyle::TextAttachmentType

  bool              m_bExtendLeaderToText;
public:
  OdDbMLeaderStyleImpl();
  ...
  static void setColorFromDxf(OdUInt32 rgbm, OdCmColor& color);
  DBENT_EXPORT static bool isModifiedForRecompute(const OdDbObject* pObj);
...
private:
  ...
  inline static void clearVersionXData( OdDbObject *pObj )
  {
    pObj->setXData(OdResBuf::newRb(OdResBuf::kDxfRegAppName, L"ACAD_MLEADERVER"));
  }

  inline static void fixVersionXData( OdDbObject *pObj )
  {
    OdString strAppName(L"ACAD_MLEADERVER");
    OdResBufPtr pRbuf = pObj->xData( strAppName );
    if( pRbuf.isNull() )
    {
      (pRbuf = OdResBuf::newRb( 1001 ))->setString( strAppName );
      pRbuf->insert( OdResBuf::newRb( 1070 ) )->setInt16( (OdInt16)2 );
      pObj->database()->newRegApp( strAppName );
      pObj->setXData( pRbuf );
    }
    else
    {
      OdResBufPtr pRbVersion = pRbuf->next();
  ...
  friend class OdDbMLeaderStyle;
  friend class OdDbMLeader;
  friend class OdDbMLeaderImpl;
};

inline void oddbVerifyDefMLeaderStyle(OdDbDatabase* pDb, OdDbAuditInfo* pAuditInfo = 0) {
  if (!pDb->usingCoreOnly())
    OdDbEntitiesModulePtr(odrxSafeLoadApp(OdDbEntitiesAppName))->VerifyDefMLeaderStyle(pDb, pAuditInfo);
}
