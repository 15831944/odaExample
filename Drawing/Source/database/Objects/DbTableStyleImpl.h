...
public:  
  ...
  bool dwgIn(OdDbDwgFiler* pFiler);
...
public:
  ...
  void dxfInCONTENTFORMAT(OdDbDxfFiler* pFiler);
  void dxfOutCONTENTFORMAT(OdDbDxfFiler* pFiler) const;
};
...
public:
  ...
  void dxfInCELLMARGIN(OdDbDxfFiler* pFiler);
  void dxfInGRIDFORMAT(OdDbDxfFiler* pFiler, OdTableGridLine& grLine);

  void dxfOut(OdDbDxfFiler* pFiler) const;
  void dxfOutTABLEFORMAT(OdDbDxfFiler* pFiler) const;
  void dxfOutCELLMARGIN(OdDbDxfFiler* pFiler) const;
  void dxfOutGRIDFORMAT(OdDbDxfFiler* pFiler, const OdTableGridLine& grLine) const;
...
class OdDbTableStyleImpl : public OdDbObjectImpl
{
  static OdDbTableStyleImpl* getImpl(const OdDbTableStyle *pObj)
  { return (OdDbTableStyleImpl*)OdDbSystemInternals::getImpl(pObj);}
public:
  OdDbTableStyleImpl();
...
//#pragma MARKMESSAGE(TODO: AC24 issue. Unknown data.)
  ...
  void setTableStyleDefault(OdDbDatabase* pDb);
  ...
  static void setModifiedForRecompute(OdDbObject *pObj, bool val);

private:
  friend  class OdDbTableStyle;

  OdInt32 findStyle(const OdString& cellStyle);
  OdInt32 findStyleById(OdInt32 cellStyle);
};

inline void oddbVerifyDefTableStyle(OdDbDatabase* pDb) {
  if (!pDb->usingCoreOnly())
    OdDbEntitiesModulePtr(odrxSafeLoadApp(OdDbEntitiesAppName))->VerifyDefTableStyle(pDb);
}
