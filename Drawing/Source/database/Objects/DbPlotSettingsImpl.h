...
class OdDbPlotSettingsImpl : public OdDbObjectImpl
{
  static OdDbPlotSettingsImpl* getImpl(const OdDbPlotSettings *pObj)
  { return (OdDbPlotSettingsImpl*)OdDbSystemInternals::getImpl(pObj);}
...
public:
  ...
  OdDbHardPointerId m_plotViewId; // Id of viewTableRecord should be stored.
  OdDbSoftPointerId m_VisualStyleId;
  OdString    m_strPaperSize;
  OdString    m_strPrinterOrConfigFileName;
  OdString    m_storedInitialMedia;

  class Pc3FileDependency : public OdDb::FileDependency
  {
  public:
    Pc3FileDependency() : OdDb::FileDependency( OD_T("Acad:PlotConfig") ){}
    void setFileName( const OdString& name, OdDbDatabase* db, bool updateDependencies = true )
    {
      if ( name.getLength() < 4 || name.right( 4 ).makeLower() != OD_T(".pc3") ) return;
#if defined(sgi) || defined(__BCPLUSPLUS__)
      OdDb::
#endif
      FileDependency::setFileName( name, db, updateDependencies );
    }
  };
  Pc3FileDependency m_pPrinterOrConfigFilePath;

  OdDb::FileDependency m_pCurrentStyleSheetPath;
  ...
  void setPlotViewName(OdDbDatabase& Db, const OdString& name); // instead field - need for load pre-r18 files
  virtual bool isModelLayout() const;

public:
  OdDbPlotSettingsImpl();
private:
  OdResult dwgInFields(OdDbDwgFiler* pFiler);
  void dwgOutFields(OdDbDwgFiler* pFiler) const;

  friend class OdDbPlotSettings;
  friend class OdDbPlotSettingsValidatorImpl;
};
