...
public:
  ...
  void dxfInCell(OdDbDxfFiler* pFiler);
  void dxfOutCell(OdDbDxfFiler* pFiler);
};

typedef OdArray<OdCellGeometry> OdRowGeometry;
typedef OdArray<OdRowGeometry> OdTableGeometry;

class OdDbTableGeometryImpl : public OdDbObjectImpl
{
public:
  static OdDbTableGeometryImpl* getImpl(const OdDbTableGeometry *pObj)
  { return (OdDbTableGeometryImpl*)OdDbSystemInternals::getImpl(pObj); }

  OdInt32 m_rows;
  OdInt32 m_columns;
  OdTableGeometry m_table;
public:
  OdDbTableGeometryImpl() : m_rows(0)
    , m_columns(0)
  {}

  friend class OdDbTableGeometry;
};
