...
class OdDbBlockLinearParameterImpl : public OdDbBlock2PtParameterImpl
{
  friend class OdDbBlockLinearParameter;
protected:
  OdString m_sLabel;
...
public:
  DECLARE_GET_IMPL(OdDbBlockLinearParameter);
  ...
  virtual bool setPropertyValue( const OdString& name, const OdResBuf* rb, const OdGeMatrix3d& m = OdGeMatrix3d::kIdentity );
  virtual bool hasInstanceData()const;
  void updateConnections();
  OdResult transformBy( const OdGeMatrix3d& m );
  bool evaluate(const OdDbEvalContext*);
  OdDbBlockLinearParameterImpl()
  {
    m_dLabelOffset = 0.;
  }
};
