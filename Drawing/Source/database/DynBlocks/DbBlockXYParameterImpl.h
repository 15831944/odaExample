...
class OdDbBlockXYParameterImpl : public OdDbBlock2PtParameterImpl
{
  friend class OdDbBlockXYParameter;
protected:
  OdString m_sXLabel;
  ...
  OdStaticRxObject<OdDbBlockParamValueSet> m_X, m_Y;
public:
  DECLARE_GET_IMPL(OdDbBlockXYParameter);

  OdDbBlockXYParameterImpl()
  {
    m_dYLabelOffset = 0.;
    m_dXLabelOffset = 0.;
  }
  ...
  virtual bool setPropertyValue( const OdString& name, const OdResBuf* rb, const OdGeMatrix3d&) ODRX_OVERRIDE;
  virtual bool hasInstanceData() const ODRX_OVERRIDE;
  virtual bool evaluate(const OdDbEvalContext*) ODRX_OVERRIDE;
};
