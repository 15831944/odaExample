...
class OdDbUndoObjFiler: public OdDbDwgFiler
{
public:
  enum EDataType
  {
...
public:
  OdDbUndoObjFiler(): m_db(NULL), m_iPos(0), m_size(0), m_aRef(100, -100)
  {
  ...
  OdDbFiler::FilerType filerType() const { return OdDbFiler::kUndoFiler; }
  static OdSmartPtr<OdDbUndoObjFiler> createObject(OdDbDatabase* pDb);
#ifdef _DEBUG
  ...
  OdDbDatabase* database() const { return m_db; }
  ...
  void wrItem(int iItem, OdDbDwgFiler* pFiler) const;
  void rdItem(int iItem, OdDbDwgFiler* pFiler);
  void deleteItemsAt(int iPos, int nItems);
  bool isItemEqual(int iItem, const OdDbUndoObjFiler& otherData, int iItemOtherData) const;
  ...
  bool isItemEqual(int iItem, EDataType eType, const OdDbObjectId& id) const
  {
    ODA_ASSERT(iItem >= 0 && iItem < (int)size());
  ...
  void copyItems(const OdDbUndoObjFiler& otherData, int iItemOtherData, int nItems);
...
typedef OdSmartPtr<OdDbUndoObjFiler> OdDbUndoObjFilerPtr;
