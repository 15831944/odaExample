...
public:
  ...
  OdDbHardPointerId      m_BaseUcsId;
  OdDbHardPointerId      m_NamedUcsId;
...
public:
  ...
  OdDbHardPointerId& baseUcsId() { return m_BaseUcsId; }
  const OdDbHardPointerId& getBaseUcsId() const { return m_BaseUcsId; }
  void setBaseUcsId(const OdDbHardPointerId& value) { m_BaseUcsId = value; }

  void setNamedUcsId(const OdDbHardPointerId& value) { m_NamedUcsId = value; }
  OdDbHardPointerId& namedUcsId() { return m_NamedUcsId; }
  const OdDbHardPointerId& getNamedUcsId() const { return m_NamedUcsId; }

  bool auditAxes(OdDbAuditInfo* pAuditInfo, OdDbObject* pObj)
  {
    ...
    if (bXInvalid || bYInvalid || !bPerp)
    {
      ...
      { //Report errors
        OdDbHostAppServices* pHostApp = pObj->database()->appServices();
  	    OdString sName = odDbGetObjectName(pObj).c_str();
        if (bXInvalid)
        {
          if (pAuditInfo)
          {
