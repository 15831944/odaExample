...
template<class T>
void writeSections(T* pIo, OdDwgFileSectionsInfo& secInfo)
{
	// writing section map...
...
/**/
class DBIO_EXPORT OdDwgFileWriter : public OdDwgFileController
{
...
protected:
  ...
  class AutoCloser
  {
    ...
    AutoCloser();
  public:
    AutoCloser(OdDwgFileWriter &p, bool bSeparateIds) : m_p(p) 
    {
...
public:
  ODRX_DECLARE_MEMBERS(OdDwgFileWriter);

	OdDwgFileWriter();
	virtual ~OdDwgFileWriter();

  virtual void writeDatabase(OdDbDatabase* );
//  void openW(OdStreamBuf* pOpenedIo, OdThumbnailImage* /*pImg*/);
  ...
  {
    OdUInt16 res = ::odDbDwgClassMap().dwgType(pClass);
    if(!res) {
      if(pClass==OdDbProxyObject::desc())
        res = OdUInt16(499);
      else if(pClass==OdDbProxyEntity::desc())
        res = OdUInt16(498);
    }
    return res;
  }
...
protected:
  ...
  inline void saveObject(OdDwgFileSplitStream* pFiler);
...
protected:
  ...
  OdStaticRxObject<OdDwgFileSplitStream> m_dwgFiler;
  OdBinaryData m_dataBuffer;
  OdBinaryData m_idsBuffer;

  ObjectMap    m_Handles;  
  OdLinkedArray<OdDbObjectId> m_IdsOfObjectsToWrite;
...
typedef OdSmartPtr<OdDwgFileWriter> OdDwgWriterPtr;
