...
//----------------------------------------------------------
//
// OdDwgR21PagedStreamMTHelper
//
//----------------------------------------------------------
class OdDwgR21PagedStreamMTHelper : public OdRxObjectImpl<OdStreamBuf>
{
public:
  static inline OdStreamBufPtr create(OdDwgR21PagedStream* pPagedStream, OdDwgFileLoader* pLoader, int threadId)
  {
    return OdStreamBufPtr((OdStreamBuf*)new OdDwgR21PagedStreamMTHelper(pPagedStream, pLoader, threadId), kOdRxObjAttach);
  }
  OdDwgR21PagedStreamMTHelper(OdDwgR21PagedStream* pPagedStream, OdDwgFileLoader* pLoader, int threadId)
    : m_curPosInPage(0)
  ...
  OdUInt64 seek(OdInt64 offset, OdDb::FilerSeekType from)
  {
    ...
    else
    {
      ...
      if (newPos != curPos())
      {
        ...
        {
          OdDwgR21PagedStream::OdPageArrayIterator itbeg, itend;
          if (newPos > m_curPage->m_dataOffset )
          {
            itbeg = m_curPage;
          ...
          m_curPage = std::lower_bound(itbeg, itend, OdDwgR21PagedStream::Page(newPos+1), PageOffsetLess());
...
protected:
  OdDwgR21PagedStream* m_pPagedStream;
  OdDwgFileLoader* m_pLoader;
  int m_threadId;
  OdDwgR21PagedStream::OdPageArrayIterator m_curPage;
  ...
  void getPage()
  {
    ...
    OdDwgR21FileLoader::R21MTContext* pR21MTContext = (OdDwgR21FileLoader::R21MTContext*)m_pLoader->getMTContext();
    OdBinaryData& pageData = pR21MTContext->m_buffs0[m_threadId];
    OdBinaryData& decData = pR21MTContext->m_buffs1[m_threadId];
    OdDwgR21Compressor& cmpr =  pR21MTContext->m_compressors[m_threadId];
