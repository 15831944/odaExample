...
OdGiRasterImagePtr PdfDirectWrapperImpl::getThumbnail(int nPage, int width, int height) const
{
  if (!security(getKey()))
  {
  ...
  try
  {
    ...
    VGSetImagePixWorH(width, height);
...
OdResult PdfDirectWrapperImpl::getImage(ImageCache& pdfInfo, int nPage, OdGiViewportDraw* pVd, const OdDbUnderlayDrawContext& context, OdGeExtents2d& clipRect,
                                    double DPI, int ImagePixW, int ImagePixH) const
{
 ...
 // Image width/height are the same as original size and may be very big (eg. 6000x4000 pixels). 
...
#ifdef PDF_DEBUG
  {
    ...
    OdRxRasterServicesPtr pRasSvcs = odrxDynamicLinker()->loadApp(RX_RASTER_SERVICES_APPNAME);
    if (!pRasSvcs.isNull())
      pRasSvcs->saveRasterImage(rast1, "f:\\pdfvi1.bmp");
  }
#endif
...
void PdfDirectWrapperImpl::viewportDraw(int nPage, OdGiViewportDraw* pVd, const OdDbUnderlayDrawContext& context) const
{
  if (!security(getKey()))
    return;
  PdfDirectWrapper::viewportDraw(nPage, pVd, context);
}

OdResult PdfDirectWrapperImpl::sliceImage(VgPsLayerInfo& layerInfo, const OdDbUnderlayDrawContext& contextWithPaleteFlag, const OdGeExtents2d& clipRect, const OdGiViewportDraw* pVd,
