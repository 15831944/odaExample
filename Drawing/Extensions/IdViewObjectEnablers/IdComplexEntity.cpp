  ...
  for( int zd = 0; zd < 6; zd++ )
  {
    ...
    case 3:
      for( k = 0; k < count; k++ )
      {
        ...
        tmpLineData->pnt[0] = pFiler->rdPoint3d();
        tmpLineData->pnt[1] = pFiler->rdPoint3d();
...
void OdIdComplexEntity::dwgOutFields(OdDbDwgFiler* pFiler) const
{
  ...
  for( int zd = 0; zd < 6; zd++ )
  {
    ...
    case 3:
      ...
      {
        pFiler->wrPoint3d( m_LineData[tmpIndex].pnt[0] );
        pFiler->wrPoint3d( m_LineData[tmpIndex].pnt[1] );
...
bool OdIdComplexEntity::subWorldDraw(OdGiWorldDraw* pWd) const
{
  ...
  {
    geom.polyline(2, m_LineData[counter].pnt, &vNormal);
  }
    ...
    OdGePoint3d const *ppoint )
{
  ...
  tmpLineData->pnt[0] = ppoint[0];
  tmpLineData->pnt[1] = ppoint[1];
