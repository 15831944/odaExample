...
OdResult OdDbImpAssocSingleDependencyActionParam::getObject(OdDbCompoundObjectId& idObject) const
{
  ...
  return pDependency->getDependentOnCompoundObject(idObject);
}
...
OdResult OdDbImpAssocVertexActionParam::setVertexRef( const OdDbVertexRef& vertexRef, bool isReadDependency, bool isWriteDependency, int dependencyOrder, OdDbObjectId actionId )
{
  if ( isReadDependency || isWriteDependency )
  {
    ...
    if ( !pSubRef.isNull() )
    {
      ...
      compoundObjectId.setEmpty();

      compoundObjectId.set( vertexRef.entity() );
...
OdStringArray CompareTwoCircArc3d(const OdGeCircArc3d& arc, const OdGeCircArc3d& arcToCompare)//type 11
{
  OdStringArray retArr;
...
OdStringArray CompareTwoLineSeg3d(const OdGeLineSeg3d& line, const OdGeLineSeg3d& lineToCompare)//type 23
{
  OdStringArray retArr;
...
OdStringArray CompareTwoEllipArc3d(const OdGeEllipArc3d& ellArc, const OdGeEllipArc3d& ellArcToCompare)// type 17
{
  OdStringArray retArr;
...
OdStringArray  CompareTwoNurbCurve3d(const OdGeNurbCurve3d& spline, const OdGeNurbCurve3d& splineToCompare) // type 42
{
  OdStringArray retArr;
...
OdStringArray  CompareTwoLine3d(const OdGeLine3d& line, const OdGeLine3d& lineToCompare)//type 19
{
  OdStringArray retArr;
      ...
      if (!m_edgeRef.isEmpty())
      {
        ...
        switch (m_edgeRef.curve()->type())
        {
          ...
          case 47:
          {
            ...
            const OdGeCurve3dPtrArray& curvelist = curve.getCurveList();

            const OdGeCurve3dPtrArray& curvelistToCompare = curveToCompare.getCurveList();
...
OdResult OdDbImpAssocEdgeActionParam::setEdgeRef( const OdDbEdgeRef& edgeRef, bool isReadDependency, /* = true */ bool isWriteDependency, /* = false */ int dependencyOrder, OdDbObjectId actionId )
{
  if ( isReadDependency || isWriteDependency )
  {
    ...
    compoundObjectId.set( edgeRef.entity() );
    pDependency->attachToObject( compoundObjectId );
    pAction->addDependency( pDependency->objectId(), true );
...
OdResult OdDbImpAssocAsmBodyActionParam::dwgInFields(OdDbDwgFiler* pFiler, OdDbObjectId objectId)
{
	...
	m_UnknownPathActionParam1 = pFiler->rdInt32();
	bool bReadEnt = pFiler->rdBool();

  if (bReadEnt)
  {
    res = OdDbAcisIO::readAcisData(pFiler, m_pModelerGeom);
  }
...
void OdDbImpAssocAsmBodyActionParam::dwgOutFields(OdDbDwgFiler* pFiler, OdDbObjectId objectId) const
{
  ...
  bool bWriteEnt = m_pModelerGeom.get() != NULL;

  pFiler->wrBool(bWriteEnt);
  if (bWriteEnt)
  {
    OdModelerGeometryPtr mGeom = m_pModelerGeom;
    OdDbAcisIO::writeAcisData(pFiler, mGeom);
  }
}
...
OdResult OdDbImpAssocAsmBodyActionParam::dxfInFields(OdDbDxfFiler* pFiler, OdDbObjectId objectId)
{
  ...
  if (bReadEnt)
  {
    m_pModelerGeom = OdDbAcisIO::readAcisData(pFiler);
  }
...
void OdDbImpAssocAsmBodyActionParam::dxfOutFields(OdDbDxfFiler* pFiler, OdDbObjectId objectId) const
{
  ...
  bool bWriteEnt = m_pModelerGeom.get() != NULL;
  pFiler->wrBool(290, bWriteEnt);
  if (bWriteEnt)
  {
    OdModelerGeometryPtr mGeom = m_pModelerGeom;
    OdDbAcisIO::writeAcisData(pFiler, mGeom, false);
  }
}
