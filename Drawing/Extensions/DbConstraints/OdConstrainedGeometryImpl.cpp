...
OdResult OdConstrainedGeometryImpl::init(OdDbObjectId depId)
{
...
OdResult OdConstrainedGeometryImpl::setOwningRigidSet(OdConstraintGroupNodeId rigidSetNodeId)
{
  if (getOwningRigidSet())
    return eAmbiguousInput;

  m_rigidSetIdx = rigidSetNodeId;
  return eOk;
}
...
OdDbObjectPtr OdConstrainedGeometryImpl::geomObject(const OdDbObjectId geomDepId, OdDb::OpenMode mode)
{
  ...
  pGeomDep->getDependentOnCompoundObject(compId);

  return compId.leafId().openObject(mode);
}
...
OdResult OdConstrainedGeometryImpl::getFullSubentPath(OdDbFullSubentPath& path) const
{
  ...
  if (pGeomDep.isNull())
    return eBadObjType;
  if (pGeomDep->status()==kErasedAssocStatus)
    return eWasErased;
