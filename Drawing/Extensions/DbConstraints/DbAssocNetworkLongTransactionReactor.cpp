...
{
  OdDbObjectIdArray aFilteredObjects(iOriginList.length());
  OdDbObjectIdArray::const_iterator it = iOriginList.begin();
  for (; it != iOriginList.end(); ++it)
  {
    OdDbDimensionPtr pDim = OdDbDimension::cast(it->openObject());
    if (pDim.isNull() || !pDim->isConstraintObject())
      aFilteredObjects.append(*it);
  }
  if (aFilteredObjects.length() != iOriginList.length())
    iOriginList = aFilteredObjects;
...
void OdDbAssocNetworkLongTransactionReactor::endCheckIn(OdDbLongTransaction& iTransaction)
{
  ...
  try
  {
    ...
    OdDbAssocManager::evaluateTopLevelNetwork(iTransaction.originBlock().database());
  }
  catch (OdError &)
  {
    m_OriginalObjectReactorsMap.clear();
