...
void OdConstraintGroupNodeImpl::audit(OdConstraintGroupNode*, OdAuditInfo* pAuditInfo)
{
  OdGroupNodeIdArray fixedConnections;
  fixedConnections.reserve(m_connections.size());

  std::set<OdConstraintGroupNodeId> sortedConnections;
  for (unsigned i = 0; i < m_connections.size(); ++i)
  {
    if (sortedConnections.insert(m_connections[i]).second)
      fixedConnections.push_back(m_connections[i]);
  }

  if (fixedConnections.size() != m_connections.size())
  {
    pAuditInfo->errorsFound(1);
    if (pAuditInfo->fixErrors())
    {
      std::swap(fixedConnections, m_connections);
