  ...
  OdArray<OdDbSubentId>& allSubentIds)
{
  ...
  else if (subentType == OdDb::kVertexSubentType)
  {
    ...
    if (!pSpline->isClosed())
    {
      allSubentIds.append(OdDbSubentId(OdDb::kVertexSubentType, kStart));
      allSubentIds.append(OdDbSubentId(OdDb::kVertexSubentType, kEnd));
    }
    for (int i = 0; i < pSpline->numControlPoints(); i++)
      allSubentIds.append(OdDbSubentId(OdDb::kVertexSubentType, i));
    //sometimes ARX also appends some kCenterPLinePrefix vertices (for invalid splines?)
