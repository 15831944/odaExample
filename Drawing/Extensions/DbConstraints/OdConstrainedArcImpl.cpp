...
OdResult OdConstrainedArcImpl::init(OdDbObjectId depId)
{
...
void OdConstrainedArcImpl::audit(OdConstraintGroupNode* pNode, OdAuditInfo* pAuditInfo)
{
  OdConstrainedCircleImpl::audit(pNode, pAuditInfo);
  OdSmartPtr<OdConstrainedArc> pThisConsArc = OdConstrainedArc::cast(pNode);
  ...
  //previous version of the Drawings SDK had different from object arx representation for bulged polyline edges
  //acad sets negated normal to define clockwise arc segment, Drawings swapped end points to make it counterclockwise.
  //To recognize Drawings old representation following checks are used
  {
    OdDbObjectPtr pGeomObj = geomObject(geomDepId);
    if (pGeomObj.isNull() || *pThisConsArc == *pRefConsArc || !pGeomObj->isKindOf(OdDbPolyline::desc()))
      return;
