...
{
  OdDbObjectPtr parentObject;
  OdResult res = parentAction().openObject(parentObject, OdDb::kForWrite);
  if(res!=eOk)
    return res;
  OdDbAssocActionPtr parentAction = OdDbAssocAction::cast(parentObject);
  if(parentAction.get())
    return parentAction->setStatus(newStatus, notifyOwningNetwork, setInOwnedActions);
...
{
  return OdDbAssocActionPtr( parentAction().safeOpenObject( OdDb::kForWrite ) )
...
OdResult OdDbAssocActionBody::dependentObjectClonedOverride(
  const OdDbAssocDependency* dependency, const OdDbObject* original, const OdDbObject* clone)
{
  ...
  return m_pImpObj->dependentObjectClonedOverride(this, dependency, original, clone);
}

OdResult OdDbAssocActionBody::addMoreObjectsToDeepCloneOverride(
  OdDbIdMapping& idMap, OdDbObjectIdArray& additionalObjectsToClone) const
{
  ...
  return m_pImpObj->addMoreObjectsToDeepCloneOverride(this, idMap, additionalObjectsToClone);
}
