...
{
  assertReadEnabled();
  OdDbAssocStatus oldStatus = m_pImpObj->m_status;
  ...
  if (notifyParentAction && m_pImpObj->m_status != oldStatus)
  {
    OdDbAssocActionPtr pAction = parentAction().openObject(OdDb::kForWrite);
    if (!pAction.isNull())
      return pAction->setStatus(newStatus, notifyParentAction, setInOwnedParams);
  }
  return eOk;
}
...
void OdDbAssocEdgeActionParam::composeForLoad(OdDb::SaveType format, OdDb::DwgVersion version, OdDbAuditInfo* pAuditInfo)
{
  OdDbImpAssocEdgeActionParam* pImpl = static_cast <OdDbImpAssocEdgeActionParam*>(m_pImpObj);
  OdDbObjectId depId = pImpl->m_DependencyId;
  if (depId.isNull())
    return;

  OdDbAssocDependencyPtr pDepPath = OdDbAssocDependency::cast(depId.openObject(OdDb::kForWrite));
  if (pDepPath.get() && pDepPath->dependentOnObject().objectClass()->isDerivedFrom(OdDbCurve::desc()))
  {
    // methods OdDbImpAssocEdgeActionParam::dwgInFields/dwgOutFields/dxfInFields/dxfOutFields 
    // do not save/restory interval of curve. So the interval is[0; 1] by default, instead of[0; length].
    // To restory the correct interval, we have to call OdDbAssocDependency::setStatus(kChangedDirectlyAssocStatus); 
    // for path curve, corresponding to the array.

    OdDbObjectId idAction = ownerId();
    OdDbAssocActionPtr pAssocAction = OdDbAssocAction::cast(idAction.openObject());
    if (pAssocAction.isNull())
      return; // CORE-17631 problem with DbAssocDimensionTest
    OdDbObjectId idBody = pAssocAction->actionBody();
    OdDbAssocActionBodyPtr pBody = OdDbAssocActionBody::cast(idBody.openObject());
    if (OdDbAssocArrayActionBody::cast(pBody.get()).isNull())
    {
      ODA_ASSERT_ONCE_X(ASSOC, OdDbCenterMarkActionBody::cast(pBody.get()).get()); // test
      return; // CORE-17631 problem with DbAssocDimensionTest
    }

    // force to update interval in OdDbImpAssocArrayActionBody::evaluateOverride()
    pDepPath->setStatus(kChangedDirectlyAssocStatus); // updateAssocArrayPathInterval // CORE-14767
  }
}
