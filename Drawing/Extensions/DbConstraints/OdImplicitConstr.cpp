...
void OdPointCurveConstraint::addMoreObjectsToDeepClone(
  OdDbIdMapping& idMap, OdDbObjectIdArray& additionalObjectsToClone) const
{
  if (additionalObjectsToClone.empty())
    additionalObjectsToClone.append(owningConstraintGroupId());
}
...
void OdConcentricConstraint::addMoreObjectsToDeepClone(
  OdDbIdMapping& idMap, OdDbObjectIdArray& additionalObjectsToClone) const
{
  if (additionalObjectsToClone.empty())
    additionalObjectsToClone.append(owningConstraintGroupId());
...
OdConstraintGroupNodeId OdParallelConstraint::getDatumLineId() const
{
  if (const OdParallelConstraintImpl* pImpl = dynamic_cast<const OdParallelConstraintImpl*>(m_pImpl))
  {
    return pImpl->getDatumLineIdx();
  }
  return 0;
}

void OdParallelConstraint::addMoreObjectsToDeepClone(
  OdDbIdMapping& idMap, OdDbObjectIdArray& additionalObjectsToClone) const
{
  if (additionalObjectsToClone.empty())
    additionalObjectsToClone.append(owningConstraintGroupId());
...
void OdEqualCurvatureConstraint::addMoreObjectsToDeepClone(
  OdDbIdMapping& idMap, OdDbObjectIdArray& additionalObjectsToClone) const
{
  if (additionalObjectsToClone.empty())
    additionalObjectsToClone.append(owningConstraintGroupId());
