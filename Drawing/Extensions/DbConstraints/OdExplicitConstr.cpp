...
inline void addObjectToDeepClone(OdDbObjectIdArray& additionalObjectsToClone, OdDbObjectId depId) {
  OdDbAssocDependencyPtr dep = depId.openObject();
  if (dep.get())
    additionalObjectsToClone.append(dep->dependentOnObject());
}

void OdExplicitConstraint::addMoreObjectsToDeepClone(
  OdDbIdMapping& idMap, OdDbObjectIdArray& additionalObjectsToClone) const
{
  ...
  ::addObjectToDeepClone(additionalObjectsToClone, dimDependencyId());
  ::addObjectToDeepClone(additionalObjectsToClone, valueDependencyId());
}
