...
OdResult OdDbImpCenterMarkActionBody::setCenterPoint(const OdGePoint3d & ptCenter, const OdDbObjectId &parentActionId)
{
  OdDbBlockReferencePtr bRef = OdDbBlockReference::cast(getArrayEntity(parentActionId).safeOpenObject(OdDb::kForWrite));
  OdDbAssocActionPtr pAction = OdDbAssocAction::cast(parentActionId.safeOpenObject(OdDb::kForWrite));
  ...
  // if CenterMark associated with circle, first removing dependency
  // then moving CenterMark
  if (!pGeomDep.isNull())
    pAction->removeDependency(pGeomDep->objectId(), true);

  bRef->setPosition(ptCenter);
...
OdResult OdDbImpCenterMarkActionBody::getRefCircle(OdGeCircArc3d &refCircle, const OdDbObjectId &parentActionId)
{
  ...
  return getRefCircle(refCircle, getAssocGeomDependencyPtr(pAction));
}

OdResult OdDbImpCenterMarkActionBody::getRefCircle(OdGeCircArc3d &refCircle, const OdDbAssocGeomDependency* pGeomDep)
{
  if (!pGeomDep)
  {
    //ODA_ASSERT(! L"Dependency is Null");
    // It's allowed if copy CM without object to associate
    return eNullObjectId;
  }
  ...
  OdResult res = pGeomDep->getEdgeSubentityGeometry(arrCurve);
...
void OdDbImpCenterMarkActionBody::evaluateOverride(OdDbObjectId parentActionId)
{
  ...
  if (pGeomDep.get() && pGeomDep->status() == kErasedAssocStatus)
    pAction->removeDependency(pGeomDep->objectId(), true);
  ...
  OdGeCircArc3d arc;
  if (getRefCircle(arc, pGeomDep) != eOk) {
    // if there is no associated circle, creating new circle using BlockReference params

    OdDbEvalVariantPtr value = OdDbEvalVariant::init(0);
    OdString expression(""), evaluatorId("");

    if (pAction->getValueParam(OD_T("CachedCircleRadius"), *value.get(), expression, evaluatorId, 0) != eOk)
    {
      pAction->setStatus(kIsUpToDateAssocStatus);
      return;
    }

    OdGeCircArc3d Circle(bRef->position(), bRef->normal(), value->getDouble());
    arc = Circle;
  }
  ...
  if (!pBR.isNull())
  {
    ...
    for (count = 0; count < 6; count++)
    {
      ...
      case 2:
      {
        ...
        //double overSh = horizontalStartOvershoot(pAction); //unused variable
    ...
    {
      pDb->endTransaction(); // Apply changes ( downgradeOpen() )
    }
    pBR->downgradeOpen();
  }
  bEntRef->downgradeOpen();
  ...
  OdDbObjectId& actionBodyId)
{
  ...
  OdDbEntityPtr assocOb = sourceEntity.openObject(OdDb::kForWrite);
  OdGeCircArc3d arc;
  ...
  compoundObjectId.setFullPath(pathArray);//, OdDbDatabase* pHostDatabase = NULL)
  pGeomDependency->attachToObject(compoundObjectId);
  ...
  compoundObjectId.set(pCenterMarkBlockRef->objectId());
  ...
  {
    compoundObjectId.set(pGeomDependency->objectId());
