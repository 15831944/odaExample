...
OdDbAssocArrayActionBodyPtr OdDbAssocArrayParameters::owner() const
{
  OdDbAssocArrayActionBodyPtr pOwner;
  if (!m_pImpl->m_ownerId.isNull()) // if (m_pOwner)
    pOwner = m_pImpl->m_ownerId.safeOpenObject(OdDb::kForRead);
  return pOwner; // return m_pImpl->m_pOwner;
}
