...
ODRX_NO_CONS_DEFINE_MEMBERS_ALTNAME(OdDbAssocDimDependencyBodyBase, OdDbAssocDependencyBody, L"AcDbAssocDimDependencyBodyBase");
...
OdResult OdDbAssocDimDependencyBodyBase::dxfInFields(OdDbDxfFiler* pFiler)
{
  ...
  if (!pFiler->atSubclassData(L"AcDbImpAssocDimDependencyBodyBase")) // Subclass marker is not class name!
  {
    ODA_FAIL_ONCE();
    return eMakeMeProxy;
  }
...
void OdDbAssocDimDependencyBodyBase::dxfOutFields(OdDbDxfFiler* pFiler) const
{
  ...
  pFiler->wrSubclassMarker(L"AcDbImpAssocDimDependencyBodyBase"); // Subclass marker is not class name!
...
OdResult /*static */OdDbAssocDimDependencyBodyBase::getFromEntity(const OdDbObjectId& entityId, OdDbObjectId& dimDepBodyId)
{
  ...
  const OdDbObjectIdArray arrPersistentReactors = pObject->getPersistentReactors();
  OdDbObjectPtr pDepBody;
  OdDbAssocDependencyPtr pDep;
  for ( unsigned int i = 0; i < arrPersistentReactors.length(); i++ )
  {
    pDep = OdDbAssocDependency::cast(arrPersistentReactors[i].openObject());
    if ( pDep.get() )
    {
      OdDbObjectId depBodyId = pDep->dependencyBody();
      if ( !depBodyId.isNull() )
      {
        pDepBody = depBodyId.openObject();
        if ( pDepBody.get() && pDepBody->isKindOf(OdDbAssocDimDependencyBodyBase::desc()) )
        {
