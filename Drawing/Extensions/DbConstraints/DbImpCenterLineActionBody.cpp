...
void OdDbImpCenterLineActionBody::evaluateOverride(OdDbObjectId parentActionId)
{
  ...
  if (pGeomDep0.isNull() || pGeomDep1.isNull() || pGeomDep0->status()==kErasedAssocStatus || pGeomDep1->status()==kErasedAssocStatus) {
    if(pGeomDep0.get())
      pAction->removeDependency(pGeomDep0->objectId(), true);
    if(pGeomDep1.get())
      pAction->removeDependency(pGeomDep1->objectId(), true);
  ...
  // Calculate intersection of two associated lines 
  ...
  double lengthForCheckingIntersection = ptLine1.length() > ptLine2.length() ? ptLine1.length() : ptLine2.length();
  ...
  if (bParallel)
  {
    OdGeVector3d vFirst = dirEndPoint - revEndPoint;
    OdGeVector3d vSecond = dirStartPoint - revStartPoint;

    if (!vFirst.isCodirectionalTo(vSecond))
    {
      dirSegOfLine.set(dirStartPoint, revEndPoint);
      revertSegOfLine.set(revStartPoint, dirEndPoint);
    }
  }
  ...
  if (!bParallel)
  {
    ...
    vFirst.setLength(lengthForCheckingIntersection);
    vSecond.setLength(lengthForCheckingIntersection);
  ...
  ODA_VERIFY(pAction->evaluateDependencies() == eOk);
  ODA_VERIFY(pAction->setStatus(kIsUpToDateAssocStatus) == eOk);
}
  ...
  OdDbObjectId& actionBodyId)
{
  ...
  res = pGeomDependencyArr[0]->getDependentOnCompoundObject(compoundObjectId);
  ...
  if (!bParallel)
  {
    ...
    OdGeLineSeg3d ptLineStartIntersect(segLineArr[0].startPoint(), intersectPoint);
    OdGeLineSeg3d ptLineEndIntersect(segLineArr[0].endPoint(), intersectPoint);
  ...
  //////////////////////////////////////////////////////////////////////
  res = setParametersAndCreateCL(inputArr, pDb, pAction, pActionBody, pDependency,
  ...
  )
{
  ...
  OdDbEntityPtr assocOb0 = sourceEntites[0].openObject(OdDb::kForWrite);
  ...
  OdDbCompoundObjectId compoundObjectId;
  compoundObjectId.setFullPath(entPath.objectIds());//, OdDbDatabase* pHostDatabase = NULL)
  pGeomDependencyArr[0]->attachToObject(compoundObjectId);
  ...
  OdDbEntityPtr assocOb1 = sourceEntites[1].openObject(OdDb::kForWrite);
  ...
  //pDependency->setOrder(0 - 2147483648u);
  compoundObjectId.setFullPath(entPath.objectIds());//, OdDbDatabase* pHostDatabase = NULL)
  pGeomDependencyArr[1]->attachToObject(compoundObjectId);
  ...
  //pDependency->setOrder(0 - 2147483648u);
  compoundObjectId.set(pCenterLineBlock->objectId());
  pDependency->attachToObject(compoundObjectId);
  pAction->addDependency(pDependency->objectId(), true);
  // TODO now ACAD can't draw correct CM for our drawing
  if (assocOb0->isKindOf(OdDbSurface::desc()) || assocOb0->isKindOf(OdDbSolid::desc()))
  {
    OdDbObjectIdArray arrObjId;
    
    OdDbCompoundObjectId resCO;
    pDependency->getDependentOnCompoundObject(resCO);
    
    if (resCO.getFullPath(arrObjId) != eOk)
      return eBadObjType;

    OdDbObjectPtr pObj;
    OdDbObjectId depId = pDependency->objectId();
  ...
  for (int i = 0; i < 2; i++)
  {
    ...
    {
      compoundObjectId.set(pGeomDependencyArr[i]->objectId());
