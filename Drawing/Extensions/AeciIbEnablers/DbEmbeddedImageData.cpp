...
public:
  ...
  OdStreamBufPtr composeTiff() const
  {
    ...
    wrIfdEntry(*ps.get(), kTiffResolutionUnit, kTiffShort, m_corrInfo.m_units); // 1 - no meaningful units, 2 - english, 3 - metric
    OdUInt32 tw = 0, tl = 0;
    getTileSize(tw, tl);
    ...
    double dResolution = 1. / m_corrInfo.m_density;
    wrRational(*ps.get(), dResolution);
    wrRational(*ps.get(), dResolution);
