  ...
  if (pImageBuf.get())
  {
    try
    {
      ...
      if (!pRasSvcs.isNull())  // catch below is not enough to prevent an access violation on Solaris
      {
        ...
        OdGiRasterImage::Units units = resolutionUnits();
        OdGeVector2d pixelSize = resolutionMMPerPixel();
        if (!pImage.isNull())
          pImage = pImage->changeImageSource(OdGiRasterImage::kFromDwg);
        OdDbRasterImageDef::setImage(pImage, modifyDatabase);
        bool bWasOpenForWrite = isWriteEnabled();
        if (!bWasOpenForWrite)
        {
          disableUndoRecording(true);
          upgradeOpen();
        }
        setResolutionMMPerPixel(pixelSize);
        setResolutionUnits(units);
        if (!bWasOpenForWrite)
        {
          downgradeOpen();
