...
{
  return m_pWrapper->getThumbnail(m_nItem, 170, 170);
}

OdGiRasterImagePtr PdfUnderlayItemImpl::getThumbnail(int width, int height) const
{
  return m_pWrapper->getThumbnail(m_nItem, width, height);
}
