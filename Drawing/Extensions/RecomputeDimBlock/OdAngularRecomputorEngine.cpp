...
void OdAngularRecomputorEngine::transformBy(const OdGeMatrix3d& matr)
{
  OdRecomputorEngine::transformBy(matr);
...
void OdAngularRecomputorEngine::intersectTextBoxWithDimArc(bool bAddArcSym) // double gap, 
{
  ...
  OdGeCircArc2d circle;

  circle.set(OdGePoint2d(m_centerDimPtArc.x,m_centerDimPtArc.y), m_radiusDimArc, m_extLine1Angle, m_extLine2Angle);
  int num = 0;
  OdGePoint2d p1, p2;
  OdGeLineSeg2d line;
  OdGePoint3d minPt = m_textExtentsWithGap.minPoint();
  ...
  ODA_TRACE2("%f,%f,0.0\n\n", line.endPoint().x, line.endPoint().y);
#endif
  if ( circle.intersectWith(line, num, p1, p2) )
  {
...
void OdArcLenRecomputorEngine::calcExtLinePoints()
{
  ...
  OdGeTol geTol = OdGeContext::gTol;
  if ( m_centerPtArc.x > 1.e6 || m_centerPtArc.y > 1.e6 )
  {
    geTol = 1.e-8;
  }
  ...
  if ( m_angle < OdaPI2 )
  {
    ...
    if ( distArcToDim < 0.0 )
    {
      ...
      circle.set(m_arcPt.convert2d(), m_sourceArcRadius);
      OdGeLine2d line;
      line.set(OdGePoint2d(m_centerPtArc.x, m_centerPtArc.y), OdGePoint2d(p.x, p.y));

      circle.intersectWith(line,numInt, p1, p2, geTol);
    ...
    circle.set(m_centerDimPtArc.convert2d(), m_radiusDimArc);
    OdGeLine2d line;

    line.set(OdGePoint2d(m_xLine1Pt.x, m_xLine1Pt.y), OdGePoint2d(m_xLine1PtEnd.x, m_xLine1PtEnd.y));

    if ( circle.intersectWith(line, numInt, p1, p2, geTol) )
    {
...
void OdAngularRecomputorEngine::intersectTextBoxWithExtDimArc(OdGePoint3dArray& intersections)
{
  addGapToTextExtents();
  OdGeCircArc2d circle;
  circle.set(OdGePoint2d(m_centerDimPtArc.x,m_centerDimPtArc.y), m_radiusDimArc, m_extLine2Angle, m_extLine1Angle );
  int num = 0;
  OdGePoint2d p1, p2;
  OdGeLineSeg2d line;
