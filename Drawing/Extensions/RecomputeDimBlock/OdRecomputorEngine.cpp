...
void OdRecomputorEngine::transformBy(const OdGeMatrix3d& matr)
{
...
void OdRecomputorEngine::calcTextExtents(OdDbMTextPtr& pText, bool bForce, bool bApplyXText)
{
  //  Result: 
  ...
  if (m_bUseXText && bApplyXText)
  {
    m_textHeight = m_textExtents.maxPoint().y - m_textExtents.minPoint().y;
    m_textWidth = m_textExtents.maxPoint().x - m_textExtents.minPoint().x;
  }
}
...
double OdRecomputorEngine::widthRotText()
{
  ...
  double minX = odmin(odmin(odmin(p1.x, p2.x), p3.x), p4.x);
  double maxX = odmax(odmax(odmax(p1.x, p2.x), p3.x), p4.x);
  return maxX - minX;
}
...
OdDbMTextPtr OdRecomputorEngine::createXText(OdDbMTextPtr pText, OdInt16 dimtmove, bool bUserDefPos, double dTextRot, bool oneText)
{
  ...
  if (xPos != -1)
  {
    if ( !bUserDefPos && OdZero(dTextRot) && !oneText )
    {
    ...
    else
    {
      ...
      text.replace(OD_T("\\X"), OD_T(""));
      pText->setContents(text);
      if ((bUserDefPos && dimtmove <= 1) || oneText)
      {
        m_bUseXText = true;
      }
    }
