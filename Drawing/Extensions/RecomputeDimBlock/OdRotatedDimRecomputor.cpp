...
{
  const OdGeMatrix3d world2Plane(OdGeMatrix3d::worldToPlane(normal()));
...
{
  const OdGeMatrix3d mWorld2Plane(OdGeMatrix3d::worldToPlane(m_vNormal));
  ...
  const OdGeVector3d vDef(-sin(m_dRotAngle), cos(m_dRotAngle), 0.0);
  const OdGeVector3d vDef1(defPoint1 - defPoint2);
                                                    ...
                                                    const OdGePoint3d& defPoint2)
{
  ...
  if(OdNonZero(first.length()))
  {
    ...
    const double t = (second.x * (defPoint1.y - dimLineDefPt.y) - second.y * (defPoint1.x - dimLineDefPt.x)) /
