...
#define INTERSECTION_TOLERANCE  1.0e-8

void OdAlignedRecomputorEngine::transformBy(const OdGeMatrix3d& matr)
{
...
//    if ( m_asz1 + m_asz2 + m_gap*2.0 <= distance )
    if ( OdNegative(m_asz1 + m_asz2 + m_gap*2.0 - distance) )
...
double OdAlignedRecomputorEngine::distanceToHorText()
{
  ...
  const double h = textHeight()/2.0;
  const double w = textWidth()/2.0;
  const double angDiag = ::atan(h/w);
  const double diag = ::sqrt(h*h+w*w);
  ang = ang > OdaPI2 ? OdaPI - ang : ang;
  ang -= angDiag; 
  double distance = diag*::cos(ang);
  if ( m_isBox ) 
    distance += m_gap;
  return distance;
}
...
OdUInt32 OdAlignedRecomputorEngine::getIntersectionPoints(OdGePoint3d& p1, OdGePoint3d& p2, bool useDimLine)
{
  ...
  */
  ...
  double rotation = m_dTextRot; 
  
  if (m_xAxis != OdGeVector3d::kXAxis ) 
    rotation = m_xAxis.angleTo(OdGeVector3d::kXAxis);

  // OdGeMatrix2d matr = OdGeMatrix2d::rotation(m_dTextRot, OdGePoint2d(m_textPosition.x, m_textPosition.y));
  OdGeMatrix2d matr = OdGeMatrix2d::rotation(rotation, OdGePoint2d(m_textPosition.x, m_textPosition.y));
...
void OdAlignedRecomputorEngine::adjustUserDefText1(double /*asz*/, double /*exe*/)
{
  ...
  bool bLeaderOnDimLine = testTextLocation1();
  if ( m_bUseXText && m_bLeader )
  {
    OdGeVector3d v = m_ldrPt1 - m_ldrPt2;
    v.normalize();
    v *= textWidth();
    m_ldrPt1 += v;
  }
  ...
  } else
  {
    if (m_bUseRotate)
    {
      ...
      v.rotateBy(m_dTextRot, OdGeVector3d::kZAxis);
      m_textPosition = m_dimOcsTextPosition + v*(textWidth(false) / 2.0);
    }
  }
...
bool OdAlignedRecomputorEngine::testTextLocation1()
{
  ...
  if ( !OdNegative( distFromTxt2DimLine - m_asz - m_gap ) )
  {
  ...
  OdGePoint2d point;
  bool isIntersect = dimLine.intersectWith(leaderLine, point, INTERSECTION_TOLERANCE);

  bool b = (dimLineDir.isParallelTo(leaderLineDir, INTERSECTION_TOLERANCE) && dimLineDir.dotProduct(leaderLineDir) >= 0.0) || !isIntersect;
  ...
  double d1 = halfmeasurement + ::fabs(p.y);
  if ( b && ( ::fabs(testPt.y) > (m_asz+m_gap) ) && ( ::fabs(testPt.y) > halfmeasurement + ::fabs(p.y) ) )
  {
    m_bLeader = true;
  } else 
  {
    ...
    if (m_leaderDir.isZeroLength())
       m_leaderDir = -m_xAxis;
    else
      m_leaderDir.normalize();

    bLeaderOnDimLine = true;
    double distToArrowPoint = 0.0;
    if (pt.x > point1.x)
    {
      distToArrowPoint = point.distanceTo(m_arrow2Pt.convert2d());
      if ( distToArrowPoint < 2*m_asz )
      { 
        pt = m_arrow2Pt + 2 * m_asz * m_dirDimLine;
        makeLeaderOnDimLine(m_arrow2Pt, pt);
      } else
    ...
    {
      distToArrowPoint = point.distanceTo(m_arrow1Pt.convert2d());
      if (distToArrowPoint < 2 * m_asz)
      {
        pt = m_arrow1Pt - 2 * m_asz * m_dirDimLine;
        makeLeaderOnDimLine(m_arrow1Pt, pt);
      }
      else
...
void OdAlignedRecomputorEngine::testTextLocation2()
{
  OdGePoint3d oscTextPosition = m_textPosition;
  double halfTextHeight = textHeight()/2.0;
