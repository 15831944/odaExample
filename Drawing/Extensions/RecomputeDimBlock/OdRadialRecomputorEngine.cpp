...
void OdRadialRecomputorEngine::preprocessDimPoints(OdUInt16 dimMove)
{
  ...
  } else // horizontal or rotated text
  {
    ...
    {
      OdGeLine2d radialLine_(m_xLine1Pt.convert2d(), m_xLine2Pt.convert2d());
      OdGeVector2d dir = radialLine_.direction();
      dir.rotateBy(OdaPI2);
      OdGeLine2d perpLine(m_dimOcsTextPosition.convert2d(), dir);
      OdGePoint2d intPt_;
      perpLine.intersectWith(radialLine_, intPt_);

      m_dogLegPt.x = intPt_.x;
      m_dogLegPt.y = intPt_.y;
  ...
  } else
  {
    ...
    } else
    {
      ...
      } else if ( !m_bRadial )
      {
        m_bArrowsInside = true;
        m_bTextInside = true;
        if (textWidth(true) + 2.0*m_gap + 2.0*m_asz > m_radius*2.0 )
        {
          m_bArrowsInside = false;
          m_bTextInside = false;
        }
...
OdUInt32 OdRadialRecomputorEngine::getIntersectionPoints(OdGePoint3d& p1, OdGePoint3d& p2, bool useDimLine, bool bWithGap)
{
  ...
  double extX2 = textWidth(bWithGap)/2.0;
  double extY2 = textHeight(bWithGap)/2.0;
...
bool OdRadialRecomputorEngine::adjustArrowPlace2()
{
  OdGePoint3d p1;
  OdGePoint3d p2;
  OdInt32 numItersect = getIntersectionPoints(p1, p2, true, false);
