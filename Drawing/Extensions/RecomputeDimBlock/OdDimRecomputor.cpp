...
void OdDbDimRecomputePEImpl::recomputeDimBlock(OdDbDimension* pDim, OdDbDimensionObjectContextData* ctx)
{
  ...
  OdArray<OdDbEntityPtr>::iterator pSrcEnt = ents.begin();
  ...
  if (ctx)
  {
    if (pDim->isDBRO())
    {
      ...
      if (pBTR.isNull())
      {
        ...
        pBTR->setName(L"*D");
        OdDbBlockTablePtr pBT = pDim->database()->getBlockTableId().safeOpenObject(OdDb::kForWrite);
        ctx->setBlockIndex(pBT->add(pBTR));
      }
    }
    else
    {
...
void OdDbDimRecomputePEImpl::recomputeDimMeasurement(OdDbDimension* pDim)
{
  ...
  pDim->getDimstyleData(pDimVars);
  OdDimRecomputorPtr pDimRc = createRecomputor();
  pDimRc->getDimParams(pDim, nullptr);
  pDimRc->m_dMeasurement = -1.;
  pDimRc->preprocessDimPoints(pDimVars);
  pDimRc->updateMeasurement(pDimVars);
  setMeasurementValue(pDim, pDimRc->m_dMeasurement);
  pDimRc->setDimParams(pDim, nullptr);
}
...
, m_dTextHeight(0.0)
, m_bInspection(false)
, m_nInspectionFrame(0)
, m_AttachPt(0)
, m_nLineweight(0)
, m_dLinetypeScale(0)
, m_dLinespacingFactor(0.0)
...
void OdDimRecomputor::getDimParams(const OdDbDimension* pDim, OdDbDimensionObjectContextData* ctx)
{
  ...
  if (m_dTextRot >= Oda2PI)
  {
    m_dTextRot = fmod(m_dTextRot, Oda2PI);
  }
...
{
  int entier = 0, numerator = 0, denominator = 0;
  // rounding value
  if(fabs(rnd) > 1.0e-10)
  {
    value = OdRound(value / rnd) * rnd;
                                                  ...
                                                  OdString dimPost, bool bPrimaryUnits, bool dimAlt)
{
  ...
  {
    if ( !dimAlt && suffix.getLength() >= 2 )
    { 
      if (suffix.getAt(suffix.getLength() - 2) == '\\' && suffix.getAt(suffix.getLength() - 1) == 'X')
        suffix.replace(OD_T("\\X"), OD_T(""));
    }
    measurement = prefix + measurement + suffix;
  }
}
                                               ...
                                               OdDbDimStyleTableRecord* pDimVars)
{
  ...
  if(m_strDimText.isEmpty() || (quotesNum != -1))
  {
    ...
    formatDimStrWithLimsAndTols(measurement, isLims, isTol, dimVal,
      ...
      dimDec, dimPost, true, getDimalt(pDimVars));
    ...
    if ( bUseAltUnitVar || bUseAltUnitStr ) // getDimalt(pDimVars))
    {
      ...
      formatDimStrWithLimsAndTols(alternative, isLims, isTol, m_dMeasurement * altFactor,
        ...
        getDimaltd(pDimVars), getDimapost(pDimVars), false, getDimalt(pDimVars));
...
static OdString convertToMText(OdString textStr)
{
  ...
  const int srcSize = textStr.getLength();
...
void OdDimRecomputor::makeInspectionFrame(OdDbDimStyleTableRecord* pDimVars, OdRecomputorEngine* pEngine, OdArray<OdDbEntityPtr>& ents)
{
  ...
  const OdGePoint3d  pos = pEngine->m_textPosition;
  const OdGeVector3d dir = pEngine->m_textDirection;
  const double textWidth = pEngine->m_textExtents.maxPoint().x - pEngine->m_textExtents.minPoint().x;
  const double labelWidth = pEngine->m_labelExtents.maxPoint().x - pEngine->m_labelExtents.minPoint().x;
  const double rateWidth = pEngine->m_rateExtents.maxPoint().x - pEngine->m_rateExtents.minPoint().x;
  ...
  } else
  {
    ...
    if ( getDimgap(pDimVars) < 0.0 )
    {
      ...
      const OdGePoint3d p1 = first;
      const OdGePoint3d p2 = second;
...
bool OdDimRecomputor::setUseDimLineIndex(OdDbDimension* pDim)
{
  ...
  if (pExtDic.get())
  {
    ...
    if ( !brData.isNull() )
    {
      ...
      const OdInt32 amPoints = pntRefAray.size();
...
OdInt32 dimLineIndex(const OdDbEntity* entity)
{
...
void removeDimLineIndex(OdDbEntity* entity)
{
  OdResBufPtr xd = entity->xData(OD_T("Index"));
  if ( xd.isNull() )
    return;
