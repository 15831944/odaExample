                                                        ...
                                                        bool bPrimaryUnits, bool dimAlt)
{
  ...
  const int numQuotesPost = dimPost.find(OD_T("<>"));  // dimpost - primary units 
  const int numQuotesApost = dimPost.find(OD_T("[]")); // dimapost - alternate units
...
{
  OdGePlane dimPlane(m_DimLineDefPt, m_vNormal);
  OdGePoint3d pCenter = m_DefPointCenter.orthoProject(dimPlane); // m_DefPointCenter;
  m_dMeasurement = m_DimLineDefPt.distanceTo(pCenter);
                                           ...
                                           OdDbDimStyleTableRecord* pDimVars)
{
  ...
  OdGePlane dimPlane(m_DimLineDefPt, m_vNormal);
  m_DefPointCenter = m_DefPointCenter.orthoProject(dimPlane);
  ...
  double asz1 = 0.0, asz2 = 0.0, asz = 0.0;
  ...
  } else
  {
    ...
    if ( engine.m_bRadial && dm == 0 && engine.m_atfit != 3 && !engine.m_bTextHor && !engine.m_bUseRotate )
    {
      ...
      OdGePoint3d pt = m_DefPointCenter;
      pt.transformBy(mWorld2Plane);

      m_DefPointCenter = engine.m_xLine2Pt;
      m_DefPointCenter.z = pt.z;
  ...
  {
    lineweight = (OdDb::LineWeight)m_nLineweight;
    // Need to check 
