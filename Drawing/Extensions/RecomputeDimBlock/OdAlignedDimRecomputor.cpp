                                            ...
                                            OdDbDimStyleTableRecord* pDimVars)
{
  ...
  if (engine.m_bUseXText && getDimtmove(pDimVars) == 1)
    tad = 0;
  ...
  { 
    ...
    if ( !pText.isNull() )
    {
      engine.calcTextExtents(pText, false, true);
      if ( m_bInspection )
      {
        engine.calcInspectionExtents(pLabel, pRate, true);
      }
  ...
  if ( !pText.isNull() )
  {
    ...
    engine.calcTextExtents(pText, true, false);
    engine.m_dTextRot = pText->rotation();
  } else
  {
    engine.m_bNeedCalcTextIntersection = false;
  ...
  ADD_POINT(/*mPlane2World */ m_DimLineDefPt);
}
