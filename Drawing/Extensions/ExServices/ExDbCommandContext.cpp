...
void ExDbCommandContext::highlight(const OdDbBaseFullSubentPath& subEntPath, 
                                   bool bDoIt) const //  = true
{
...
bool ExDbCommandContext::merge(OdSelectionSet* pRes,
                               int options,
                               ...
                               OdEdUserIO* pIO) //  = NULL
{
  ...
  OdUInt32 numResEntities = pRes->numEntities(),
           numResSubentities = pRes->subentCount(),
  ...
  for (pIter = pSSet->newIterator(); !pIter->done(); pIter->next())
  {
    ...
    if (bAppending)
    {
      if (!pTracker || bTrackSubentities && numSubent>0 || pTracker->append(id, pIter->method()))
      {
        ...
        for (OdUInt32 se = 0; se < numSubent; se++)
        {
          OdDbBaseFullSubentPath subEntPath;
          if(pIter->getSubentity(se, subEntPath)) {
            if(!pTracker || !bTrackSubentities || pTracker->append(subEntPath, pIter->method())) {
    ...
    {
      if (!pTracker || bTrackSubentities && numSubent>0 || pTracker->remove(id, pIter->method()))
      {
        if (!bTrackSubentities || numSubent==0)
          highlight(pIter, bAppending);
               ...
               !pResIter->done(); pResIter->next())
          {
            ...
            if (pResIter->subentCount())
            {
              for (OdUInt32 se = 0; se < numSubent; se++)
              {
                OdDbBaseFullSubentPath subEntPath;
                if (pIter->getSubentity(se, subEntPath)) {
                  if(!pTracker || !bTrackSubentities || pTracker->remove(subEntPath, pIter->method())) {
                    if(bTrackSubentities)
                      highlight(subEntPath, bAppending);
  ...
  do
  {
    ...
    try
    {
      try
      {
        ...
        if(!bSelected && !GETBIT(options, OdEd::kSelSingleEntity))
        {
...
inline void printInvalidInputMessage(OdDbUserIO& io, const OdString &keywords) {
                                   ...
                                   OdEdIntegerTracker* pTracker)
{
  ...
  if(bDef)
  {
    if (prompt.find(L'<') < 0) { //skip if already present
  ...
  for(;;)
  {
    ...
    try
    {
      ...
      sInput = getString(prompt, options,
                         bDef ? kwIndex.at(defVal).keyword() : OdString::kEmpty, sKeywords, pTracker ? &trackerProxy : 0);
    }
