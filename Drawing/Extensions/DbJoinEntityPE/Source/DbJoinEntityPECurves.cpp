...
class ComparerPositionAndIndex
{
public:
...
bool findCurveSequence(std::list<CurveIndex>& listIndex, const OdDbCurvePtr primaryEntity, const OdDbEntityPtrArray& otherEntities, const OdRxClass** aObjDesc, int nObjDesc, bool bCheckPlanarity)
{
  ...
  for(i = 0; i < nEnt; ++i)
  {
    ...
    if(bCheckPlanarity)
    {
      ...
      if (planarity == OdDb::kLinear)
        plane.set(ptStart, normalPrimary); //plane of linear entities is irrelevant
  ...
  case OdGe::kCompositeCrv3d:
    {
      ...
      const OdGeCurve3dPtrArray& aCrv = pComposite->getCurveList();
      unsigned int i;
