...
public:
  DwfProperties() : _paperWidth( 297 ), _paperHeight( 210 ),_background(0xffffffff), 
    _preserveColorIndices(false), // true uses only for roundtrip of ODA export/import in old DWF formats with palette // CORE-14680
    _layoutNumber(-1), 
    _importW3d(true), 
  ...
  bool get_MarkupFrozen()const { return _markupFrozen; }
  void put_MarkupFrozen(bool b) { _markupFrozen = b; }
...
ODRX_DECLARE_PROPERTY(MarkupFrozen)
...
ODRX_DEFINE_PROPERTY(MarkupFrozen, DwfProperties, getBool)
...
ODRX_BEGIN_DYNAMIC_PROPERTY_MAP( DwfProperties );
  ...
  ODRX_GENERATE_PROPERTY( MarkupFrozen )
...
OdDwfImport::ImportResult DwfImporter::import(OdString* pWarnings) // = NULL
{
  OdString sTemp;
  if (!pWarnings)
    pWarnings = &sTemp;
  OdString& sWarnings = *pWarnings;
  ...
  DWFPackageReader::tePackageType type = dwfImp::DWFVersion(stream);
  switch (type)
  {
	...
	case DWFPackageReader::eDWFPackageEncrypted:  // Password protected DWF 6 (and higher) zip package format.
    {
      ...
      w.setProgressMeter(&pm);
		  return loadPackage( w, _properties->get_Password(), pm, sWarnings);
    }
	...
	case DWFPackageReader::eW2DStream:            // 2D graphics stream/file resource.
      ...
      catch (const char* pcszErr)
      {
        ...
        sWarnings.format(L"Invalid format of DWF file"); // CORE-17757
        res = OdDwfImport::bad_file;
      }
...
OdDwfImport::ImportResult DwfImporter::load2dResourceXps(DWFToolkit::DWFPackageReader* pReader, 
                                                         DWFToolkit::DWFSection* pSection, 
                                                         const wchar_t* pRoleName, 
                                                         double dPaperWidth, double dPaperHeight, 
                                                         OdDbPlotSettings::PlotPaperUnits& units, 
                                                         DWFImportProgressMeter& pm, 
                                                         int layoutCount, bool bForExtents)
{
  _ignoreMetadata = false;
  ...
  if (DWFToolkit::DWFResourceContainer::ResourceIterator* piResources = pSection->findResourcesByRole( DWFXML::kzRole_Font))
  {
    for (; piResources->valid(); piResources->next())
    {
      if (DWFFontResource* pFontRes = dynamic_cast<DWFFontResource*>(piResources->get()))
      {
        ...
        try { 
        ...
        catch (const DWFException& err) {
          // CORE-16665 // ODA_FAIL_ONCE_X(TDWF);
          DWFCORE_FREE_OBJECT(ef);
          OdString sMsg = err.message();
          bool bMissing = sWarnings.find(sMsg) < 0;
          if (bMissing)
          {
          ...
          if (!name.isEmpty())
            sWarnings += (bMissing ? L" " : L",") + name;
          continue; // throw OdError(err.message());
        }

        OdString loadedName = (const wchar_t*)OdUS2W(ef->loadedFontName());
...
void DwfImporter::loadRasterResource(DWFToolkit::DWFPackageReader* pReader, DWFToolkit::DWFSection* pSection, const wchar_t* pRoleName,
                                     double& dPaperWidth, double& dPaperHeight, OdDbPlotSettings::PlotPaperUnits& units, // in/out
                                     DWFToolkit::DWFPaper* pPaper) // = NULL
{
  if (DWFToolkit::DWFResourceContainer::ResourceIterator* piResources = pSection->findResourcesByRole( pRoleName ))
  {
    for (; piResources->valid(); piResources->next())
    {
      if (DWFImageResource* pRasterRes = dynamic_cast<DWFImageResource*>(piResources->get()))
      {
        ...
        if (width < OdGeContext::gTol.equalPoint() || height < OdGeContext::gTol.equalPoint())
        {
          // CORE-15746 DwfImport produces corrupted result in OdaMfcApp
          minpt = OdGePoint2d(pRasterRes->originalExtents()[0], pRasterRes->originalExtents()[1]);
          maxpt = OdGePoint2d(pRasterRes->originalExtents()[2], pRasterRes->originalExtents()[3]);
          width = fabs(maxpt.x - minpt.x);
          height = fabs(maxpt.y - minpt.y);
          ODA_ASSERT_ONCE_X(TDWF,    width > OdGeContext::gTol.equalPoint() 
                                  && height > OdGeContext::gTol.equalPoint());
          if (!pPaper || !pPaper->show())
          {
            ...
            ODA_ASSERT_VAR(else)
              ODA_ASSERT_ONCE_X(TDWF, units == OdDbPlotSettings::kMillimeters);
          }
        }
...
OdDwfImport::ImportResult DwfImporter::load2dResource(DWFToolkit::DWFPackageReader* pReader, 
                                                      DWFToolkit::DWFSection* pSection, 
                                                      const wchar_t* pRoleName, 
                                                      double dPaperWidth, double dPaperHeight, 
                                                      OdDbPlotSettings::PlotPaperUnits& units, 
                                                      DWFImportProgressMeter& pm, 
                                                      int layoutCount, bool bForExtents, OdString& sWarnings)
{
  bool bMarkUp = (OdString(pRoleName) == DWFXML::kzRole_Graphics2dMarkup);
  ...
  if (DWFToolkit::DWFResourceContainer::ResourceIterator* piResources = pSection->findResourcesByRole( pRoleName ))
  {
    for (; piResources->valid(); piResources->next())
    {
      if (DWFGraphicResource* pW2D = static_cast<DWFGraphicResource*>(piResources->get()))
      {
        OdString sMime = (const wchar_t*) pW2D->mime(); //kzMIMEType_W2D
        if (sMime == DWFCore::DWFMIME::kzMIMEType_W2D_S)
        {
        ...
        if (bForExtents)
        {
          ...
          try {
            WT_File wtFile;
            //CORE-16665 seek is unsupported directly for pW2DStream //res = loadStream(pW2DStream, wtFile);
            OdStreamBufPtr pStream = dwfImp::toOdStreamBuf(pW2DStream);
            DWFInputStreamWrapper w(pStream);
            res = loadStream(&w, wtFile);
          }
          catch (const DWFException& err) {
            // CORE-16665 // ODA_FAIL_ONCE_X(TDWF);
...
OdDwfImport::ImportResult DwfImporter::getPrimaryUnits(DWFToolkit::DWFPackageReader* pReader,
                                                       DWFToolkit::DWFSection* pSection,
                                                       const wchar_t* pRoleName, // DWFXML::kzRole_Graphics2d
                                                       OdString& sWarnings)
{
  if (!pRoleName || !*pRoleName)
    pRoleName = DWFXML::kzRole_Graphics2d;
    ...
    for (; piResources->valid(); piResources->next())
    {
      if (DWFGraphicResource* pW2D = static_cast<DWFGraphicResource*>(piResources->get()))
      {
        ...
        _extent._collectBounds = true;
        {
          try {
            WT_File wtFile;
            //CORE-16665 seek is unsupported directly for pW2DStream //retval = loadStream(pW2DStream, wtFile);
            OdStreamBufPtr pStream = dwfImp::toOdStreamBuf(pW2DStream);
            DWFInputStreamWrapper w(pStream); 
            retval = loadStream(&w, wtFile);
          }
          catch (const DWFException& err) {
            // CORE-16665 // ODA_FAIL_ONCE_X(TDWF);
            DWFCORE_FREE_OBJECT(pW2DStream);
        ...
        if (_extent._useUnits)
        {
          ...
          _extent._units.setToIdentity(); // should be changed later
        }
        ODA_ASSERT_VAR(else)
          ODA_ASSERT(true); // brk
        break;
...
OdDwfImport::ImportResult DwfImporter::loadPackage(DWFInputStream& stream, const OdChar* password,
                                                   DWFImportProgressMeter& pm, OdString& sWarnings)
{
  ImportResult retval = OdDwfImport::success;
    ...
    for (; piSections->valid(); piSections->next())
    {
      ...
      if (pSection && (pSection->type() == DWFString(_DWF_FORMAT_EPLOT_TYPE_STRING)))
      {
        ...
        loadRasterResource(pReader, pSection, DWFXML::kzRole_RasterOverlay, 
                           dPaperWidth, dPaperHeight, units, // in/out
                           pPaper);
        if (_xps)
        {
        ...
        else
        {
          ...
          if (pPaper && units == OdDbPlotSettings::kInches)
          {
            ...
            _extent.setPaperInfo(dPaperWidth, dPaperHeight, OdGePoint3d(dPaperWidth / 2.0, dPaperHeight / 2.0, 0.0), units, // CORE-10319
                                 pPaper->show()); // for problem with paper size of CORE-11621
            if (!pPaper->show())
              matchPrintableArea = true;
          }
	        loadFontResources(pReader, pSection, sWarnings);
          CHECK_RESULT(getPrimaryUnits(pReader, pSection, DWFXML::kzRole_Graphics2d, sWarnings));
          CHECK_RESULT(load2dResource(pReader, pSection, DWFXML::kzRole_Graphics2dOverlay, dPaperWidth, dPaperHeight, units, pm, layoutCount, true, sWarnings));
          _extent.setPlotSettings(dPaperWidth, dPaperHeight, units, matchPrintableArea);
          CHECK_RESULT(load2dResource(pReader, pSection, DWFXML::kzRole_Graphics2d, dPaperWidth, dPaperHeight, units, pm, layoutCount, false, sWarnings));
          CHECK_RESULT(load2dResource(pReader, pSection, DWFXML::kzRole_Graphics2dOverlay, dPaperWidth, dPaperHeight, units, pm, layoutCount, false, sWarnings));
          CHECK_RESULT(load2dResource(pReader, pSection, DWFXML::kzRole_Graphics2dMarkup, dPaperWidth, dPaperHeight, units, pm, layoutCount, false, sWarnings));
        }
