...
class DWFImportProgressMeter
{
  ...
  OdDbDatabase* _db;
public:
  DWFImportProgressMeter(OdDbDatabase* db, const OdString& sText = L"DWF Package loading") : _current(0), _db(db)
  {
    m_pOuterMeter = db->appServices()->newProgressMeter();
    m_pOuterMeter->setLimit(100);
    m_pOuterMeter->start(sText);
  }
...
private:
  DWFImportProgressMeter(); // disabled // CORE-16301 
  DWFImportProgressMeter(const DWFImportProgressMeter& other); // disabled
};
...
public:
  ...
  virtual void notify( const void* const /*pBuffer*/, size_t /*nBytesRequested*/, size_t nBytesRead ) 
  {
    _current += nBytesRead;
