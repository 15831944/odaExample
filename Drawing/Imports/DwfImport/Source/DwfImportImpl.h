...
// VIS-886 //#define DWFIMPORT_TEXTURE_PREFIX_BY_SOURCE_DWF_FILE // CORE-14394 // TODO comment define DWFIMPORT_TEXTURE_PREFIX_BY_SOURCE_DWF_FILE in code & check result when VIS-886 will be fixed
// CORE-14394 //#define DWFIMPORT_PRESAVE_TEXTURE_FILES

#if defined(DWFIMPORT_PRESAVE_TEXTURE_FILES)
#  define OdGiRasterImageFileTexturePtr OdGiImageFileTexturePtr
#  define OdGiRasterImageFileTexture OdGiImageFileTexture
#endif

inline void textureToMaterialMap(OdGiMaterialTexturePtr pTexture, OdGiMaterialMap& materialMap)
{
  if (pTexture.isNull())
    return;
  materialMap.setTexture(pTexture);
}
...
public:
  ...
  std::map<OdString, OdGiRasterImageFileTexturePtr> m_textures; // for W3D only
  DwfImporter();
  OdDbDatabase* database(){return _db;}
  OdRxDictionaryPtr properties();
  ImportResult import(OdString* pWarnings = NULL);
  std::map<OdString, OdString> _embeddedFontMap;
private:
	
	// load DWF (ver >= 600)
  // return values described in "Dwfimport.h"
	ImportResult loadPackage(DWFInputStream& stream, const OdChar* password, DWFImportProgressMeter& pm, OdString& sWarnings);
  ImportResult getPrimaryUnits(DWFToolkit::DWFPackageReader* pReader, DWFToolkit::DWFSection* pSection, 
                               const wchar_t* pRoleName, OdString& sWarnings);
  ImportResult load2dResource(DWFToolkit::DWFPackageReader* pReader, DWFToolkit::DWFSection* pSection, const wchar_t* pRoleName,
                              double dPaperWidth, double dPaperHeight, OdDbPlotSettings::PlotPaperUnits& units, 
                              DWFImportProgressMeter& pm, int layoutCount, bool bForExtents, OdString& sWarnings);
  ImportResult load2dResourceXps(DWFToolkit::DWFPackageReader* pReader, DWFToolkit::DWFSection* pSection, const wchar_t* pRoleName,
                                 double dPaperWidth, double dPaperHeight, OdDbPlotSettings::PlotPaperUnits& units, 
                                 DWFImportProgressMeter& pm, int layoutCount, bool bForExtents);
  ...
  void loadFontResources(DWFToolkit::DWFPackageReader* pReader, DWFToolkit::DWFSection* pSection, OdString& sWarnings);
...
namespace dwfImp
{
  ...
  class OdGeMatrix3d& normalizeScale(OdGeMatrix3d& matrix);
  ...
  // convert DWFInputStream to ODA stream
