...
namespace dwfImp
{
  ...
  OdDb::UnitsValue convertUnits(const WT_Units& units)
  {
    ...
    OdString sUnits = WTS2ODS(units.units());
    sUnits.makeLower();
    for (int i = 0, sz = int(sizeof(Units) / sizeof(Units[0])); i < sz; ++i)
    {
      if (sUnits == OdString(Units[i]).makeLower())
...
WT_Result DwfCallbackManager::process_view(WT_View& view, WT_File& file )
{
  ...
  if (idBlock == imp->database()->getModelSpaceId())
  {
    OdDbViewportTablePtr t = imp->database()->getViewportTableId().safeOpenObject();
    OdDbViewportTableRecordPtr vp = t->getActiveViewportId().safeOpenObject(OdDb::kForWrite);
    if (!OdZero(maxpt.y - minpt.y)) // CORE-14977
      vp->setHeight(fabs(maxpt.y - minpt.y));
    vp->setWidth(fabs(maxpt.x - minpt.x));
    vp->setCenterPoint(OdGePoint2d(center.x, center.y));
  }
...
WT_Result DwfCallbackManager::process_png_group4_image(WT_PNG_Group4_Image& image, WT_File& file)
{
  ...
  ODA_ASSERT_ONCE_X(TDWF,    bSwapped == (image.identifier() != 0) // image.m_identifier
                          || !file.filename().length() || dwfImp::WTS2ODS(file.filename()).makeLower().find(L".dwfx") > 0); // Xps
  if (bSwapped)
...
{
  //ODA_ASSERT_VAR(static int s_idx = 0;) ODA_ASSERT_VAR(static int s_catch = 85;)
  //ODA_ASSERT_VAR(if (s_catch && s_catch == s_idx++))
  //  ODA_ASSERT(true);
  ...
  if (!(bool)OdRxVariantValue(importer(file)->properties()->getAt(L"ProcessGradients")))
    return WT_Result::Success;
  int cnt = gpt.count();
  if (cnt < 3) // CORE-16191 // if (gpt.count() != 3)
  {
    ODA_FAIL_ONCE();
    return WT_Result::Success;
  }
  ODA_ASSERT_VAR(else if (cnt > 3))
    ODA_ASSERT(true); // brk

  for (int idx = 0; (idx + 2) < cnt; idx++) // CORE-16191
  {
    OdDbHatchPtr h = OdDbHatch::createObject();
    h->setDatabaseDefaults(importer(file)->database());
    WT_Logical_Point pts[3] = { gpt.points()[idx], gpt.points()[idx + 1], gpt.points()[idx + 2] };
    WT_RGBA32 cols[3] = {gpt.colors()[idx], gpt.colors()[idx + 1], gpt.colors()[idx + 2]};
    bool bForceColor0 = false;
    if (cnt == 3) // CORE-16191 we have a bit better result in ACAD without sorting (cnt is 5 for attached DWF)
    ...
    if (cols[0].m_whole == cols[2].m_whole)
    {
      ...
      // CORE-16191
      if (cols[0].m_rgb.a && cols[0].m_rgb.a != 0xff)
      {
        OdCmTransparency trn((cols[0].m_rgb.a + 0.5) / 255.0);
        h->setTransparency(trn);
      }
      // CORE-17873 // if (cols[0].m_rgb.r != 255 || cols[0].m_rgb.g != 255 || cols[0].m_rgb.b != 255)
      {
        OdCmColor gradientColors[2]; gradientColors[0] = gradientColors[1] = c;
        double gradientValues[2] = { 0.0, 1.0 }; // OdDbHatchImpl::setGradientColors is hardcoded fot these values
    ...
    WT_Rendition& rend = file.rendition();
    WT_Color colorSv;
    if (bForceColor0)
      bForceColor0 = (rend.color().materialized() != 0); // CORE-17873
    if (bForceColor0)
    {
      colorSv = rend.color();
      rend.color() = cols[0]; // CORE-17873
    }

    importer(file)->blocks().addEntity(h, file);

    if (bForceColor0)
      rend.color() = colorSv; // CORE-17873
...
WT_Result DwfCallbackManager::process_text(WT_Text& text, WT_File& file)
{
  ...
  if (ts.isTtfFont())
  {
    ...
    giContext.setUseTtfTriangleCache(false);
...
WT_Result DwfCallbackManager::process_viewport( WT_Viewport& vp, WT_File& file )
{
  ...
  if (mngExtent._collectBounds && vp.contour() != 0)
  {
    ...
    if (vp.contour() != 0) 
    {
      WT_Logical_Box box = const_cast<WT_Contour_Set&>(*vp.contour()).bounds();

      ODA_ASSERT_VAR(double width = box.m_max.m_x - box.m_min.m_x;)
      ODA_ASSERT_VAR(double height = box.m_max.m_y - box.m_min.m_y;)

      mngExtent.setClipRect(OdGePoint2d(box.m_min.m_x, box.m_min.m_y),
                            OdGePoint2d(box.m_max.m_x, box.m_max.m_y));
    }
    else
    {
      mngExtent.setClipRect(OdGePoint2d::kOrigin, OdGePoint2d::kOrigin); // WT_Logical_Box(0, 0, 0, 0));
    }
    return WT_Result::Success;
  }
  ...
  ODA_ASSERT_VAR(double width = odmax(fabs(bounds[1].x - bounds[0].x), fabs(bounds[2].x - bounds[0].x));)
  ODA_ASSERT_VAR(double height = odmax(fabs(bounds[1].y - bounds[0].y), fabs(bounds[2].y - bounds[0].y));)
