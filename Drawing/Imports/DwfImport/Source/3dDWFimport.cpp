...
DECLARE_OPCODE_CLASS(DD_TK_Cylinder, TK_Cylinder);

TK_Status DD_TK_Cylinder::Execute(BStreamFileToolkit & tk)
{
  TK_Status res = TK_Cylinder::Execute(tk);

  OdGePoint3d ptStart(GetStart()[0], GetStart()[1], GetStart()[2]),
              ptEnd(GetEnd()[0], GetEnd()[1], GetEnd()[2]);
  OdGeVector3d vX, vY, vZ = ptEnd - ptStart;
  double height = vZ.normalizeGetLength(),
         radius = GetRadius();
  if (OdZero(height) || OdZero(radius))
    return res; // CORE-15064 skip entity with invalid date to avoid Invalid Input error 
  OdGePoint3d ptCenter = ptStart + (vZ * (height / 2.0));
  // it looks like GetCaps() is unsupported by Design Review too

#if 1
  OdDb3dSolidPtr pEnt = OdDb3dSolid::createObject();
  pEnt->setDatabaseDefaults(m_pImp->database());
  pEnt->createFrustum(height, radius, radius, radius);
#else

  switch (GetCaps()) // m_flags
  {
  default:
  case TKCYL_FIRST: // 1 cap 'first' side
  case TKCYL_SECOND: // 2 cap 'second' side 
    ODA_FAIL_ONCE_X(TDWF);
  case TKCYL_NONE: // 0 no caps
    break; // TODO implement pipe for split cylinder by Navisworks export  CORE-13825
  case TKCYL_BOTH: // 3 cap both sides
    break;
  }
  ...
  //pEnt->setDatabaseDefaults(m_pImp->database());
  //pEnt->setStartPoint(OdGePoint3d::kOrigin - (OdGeVector3d::kZAxis * (height / 2.0)));
  //pEnt->setEndPoint(OdGePoint3d::kOrigin + (OdGeVector3d::kZAxis * (height / 2.0)));
  //
  OdDbSubDMeshPtr pEnt = OdDbSubDMesh::createObject();
  pEnt->setDatabaseDefaults(m_pImp->database());
  //OdInt32 divAxis = 10, divHeight = 1, divCap = 3, // divAxis = 3, divHeight = 3, divCap = 3, 
  //        subDLevel = 4;
  OdInt32 divAxis = 10, divHeight = 1, divCap = 1, // divAxis = 3, divHeight = 3, divCap = 3, 
          subDLevel = 4;
  pEnt->setCylinder(radius, radius, height, divAxis, divHeight, divCap, subDLevel);
#endif
  ODA_ASSERT_ONCE_X(TDWF, pEnt.get());
  if (pEnt.isNull())
    return TK_Error;

  OdGeMatrix3d matSeg = m_pImp->_currentSegment->_matrix;
  m_pImp->_currentSegment->_matrix.setToIdentity();
  ...
  vY.normalize();
  mat.setCoordSystem(ptCenter, vX, vY, vZ);
  ODA_ASSERT_ONCE_X(TDWF,    (mat.isUniScaledOrtho() && !mat.isSingular()) 
                          || OdDb3dSolid::cast(pEnt).isNull());
  mat.preMultBy(matSeg);

  if (OdDb3dSolid::cast(pEnt).get() && !mat.isUniScaledOrtho())
  {
    ODA_ASSERT_ONCE_X(TDWF, !matSeg.isUniScaledOrtho()); // via tolerance problem
    // unsupported by OdDb3dSolid::transformBy

    OdGePoint3d origin; OdGeVector3d x, y, z;
    mat.getCoordSystem(origin, x, y, z);
    ODA_ASSERT_ONCE_X(TDWF,    OdEqual(x.length(), 1.0, 1e-5)
                            && OdEqual(x.length(), 1.0, 1e-5)
                            && OdEqual(x.length(), 1.0, 1e-5));
    mat.setCoordSystem(origin, x.normalize(), y.normalize(), z.normalize());
  }
  ODA_ASSERT_ONCE_X(TDWF, mat.isUniScaledOrtho() && !mat.isSingular());

  m_pImp->_currentSegment->_matrix = mat;
  m_pImp->add3dObject(pEnt, TKO_Geo_Face);
  m_pImp->_currentSegment->_matrix = matSeg; // restore
...
TK_Status DD_TK_Shell::Execute(BStreamFileToolkit & tk)
{
  ...
  if (GetFaceExists() || GetFaces()) // CORE-14678 // if (GetFaceExists() || (useCollector && GetFaces()))
  {
  ...
  {
    ODA_ASSERT_ONCE_X(TDWF,    mp_paramwidth == 3
                            || mp_paramwidth == 2); //  attachments of CORE-13825 // TODO
    if (mp_paramwidth == 3)
    {
  ...
  //ODA_ASSERT_VAR(OdString sHandle = m_pImp->database()->handseed().ascii();)
...
OdResult DwfImporter::add3dObject(OdDbEntity* ent, TKO_Geometry_Bits mask)
{
...
#ifdef _DEBUG
  OdString sDebUrl = L"handle:";
  if (_currentSegment->_parent && !_currentSegment->_parent->_name.isEmpty())
    sDebUrl.format(L"parent:%ls ", _currentSegment->_parent->_name.c_str());
  if (!_currentSegment->_name.isEmpty())
    sDebUrl += OdString().format(L"seg:%ls ", _currentSegment->_name.c_str());
  OdString sHandle = ent->objectId().getHandle().ascii();
  ...
  ODA_ASSERT_VAR(static char pszCatch[5] = { '\0' };)
  ...
  OdResult res = ent->transformBy(_currentSegment->_matrix);
  if (res == eCannotScaleNonUniformly)
  {
    OdGePoint3d origin; OdGeVector3d x, y, z;
    _currentSegment->_matrix.getCoordSystem(origin, x, y, z);
    if (x.isPerpendicularTo(y) && y.isPerpendicularTo(z) && z.isPerpendicularTo(x))
    {
      // CORE-16539
      double xLen = x.length(), yLen = y.length(), zLen = z.length(),
             dTolRude = 1e-7;
      if (   OdEqual(xLen, yLen, dTolRude) && OdEqual(xLen, zLen, dTolRude) 
          && OdEqual(yLen, zLen, dTolRude))
      {
        x.normalize(); y.normalize(); z.normalize();
        _currentSegment->_matrix.setCoordSystem(origin, x, y, z);

        res = ent->transformBy(_currentSegment->_matrix);
      }
    }
  }
  ODA_ASSERT_ONCE_X(TDWF, res == eOk);
                                         ...
                                         OdGiMaterialMap* pMap = NULL)
{
  ...
  if (sName.find(L'/') >= 0)
  {
    sName = sName.mid(sName.reverseFind(L'/') + 1);
    if (sName.isEmpty())
      return OdGiMaterialTexturePtr();
  }
  std::map<OdString, OdGiRasterImageFileTexturePtr>::const_iterator pItr = pCtx->m_textures.find(sName);
...
TK_Status DD_TK_Color::Execute(BStreamFileToolkit & tk)
{
  ...
  if (channels & (1 << TKO_Channel_Diffuse))
  {
    ...
    textureToMaterialMap(pTexture, diffuseMap);
  ...
  if (channels & (1 << TKO_Channel_Specular))
  {
    ...
    textureToMaterialMap(pTexture, specularMap);
      ...
      && (GetTransmission()[0] > 0. || GetTransmission()[1] > 0. || GetTransmission()[2] > 0.))
  {
    ...
    textureToMaterialMap(pTexture, opacityMap);
...
//#endif
      ...
      && (GetEmission()[0] > 0. || GetEmission()[1] > 0. || GetEmission()[2] > 0.)) //core-10376
  {
    ...
    textureToMaterialMap(pTexture, emissionMap);
  ...
  if (channels & (1 << TKO_Channel_Bump))
  {
    ...
    if (pTexture.get())
    {
      ...
      textureToMaterialMap(pTexture, bumpMap);
...
TK_Status DD_TK_Text::Execute(BStreamFileToolkit & tk)
{
  ...
  text->setDatabaseDefaults(m_pImp->database()); // apply current text style ... // CORE-16539
  ...
  OdString sText;
  enum TKO_Text_Encodings encd = (enum TKO_Text_Encodings) GetEncoding();
  switch (encd)
  {
  case TKO_Enc_Unicode32: case TKO_Enc_Unicode: case TKO_Enc_Raw_16:
    sText = dwfImp::WTS2ODS(m_string, m_length); // CORE-16581 Linux problem // = (const wchar_t*)GetString();
    break;
  default:
    sText = OdString(GetString());
    break;
  }
  ODA_ASSERT_ONCE_X(TDWF, !sText.isEmpty());
  text->setTextString(sText);
...
TK_Status DD_TK_Matrix::Execute(BStreamFileToolkit & tk)
{
  ...
  if (Opcode() == TKE_Modelling_Matrix)
  {
    ...
    ODA_ASSERT_VAR(if (m != OdGeMatrix3d::kIdentity))
      ODA_ASSERT(true); // brk
...
TK_Status DD_TK_Open_Segment::Execute(BStreamFileToolkit & tk)
{
  ...
  ODA_ASSERT_VAR(if (segName == L"1327"))
    ODA_ASSERT(true); // brk at roof
  //ODA_ASSERT_VAR(if (segName == L"?Include Library/399"))
  //  ODA_ASSERT(true); // brk at window frame
...
: _parent(parent), font_size(0), _colorGeometryMask(0),_visibilityMask(0),_visibility(-1),_name(name),_anonymousSegments(0)
{
  if (parent)
  {
    *this = *parent;
    ...
    //ODA_ASSERT_VAR(if (parent->_matrix != OdGeMatrix3d::kIdentity))
    //  ODA_ASSERT(true); // brk
  ...
  if (pRxSrv.get())
  {
    ...
    for (; piResources && piResources->valid(); piResources->next())
    {
      ...
      if (iSizeReq && !sName.isEmpty() && m_textures.find(sName) == m_textures.end())
      {
        ...
        // way for DwfImport
#      if defined(DWFIMPORT_PRESAVE_TEXTURE_FILES)
...
#      else
        // CORE-14394
        OdGiRasterImageFileTexturePtr pTexture = OdRxObjectImpl<OdGiRasterImageFileTexture>::createObject();
        m_textures[sName] = pTexture;
        ...
        database()->addOdDbObject(pImageDef); // CORE-18363 problem with saving
        // add image to reactor, to be notified of db saving
        DwfEventReactor::getReactor(database())->addImage(pImageDef);
        pStreamBuf->rewind(); pImageDef->setData(pStreamBuf);

        OdString sFileName = database()->getFilename();
        if (sFileName.isEmpty())
        {
          OdRxVariantValue rxPath = (OdRxVariantValue)properties()->getAt(L"DwfPath");
          ODA_ASSERT_ONCE_X(TDWF, !rxPath.isNull());
          sFileName = rxPath->getString();
        }
        sFileName.replace(L'\\', L'/');
        int pos;
        if ((pos = sFileName.reverseFind(L'.')) > 0)
          sFileName = sFileName.left(pos);
        if ((pos = sFileName.reverseFind(L'/')) > 0)
          sFileName = sFileName.mid(++pos);
        sName.remove(L'\"'); sName.remove(L'\'');
        sName = sFileName + L"_" + sName;

        pImageDef->setName(sName);
        pImageDef->setExtension(sExt);
        pImageDef->setTexture(pTexture);
        pTexture->setSourceFileName(sName + sExt);
        OdGiRasterImagePtr pImage = pRxSrv->loadRasterImage(pStreamBuf); pStreamBuf->rewind();
        pTexture->setRasterImage(pImage); 
#      endif
      }
  ...
  oW3DStreamParser.SetOpcodeHandler (TKE_Cylinder,             new DD_TK_Cylinder(this));
