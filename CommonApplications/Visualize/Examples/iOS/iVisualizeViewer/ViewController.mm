...
- (void)viewDidLoad {
    ...
    for (NSString * filename in filenames)
    {
        ...
        if ([extension isEqualToString:@"dwg"] ||
            [extension isEqualToString:@"dxf"] ||
            [extension isEqualToString:@"vsf"] ||
            [extension isEqualToString:@"obj"])
        {
...
- (void)prepareForSegue:(UIStoryboardSegue *)segue sender:(id)sender {
    if ([segue.identifier isEqualToString:@"ShowFile"]) {
        ...
        NSLog(@"%@", dataPath);
        NSString *extension = [[dataPath pathExtension] lowercaseString];
        VisualizeViewController *destViewController = segue.destinationViewController;
