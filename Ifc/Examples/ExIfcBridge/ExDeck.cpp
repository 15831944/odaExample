...
ExDeckProfile::ExDeckProfile(ExIfcModelFiller* filler,
                             ...
                             double girderDepth)
{
  ...
  // left wing
  points.append(OdGePoint2d(left, ydim));
  points.append(OdGePoint2d(left, ydim - plateThickness * 0.5));
  points.append(OdGePoint2d(left + wingWidth, ydim - plateThickness));

  // right wing
  points.append(OdGePoint2d(right - wingWidth, ydim - plateThickness));
  points.append(OdGePoint2d(right, ydim - plateThickness * 0.5));
  points.append(OdGePoint2d(right, ydim));

  m_profile = profileBuilder->createArbitraryClosedProfileDef(IfcProfileBuilder::kAREA, name, points);
  ODA_ASSERT(!m_profile.isNull());
}

OdDAIObjectId ExDeckProfile::profile() const
{
