...
/* Calling sequence:                                                    */
/*                                                                      */
/* ExIfcModelEditor <destination dump filename>                         */
/*                                                                      */
/************************************************************************/
...
//
// Ifc2x3 schema early binding classes
//
...
bool isDir(OdString &szSource, bool &upperLevel)
{
  ...
  namespace {
	...
	void appendToModel(OdIfcModel * model, OdIfc::OdIfcEntityPtr entity)
	{
		model->appendEntityInstance(entity);	
	}
    ...
    void fillModel(OdIfcFile *pDb)
    {
	  ...
	  project_18 = OdIfc::Utils::createProject(model, L"Sample project made with ODA ExIfcModelFiller app", representationContext_18, unitAssignment_17, handleOwnerHistory);
      ...
      OdIfc::OdIfcEntityPtr pEnt = model->createEntityInstance("IfcDirection");                         
	  if (pEnt.isNull())                                                                                 
		  throw OdError(eIllegalEntityType);                                                               

	  appendToModel(model, pEnt);
	  
	  OdDAI::InstanceIteratorPtr it = model->newIterator();
	  OdIfc::OdIfcEntityPtr pInst;

	  OdArray<OdIfc::OdIfcEntityPtr> copies;
	  ...
	  for (entIdx = 0; !it->done(); it->step(), ++entIdx)
	  {
		  ...
		  if (!pInst.isNull())
		  {
			  ...
			  OdIfc::OdIfcEntityPtr pEnt = model->createEntityInstance(typeNameW);
			  if (pEnt.isNull())
				  throw OdError(eIllegalEntityType);
			  copies.append(pEnt);			  
	  ...
	  for (OdIfc::OdIfcEntityPtr pEnt : copies)
	  {
		  appendToModel(model, pEnt);	  
	  }
