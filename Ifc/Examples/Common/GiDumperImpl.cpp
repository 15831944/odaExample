...
OdArray<OdIfcStlTriangleFace> OdGiDumper::m_arrTriangles;
                                      ...
                                      const OdString& name)
{
  if (trueColors)
  {
    ...
    for (i = 0; i < numColors; i++)
    {
      OdCmEntityColor dgColor = trueColors[i];
                               ...
                               const OdString& table)
{
  if (ids)
  {
    ...
    for (i = 0; i < numIds; i++)
    {
      output(toString(name + OD_T("[%d]"), (int)i));
    }
    popIndent();
  }
}
...
void OdGiDumper::addStlTriangle( const OdIfcStlTriangleFace& triangleData )
{
  m_arrTriangles.push_back( triangleData );
}

OdArray<OdIfcStlTriangleFace> OdGiDumper::getStlTriangles()
{
...
void OdGiDumper::writeFaceDataToStlFile( const OdString& strFileName, 
                                           const OdArray<OdIfcStlTriangleFace>& arrTriangles )
{
