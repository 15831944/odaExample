...
OdIfc::OdIfcEntityPtr ExRightCircularCylinder::subProcess()
{
  ...
  OdIfc::OdIfcEntityPtr cylinder = m_b->createRightCircularCylinder(m_objectPlacement, m_h, m_r);

  if (m_createBoolean)
  {
    OdGeMatrix3d secondObjectPlacement = m_objectPlacement * OdGeMatrix3d().setTranslation(OdGeVector3d(0, 0, -0.5 * m_h));
    OdIfc::OdIfcEntityPtr block = m_b->createBlock(secondObjectPlacement, m_r, m_r, m_h);

    cylinder = m_b->createIfcBooleanResult(cylinder, block, "union");
  }
  setStyledItem(cylinder, "Dark");

  OdIfc::OdIfcEntityPtr pS = model->createEntityInstance("ifccsgsolid");
  pS->putAttr(OdIfc::kTreeRootExpression, OdDAIObjectId(cylinder->id()));
...
ExRightCircularCylinder::ExRightCircularCylinder(
