...
OdIfc::OdIfcEntityPtr ExRectangularPyramid::subProcess()
{
  ...
  OdIfc::OdIfcEntityPtr pyramid = m_b->createRectangularPyramid(m_objectPlacement, m_x, m_y, m_h);

  if (m_createBoolean)
  {
    OdGeMatrix3d secondObjectPlacement = m_objectPlacement * OdGeMatrix3d().setTranslation(OdGeVector3d(0, 0, -0.5 * m_h));
    OdIfc::OdIfcEntityPtr block = m_b->createBlock(secondObjectPlacement, m_x, m_y, m_h);

    pyramid = m_b->createIfcBooleanResult(pyramid, block, "union");
  }
  setStyledItem(pyramid, "Dark");

  OdIfc::OdIfcEntityPtr pS = model->createEntityInstance("ifccsgsolid");
  pS->putAttr(OdIfc::kTreeRootExpression, OdDAIObjectId(pyramid->id()));
...
ExRectangularPyramid::ExRectangularPyramid(
