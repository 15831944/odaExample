...
#ifndef _IFC2X2_FINAL_IFCLINE_AUTO_IMPL_H
#define _IFC2X2_FINAL_IFCLINE_AUTO_IMPL_H
...
class IFC2X2_FINAL_EXPORT IfcLine : public IfcCurve
{
  //DOM-IGNORE-BEGIN
  OD_EXP_DECLARE_MEMBERS(OdIfc2x2_final::IfcLine);
  //DOM-IGNORE-END
...
public:
  ...
  const OdDAIObjectId& getPnt() const;
  void setPnt(const OdDAIObjectId& Pnt);
  const OdDAIObjectId& getDir() const;
  void setDir(const OdDAIObjectId& Dir);
...
protected:
  // Fields declaration: 
  OdDAIObjectId m_Pnt;   // IfcCartesianPoint
  OdDAIObjectId m_Dir;   // IfcVector
};

/** \details
  A data type that represents a smart pointer to a <link OdIfc2x2_final::IfcLine, IfcLine> object.
*/
typedef OdSmartPtr<IfcLine> IfcLinePtr;

} // namespace

#endif // _IFC2X2_FINAL_IFCLINE_AUTO_IMPL_H
