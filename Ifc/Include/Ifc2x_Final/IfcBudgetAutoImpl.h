...
#ifndef _IFC2X_FINAL_IFCBUDGET_AUTO_IMPL_H
#define _IFC2X_FINAL_IFCBUDGET_AUTO_IMPL_H
...
class IFC2X_FINAL_EXPORT IfcBudget : public IfcCostSchedule
{
  //DOM-IGNORE-BEGIN
  OD_EXP_DECLARE_MEMBERS(OdIfc2x_final::IfcBudget);
  //DOM-IGNORE-END
...
public:
  ...
  const OdAnsiString& getBudgetSource() const;
  void setBudgetSource(const OdAnsiString& BudgetSource);
  const OdDAIObjectId& getBalance() const;
  void setBalance(const OdDAIObjectId& Balance);
...
protected:
  // Fields declaration: 
  OdAnsiString m_BudgetSource;   // IfcLabel  [optional]
  OdDAIObjectId m_Balance;   // IfcCostValue
};

/** \details
  A data type that represents a smart pointer to a <link OdIfc2x_final::IfcBudget, IfcBudget> object.
*/
typedef OdSmartPtr<IfcBudget> IfcBudgetPtr;

} // namespace

#endif // _IFC2X_FINAL_IFCBUDGET_AUTO_IMPL_H
