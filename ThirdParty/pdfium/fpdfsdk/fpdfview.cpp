...
class CPdfiumMemFile final : public IFX_SeekableReadStream {
 public:
  static RetainPtr<CPdfiumMemFile> Create(const uint8_t* pBuf, FX_FILESIZE size) {
    return RetainPtr<CPdfiumMemFile>(new CPdfiumMemFile(pBuf, size));
  }
 ...
 private:
   CPdfiumMemFile(const uint8_t* pBuf, FX_FILESIZE size)
...
FPDF_LoadMemDocument(const void* data_buf, int size, FPDF_BYTESTRING password) {
  return LoadDocumentImpl(
    CPdfiumMemFile::Create(static_cast<const uint8_t*>(data_buf), size), password);
}
