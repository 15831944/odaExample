...
bool CPDF_HintTables::ReadPageHintTable(CFX_BitStream* hStream) {
  ...
  for (int i = 0; i < nPages; ++i) {
    ...
    if (!safeDeltaObj.IsValid())
      return false;
    uint32_t tmpVal = safeDeltaObj.ValueOrDie();
    m_dwDeltaNObjsArray.push_back(tmpVal);
  }
  hStream->ByteAlign();
  ...
  for (int i = 0; i < nPages; ++i) {
    ...
    if (!safePageLen.IsValid())
      return false;
    uint32_t tmpVal = safePageLen.ValueOrDie();
    dwPageLenArray.push_back(tmpVal);
  }
  ...
  for (uint32_t i = 0; i < dwSharedObjTotal; ++i) {
    ...
    if (i < m_nFirstPageSharedObjs) {
      ...
      if (!safeObjNum.IsValid())
        return false;
      uint32_t tmpVal = safeObjNum.ValueOrDie();
      m_dwSharedObjNumArray.push_back(tmpVal);
      if (i == m_nFirstPageSharedObjs) {
        FX_SAFE_FILESIZE safeLoc = dwFirstSharedObjLoc;
        if (!safeLoc.IsValid())
          return false;
        FX_FILESIZE tmpVal = safeLoc.ValueOrDie();
        m_szSharedObjOffsetArray.push_back(tmpVal);
      }
    }
