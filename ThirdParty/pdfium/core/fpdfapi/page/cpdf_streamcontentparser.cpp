...
#ifdef ODA_CHANGES
ByteString CPDF_StreamContentParser::GetString(uint32_t index, bool* is_hex)
{
  if (index >= m_ParamCount) {
    return ByteString();
  }
  ...
  {
    if (nullptr != is_hex && param.m_pObject->IsString())
    {
      CPDF_String* str = param.m_pObject->AsString();
      *is_hex = str->IsHex();
    }
...
#ifdef ODA_CHANGES
void CPDF_StreamContentParser::AddTextObject(ByteString* pStrs, float fInitKerning, float* pKerning, int nsegs, bool is_hex)
{
...
#ifdef ODA_CHANGES
    pText->SetIsHexStringSource(is_hex);
#endif
...
{
  bool is_hex = true;
  ByteString str = GetString(0, &is_hex);
  if (str.IsEmpty()) 
  {
    return;
  }
  AddTextObject(&str, 0, nullptr, 1, is_hex);
}
#else
void CPDF_StreamContentParser::Handle_ShowText() {
...
void CPDF_StreamContentParser::Handle_ShowText_Positioning() {
  ...
  for (size_t i = 0; i < n; i++) {
    ...
    if (pObj->IsString())
    {
      CPDF_String* str_obj = pObj->AsString();
      ByteString str = str_obj->GetString();
      is_hex = str_obj->IsHex();
      if (str.IsEmpty())
        continue;
      strs[iSegment] = str;
      kernings[iSegment++] = 0;
  ...
  AddTextObject(strs.data(), fInitKerning, kernings.data(), iSegment, is_hex);
#else
  for (size_t i = 0; i < n; i++) {
    CPDF_Object* pObj = pArray->GetDirectObjectAt(i);
    if (pObj->IsString()) {
