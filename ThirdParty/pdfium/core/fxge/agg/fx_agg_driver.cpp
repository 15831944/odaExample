...
void RasterizeStroke(od_agg::rasterizer_scanline_aa* rasterizer,
                     od_agg::path_storage* path_data,
                     const CFX_Matrix* pObject2Device,
                     const CFX_GraphStateData* pGraphState,
                     float scale,
                     bool bStrokeAdjust,
                     bool bTextMode) {
  od_agg::line_cap_e cap;
  switch (pGraphState->m_LineCap) {
    case CFX_GraphStateData::LineCapRound:
      cap = od_agg::round_cap;
      break;
    case CFX_GraphStateData::LineCapSquare:
      cap = od_agg::square_cap;
      break;
    default:
      cap = od_agg::butt_cap;
      break;
  }
  od_agg::line_join_e join;
  switch (pGraphState->m_LineJoin) {
    case CFX_GraphStateData::LineJoinRound:
      join = od_agg::round_join;
      break;
    case CFX_GraphStateData::LineJoinBevel:
      join = od_agg::bevel_join;
      break;
    default:
      join = od_agg::miter_join_revert;
      break;
  }
  ...
  if (pGraphState->m_DashArray) {
    typedef od_agg::conv_dash<od_agg::path_storage> dash_converter;
    ...
    typedef od_agg::conv_stroke<dash_converter> dash_stroke;
  ...
  od_agg::conv_stroke<od_agg::path_storage> stroke(*path_data);
  stroke.line_join(join);
  stroke.line_cap(cap);
  stroke.miter_limit(pGraphState->m_MiterLimit);
  stroke.width(width);
  ...
  for (size_t i = 0; i < pPoints.size(); i++) {
    ...
    if (point_type == FXPT_TYPE::MoveTo) {
      ...
      if (i > 0 && i + 2 < pPoints.size()) {
        ...
        pos2 = HardClip(pos2);
        pos3 = HardClip(pos3);
        od_agg::curve4 curve(pos0.x, pos0.y, pos.x, pos.y, pos2.x, pos2.y, pos3.x,
                          pos3.y);
        i += 2;
...
void CFX_AggDeviceDriver::SetClipMask(od_agg::rasterizer_scanline_aa& rasterizer) {
  ...
  pThisLayer->Clear(0);
  od_agg::rendering_buffer raw_buf(pThisLayer->GetBuffer(), pThisLayer->GetWidth(),
                                pThisLayer->GetHeight(),
                                pThisLayer->GetPitch());
  od_agg::pixfmt_gray8 pixel_buf(raw_buf);
  od_agg::renderer_base<od_agg::pixfmt_gray8> base_buf(pixel_buf);
  od_agg::renderer_scanline_aa_offset<od_agg::renderer_base<od_agg::pixfmt_gray8> >
      final_render(base_buf, path_rect.left, path_rect.top);
  final_render.color(od_agg::gray8(255));
  od_agg::scanline_u8 scanline;
  od_agg::render_scanlines(rasterizer, scanline, final_render,
...
bool CFX_AggDeviceDriver::SetClip_PathFill(const CFX_PathData* pPathData,
  ...
  od_agg::rasterizer_scanline_aa rasterizer;
  ...
  rasterizer.filling_rule((fill_mode & 3) == FXFILL_WINDING
                              ? od_agg::fill_non_zero
                              : od_agg::fill_even_odd);
...
bool CFX_AggDeviceDriver::SetClip_PathStroke(
  ...
  rasterizer.filling_rule(od_agg::fill_non_zero);
  SetClipMask(rasterizer);
  return true;
}
...
bool CFX_AggDeviceDriver::RenderRasterizer(
  ...
  od_agg::scanline_u8 scanline;
  od_agg::render_scanlines(rasterizer, scanline, render,
                        (m_FillFlags & FXFILL_NOPATHSMOOTH) != 0);
  return true;
}
