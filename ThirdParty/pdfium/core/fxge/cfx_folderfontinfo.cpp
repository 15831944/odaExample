...
#ifdef ODA_CHANGES
void CFX_FolderFontInfo::AddPathEnumFonts(const char* path)
{
  ByteString str_path(path);
  auto iter = std::find(m_PathList.begin(), m_PathList.end(), str_path);
  if (m_PathList.end() == iter)
  {
    m_PathList.push_back(path);
    ScanPath(path);
  }
