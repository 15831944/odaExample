...
namespace XamlDrawableAttributes
{
    ...
    public:
        IndexArrayRef();
        IndexArrayRef( int* pnArray, size_t nCount ) ;
        const IndexArrayRef& operator=( const IndexArrayRef& ) ;
        const IndexArrayRef& operator=( vector<int> ) ;
        bool operator==(const IndexArrayRef& s) const;
        virtual ~IndexArrayRef();

    public:
        operator const int*() const { return _pnArray; }
        const int operator[]( unsigned int nIndex ) const ;
