...
#ifndef SSRWINPUTSOURCEPALMVFSFILE_H_INCLUDED
#define SSRWINPUTSOURCEPALMVFSFILE_H_INCLUDED
 ...
 */
_OLESS_API size_t SsrwPalmVFSFileFread(
    void*   in_pv,
    size_t  in_size,
    size_t  in_num,
    SSRWIS* in_pIS);
 ...
 */
_OLESS_API size_t SsrwPalmVFSFileFwrite(
    const void* in_pv,
    size_t      in_size,
    size_t      in_num,
    SSRWIS*     in_pIS);
 ...
 */
_OLESS_API int SsrwPalmVFSFileFclose(
    SSRWIS* in_pv);
 ...
 *                      should be moved
 * @param in_lOffset -  offset to which to move pointer
 * @param in_whence -   SEEK_SET, etc. origin from which to move pointer
 * @return 0 on success, nonzero on failure
 */
_OLESS_API int SsrwPalmVFSFileFseek(
    SSRWIS* in_pIS,
    SINT8   in_llOffset,
    int     in_whence);
 ...
 */
_OLESS_API SINT8 SsrwPalmVFSFileFtell(
    SSRWIS* in_pIS);

/**
 * Truncates a file, if possible.
 * THIS FUNCTION SHOULD NOT BE USED BY EXTERNAL CALLERS.
 *
 * @param in_pIS -        pointer to a palm file input stream that should be 
 *                        truncated
 * @param in_llNewSize -  new size of input stream
 * @return 0 on success, nonzero on failure
 */
_OLESS_API SINT4 SsrwPalmVFSFileFtruncate(
    SSRWIS* in_pIS,
    SINT8   in_llNewSize);

#endif /* SSTG_PALMOS */

#endif /* SSRWINPUTSOURCEPALMVFSFILE_H_INCLUDED */
